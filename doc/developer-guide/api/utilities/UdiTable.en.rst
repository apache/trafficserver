...
and must not reenter the same table. Predicate locks follow the table lock.
A contest examines at most the smaller of the capacity and 1024 slots, so
...
``data_snapshot()`` returns shared ownership of occupied data for maintenance
outside the table lock. ``clear()`` removes all entries without resetting
metrics; existing shared pointers remain valid.
