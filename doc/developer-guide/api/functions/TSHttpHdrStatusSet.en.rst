...
Sets the HTTP status code on an existing HTTP header object. An overload also accepts the transaction and an identifying setter label. When provided, the setter is recorded on the transaction for logging via the `plss` log field.
