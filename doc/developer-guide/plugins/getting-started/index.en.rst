...
-  HTTP processing: plugins can filter, blocklist, authorize users,
   transform content
...
-  **Blocklisting plugin**: denies attempts to access web sites that are
   off-limits.
...
-  ``blocklist_1.c`` reads blocklisted servers from a configuration file
   and denies client access to these servers. This plugin is explained
   in :ref:`developer-plugins-examples-blocklist`.
