...
the selected cache rule configuration. This layout is recomputed from scratch
when :program:`traffic_server` starts. Therefore, any change to those files can
...
Objects can only be pinned via the selected cache rule configuration and while
