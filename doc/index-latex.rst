.. Licensed to the Apache Software Foundation (ASF) under one
   or more contributor license agreements.  See the NOTICE file
   distributed with this work for additional information
   regarding copyright ownership.  The ASF licenses this file
   to you under the Apache License, Version 2.0 (the
   "License"); you may not use this file except in compliance
   with the License.  You may obtain a copy of the License at

   http://www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing,
   software distributed under the License is distributed on an
   "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
   KIND, either express or implied.  See the License for the
   specific language governing permissions and limitations
   under the License.

.. include:: common.defs

Apache Traffic Server Documentation
************************************

This is the LaTeX-only master document for improved PDF structure.
HTML builds use index.rst instead.

.. raw:: latex

   \part{Introduction and Getting Started}
   \setcounter{chapter}{0}

.. toctree::
   :maxdepth: 1

   preface/index.en
   getting-started/index.en
   release-notes/index.en

.. raw:: latex

   \addtocontents{toc}{\protect\clearpage}
   \part{Administrator's Guide}
   \setcounter{chapter}{0}

.. toctree::
   :maxdepth: 1

   admin-guide/introduction.en
   admin-guide/installation/index.en
   admin-guide/configuration/index.en
   admin-guide/interaction/index.en
   admin-guide/security/index.en
   admin-guide/storage/index.en

.. raw:: latex

   \addtocontents{toc}{\protect\setcounter{tocdepth}{2}}
   % Make plugin subsections (individual plugins) start on new pages
   \let\oldsubsection\subsection
   \renewcommand{\subsection}{\clearpage\oldsubsection}

.. toctree::
   :maxdepth: 1

   admin-guide/plugins/index.en

.. raw:: latex

   \addtocontents{toc}{\protect\setcounter{tocdepth}{1}}
   % Restore normal subsection behavior
   \let\subsection\oldsubsection

.. toctree::
   :maxdepth: 1

   developer-guide/cripts/index.en
   admin-guide/logging/index.en
   admin-guide/monitoring/index.en
   admin-guide/configuring-traffic-server.en
   admin-guide/layer-4-routing.en
   admin-guide/performance/index.en

.. toctree::
   :maxdepth: 1

   admin-guide/files/index.en
   admin-guide/tools/index.en
   appendices/index.en

.. raw:: latex

   \addtocontents{toc}{\protect\clearpage}
   \part{Developer's Guide}
   \setcounter{chapter}{0}

.. toctree::
   :maxdepth: 1

   developer-guide/introduction/index.en
   developer-guide/release-process/index.en
   developer-guide/contributing/index.en
   developer-guide/debugging/index.en
   developer-guide/threads-and-events.en
   developer-guide/cache-architecture/index.en
   developer-guide/logging-architecture/index.en
   developer-guide/internal-libraries/index.en
   developer-guide/plugins/index.en
   developer-guide/config-vars.en
   developer-guide/api/index.en
   developer-guide/continuous-integration/index.en
   developer-guide/documentation/index.en
   developer-guide/host-resolution-proposal.en
   developer-guide/client-session-architecture.en
   developer-guide/core-architecture/index.en
   developer-guide/design-documents/index.en
   developer-guide/layout/index.en
   developer-guide/testing/index.en
   developer-guide/jsonrpc/index.en

Indices and tables
==================

* :ref:`genindex`
* :ref:`search`
