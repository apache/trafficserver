...
policies for selected requests and is the default cache rule format. The
legacy :file:`cache.config` format remains supported when selected with
``proxy.config.cache.control.filename``. After modifying the file, run
...
The top-level ``cache`` key contains a sequence of rules. Each rule has a
required ``action`` map and may have a ``match`` map:
