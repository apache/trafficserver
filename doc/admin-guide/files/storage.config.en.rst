...
.. deprecated:: 11.0
   ...
   Use :file:`storage.yaml` instead. :file:`storage.config` and
   :file:`volume.config` are still loaded if :file:`storage.yaml` is absent,
   for backward compatibility. Use ``traffic_ctl config convert storage`` to
   migrate. If both :file:`storage.yaml` and :file:`storage.config` exist,
   :file:`storage.yaml` takes precedence and a warning is logged.
