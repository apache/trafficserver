...
The :file:`remap.yaml` file (by default, located in ``/usr/local/etc/trafficserver/``) contains
...
The file contents are parsed as `YAML 1.2 <https://yaml.org/spec/1.2/spec.html>`__ formatted text.

The top level key is ``url_rewrite``. Under this key are two elements, a ``rules`` key and a
``filters`` key.
...
``target``
   ...
   support for :ref:`limited regular expressions <remap-yaml-regex>`. This is indicated by marking
...
.. _remap-yaml-format-target:

``target``
   Enter the original ("from") URL. This is the URL in the client requiest. There are up to four components:

      *<scheme>*\ ://\ *<host>*\ [:\ *<port>*\ ]\ *<path_prefix>*

   *scheme* must be one of a fixed set of strings - ``http``, ``https``, ``wss``.

.. _remap-yaml-format-replacement:

``replacement``
   Enter the replacement ("from") URL. This has the same structure as the target URL.

   .. note::

      A remap rule for requests that upgrade from HTTP to WebSocket still require a remap   .rule
      with the ``ws`` or ``wss`` scheme.

.. _remap-yaml-precedence:
...
be used with care, and certainly only once at the end of the remap.yaml file. E.g. ::
...
takes precedence because it appears earlier in the :file:`remap.yaml`
file.
...
.. note::
   ...
   unless :file:`remap.yaml` has also changed.

Examples
--------

In this example, a top-level :file:`remap.yaml` file simply
