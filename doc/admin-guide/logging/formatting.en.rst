...
cfl   Proxy Cache    Freshness limit, in seconds, for an object served from or
                     written to cache. The limit is calculated from response
                     headers and the configured guaranteed and heuristic
                     lifetime bounds. The value is ``-1`` when not applicable.
