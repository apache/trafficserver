...
.. _admin-plugins-uri-signing:

URI Signing Plugin
******************

This remap plugin implements the draft URI Signing protocol documented `here <https://tools.ietf.org/html/draft-ietf-cdni-uri-signing-21>`_.

It takes a single argument: the name of a config file that contains key information.

.. note::

    Take care in ordering the plugins. In general, this plugin
    ...
    verified *before* any other plugins modify the request. If another plugin drops
...
In addition, the ``cdniuc`` container of ``hash`` is **not supported**.
