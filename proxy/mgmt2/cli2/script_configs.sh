...
export TS_CONFIG_FILES="logs.config storage.config socks.config proxy.pac lm.config vaddrs.config cache.config icp.config mgmt_allow.config ip_allow.config parent.config filter.config nntp_servers.config nntp_access.config remap.config snmpd.cnf snmpinfo.dat mgr.cnf update.config ${MAIN_CONFIG_FILE}"
