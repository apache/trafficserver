                            ...
                            HpackIndexingTable &indexing_table, uint32_t header_field_max_size)
{
  ...
  if (index) {
    ...
    len = xpack_decode_string(arena, &name_str, name_str_len, p, buf_end, header_field_max_size);
    if (len == XPACK_ERROR_COMPRESSION_ERROR) {
      return HPACK_ERROR_COMPRESSION_ERROR;
    }
  ...
  len = xpack_decode_string(arena, &value_str, value_str_len, p, buf_end, header_field_max_size);
  if (len == XPACK_ERROR_COMPRESSION_ERROR) {
    return HPACK_ERROR_COMPRESSION_ERROR;
  }
