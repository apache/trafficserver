/** @file
 *
 *  Remap text (proprietary TS) configuration file parsing.
 *
...
struct RemapTextBuilder : public RemapBuilder {
  ...
  TextView parse_rule(int line_no, ErrBuff &errw);

  /// Reset state for a new rule.
  /// Filters are not cleared.
  void clear();

  // These are views of strings in the configuration file that have been localized. It is critical
  // that the result can be passed to the plugin API as a C string.
