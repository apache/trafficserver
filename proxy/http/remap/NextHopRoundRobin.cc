...
NextHopRoundRobin::findNextHop(TSHttpTxn txnp, void *ih, time_t now)
{
  ...
  do {
    ...
    NH_Debug(NH_DEBUG_TAG,
             "[%" PRIu64 "] Selected a parent, %s,  failCount (faileAt: %jd failCount: %d), FailThreshold: %" PRIu64
             ", request_info->xact_start: %ld",
             sm_id, cur_host->hostname.c_str(), (int64_t)cur_host->failedAt, cur_host->failCount.load(), fail_threshold,
    ...
    if ((cur_host->failedAt == 0) || (cur_host->failCount.load() < fail_threshold)) {
      if (cur_host->available.load() && host_stat == TS_HOST_STATUS_UP) {
        NH_Debug(NH_DEBUG_TAG,
                 "[%" PRIu64
                 "] Selecting a parent, %s,  due to little failCount (failedAt: %jd failCount: %d), FailThreshold: %" PRIu64,
                 sm_id, cur_host->hostname.c_str(), (int64_t)cur_host->failedAt, cur_host->failCount.load(), fail_threshold);
        parentUp = true;
      }
      ...
      if (((result->wrap_around) || (cur_host->failedAt + retry_time) < (int64_t)_now) && host_stat == TS_HOST_STATUS_UP) {
        // Reuse the parent
        parentUp    = true;
        parentRetry = true;
