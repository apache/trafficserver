...
algorithm_version(int i)
{
  switch (i) {
  case 4:
    return PreWarm::Algorithm::V4;
  case 3:
    return PreWarm::Algorithm::V3;
  case 2:
...
/**
   How many connections needs to be pre-warmed

   Try to keep pool size more than min

   @params min : min connections (configured)
   @params max : max connections (configured), -1 : unlimited
   @return gap to the min
 */
inline uint32_t
...
/**
   Pre-warming Algorithm v3 Implementation (on event interval)
 */
inline uint32_t
prewarm_size_v3_on_event_interval(uint32_t hit, uint32_t miss, uint32_t current_size, uint32_t min, int32_t max)
{
...
/**
   Pre-warming Algorithm v4 Implementation (on event interval)

   V3 + rate setting
 */
inline uint32_t
prewarm_size_v4_on_event_interval(uint32_t hit, uint32_t miss, uint32_t current_size, uint32_t min, int32_t max, double rate)
{
  if (hit + miss + current_size < min) {
    // fallback to v1 to keep min size
    return prewarm_size_v1_on_event_interval(hit + miss, current_size, min, max);
