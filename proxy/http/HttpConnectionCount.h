...
public:
  ...
  getCount(const IpEndpoint &addr, const INK_MD5 &hostname_hash, TSServerSessionSharingMatchType match_type)
  {
    ...
    auto host_count = _hostCount.find(ConnAddr(addr, hostname_hash, match_type));
    int count = (host_count != _hostCount.end()) ? host_count->second : 0;
                 ...
                 const int delta = 1)
  {
    ...
    ink_mutex_acquire(&_mutex);
    auto count = _hostCount.find(caddr);
    if (count != _hostCount.end())
      _hostCount.insert({caddr, count->second + delta});
    ink_mutex_release(&_mutex);
  }
  /**
  ...
  struct ConnAddr {
    IpEndpoint _addr;
    mutable INK_MD5 _hostname_hash;
    ...
    operator bool() const { return ats_is_ip(&_addr); }
    std::string
    getIpStr() const
    {
      std::string str;
  ...
  class ConnAddrHashFunc
  {
  public:
    std::size_t operator()(const ConnAddr &addr) const
    {
      if (addr._match_type == TS_SERVER_SESSION_SHARING_MATCH_IP) {
        return (std::size_t)ats_ip_port_hash(&addr._addr.sa);
      } else if (addr._match_type == TS_SERVER_SESSION_SHARING_MATCH_HOST) {
        return (std::size_t)addr._hostname_hash.u64[0];
      } else if (addr._match_type == TS_SERVER_SESSION_SHARING_MATCH_BOTH) {
        return ((std::size_t)ats_ip_port_hash(&addr._addr.sa) ^ (std::size_t)addr._hostname_hash.u64[0]);
      } else {
  ...
  class ConnAddrEqualToFunc
  {
  public:
    bool operator()(const ConnAddr &a, const ConnAddr &b) const
    {
...
protected:
  ...
  std::unordered_map<ConnAddr, int, ConnAddrHashFunc, ConnAddrEqualToFunc> _hostCount;
  ink_mutex _mutex;
