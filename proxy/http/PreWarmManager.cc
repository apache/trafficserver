...
/**
   @brief Give ownership of netvc to caller
...
PreWarmSM::state_open(int event, void *data)
{
  switch (event) {
  case VC_EVENT_READ_READY: {
    // When origin server sent something, keep it in buffer. Forward it to the UA when tunnel is setup (HttpSM::setup_blind_tunnel)
...
PreWarmSM::_connect(const IpEndpoint &addr)
{
  ...
  case SNIRoutingType::PARTIAL_BLIND: {
    ...
    // ALPN
    if (_dst->alpn_index == TS_ALPN_PROTOCOL_INDEX_HTTP_2_0) {
      opt.alpn_protos = TS_ALPN_PROTO_ID_OPENSSL_HTTP_2;
    } else if (_dst->alpn_index == TS_ALPN_PROTOCOL_INDEX_HTTP_1_1) {
      opt.alpn_protos = TS_ALPN_PROTO_ID_OPENSSL_HTTP_1_1;
    }
...
/**
   ...
   V1: and expand pool size to requested size
   V2: same as V1 (event_period can be long)
   V3: add stat.miss to the pool
   V4: add (stat.miss * conf.rate) to the pool
 */
void
PreWarmQueue::_prewarm_on_event_interval(const PreWarm::SPtrConstDst &dst, const Info &info)
{
  ...
  switch (_algorithm) {
  case PreWarm::Algorithm::V4: {
    n = PreWarm::prewarm_size_v4_on_event_interval(info.stat.hit, info.stat.miss, current_size, info.conf->min, info.conf->max,
                                                   info.conf->rate);
    break;
  }
  case PreWarm::Algorithm::V3: {
    n = PreWarm::prewarm_size_v3_on_event_interval(info.stat.hit, info.stat.miss, current_size, info.conf->min, info.conf->max);
    break;
  }
...
/**
   V1: do nothing
   V2: start prewarming a new netvc
   V3: same as V2
   V4: same as V2
 */
void
...
/**
   Delete closed state PreWarmSM(s) in the queue
 */
void
...
PreWarmManager::_parse_sni_conf(PreWarm::ParsedSNIConf &parsed_conf, const SNIConfigParams *sni_conf) const
{
  ...
  for (const auto &item : sni_conf->Y_sni.items) {
    if (item.tunnel_routing_type != SNIRoutingType::FORWARD && item.tunnel_routing_type != SNIRoutingType::PARTIAL_BLIND) {
      continue;
    }
    ...
    if (!item.tunnel_alpn.empty() && item.tunnel_routing_type == SNIRoutingType::PARTIAL_BLIND) {
    ...
    for (int id : alpn_ids) {
      Debug("prewarm_m", "sni=%s dst=%s type=%d alpn=%d min=%d max=%d c_timeout=%d i_timeout=%d srv=%d", item.fqdn.c_str(),
            item.tunnel_destination.c_str(), (int)item.tunnel_routing_type, id, (int)item.tunnel_prewarm_min,
            (int)item.tunnel_prewarm_max, (int)item.tunnel_prewarm_connect_timeout, (int)item.tunnel_prewarm_inactive_timeout,
      ...
      if (port < 0) {
        port = item.tls_upstream ? 443 : 80;
      }

      PreWarm::SPtrConstDst dst = std::make_shared<const PreWarm::Dst>(dst_fqdn, port, item.tunnel_routing_type, id);
      ...
      PreWarm::SPtrConstConf conf = std::make_shared<const PreWarm::Conf>(
        ...
        (item.tunnel_pristine_server_name)? item.fqdn : dst_fqdn
      );
      // clang-format on
