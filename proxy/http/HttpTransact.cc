...
find_server_and_update_current_info(HttpTransact::State* s)
{
  ...
  if (ptr_len_cmp(host, host_len, local_host_ip_str, sizeof(local_host_ip_str) - 1) == 0) {
    ...
    case PARENT_SPECIFIED:
      ...
      if (s->parent_result.r == PARENT_DIRECT && s->http_config_param->no_dns_forward_to_parent != 0) {
        ink_assert(!ats_is_ip(&s->server_info.dst_addr));
  ...
  case PARENT_SPECIFIED:
    ...
    s->parent_info.dst_addr.port() = s->parent_result.port;
  ...
  default:
    ...
    s->current.server->dst_addr.port() = s->hdr_info.server_request.port_get();
    update_dns_info(&s->dns_info, &s->current, 0, &s->arena);
...
HttpTransact::StartRemapRequest(State* s)
{
  ...
  s->cop_test_page = (ptr_len_cmp(host, host_len, local_host_ip_str, sizeof(local_host_ip_str) - 1) == 0) &&
    ...
    (!s->http_config_param->autoconf_localhost_only || ats_ip4_addr_cast(&s->client_info.src_addr.sa) == htonl(INADDR_LOOPBACK));
...
HttpTransact::PPDNSLookup(State* s)
{
  ...
  if (!s->dns_info.lookup_success) {
    ...
    DebugTxn("http_trans", "[PPDNSLookup] DNS lookup for sm_id[%" PRId64"] successful IP: %s",
          s->state_machine->sm_id, ats_ip_ntop(&s->parent_info.dst_addr.sa, addrbuf, sizeof(addrbuf)));
  }
...
HttpTransact::OSDNSLookup(State* s)
{
  ...
  DebugTxn("http_trans", "[OSDNSLookup] DNS lookup for O.S. successful "
        "IP: %s", ats_ip_ntop(&s->server_info.dst_addr.sa, addrbuf, sizeof(addrbuf)));
...
HttpTransact::HandleCacheOpenReadHit(State* s)
{
  ...
  if (send_revalidate) {
    ...
    if (server_up || s->stale_icp_lookup) {
      ...
      if (!s->stale_icp_lookup && (check_hostdb || !ats_is_ip(&s->current.server->dst_addr))) {
...
HttpTransact::HandleICPLookup(State* s)
{
  ...
  if (s->icp_lookup_success == true) {
    ...
    ink_release_assert(
        (s->icp_info.dst_addr.port() != s->client_info.dst_addr.port()) ||
...
HttpTransact::initialize_state_variables_for_origin_server(State* s, HTTPHdr* incoming_request, bool second_time)
{
  ...
  ats_ip_copy(&s->server_info.dst_addr, &s->request_data.src_ip); //initialize w/ junk
  s->server_info.dst_addr.port() = incoming_request->port_get();
  ip_port_text_buffer ip_s;
  DebugTxn("http_trans", "init_for_origin server: %s port: %d incoming port: %d", ats_ip_nptop(&s->server_info.dst_addr.sa, ip_s, sizeof(ip_s)), s->server_info.dst_addr.port(), incoming_request->port_get());
...
HttpTransact::initialize_state_variables_from_request(State* s, HTTPHdr* obsolete_incoming_request)
{
  ...
  if (!s->server_info.name || s->redirect_info.redirect_in_process) {
    ...
    s->server_info.dst_addr.port() = incoming_request->port_get();
    ip_port_text_buffer ip_s;
    DebugTxn("http_trans", "init_from_request server: %s port: %d incoming port: %d", ats_ip_nptop(&s->server_info.dst_addr.sa, ip_s, sizeof(ip_s)), s->server_info.dst_addr.port(), incoming_request->port_get());
  }
...
HttpTransact::will_this_request_self_loop(State* s)
{
  ...
  if (s->dns_info.lookup_success) {
    if (ats_ip_addr_eq(s->host_db_info.ip(), &Machine::instance()->ip.sa)) {
      ...
      uint16_t local_port = ats_ip_port_host_order(&s->client_info.src_addr);
                                   ...
                                   const char *error_body_type, const char *format, ...)
{
  ...
  if (s->http_config_param->errors_log_error_pages && status_code >= HTTP_STATUS_BAD_REQUEST) {
    ...
    Log::error("RESPONSE: sent %s status %d (%s) for '%s'",
        ats_ip_ntop(&s->client_info.src_addr.sa, ip_string, sizeof(ip_string)),
        status_code,
        reason_phrase,
