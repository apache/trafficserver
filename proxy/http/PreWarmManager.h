...
/**
   @class PreWarmQueue
   @detail
   - Each ET_NET thread has this manager
   - Have resposibility of life cycle of PreWarmSM (except giving PreWarmSM by dequeue to HttpSM )
   - Have queue of pre-warmed connections per destination
...
private:
  ...
  // hooks for pre-warmed connection pool size algorithm
  ...
  // Periodically force PreWarmSM to open new netvc to keep the connection warm
...
private:
  ...
  // Variables
  //
  // For race of Main Thread (start up) vs Task Thread (config reload)
  Ptr<ProxyMutex> _mutex;

  // Should be merged these maps?
