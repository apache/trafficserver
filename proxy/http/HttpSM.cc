...
HttpSM::init()
{
  milestones[TransactionMilestones::SM_START] = ink_get_hrtime();
...
{
  milestones[TransactionMilestones::UA_BEGIN] = ink_get_hrtime();
...
HttpSM::state_read_client_request_header(int event, void *data)
{
  ...
  //
  if (client_request_hdr_bytes == 0) {
    milestones[TransactionMilestones::UA_FIRST_READ] = ink_get_hrtime();
  ...
  if (state != PARSE_CONT || ua_entry->eos || (state == PARSE_CONT && event == VC_EVENT_READ_COMPLETE)) {
    ...
    milestones[TransactionMilestones::UA_READ_HEADER_DONE] = ink_get_hrtime();
  }
...
HttpSM::state_watch_for_client_abort(int event, void *data)
{
  ...
  case VC_EVENT_INACTIVITY_TIMEOUT: {
    ...
    milestones[TransactionMilestones::UA_CLOSE] = ink_get_hrtime();
    set_ua_abort(HttpTransact::ABORTED, event);
    terminate_sm = true;
    break;
  }
...
HttpSM::state_read_push_response_header(int event, void *data)
{
  ...
  if (state != PARSE_CONT) {
    ...
    ua_entry->read_vio->nbytes = ua_entry->read_vio->ndone;
    http_parser_clear(&http_parser);
    milestones[TransactionMilestones::SERVER_READ_HEADER_DONE] = ink_get_hrtime();
  }
...
HttpSM::state_raw_http_server_open(int event, void *data)
{
  ...
  ink_assert(server_entry == NULL);
  milestones[TransactionMilestones::SERVER_CONNECT_END] = ink_get_hrtime();
...
HttpSM::state_read_server_response_header(int event, void *data)
{
  ...
  if (server_response_hdr_bytes == 0) {
    milestones[TransactionMilestones::SERVER_FIRST_READ] = ink_get_hrtime();
...
HttpSM::process_hostdb_info(HostDBInfo *r)
{
  ...
  milestones[TransactionMilestones::DNS_LOOKUP_END] = ink_get_hrtime();
...
HttpSM::state_cache_open_write(int event, void *data)
{
  ...
  milestones[TransactionMilestones::CACHE_OPEN_WRITE_END] = ink_get_hrtime();
  pending_action = NULL;
...
HttpSM::state_cache_open_read(int event, void *data)
{
  ...
  milestones[TransactionMilestones::CACHE_OPEN_READ_END] = ink_get_hrtime();
...
HttpSM::tunnel_handler_server(int event, HttpTunnelProducer *p)
{
  ...
  milestones[TransactionMilestones::SERVER_CLOSE] = ink_get_hrtime();
...
HttpSM::do_hostdb_lookup()
{
  ...
  milestones[TransactionMilestones::DNS_LOOKUP_BEGIN] = ink_get_hrtime();
...
HttpSM::do_cache_lookup_and_read()
{
  ...
  milestones[TransactionMilestones::CACHE_OPEN_READ_BEGIN] = ink_get_hrtime();
  t_state.cache_lookup_result = HttpTransact::CACHE_LOOKUP_NONE;
...
HttpSM::do_cache_prepare_write()
{
  ...
  milestones[TransactionMilestones::CACHE_OPEN_WRITE_BEGIN] = ink_get_hrtime();
...
HttpSM::do_http_server_open(bool raw)
{
  ...
  milestones[TransactionMilestones::SERVER_CONNECT] = ink_get_hrtime();
...
HttpSM::do_api_callout_internal()
{
  ...
  case HttpTransact::SM_ACTION_API_SEND_RESPONSE_HDR:
    ...
    milestones[TransactionMilestones::UA_BEGIN_WRITE] = ink_get_hrtime();
    break;
  case HttpTransact::SM_ACTION_API_SM_SHUTDOWN:
...
HttpSM::mark_server_down_on_client_abort()
{
  ...
  if (t_state.current.request_to == HttpTransact::ORIGIN_SERVER && t_state.hdr_info.request_content_length == 0) {
    if (milestones[TransactionMilestones::SERVER_FIRST_CONNECT] != 0 && milestones[TransactionMilestones::SERVER_FIRST_READ] == 0) {
      ...
      ink_hrtime wait = ink_get_hrtime() - milestones[TransactionMilestones::SERVER_FIRST_CONNECT];
      if (wait < 0) {
        wait = 0;
      }
...
HttpSM::setup_server_send_request()
{
  ...
  milestones[TransactionMilestones::SERVER_BEGIN_WRITE] = ink_get_hrtime();
...
HttpSM::update_stats()
{
  milestones[TransactionMilestones::SM_FINISH] = ink_get_hrtime();
