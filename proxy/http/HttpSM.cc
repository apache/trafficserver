...
DbgCtl dbg_ctl_http_parse{"http_parse"};
DbgCtl dbg_ctl_http_websocket{"http_websocket"};
DbgCtl dbg_ctl_http_connect{"http_connect"};
DbgCtl dbg_ctl_http_track{"http_track"};
...
DbgCtl dbg_ctl_http_range{"http_range"};
DbgCtl dbg_ctl_http_trans{"http_trans"};
DbgCtl dbg_ctl_http_cache_write{"http_cache_write"};
DbgCtl dbg_ctl_http_ss_auth{"http_ss_auth"};
DbgCtl dbg_ctl_http_redir_error{"http_redir_error"};
...
#define SMDebug(tag, fmt, ...) SpecificDbg(debug_on, dbg_ctl_##tag, "[%" PRId64 "] " fmt, sm_id, ##__VA_ARGS__)
  ...
  case NET_EVENT_OPEN_FAILED: {
    ...
    */
    if (EADDRNOTAVAIL == t_state.current.server->connect_result && t_state.client_info.is_transparent) {
      if (is_dbg_ctl_enabled(dbg_ctl_http_tproxy)) {
...
HttpSM::process_hostdb_info(HostDBInfo *r)
{
  ...
  if (is_dbg_ctl_enabled(dbg_ctl_http_timeout)) {
...
HttpSM::do_http_server_open(bool raw)
{
  ...
  if (!t_state.url_remap_success || t_state.url_map.getMapping()->ip_allow_check_enabled_p) {
    ...
    if (deny_request) {
      if (is_dbg_ctl_enabled(dbg_ctl_ip_allow)) {
        ip_text_buffer ipb;
        if (method != -1) {
  ...
  if (t_state.txn_conf->outbound_conntrack.max > 0) {
    ...
    if (ccount > t_state.txn_conf->outbound_conntrack.max) {
      ...
      ct_state.Warn_Blocked(&t_state.txn_conf->outbound_conntrack, sm_id, ccount - 1, &t_state.current.server->dst_addr.sa,
                            debug_on && is_dbg_ctl_enabled(dbg_ctl_http) ? "http" : nullptr);
      send_origin_throttled_response();
...
HttpSM::set_next_state()
{
  ...
  case HttpTransact::SM_ACTION_DNS_LOOKUP: {
    ...
    if (t_state.api_server_addr_set) {
       ...
       */
      ip_text_buffer ipb;
      SMDebug(dns, "Skipping DNS lookup for API supplied target %s.", ats_ip_ntop(&t_state.server_info.dst_addr, ipb, sizeof(ipb)));
