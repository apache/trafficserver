...
HttpSM::state_read_server_response_header(int event, void *data)
{
  ...
  case PARSE_RESULT_DONE:
    ...
    do_api_callout();
    if (server_entry) {
      // Don't want to accidentally pick this method up for the handler
...
HttpSM::setup_server_read_response_header()
{
  ...
  // Change the default handler to one that will fail the state machine if it gets an unexpected event
  HTTP_SM_SET_DEFAULT_HANDLER(&HttpSM::tunnel_handler);
