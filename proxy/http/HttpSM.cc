...
HttpSM::do_http_server_open(bool raw)
{
  ...
  if (scheme_to_use == URL_WKSIDX_HTTPS) {
    ...
    if (t_state.txn_conf->client_cert_file != nullptr) {
      opt.set_client_certname(t_state.txn_conf->client_cert_file);
      auto tCTX = params->getCTX(t_state.txn_conf->client_cert_file);

      if (tCTX == nullptr) {
        // make new client ctx and add it to the ctx list
        auto tctx = params->getNewCTX(t_state.txn_conf->client_cert_file);
        params->InsertCTX(t_state.txn_conf->client_cert_file, tctx);
      }
    }
