      ...
      if (state == CHUNK_READ_SIZE) {
        ...
        if (in_quoted_string) {
          ...
          if (*tmp == '"') {
            ...
            Debug("http_chunk", "Found an LF without a preceding CR (protocol violation) in chunk extension");
...
HttpTunnel::producer_run(HttpTunnelProducer *p)
{
  ...
  if (p->alive) {
    ...
    for (c = p->consumer_list.head; c; c = c->link.next) {
      if (c->alive && c->write_vio) {
        Debug("http_tunnel", "re-enable %s write_vio", c->name);
...
HttpTunnel::_throttle_chunked_producer(HttpTunnelProducer *p)
{
  for (HttpTunnelConsumer *ci = p->consumer_list.head; ci; ci = ci->link.next) {
    ...
    if (ci->buffer_reader->high_water()) {
      if (p->read_vio) {
        Debug("http_tunnel", "disable %s read_vio - %s read_avail = %" PRId64 " / %" PRId64, p->name, ci->name,
              ci->buffer_reader->read_avail(), ci->buffer_reader->mbuf->water_mark);
        HTTP_INCREMENT_DYN_STAT(http_tunnel_chunked_throttle_stat);
        p->read_vio->disable();
      }
      return ci;
    }
  ...
  if (p->is_handling_chunked_content()) {
    ...
    if (event == VC_EVENT_READ_READY && !plain_passthru) {
      ...
      if (HttpTunnelConsumer *ci = _throttle_chunked_producer(p); ci != nullptr) {
        if (ci->write_vio) {
          Debug("http_tunnel", "re-enable %s write_vio", ci->name);
...
HttpTunnel::_should_reenable_for_chunk_handler_fc(HttpTunnelConsumer *c)
{
  ...
  for (auto *ci = p->consumer_list.head; ci; ci = ci->link.next) {
    ...
    Debug("http_tunnel", "check %s consumer - read_avail = %" PRId64 " / %" PRId64, ci->name, ci->buffer_reader->read_avail(),
          ci->buffer_reader->mbuf->water_mark);

    if (ci->buffer_reader->high_water()) {
      Debug("http_tunnel", "wait until data in the buffer is consumed");
      return false;
    }
  }
    ...
    if (backlog >= flow_state.high_water) {
      ...
      if (srcp && srcp->alive && c->is_sink()) {
        ...
        if (backlog < flow_state.low_water) {
          ...
          srcp->unthrottle();
          if (srcp->read_vio) {
            Debug("http_tunnel", "re-enable %s read_vio", srcp->name);
...
HttpTunnel::consumer_handler(int event, HttpTunnelConsumer *c)
{
  ...
  case VC_EVENT_WRITE_READY: {
    if (_should_reenable_for_tunnel_chain_fc(c) && _should_reenable_for_chunk_handler_fc(c) && p->read_vio) {
      Debug("http_tunnel", "re-enable %s read_vio", p->name);
      p->read_vio->reenable();
