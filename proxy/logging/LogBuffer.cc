...
LogBufferHeader::fmt_name()
{
  char *addr = nullptr;
  if (fmt_name_offset) {
    addr = reinterpret_cast<char *>(this) + fmt_name_offset;
  }
  return addr;
}
  ...
  : m_size(size), m_buf_align(buf_align), m_write_align(write_align), m_owner(owner), m_references(0)
{
  ...
  m_id = static_cast<uint32_t>(ink_atomic_increment(static_cast<pvint32>(&M_ID), 1));
      ...
      if (field != nullptr) {
        ...
        if (field->aggregate() == LogField::NO_AGGREGATE) {
          ...
          if (strcmp(sym, "cqts") == 0) {
            ...
            res       = static_cast<int>(::strlen(str));
