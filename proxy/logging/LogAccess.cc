...
{
  const char *protocol_str = m_http_sm->get_client_protocol();
...
{
  if (buf) {
    marshal_int(buf, m_http_sm->get_client_tcp_reused() ? 1 : 0);
  }
  return INK_MIN_ALIGN;
}
...
{
  if (buf) {
    marshal_int(buf, m_http_sm->get_client_connection_is_ssl() ? 1 : 0);
  }
  return INK_MIN_ALIGN;
}
...
{
  if (buf) {
    marshal_int(buf, m_http_sm->get_client_ssl_reused() ? 1 : 0);
  }
  return INK_MIN_ALIGN;
}
...
{
  const char *proto = m_http_sm->get_client_sec_protocol();
...
{
  const char *cipher = m_http_sm->get_client_cipher_suite();
...
{
  const char *curve = m_http_sm->get_client_curve();
...
{
  const char *alpn = "-";
  if (const int alpn_id = m_http_sm->get_client_alpn_id(); alpn_id != SessionProtocolNameRegistry::INVALID) {
