...
HdrTest::test_error_page_selection()
{
  ...
  for (i = 0; i < nsets; i++) {
    ...
    body_set                   = static_cast<HttpBodySetRawData *>(ats_malloc(sizeof(HttpBodySetRawData)));
    body_set->magic            = 0;
    body_set->set_name         = const_cast<char *>(sets[i].set_name);
    body_set->content_language = const_cast<char *>(sets[i].content_language);
    body_set->content_charset  = const_cast<char *>(sets[i].content_charset);
    body_set->table_of_pages   = (RawHashTable *)1; // hack --- can't be NULL

    table_of_sets->setValue(static_cast<RawHashTable_Key>(body_set->set_name), (RawHashTable_Value)body_set);
  }
...
HdrTest::test_http_parser_eos_boundary_cases()
{
  ...
  for (i = 0; tests[i].msg != nullptr; i++) {
    ...
    bytes_consumed = static_cast<int>(start - orig_start);

    printf("======== test %d (length=%d, consumed=%d)\n", i, static_cast<int>(strlen(tests[i].msg)), bytes_consumed);
    printf("[%s]\n", tests[i].msg);
    printf("\n[");
