...
convert_from_2_to_1_1_header(HTTPHdr* headers)
{
  ...
  if (http_hdr_type_get(headers->m_http) == HTTP_TYPE_REQUEST) {
    ...
    if ((field = headers->field_find(HPACK_HEADER_SCHEME_STRING, strlen(HPACK_HEADER_SCHEME_STRING))) != NULL) {
      scheme = field->value_get(&scheme_len);
    } else {
      return PARSE_ERROR;
    }
    if ((field = headers->field_find(HPACK_HEADER_AUTHORITY_STRING, strlen(HPACK_HEADER_AUTHORITY_STRING))) != NULL) {
      authority = field->value_get(&authority_len);
    } else {
      return PARSE_ERROR;
    }
    if ((field = headers->field_find(HPACK_HEADER_PATH_STRING, strlen(HPACK_HEADER_PATH_STRING))) != NULL) {
    ...
    // Get value of :method
    if ((field = headers->field_find(HPACK_HEADER_METHOD_STRING, strlen(HPACK_HEADER_METHOD_STRING))) != NULL) {
    ...
    headers->field_delete(HPACK_HEADER_SCHEME_STRING, strlen(HPACK_HEADER_SCHEME_STRING));
    headers->field_delete(HPACK_HEADER_METHOD_STRING, strlen(HPACK_HEADER_METHOD_STRING));
    headers->field_delete(HPACK_HEADER_AUTHORITY_STRING, strlen(HPACK_HEADER_AUTHORITY_STRING));
    headers->field_delete(HPACK_HEADER_PATH_STRING, strlen(HPACK_HEADER_PATH_STRING));
  } else {
    int status_len;
    const char* status;

    if ((field = headers->field_find(HPACK_HEADER_STATUS_STRING, strlen(HPACK_HEADER_STATUS_STRING))) != NULL) {
    ...
    // Remove HTTP/2 style headers
    headers->field_delete(HPACK_HEADER_STATUS_STRING, strlen(HPACK_HEADER_STATUS_STRING));
  }
...
MIMEParseResult
http2_parse_req(HdrHeap *heap, HTTPHdrImpl *hh, uint8_t* buf_start, uint8_t* buf_end, bool /* eof */, Http2HeaderTable& header_table)
{
  ...
  // XXX I think that we need to check the eof flag so we can accurately report whether we parsed the whole
