...
TSHttpTxnServerAddrSet(TSHttpTxn txnp, struct sockaddr const* addr)
{
  ...
  if (ats_ip_copy(&sm->t_state.server_info.dst_addr.sa, addr)) {
    ats_ip_port_cast(&sm->t_state.server_info.dst_addr.sa) = ats_ip_port_cast(addr);
    sm->t_state.server_info.dst_addr.port() = htons(ats_ip_port_cast(addr));
...
TSHttpTxnClientIncomingPortSet(TSHttpTxn txnp, int port)
{
  ...
  HttpSM *sm = (HttpSM *) txnp;
  sm->t_state.client_info.dst_addr.port() = port;
}
