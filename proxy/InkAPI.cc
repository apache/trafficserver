...
void
TSVConnInternalSet(TSVConn connp, int internal)
{
  ...
  // If this is a PluginVC generated by TSHttpConnect, then mark
  // both sides as requested.
  if ((pvc = dynamic_cast<PluginVC *>(vc))) {
    pvc->get_other_side()->set_is_internal_request(internal ? true : false);
  }

  vc->set_is_internal_request(internal ? true : false);
}
