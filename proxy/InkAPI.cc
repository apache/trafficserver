...
{
  std::unordered_map<std::string, TXN_CONFIG_PAIR>::iterator i = TXN_CONFIG_LOOKUP_TABLE.find(name);

  if (i == TXN_CONFIG_LOOKUP_TABLE.end()) {
    return NULL;
  } else {
    // found
    return &i->second;
...
TSHttpTxnConfigFind(const char *name, int length, TSOverridableConfigKey *conf, TSRecordDataType *type)
{
  ...
  TXN_CONFIG_PAIR *opts = TXN_CONFIG_LOOKUP(std::string(name, length));
  if (opts) {
    cnf = opts->cnf;
    typ = opts->typ;
  }
