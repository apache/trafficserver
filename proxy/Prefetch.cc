...
static ink32 udp_seq_no;

static inline inku32
get_udp_seq_no()
{
...
static inline void
setup_udp_header(char *header, inku32 seq_no, inku32 pkt_no, bool last_pkt)
{
...
static inline void
setup_object_header(char *header, inku32 size, bool url_promise)
{
  inku32 *hdr = (inku32 *) header;
  hdr[0] = htonl(size);
...
PrefetchConfiguration::readConfiguration()
{
  ...
  fd = ink_open(conf_path, O_RDONLY);
  if (fd < 0) {
