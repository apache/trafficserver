...
typedef MTHashTable<inku64, CongestionEntry *>CongestionTable;
typedef HashTableIteratorState<inku64, CongestionEntry *>Iter;
...
void remove_congested_entry(inku64 key);
...
public:
  CongestionDB(int tablesize);
   ~CongestionDB();
  bool congested(inku64 key);

// add an entry to the db
  void addRecord(inku64 key, CongestionEntry * pEntry);
// remove an entry from the db
  void removeRecord(inku64 key);
