...
TSSslSecretGet(const char *secret_name, int secret_name_length)
{
  ...
  TSAllocatedVarLenData vld;
  if (secret_data.empty()) {
    vld.data = nullptr;
    vld.size = 0;

  } else {
    vld.data = static_cast<char *>(ats_malloc(secret_data.size()));
    memcpy(vld.data, secret_data.data(), secret_data.size());
    vld.size = secret_data.size();
  }
  if (loading) {
