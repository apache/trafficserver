...
Http1ClientSession::free()
{
  ...
  if (conn_decrease) {
    Counter::decrement(http_rsb.current_client_connections);
    conn_decrease = false;
  }
...
Http1ClientSession::new_connection(NetVConnection *new_vc, MIOBuffer *iobuf, IOBufferReader *reader)
{
  ...
  Counter::increment(http_rsb.current_client_connections);
  conn_decrease = true;
  Counter::increment(http_rsb.total_client_connections);
  if (static_cast<HttpProxyPort::TransportType>(new_vc->attributes) == HttpProxyPort::TRANSPORT_SSL) {
    Counter::increment(http_rsb.https_total_client_connections);
  }

  /* inbound requests stat should be incremented here, not after the
   * header has been read */
  Counter::increment(http_rsb.total_incoming_connections);
  ...
  case AF_INET:
    Counter::increment(http_rsb.total_client_connections_ipv4);
    break;
  case AF_INET6:
    Counter::increment(http_rsb.total_client_connections_ipv6);
    break;
  default:
...
{
  Counter::increment(http_rsb.current_active_client_connections);
}
void
Http1ClientSession::decrement_current_active_connections_stat()
{
  Counter::decrement(http_rsb.current_active_client_connections);
}
