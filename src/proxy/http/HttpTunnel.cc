...
HttpTunnel::chain_abort_all(HttpTunnelProducer *p)
{
  ...
  if (p->alive) {
    ...
    Counter::increment(http_rsb.origin_shutdown_tunnel_abort);
...
HttpTunnel::mark_tls_tunnel_active()
{
  ...
  Counter::increment(http_rsb.tunnel_current_active_connections);
...
HttpTunnel::mark_tls_tunnel_inactive()
{
  ...
  Counter::decrement(http_rsb.tunnel_current_active_connections);
