          ...
          if (server_vc) {
            ...
            if (new_vc != server_vc) {
              ...
              if (!new_vc) {
                ...
                Counter::increment(http_rsb.origin_shutdown_migration_failure);
...
ServerSessionPool::removeSession(PoolableSession *to_remove)
{
  ...
  if (m_ip_pool.erase(to_remove)) {
    Counter::decrement(http_rsb.pooled_server_connections);
  }
  if (is_debug_tag_set("http_ss")) {
...
ServerSessionPool::addSession(PoolableSession *ss)
{
  ...
  Counter::increment(http_rsb.pooled_server_connections);
