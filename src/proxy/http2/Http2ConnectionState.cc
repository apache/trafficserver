...
Http2ConnectionState::rcv_settings_frame(const Http2Frame &frame)
{
  ...
  //
  // Exempt only origin ACKs. Dynamic outbound flow control (policy_out=2)
  // can elicit many legitimate ACKs. _process_incoming_settings_ack_frame()
  ...
  if (!(is_ack && this->session->is_outbound())) {
...
Http2ConnectionState::rcv_continuation_frame(const Http2Frame &frame)
{
  ...
  if (this->get_continued_stream_id() != stream_id || stream_id == 0) {
    return Http2Error(Http2ErrorClass::HTTP2_ERROR_CLASS_CONNECTION, Http2ErrorCode::HTTP2_ERROR_PROTOCOL_ERROR,
