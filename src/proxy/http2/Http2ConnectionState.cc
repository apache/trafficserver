...
Http2ConnectionState::rcv_priority_frame(const Http2Frame &frame)
{
  ...
  if (configured_max_priority_frames_per_minute != 0 &&
    ...
    Counter::increment(http2_rsb.max_priority_frames_per_minute_exceeded);
...
Http2ConnectionState::rcv_rst_stream_frame(const Http2Frame &frame)
{
  ...
  if (configured_max_rst_stream_frames_per_minute != 0 &&
    ...
    Counter::increment(http2_rsb.max_rst_stream_frames_per_minute_exceeded);
...
Http2ConnectionState::rcv_settings_frame(const Http2Frame &frame)
{
  ...
  if (configured_max_settings_frames_per_minute != 0 &&
    ...
    Counter::increment(http2_rsb.max_settings_frames_per_minute_exceeded);
  ...
  while (nbytes < frame.header().length) {
    if (n_settings >= Http2::max_settings_per_frame) {
      Counter::increment(http2_rsb.max_settings_per_frame_exceeded);
  ...
  if (this->get_received_settings_count() > Http2::max_settings_per_minute) {
    Counter::increment(http2_rsb.max_settings_per_minute_exceeded);
...
Http2ConnectionState::rcv_ping_frame(const Http2Frame &frame)
{
  ...
  if (configured_max_ping_frames_per_minute != 0 && this->get_received_ping_frame_count() > configured_max_ping_frames_per_minute) {
    Counter::increment(http2_rsb.max_ping_frames_per_minute_exceeded);
...
Http2ConnectionState::create_stream(Http2StreamId new_id, Http2Error &error)
{
  ...
  if (check_max_concurrent_limit >= 0 && check_count >= check_max_concurrent_limit) {
    Counter::increment(is_client_streamid ? http2_rsb.max_concurrent_streams_exceeded_in :
...
Http2ConnectionState::send_rst_stream_frame(Http2StreamId id, Http2ErrorCode ec)
{
  ...
  if (ec != Http2ErrorCode::HTTP2_ERROR_NO_ERROR) {
    Counter::increment(http2_rsb.stream_errors_count);
    ++stream_error_count;
  }
...
Http2ConnectionState::send_goaway_frame(Http2StreamId id, Http2ErrorCode ec)
{
  ...
  if (ec != Http2ErrorCode::HTTP2_ERROR_NO_ERROR) {
    Counter::increment(http2_rsb.connection_errors_count);
  }
...
Http2ConnectionState::_adjust_concurrent_stream()
{
  ...
  int64_t current_client_streams = Counter::read(http2_rsb.current_client_stream_count);
...
Http2ConnectionState::increment_peer_rwnd(size_t amount)
{
  ...
  if (avg < Http2::min_avg_window_update) {
    Counter::increment(http2_rsb.insufficient_avg_window_update);
