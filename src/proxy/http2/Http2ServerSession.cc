...
Http2ServerSession::free()
{
  ...
  if (Http2CommonSession::common_free(this)) {
    Counter::decrement(http2_rsb.current_server_session_count);
...
Http2ServerSession::new_connection(NetVConnection *new_vc, MIOBuffer *iobuf, IOBufferReader *reader)
{
  ...
  Counter::increment(http2_rsb.current_server_session_count);
  Counter::increment(http2_rsb.total_server_connection_count);
