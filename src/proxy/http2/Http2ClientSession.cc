...
Http2ClientSession::free()
{
  ...
  if (Http2CommonSession::common_free(this)) {
    Counter::decrement(http2_rsb.current_client_session_count);
...
Http2ClientSession::new_connection(NetVConnection *new_vc, MIOBuffer *iobuf, IOBufferReader *reader)
{
  ...
  Counter::increment(http2_rsb.current_client_session_count);
  Counter::increment(http2_rsb.total_client_connection_count);
...
{
  Counter::increment(http2_rsb.current_active_client_connection_count);
}

void
Http2ClientSession::decrement_current_active_connections_stat()
{
  Counter::decrement(http2_rsb.current_active_client_connection_count);
}
