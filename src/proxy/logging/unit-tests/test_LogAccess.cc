...
void
set_socket_address(sockaddr_storage &addr, std::string_view text)
{
  REQUIRE(0 == ats_ip_pton(text, reinterpret_cast<sockaddr *>(&addr)));
}

void
populate_pre_transaction_data(LogAccess::PreTransactionLogData &data, std::string_view method, std::string_view scheme,
                              std::string_view authority, std::string_view path)
{
  initialize_headers_once();

  auto *heap = new_HdrHeap(HdrHeap::DEFAULT_SIZE + 64);
  data.client_request.create(HTTPType::REQUEST, HTTP_2_0, heap);
  data.client_request.method_set(method);
  data.has_client_request       = true;
  data.client_connection_is_ssl = true;
  data.log_code                 = SquidLogCode::ERR_INVALID_REQ;
  data.hit_miss_code            = SQUID_MISS_NONE;
  data.hier_code                = SquidHierarchyCode::NONE;
  data.server_transact_count    = 0;
  data.client_protocol          = "http/2";
  ...
  data.url = synthesize_target(method, scheme, authority, path);
  set_socket_address(data.client_addr, "192.0.2.10:4321"sv);

  add_header_field(data.client_request, PSEUDO_HEADER_METHOD, method);
  if (!scheme.empty()) {
    add_header_field(data.client_request, PSEUDO_HEADER_SCHEME, scheme);
  }
  if (!authority.empty()) {
    add_header_field(data.client_request, PSEUDO_HEADER_AUTHORITY, authority);
  }
  if (!path.empty()) {
    add_header_field(data.client_request, PSEUDO_HEADER_PATH, path);
  }
  add_header_field(data.client_request, static_cast<std::string_view>(MIME_FIELD_USER_AGENT), "TikTok/1.0");

  data.milestones[TS_MILESTONE_SM_START]            = ink_hrtime_from_msec(10);
  data.milestones[TS_MILESTONE_UA_BEGIN]            = ink_hrtime_from_msec(10);
  data.milestones[TS_MILESTONE_UA_FIRST_READ]       = ink_hrtime_from_msec(12);
  data.milestones[TS_MILESTONE_UA_READ_HEADER_DONE] = ink_hrtime_from_msec(14);
  data.milestones[TS_MILESTONE_SM_FINISH]           = ink_hrtime_from_msec(15);
}
