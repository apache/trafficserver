...
LogBufferManager::preproc_buffers(LogBufferSink *sink)
{
  ...
  while ((b = q.pop())) {
    if (b->m_references || b->m_state.s.num_writers) {
      ...
      Counter::increment(log_rsb.bytes_lost_before_preproc, b->header()->byte_count);
      delete b;
    } else {
      new_q.push(b);
    }
...
LogObjectManager::log(LogAccess *lad)
{
  ...
  if (unlikely(ret & Log::FAIL)) {
    Counter::increment(log_rsb.event_log_access_fail);
  } else if (unlikely(ret & Log::FULL)) {
    Counter::increment(log_rsb.event_log_access_full);
  } else if (likely(ret & Log::LOG_OK)) {
    Counter::increment(log_rsb.event_log_access_ok);
  } else if (unlikely(ret & Log::AGGR)) {
    Counter::increment(log_rsb.event_log_access_aggr);
