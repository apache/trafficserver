...
Log::access(LogAccess *lad)
{
  ...
  if (Log::config->sampling_frequency > 1) {
    ...
    if (this_sample && this_sample % Log::config->sampling_frequency) {
      ...
      Counter::increment(log_rsb.event_log_access_skip);
      ret = Log::SKIP;
      goto done;
    } else {
...
Log::va_error(const char *format, va_list ap)
{
  ...
  if (error_log) {
    ...
    switch (ret_val) {
    case Log::LOG_OK:
      Counter::increment(log_rsb.event_log_error_ok);
      break;
    case Log::SKIP:
      Counter::increment(log_rsb.event_log_error_skip);
      break;
    case Log::AGGR:
      Counter::increment(log_rsb.event_log_error_aggr);
      break;
    case Log::FULL:
      Counter::increment(log_rsb.event_log_error_full);
      break;
    case Log::FAIL:
      Counter::increment(log_rsb.event_log_error_fail);
      break;
    default:
  ...
  while (true) {
    ...
    while ((fdata = invert_link.pop())) {
      ...
      if (!logfile->is_open()) {
        ...
        Counter::increment(log_rsb.bytes_lost_before_written_to_disk, total_bytes);
        delete fdata;
        continue;
      }
      ...
      while (total_bytes - bytes_written) {
        if (Log::config->logging_space_exhausted) {
          ...
          Counter::increment(log_rsb.bytes_lost_before_written_to_disk, total_bytes - bytes_written);
          break;
        }
      ...
      Counter::increment(log_rsb.bytes_written_to_disk, bytes_written);
