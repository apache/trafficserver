...
TEST_CASE("UdiTable rotates through all occupied slots", "[tsutil][UdiTable][contest]")
{
  TestTable table(4);
  for (int i = 0; i < 4; ++i) {
    REQUIRE(table.process_event(std::to_string(i), 1));
  }
  REQUIRE(table.slots_used() == 4);
  for (int i = 4; i < 8; ++i) {
    REQUIRE(table.process_event(std::to_string(i), 100));
  }
  for (int i = 0; i < 4; ++i) {
    CHECK_FALSE(table.find(std::to_string(i)));
    CHECK(table.find(std::to_string(i + 4)));
  }
  auto snapshot = table.data_snapshot();
  REQUIRE(snapshot.size() == 4);
  table.clear();
  CHECK(table.slots_used() == 0);
  CHECK(table.data_snapshot().empty());
  CHECK(snapshot.front());
  CHECK(table.evictions() == 4);
}
