...
using AtomicStackWithOffset = AtomicStack_<AtomicStackBaseWithOffset>;
...
{
  return AtomicStackWithOffset(l->head, l->offset).pop<>();
}

void *
ink_atomiclist_popall(InkAtomicList *l)
{
  return AtomicStackWithOffset(l->head, l->offset).pop<true>();
}

void *
ink_atomiclist_push(InkAtomicList *l, void *item)
{
  return AtomicStackWithOffset(l->head, l->offset).push(item);
}

void *
ink_atomiclist_next(InkAtomicList *l, void *item)
{
  return item ? AtomicStackWithOffset(l->head, l->offset).link(item) : nullptr;
}

void *
ink_atomiclist_remove(InkAtomicList *l, void *item)
{
  return AtomicStackWithOffset(l->head, l->offset).remove(item) ? item : nullptr;
}
