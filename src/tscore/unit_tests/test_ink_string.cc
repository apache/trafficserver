...
constexpr uint64_item uint64_tests[] = {
  {{0},                     {"0"}                   },
  {{1},                     {"1"}                   },
  {{10},                    {"10"}                  },
  {{100},                   {"100"}                 },
  ...
  {{100000000000000000},    {"100000000000000000"}  },
  {{1000000000000000000},   {"1000000000000000000"} },
  {{10000000000000000000U}, {"10000000000000000000"}},
  {{UINT64_MAX},            {"18446744073709551615"}},
};

TEST_CASE("ink_fast_ultoa", "[libts][ink_fast_ultoa]")
{
  char buffer[21];
  for (auto const &test : uint64_tests) {
    REQUIRE(ink_atoui64(test.s.data()) == test.n);
    int length = 0;
    REQUIRE((length = ink_fast_ultoa(test.n, buffer, sizeof(buffer))) == (int)test.s.length());
    REQUIRE(std::string_view(buffer, length) == test.s);
  }
}
