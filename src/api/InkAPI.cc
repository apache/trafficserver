...
TSHttpHdrStatusSet(TSMBuffer bufp, TSMLoc obj, TSHttpStatus status, TSHttpTxn txnp, std::string_view setter)
{
  ...
  if (txnp != nullptr && !setter.empty()) {
    ...
    TSHttpStatusSetterSet(txnp, setter);
  }
  return TS_SUCCESS;
}
...
void
TSHttpStatusSetterSet(TSHttpTxn txnp, std::string_view setter)
{
  sdk_assert(sdk_sanity_check_txn(txnp) == TS_SUCCESS);
  ...
  if (!setter.empty()) {
    ...
    sm->t_state.http_return_code_setter_name.clear();
  }
}

std::string_view
TSHttpStatusSetterGet(TSHttpTxn txnp)
{
  sdk_assert(sdk_sanity_check_txn(txnp) == TS_SUCCESS);

  HttpSM            *sm  = reinterpret_cast<HttpSM *>(txnp);
  const std::string &tag = sm->t_state.http_return_code_setter_name;
