...
TSSslClientCertUpdate(const char *cert_path, const char *key_path)
{
  ...
  for (auto const &ca_paths_key : ca_paths_keys) {
    ...
    shared_SSL_CTX client_ctx(SSLCreateClientContext(params, ca_bundle_path.empty() ? nullptr : ca_bundle_path.c_str(),
                                                     ca_bundle_file.empty() ? nullptr : ca_bundle_file.c_str(), cert_path,
                                                     key_path),
                              SSL_CTX_free);
