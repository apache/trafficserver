...
TEST_CASE("CacheConfigParser parses legacy cache.config", "[cache][config][legacy]")
{
  ...
  CacheConfigParser::Format format;
  auto                      result = parser.parse_content(LEGACY_CONFIG, "cache.config", &format);

  REQUIRE(result.ok());
  CHECK(format == CacheConfigParser::Format::Legacy);
...
TEST_CASE("CacheConfigParser parses cache.yaml", "[cache][config][yaml]")
{
  CacheConfigParser         parser;
  CacheConfigParser::Format format;
  auto                      result = parser.parse_content(YAML_CONFIG, "cache.yaml", &format);

  REQUIRE(result.ok());
  CHECK(format == CacheConfigParser::Format::YAML);
  REQUIRE(result.value.size() == 3);
...
TEST_CASE("CacheConfigMarshaller round trips legacy rules through YAML", "[cache][config][marshaller]")
{
  ...
  CHECK(yaml.ends_with('\n'));
  CHECK_FALSE(yaml.ends_with("\n\n"));
...
cache:
  ...
  auto embedded_newline = parser.parse_content(R"(
cache:
  - match:
    ...
    action:
      cache: never
                                               ...
                                               "cache.yaml");
  CHECK_FALSE(embedded_newline.ok());
}
