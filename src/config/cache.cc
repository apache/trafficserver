...
CacheConfigParser::parse(std::string const &filename, Format *format) const
{
  std::error_code ec;
  ...
  return parse_content(content, filename, format);
}

ConfigResult<CacheConfig>
CacheConfigParser::parse_content(std::string_view content, std::string_view filename, Format *format) const
{
  Format const detected_format = detect_format(content, filename);
  ...
  return detected_format == Format::YAML ? parse_yaml(content) : parse_legacy(content);
}
