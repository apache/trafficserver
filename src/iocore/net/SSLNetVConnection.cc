...
SSLNetVConnection::_ssl_read_from_net(EThread *lthread, int64_t &ret)
{
  ...
  while (sslErr == SSL_ERROR_NONE && bytes_read < toread) {
    ...
    case SSL_ERROR_SYSCALL:
      if (nread != 0) {
        // not EOF
        Counter::increment(ssl_rsb.error_syscall);
    ...
    default: {
      ...
      Counter::increment(ssl_rsb.error_ssl);
    } break;
    } // switch
  }   // while
    ...
    if (redoWriteSize) {
      ...
      if (SSLConfigParams::ssl_maxrecord > 0 && l > SSLConfigParams::ssl_maxrecord) {
        ...
        if (sslTotalBytesSent < SSL_DEF_TLS_RECORD_BYTE_THRESHOLD) {
          ...
          Counter::increment(ssl_rsb.total_dyn_def_tls_record_count);
        } else {
          dynamic_tls_record_size = SSL_MAX_TLS_RECORD_SIZE;
          Counter::increment(ssl_rsb.total_dyn_max_tls_record_count);
        }
    ...
    if (this->ssl == nullptr) {
      ...
      if (tlsext_host_name) {
        if (SSL_set_tlsext_host_name(this->ssl, tlsext_host_name)) {
          ...
          Counter::increment(ssl_rsb.sni_name_set_failure);
        }
      }
...
SSLNetVConnection::sslServerHandShakeEvent(int &err)
{
  ...
  if (sslHandshakeHookState == HANDSHAKE_HOOKS_PRE) {
    Counter::increment(ssl_rsb.total_attempts_handshake_count_in);
    if (!curHook) {
  ...
  case SSL_ERROR_NONE:
    ...
    if (this->get_tls_handshake_begin_time()) {
      ...
      Counter::increment(ssl_rsb.total_success_handshake_count_in);
    }
...
// enable the sni callback to break out of the SSL_accept processing
  ...
  case SSL_ERROR_WANT_ASYNC:
    Counter::increment(ssl_rsb.error_async);
...
SSLNetVConnection::sslClientHandShakeEvent(int &err)
{
  ...
  if (sslHandshakeHookState == HANDSHAKE_HOOKS_OUTBOUND_PRE) {
    Counter::increment(ssl_rsb.total_attempts_handshake_count_out);
    if (!curHook) {
  ...
  case SSL_ERROR_NONE:
    ...
    Counter::increment(ssl_rsb.total_success_handshake_count_out);
...
SSLNetVConnection::_out_context_tunnel()
{
  ...
  Counter::increment(net_rsb.tunnel_total_server_connections_tls);
  Counter::increment(net_rsb.tunnel_current_server_connections_tls);
}
...
SSLNetVConnection::increment_ssl_version_metric(int version) const
{
  switch (version) {
  case SSL3_VERSION:
    Counter::increment(ssl_rsb.total_sslv3);
    break;
  case TLS1_VERSION:
    Counter::increment(ssl_rsb.total_tlsv1);
    break;
  case TLS1_1_VERSION:
    Counter::increment(ssl_rsb.total_tlsv11);
    break;
  case TLS1_2_VERSION:
    Counter::increment(ssl_rsb.total_tlsv12);
    break;
#ifdef TLS1_3_VERSION
  case TLS1_3_VERSION:
    Counter::increment(ssl_rsb.total_tlsv13);
    break;
#endif
  default:
    ...
    while (true) {
      ...
      if (had_error_on_reading_early_data) {
        ...
        if (nread > 0) {
          ...
          Counter::increment(ssl_rsb.early_data_received_count);
...
SSLNetVConnection::_ssl_connect()
{
  ...
  if (ret > 0) {
    if (SSL_session_reused(ssl)) {
      Counter::increment(ssl_rsb.origin_session_reused_count);
