...
public:
  ...
  check_inactivity(int event, Event *e)
  {
    ...
    while (NetEvent *ne = nh.cop_list.pop()) {
      ...
      if (!lock.is_locked()) {
        Counter::increment(net_rsb.inactivity_cop_lock_acquire_failure);
        continue;
      }
      ...
      if (ne->next_inactivity_timeout_at == 0 && ne->default_inactivity_timeout_in > 0 && (ne->read.enabled || ne->write.enabled)) {
        ...
        Counter::increment(net_rsb.default_inactivity_timeout_applied);
      }
      ...
      if (ne->next_inactivity_timeout_at && ne->next_inactivity_timeout_at < now) {
        if (ne->is_default_inactivity_timeout()) {
          ...
          Counter::increment(net_rsb.default_inactivity_timeout_count);
        }
        if (nh.keep_alive_queue.in(ne)) {
          ...
          Counter::increment(net_rsb.keep_alive_queue_timeout_total, diff);
          Counter::increment(net_rsb.keep_alive_queue_timeout_count);
        }
