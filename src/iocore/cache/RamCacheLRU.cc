...
RamCacheLRU::get(CryptoHash *key, Ptr<IOBufferData> *ret_data, uint64_t auxkey)
{
  ...
  while (e) {
    if (e->key == *key && e->auxkey == auxkey) {
      ...
      lru.enqueue(e);
      (*ret_data) = e->data;
      DDbg(get_dbg_ram_cache(), "get %X %" PRIu64 " HIT", key->slice32(3), auxkey);
  ...
  DDbg(get_dbg_ram_cache(), "get %X %" PRIu64 " MISS", key->slice32(3), auxkey);
...
RamCacheLRU::remove(RamCacheLRUEntry *e)
{
  ...
  DDbg(get_dbg_ram_cache(), "put %X %" PRIu64 " FREED", e->key.slice32(3), e->auxkey);
  e->data = nullptr;
...
RamCacheLRU::put(CryptoHash *key, IOBufferData *data, [[maybe_unused]] uint32_t len, bool, uint64_t auxkey)
{
  ...
  if ((cache_config_ram_cache_use_seen_filter == 1) ||
    ...
    if (!(*seen)[j]) {
      DDbg(get_dbg_ram_cache(), "put %X %" PRIu64 " len %d UNSEEN", key->slice32(3), auxkey, len);
      (*seen)[j] = true;
      return 0;
    } else {
  ...
  DDbg(get_dbg_ram_cache(), "put %X %" PRIu64 " INSERTED", key->slice32(3), auxkey);
