...
CacheDisk::incrErrors(const AIOCallback *io)
{
  if (0 == this->num_errors) {
    ...
    Counter::decrement(cache_rsb.span_online);
    Counter::increment(cache_rsb.span_failing);
  }
  this->num_errors++;
  ...
  case LIO_READ:
    opname = "READ";
    Counter::increment(cache_rsb.span_errors_read);
    break;
  case LIO_WRITE:
    opname = "WRITE";
    Counter::increment(cache_rsb.span_errors_write);
    break;
  default:
    break;
  }
