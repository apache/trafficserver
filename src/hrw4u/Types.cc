...
var_type_info(VarType type)
{
  static const VarTypeInfo info_table[] = {
    {.name = "bool",  .cond_tag = "FLAG",  .op_tag = "set-state-flag",  .op_type = hrw::OperatorType::SET_STATE_FLAG,  .limit = 16},
    {.name = "int8",  .cond_tag = "INT8",  .op_tag = "set-state-int8",  .op_type = hrw::OperatorType::SET_STATE_INT8,  .limit = 4 },
    {.name = "int16", .cond_tag = "INT16", .op_tag = "set-state-int16", .op_type = hrw::OperatorType::SET_STATE_INT16, .limit = 1 },
  };
...
namespace
{
  ...
  const std::vector<std::string_view> NEXTHOP_FIELDS_VEC = {"host", "port", "strategy"};
...
validate_suffix(SuffixGroup group, std::string_view suffix)
{
  ...
  return std::ranges::find(valid, to_lower(suffix)) != valid.end();
}
...
get_valid_suffixes(SuffixGroup group)
{
  ...
  case SuffixGroup::NEXTHOP_FIELDS:
    return NEXTHOP_FIELDS_VEC;
  }
  return EMPTY_VEC;
}
