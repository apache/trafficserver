...
struct ProcedureParam {
  std::string                name;
  hrw4uParser::ValueContext *default_ctx = nullptr;
};

struct ProcedureSignature {
  std::string                 qualified_name;
  std::vector<ProcedureParam> params;
  hrw4uParser::BlockContext  *body_ctx = nullptr;
  std::string                 source_file;

  std::shared_ptr<antlr4::ANTLRInputStream>  input_stream;
  std::shared_ptr<hrw4uLexer>                lexer;
  std::shared_ptr<antlr4::CommonTokenStream> token_stream;
  std::shared_ptr<hrw4uParser>               parser;
};
...
public:
  ...
  std::any visitSessionVarSection(hrw4uParser::SessionVarSectionContext *ctx) override;
  std::any visitUseDirective(hrw4uParser::UseDirectiveContext *ctx) override;
  std::any visitProcedureDecl(hrw4uParser::ProcedureDeclContext *ctx) override;
...
private:
  ...
  void  set_conjunction(bool last, bool followed_by_or);
  ...
  void                      track_object(void *obj, ObjKind kind);
  ResolveResult             resolve_identifier(const std::string &ident);
  [[nodiscard]] std::string get_source_line(size_t line_number) const;

  // Procedure support
  std::string resolve_use_path(const std::string &spec);
  void        load_proc_file(const std::string &path, std::vector<std::string> &load_stack, const std::string &use_spec = "");
  std::vector<ProcedureParam> collect_proc_params(hrw4uParser::ParamListContext *ctx);
  void                        expand_procedure_call(const std::string &name, hrw4uParser::ArgumentListContext *args_ctx,
  ...
  std::vector<std::pair<void *, ObjKind>> _allocated_objects;
  ...
  VarScope      _current_var_scope = VarScope::TXN;
  bool          _section_has_ops   = false;

  std::map<std::string, ProcedureSignature> _proc_registry;
  std::set<std::string>                     _proc_loaded;
  std::map<std::string, std::string>        _proc_bindings;
  std::vector<std::string>                  _proc_call_stack;
