...
SymbolResolver::SymbolResolver()
{
  ...
  _condition_map = {
    ...
    {"capture.",                       {.cond_type = hrw::ConditionType::COND_LAST_CAPTURE, .prefix = true}                            },
    {"from.url.query.",
     {.target = "FROM-URL:QUERY", .sections = HTTP_SECTIONS, .cond_type = hrw::ConditionType::COND_FROM_URL, .prefix = true}           },
    {"from.url.",
     {.sections              = HTTP_SECTIONS,
      .suffix_group          = SuffixGroup::URL_FIELDS,
    ...
    {"inbound.conn.",
     {.target                = "INBOUND",
      ...
      .prefix                = true,
      .has_suffix_validation = true}                                                                                                   },
    {"inbound.cookie.",                {.sections = HTTP_SECTIONS, .cond_type = hrw::ConditionType::COND_COOKIE, .prefix = true}       },
    {"inbound.req.",                   {.sections = HTTP_SECTIONS, .cond_type = hrw::ConditionType::COND_CLIENT_HEADER, .prefix = true}},
    {"inbound.resp.",
     {.sections  = {SectionType::READ_RESPONSE, SectionType::SEND_RESPONSE, SectionType::TXN_CLOSE},
      .cond_type = hrw::ConditionType::COND_HEADER,
      .prefix    = true}                                                                                                               },
    {"inbound.url.query.",
     {.target = "CLIENT-URL:QUERY", .sections = HTTP_SECTIONS, .cond_type = hrw::ConditionType::COND_CLIENT_URL, .prefix = true}       },
    {"inbound.url.",
     {.sections              = HTTP_SECTIONS,
      .suffix_group          = SuffixGroup::URL_FIELDS,
      ...
      .has_suffix_validation = true}                                                                                                   },
    {"nexthop.",
     {.sections              = {SectionType::SEND_REQUEST, SectionType::READ_RESPONSE, SectionType::SEND_RESPONSE},
      .suffix_group          = SuffixGroup::NEXTHOP_FIELDS,
    ...
    {"outbound.url.",
     {.target                = "SERVER-URL",
      ...
      .has_suffix_validation = true}                                                                                                   },
    {"to.url.query.",
     {.target = "TO-URL:QUERY", .sections = HTTP_SECTIONS, .cond_type = hrw::ConditionType::COND_TO_URL, .prefix = true}               },
    {"to.url.",
     {.sections              = HTTP_SECTIONS,
      .suffix_group          = SuffixGroup::URL_FIELDS,
  ...
  _function_map = {
    {"access",        {.cond_type = hrw::ConditionType::COND_ACCESS, .bare = true}                },
    {"cache",         {.cond_type = hrw::ConditionType::COND_CACHE, .bare = true}                 },
    {"cidr",          {.cond_type = hrw::ConditionType::COND_CIDR, .bare = true}                  },
    {"internal",      {.cond_type = hrw::ConditionType::COND_INTERNAL_TXN, .bare = true}          },
    {"random",        {.cond_type = hrw::ConditionType::COND_RANDOM, .bare = true}                },
    {"ssn-txn-count", {.cond_type = hrw::ConditionType::COND_SESSION_TRANSACT_COUNT, .bare = true}},
    {"txn-count",     {.cond_type = hrw::ConditionType::COND_TRANSACT_COUNT, .bare = true}        },
  };
                                 ...
                                 SectionType section) const
{
  ...
  auto          it           = table.find(symbol_lower);
  ...
  for (const auto &[prefix, params] : table) {
    if (prefix.back() == '.' && symbol_lower.size() >= prefix.size() && symbol_lower.starts_with(prefix)) {
...
{
  auto result = section_type_from_string(name);

  if (result != SectionType::UNKNOWN) {
    return result;
  }
...
{
  return var_type_from_string(name);
}
