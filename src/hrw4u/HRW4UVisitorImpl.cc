...
HRW4UVisitorImpl::get_or_create_ruleset()
{
  if (_current_ruleset == nullptr && _callbacks.create_ruleset) {
    ...
    track_object(_current_ruleset, ObjKind::RULESET);
...
HRW4UVisitorImpl::create_condition(const ParserContext &pctx)
{
  ...
  if (cond) {
    track_object(cond, ObjKind::CONDITION);
  }
  return cond;
}
...
HRW4UVisitorImpl::create_operator(const ParserContext &pctx)
{
  ...
  if (op) {
    track_object(op, ObjKind::OPERATOR);
  }
  return op;
}
...
HRW4UVisitorImpl::cleanup_on_error()
{
  if (_callbacks.destroy) {
    for (auto &[obj, kind] : _allocated_objects) {
      if (obj && kind == ObjKind::RULESET) {
        _callbacks.destroy(obj, kind);
      }
    }
  }
...
void
HRW4UVisitorImpl::track_object(void *obj, ObjKind kind)
{
  if (obj) {
    _allocated_objects.emplace_back(obj, kind);
  }
}
  ...
  if (is_section_level && !has_elif_else) {
    ...
    if (!has_elif_else) {
      ...
      if (_callbacks.create_if_operator) {
        ...
        if (op_if) {
          track_object(op_if, ObjKind::OPERATOR_IF);
        }
      }
...
void
HRW4UVisitorImpl::set_conjunction(bool last, bool followed_by_or)
{
  if (!last) {
    if (followed_by_or) {
...
HRW4UVisitorImpl::process_factor(hrw4uParser::FactorContext *ctx, bool last, bool followed_by_or, bool negated)
{
  ...
  if (ctx->LPAREN()) {
    set_conjunction(last, followed_by_or);
