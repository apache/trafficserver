...
on_send_response_header(TSHttpTxn txnp, TSCont contp, PurgeInstance *purge)
{
  ...
  if (TS_SUCCESS == TSHttpTxnClientRespGet(txnp, &bufp, &hdr_loc)) {
    ...
    TSHttpHdrStatusSet(bufp, hdr_loc, TS_HTTP_STATUS_OK, PLUGIN_NAME);
...
TSRemapNewInstance(int argc, char *argv[], void **ih, char * /* errbuf ATS_UNUSED */, int /* errbuf_size ATS_UNUSED */)
{
  ...
  static const struct option longopt[] = {
    ...
    {(char *)"header",     required_argument, nullptr, 'h' },
    {(char *)"state-file", required_argument, nullptr, 'f' },
    {(char *)"allow-get",  no_argument,       nullptr, 'a' },
  ...
  if ((nullptr == purge->secret) || (nullptr == purge->state_file) || !purge->secret_len) {
