...
parse_jwt(json_t *raw)
{
  ...
  struct jwt *jwt = static_cast<struct jwt *>(malloc(sizeof *jwt));
  jwt->raw        = raw;
...
jwt_check_uri(const char *cdniuc, const char *uri)
{
  ...
  // Block ending at fail_jwt label.
  {
...
renew(struct jwt *jwt, const char *iss, cjose_jwk_t *jwk, const char *alg, const char *package, const char *uri, size_t uri_ct)
{
  ...
  if (!normal_err) {
    ...
    // Block ending at fail_path label.
    {
      ...
      cjose_header_t *hdr = cjose_header_new(nullptr);
      if (!hdr) {
