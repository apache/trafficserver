...
void shutdown(TSCont contp, Data* const data)
{
...
int request_block(TSCont contp, Data* const data)
{
...
int handle_client_req(TSCont contp, TSEvent event, Data* const data)
{
  if (TS_EVENT_VCONN_READ_READY == event ||
    ...
    if (TS_PARSE_DONE !=
        data->m_req_hdrmgr.populateFrom(data->m_http_parser,
...
*/
    ...
    header.setKeyVal(TS_MIME_FIELD_HOST, TS_MIME_LEN_HOST,
    ...
    if (rstat) {
      ...
      header.setKeyVal(SLICER_MIME_FIELD_INFO,
      ...
      header.setKeyVal(SLICER_MIME_FIELD_INFO,
                       strlen(SLICER_MIME_FIELD_INFO), valstr, vallen);
      data->m_statustype = TS_HTTP_STATUS_OK;
    }
    ...
    header.removeKey(TS_MIME_FIELD_X_FORWARDED_FOR,
...
int64_t transfer_content_bytes(Data* const data)
{
  ...
  if (0 < read_avail) {
    ...
    int64_t const copied(TSIOBufferCopy(data->m_dnstream.m_write.m_iobuf,
...
void form416HeaderAndBody(HttpHeader& header, int64_t const contentlen,
                          std::string const& bodystr)
{
  ...
  header.setKeyVal(TS_MIME_FIELD_CONTENT_LENGTH,
                   TS_MIME_LEN_CONTENT_LENGTH, bufstr, buflen);

  static char const* const ctypestr = "text/html";
  static int const ctypelen = strlen(ctypestr);
  header.setKeyVal(TS_MIME_FIELD_CONTENT_TYPE, TS_MIME_LEN_CONTENT_TYPE,
                   ctypestr, ctypelen);

  buflen = snprintf(bufstr, 255, "*/%" PRId64, contentlen);
  header.setKeyVal(TS_MIME_FIELD_CONTENT_RANGE, TS_MIME_LEN_CONTENT_RANGE,
                   bufstr, buflen);
}

// this is called every time the server has data for us
int handle_server_resp(TSCont contp, TSEvent event, Data* const data)
{
  if (data->m_bail) {
  ...
  if (TS_EVENT_VCONN_READ_READY == event ||
    ...
    if (!data->m_server_block_header_parsed) {
      ...
      */
      ...
      if (!header.valueForKey(TS_MIME_FIELD_CONTENT_RANGE,
                              TS_MIME_LEN_CONTENT_RANGE, rangestr,
                              &rangelen)) {
        /*
      ...
      if (!data->m_server_first_header_parsed) {
        ...
        if (data->m_req_range.isEndBytes())  // special last N bytes
        {
          ...
          data->m_req_range.m_beg =
              std::max((int64_t)0, data->m_req_range.m_beg);
        }
        else {
          // fix up request range end
          data->m_req_range.m_end =
              std::min(data->m_contentlen, data->m_req_range.m_end);
        }
        ...
        if (data->m_bytestosend <= 0)  // assume 416 needs to be sent
        {
          ...
          TSIOBufferWrite(data->m_dnstream.m_write.m_iobuf, bodystr.data(),
                          bodystr.size());
        ...
        else if (TS_HTTP_STATUS_PARTIAL_CONTENT == data->m_statustype) {
          ...
          header.setKeyVal(TS_MIME_FIELD_CONTENT_RANGE,
          ...
          header.setReason(reason, strlen(reason));
          header.removeKey(TS_MIME_FIELD_CONTENT_RANGE,
                           TS_MIME_LEN_CONTENT_RANGE);
        }

        char bufstr[256];
        int buflen =
            snprintf(bufstr, 255, "%" PRId64, data->m_bytestosend);
        header.setKeyVal(TS_MIME_FIELD_CONTENT_LENGTH,
                         TS_MIME_LEN_CONTENT_LENGTH, bufstr, buflen);

        // add the response header length to the total bytes to send
        data->m_bytestosend +=
            TSHttpHdrLengthGet(header.m_buffer, header.m_lochdr);
    ...
    if (!data->m_client_header_sent) {
      ...
      // write the (previously) manipulated server resp header to the
      // client
      HttpHeader header(data->m_resp_hdrmgr.m_buffer,
                        data->m_resp_hdrmgr.m_lochdr);
    ...
    if (adjusted || data->m_req_range.blockIsInside(data->m_blockbytes,
                                                    data->m_blocknum)) {
...
int handle_client_resp(TSCont contp, TSEvent event, Data* const data)
{
  if (data->m_bail) {
    shutdown(contp, data);
    return TS_EVENT_CONTINUE;
  ...
  {
    DEBUG_LOG(
        "got a TS_EVENT_ERROR from the client -- it probably bailed");
    /*
    std::cerr << __func__ << ": " << "TS_EVENT_ERROR" << std::endl;
    std::cerr << "bytes: sent " << bytessent << " of " <<
    data->m_bytestosend << std::endl;
    */
    shutdown(contp, data);
  }
...
int intercept_hook(TSCont contp, TSEvent event, void* edata)
{
  ...
  else if (TS_EVENT_VCONN_INACTIVITY_TIMEOUT == event ||
