...
/**
  represents value parsed from a blocked Content-Range reponse header
  field. Range is converted from closed range into a half open range for.
 */
struct ContentRange {
  int64_t m_beg;
  ...
  explicit ContentRange(int64_t const begin, int64_t const end,
                        int64_t const len)
      : m_beg(begin), m_end(end), m_length(len)
  {
  }
  ...
  int64_t rangeSize() const { return m_end - m_beg; }
};
