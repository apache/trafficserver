...
struct Channel {
  ...
  bool setForRead(TSVConn vc, TSCont contp,
  ...
  bool setForWrite(TSVConn vc, TSCont contp,
...
struct Stage  // upstream or downstream (server or client)
{
  ...
  void setupConnection(TSVConn vc)
  {
    if (nullptr != m_vc) {
      TSVConnClose(m_vc);
    }
  ...
  void setupVioRead(TSCont contp, int64_t const bytesin = INT64_MAX)
  {
    m_read.setForRead(m_vc, contp, bytesin);
  }

  void setupVioWrite(TSCont contp, int64_t const bytesout = INT64_MAX)
  {
