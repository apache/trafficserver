...
std::string testContentRange()
{
  ...
  if (!exprange.isValid()) {
    ...
    oss << exprange.m_beg << ' ' << exprange.m_end << ' '
        << exprange.m_length << std::endl;
  }
  ...
  else if (gotrange.m_beg != exprange.m_beg ||
...
std::string testParseRange()
{
  ...
  std::vector<bool> const expsres = {true, true, true, true, false,
  ...
  for (size_t index(0); index < gots.size(); ++index) {
    if (exps[index] != gots[index] || expsres[index] != gotsres[index]) {
      ...
      oss << "exp: " << exps[index].m_beg << ' ' << exps[index].m_end
          << std::endl;
      oss << "expsres: " << (int)expsres[index] << std::endl;
      oss << "got: " << gots[index].m_beg << ' ' << gots[index].m_end
          << std::endl;
...
struct Tests {
  ...
  void add(TestFunc const &func, char const *const fname)
  {
