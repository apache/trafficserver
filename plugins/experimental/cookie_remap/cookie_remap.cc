    ...
    for (SubOpQueue::const_iterator it = subops.begin(); it != subops.end(); ++it) {
      ...
      if (c.length()) {
        ...
        if (cookie_found == false) { // cookie name or sub-key not found
          ...
          if (subop_type == NOTEXISTS) {
            TSDebug(MY_NAME, "cookie %s was not "
                             "found (and we wanted "
                             "that)",
                    object_name.c_str());
          ...
          if (subop_type == NOTEXISTS) { // we found the cookie
                                         // but are asking
            // for non existence
            TSDebug(MY_NAME, "cookie %s was found, "
                             "but operation "
                             "requires "
                             "non-existence",
                    object_name.c_str());
      ...
      if (subop_type == REGEXP) {
        ...
        if (ret >= 0) {
          ...
          TSDebug(MY_NAME, "successful regex "
                           "match of: %s with %s "
                           "rewriting string: %s",
          ...
          TSDebug(MY_NAME, "could not match "
                           "regular expression "
                           "%s to %s",
      ...
      if (subop_type == BUCKET) {
        ...
        if (taking == 0 || out_of == 0) {
          TSDebug(MY_NAME, "taking %d out of %d "
                           "makes no sense?!",
                  taking, out_of);
          retval &= 0;
          break;
        ...
        TSDebug(MY_NAME, "we hashed this to bucket: %u "
                         "taking: %u out of: %u",
                hash, taking, out_of);
