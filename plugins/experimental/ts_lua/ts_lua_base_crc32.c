 ...
 * the additional code.  For example, ts_crc32_short() of 16 bytes of data
 * takes half as much CPU clocks than ts_crc32_long().
 */

static uint32_t ts_crc32_table16[] = {0x00000000, 0x1db71064, 0x3b6e20c8, 0x26d930ac, 0x76dc4190, 0x6b6b51f4,
                                      0x4db26158, 0x5005713c, 0xedb88320, 0xf00f9344, 0xd6d6a3e8, 0xcb61b38c,
                                      0x9b64c2b0, 0x86d3d2d4, 0xa00ae278, 0xbdbdf21c};

uint32_t ts_crc32_table256[] = {
...
uint32_t *ts_crc32_table_short = ts_crc32_table16;

int
ts_crc32_table_init(void)
{
  void *p;

  if (((unsigned long)ts_crc32_table_short & ~((unsigned long)128 - 1)) == (unsigned long)ts_crc32_table_short) {
    return TS_SUCCESS;
  }

  p = TSmalloc(16 * sizeof(uint32_t) + 128);
  if (p == NULL) {
    return TS_ERROR;
  }

  p = (unsigned char *)(((unsigned long)(p) + ((unsigned long)128 - 1)) & ~((unsigned long)128 - 1));

  memcpy(p, ts_crc32_table16, 16 * sizeof(uint32_t));

  ts_crc32_table_short = p;
