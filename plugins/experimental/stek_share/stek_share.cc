...
cert_verification(const std::string &sn)
{
  if (sn.compare(stek_share_server.cert_verify_str_) == 0) {
    TSDebug(PLUGIN, "SSL cert verified");
    return true;
  }
  TSDebug(PLUGIN, "SSL cert not verified");
  return false;
}
...
{
  // Get server ID.
  YAML::Node server_conf = YAML::LoadFile(argv[1]);
  ...
  if (server_conf["server_list_file"]) {
    YAML::Node server_list       = YAML::LoadFile(server_conf["server_list_file"].as<std::string>());
...
stek_updater(void *arg)
{
  ...
  if (create_new_stek(&curr_stek, 0, 1) == 0) {
    ...
    TSSslTicketKeyUpdate(reinterpret_cast<char *>(stek_share_server.ticket_keys_), SSL_TICKET_KEY_SIZE);
