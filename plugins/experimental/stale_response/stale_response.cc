...
parse_args(int argc, char const *argv[])
{
  ...
  SRDBG(TAG, "[%s] global stale if error override = %d", __FUNCTION__, (int)plugin_config->stale_if_error_override);
  SRDBG(TAG, "[%s] global stale while revalidate override = %d", __FUNCTION__, (int)plugin_config->stale_while_revalidate_override);
  SRDBG(TAG, "[%s] global stale if error default = %d", __FUNCTION__, (int)plugin_config->stale_if_error_default);
  SRDBG(TAG, "[%s] global stale while revalidate default = %d", __FUNCTION__, (int)plugin_config->stale_while_revalidate_default);
