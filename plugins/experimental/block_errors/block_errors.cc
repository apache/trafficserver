...
std::string &
ipaddr_to_string(const swoc::IPAddr &ip, std::string &address)
{
  swoc::LocalBufferWriter<128> writer;
  writer.print("{}", ip);
  address = writer.view();
  ...
  clean()
  {
    ...
    for (auto item = _table.begin(); item != _table.end();) {
      if (item->second._count <= RESET_LIMIT || item->second._cycles >= TIMEOUT_CYCLES) {
        ...
        Dbg(dbg_ctl_clean, "ip=%s count=%d removing", ipaddr_to_string(item->first, address).c_str(), item->second._count);
        ...
        ++item->second._cycles;
        Dbg(dbg_ctl_clean, "ip=%s count=%d incrementing cycles=%d", ipaddr_to_string(item->first, address).c_str(),
...
handle_start_hook(TSCont *contp, TSEvent event, void *edata)
{
  ...
  if (TSVConnIsSsl(vconn) == 0) {
    Dbg(dbg_ctl, "not a ssl connection");
    TSVConnReenable(vconn);
    return 0;
  }
  ...
  if (count > RESET_LIMIT) {
    ...
    if (shutdown_connection == true) {
      ...
      Dbg(dbg_ctl, "ip=%s count=%d is over the limit, shutdown connection on start", ipaddr_to_string(ipaddr, address).c_str(),
          count);
      ...
      Dbg(dbg_ctl, "ip=%s count=%d is over the limit, downgrading connection", ipaddr_to_string(ipaddr, address).c_str(), count);
...
handle_close_hook(TSCont *contp, TSEvent event, void *edata)
{
  ...
  if (transaction.cls != 0 || session.cls != 0 || transaction.code != 0 || session.code != 0) {
    Dbg(dbg_ctl, "transaction error class=%d code=%" PRIu64 " session error class=%d code=%" PRIu64, transaction.cls,
...
TSPluginInit(int argc, const char *argv[])
{
  ...
  if (argc == 5) {
    ...
    Dbg(dbg_ctl, "block_errors: invalid number of arguments, using the defaults - usage: block_errors.so <reset limit> <timeout "
