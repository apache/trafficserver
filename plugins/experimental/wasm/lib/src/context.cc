...
WasmResult
BufferBase::copyTo(WasmBase *wasm, size_t start, size_t length, uint64_t ptr_ptr, uint64_t size_ptr) const
{
  if (owned_data_) {
...
std::string
PluginBase::makeLogPrefix() const
{
  std::string prefix;
  if (!name_.empty()) {
    prefix = prefix + " " + name_;
...
ContextBase::ContextBase(WasmBase *wasm, uint32_t parent_context_id, const std::shared_ptr<PluginHandleBase> &plugin_handle)
  : wasm_(wasm),
    id_(wasm != nullptr ? wasm->allocContextId() : 0),
    parent_context_id_(parent_context_id),
    plugin_(plugin_handle->plugin()),
...
WasmVm *
ContextBase::wasmVm() const
{
  return wasm_->wasm_vm();
}

bool
ContextBase::isFailed()
{
  return (wasm_ == nullptr || wasm_->isFailed());
}

std::string
ContextBase::makeRootLogPrefix(std::string_view vm_id) const
{
  std::string prefix;
  if (!root_id_.empty()) {
    prefix = prefix + " " + std::string(root_id_);
...
//
bool
ContextBase::onStart(std::shared_ptr<PluginBase> plugin)
{
  ...
  if (wasm_->on_vm_start_) {
    ...
    result = wasm_->on_vm_start_(this, id_, static_cast<uint32_t>(wasm()->vm_configuration().size())).u64_ != 0;
  }
  return result;
}

bool
ContextBase::onConfigure(std::shared_ptr<PluginBase> plugin)
{
  if (isFailed()) {
    return true;
  }
  ...
  DeferAfterCallActions actions(this);
  temp_plugin_ = plugin;
  auto result  = wasm_->on_configure_(this, id_, static_cast<uint32_t>(plugin->plugin_configuration_.size())).u64_ != 0;
  temp_plugin_.reset();
  return result;
}

void
ContextBase::onCreate()
{
...
// Shared Data
WasmResult
ContextBase::getSharedData(std::string_view key, std::pair<std::string, uint32_t> *data)
{
  return getGlobalSharedData().get(wasm_->vm_id(), key, data);
}

WasmResult
ContextBase::setSharedData(std::string_view key, std::string_view value, uint32_t cas)
{
  return getGlobalSharedData().set(wasm_->vm_id(), key, value, cas);
}

WasmResult
ContextBase::getSharedDataKeys(std::vector<std::string> *result)
{
  return getGlobalSharedData().keys(wasm_->vm_id(), result);
}

WasmResult
ContextBase::removeSharedDataKey(std::string_view key, uint32_t cas, std::pair<std::string, uint32_t> *result)
{
...
WasmResult
ContextBase::registerSharedQueue(std::string_view queue_name, SharedQueueDequeueToken *token_ptr)
{
  // Get the id of the root context if this is a stream context because onQueueReady is on the
  // root.
  *token_ptr = getGlobalSharedQueue().registerQueue(wasm_->vm_id(), queue_name, isRootContext() ? id_ : parent_context_id_,
                                                    wasm_->callOnThreadFunction(), wasm_->vm_key());
  return WasmResult::Ok;
}

WasmResult
ContextBase::lookupSharedQueue(std::string_view vm_id, std::string_view queue_name, SharedQueueDequeueToken *token_ptr)
{
  SharedQueueDequeueToken token = getGlobalSharedQueue().resolveQueue(vm_id.empty() ? wasm_->vm_id() : vm_id, queue_name);
...
WasmResult
ContextBase::dequeueSharedQueue(uint32_t token, std::string *data)
{
  return getGlobalSharedQueue().dequeue(token, data);
}

WasmResult
ContextBase::enqueueSharedQueue(uint32_t token, std::string_view value)
{
...
void
ContextBase::onForeignFunction(uint32_t foreign_function_id, uint32_t data_size)
{
...
FilterStatus
ContextBase::onDownstreamData(uint32_t data_length, bool end_of_stream)
{
  CHECK_FAIL_NET(FilterStatus::Continue, FilterStatus::StopIteration);
  ...
  DeferAfterCallActions actions(this);
  auto result = wasm_->on_downstream_data_(this, id_, static_cast<uint32_t>(data_length), static_cast<uint32_t>(end_of_stream));
...
FilterStatus
ContextBase::onUpstreamData(uint32_t data_length, bool end_of_stream)
{
  CHECK_FAIL_NET(FilterStatus::Continue, FilterStatus::StopIteration);
  ...
  DeferAfterCallActions actions(this);
  auto result = wasm_->on_upstream_data_(this, id_, static_cast<uint32_t>(data_length), static_cast<uint32_t>(end_of_stream));
  // TODO(PiotrSikora): pull Proxy-WASM's FilterStatus values.
  CHECK_FAIL_NET(FilterStatus::Continue, FilterStatus::StopIteration);
  return result == 0 ? FilterStatus::Continue : FilterStatus::StopIteration;
}
...
void
ContextBase::onDownstreamConnectionClose(CloseType close_type)
{
...
void
ContextBase::onUpstreamConnectionClose(CloseType close_type)
{
...
template <typename P>
static uint32_t
headerSize(const P &p)
{
  return p ? p->size() : 0;
}

FilterHeadersStatus
ContextBase::onRequestHeaders(uint32_t headers, bool end_of_stream)
{
...
FilterDataStatus
ContextBase::onRequestBody(uint32_t body_length, bool end_of_stream)
{
  ...
  DeferAfterCallActions actions(this);
  const auto result = wasm_->on_request_body_(this, id_, body_length, static_cast<uint32_t>(end_of_stream));
  CHECK_FAIL_HTTP(FilterDataStatus::Continue, FilterDataStatus::StopIterationNoBuffer);
  return convertVmCallResultToFilterDataStatus(result);
}

FilterTrailersStatus
ContextBase::onRequestTrailers(uint32_t trailers)
{
...
ContextBase::onRequestMetadata(uint32_t elements)
{
...
FilterHeadersStatus
ContextBase::onResponseHeaders(uint32_t headers, bool end_of_stream)
{
  CHECK_FAIL_HTTP(FilterHeadersStatus::Continue, FilterHeadersStatus::StopAllIterationAndWatermark);
  ...
  const auto result = wasm_->on_response_headers_abi_01_ ?
                        ...
                        wasm_->on_response_headers_abi_02_(this, id_, headers, static_cast<uint32_t>(end_of_stream));
  CHECK_FAIL_HTTP(FilterHeadersStatus::Continue, FilterHeadersStatus::StopAllIterationAndWatermark);
  return convertVmCallResultToFilterHeadersStatus(result);
}

FilterDataStatus
ContextBase::onResponseBody(uint32_t body_length, bool end_of_stream)
{
  CHECK_FAIL_HTTP(FilterDataStatus::Continue, FilterDataStatus::StopIterationNoBuffer);
  ...
  DeferAfterCallActions actions(this);
  const auto result = wasm_->on_response_body_(this, id_, body_length, static_cast<uint32_t>(end_of_stream));
  CHECK_FAIL_HTTP(FilterDataStatus::Continue, FilterDataStatus::StopIterationNoBuffer);
  return convertVmCallResultToFilterDataStatus(result);
}

FilterTrailersStatus
ContextBase::onResponseTrailers(uint32_t trailers)
{
  CHECK_FAIL_HTTP(FilterTrailersStatus::Continue, FilterTrailersStatus::StopIteration);
...
FilterMetadataStatus
ContextBase::onResponseMetadata(uint32_t elements)
{
  CHECK_FAIL_HTTP(FilterMetadataStatus::Continue, FilterMetadataStatus::Continue);
...
void
ContextBase::onHttpCallResponse(uint32_t token, uint32_t headers, uint32_t body_size, uint32_t trailers)
{
...
void
ContextBase::onQueueReady(uint32_t token)
{
...
void
ContextBase::onGrpcReceiveInitialMetadata(uint32_t token, uint32_t elements)
{
...
void
ContextBase::onGrpcReceiveTrailingMetadata(uint32_t token, uint32_t trailers)
{
...
void
ContextBase::onGrpcReceive(uint32_t token, uint32_t response_size)
{
...
void
ContextBase::onGrpcClose(uint32_t token, uint32_t status_code)
{
...
WasmResult
ContextBase::setTimerPeriod(std::chrono::milliseconds period, uint32_t *timer_token_ptr)
{
...
FilterHeadersStatus
ContextBase::convertVmCallResultToFilterHeadersStatus(uint64_t result)
{
  if (wasm()->isNextIterationStopped() || result > static_cast<uint64_t>(FilterHeadersStatus::StopAllIterationAndWatermark)) {
...
FilterDataStatus
ContextBase::convertVmCallResultToFilterDataStatus(uint64_t result)
{
  if (wasm()->isNextIterationStopped() || result > static_cast<uint64_t>(FilterDataStatus::StopIterationNoBuffer)) {
...
FilterTrailersStatus
ContextBase::convertVmCallResultToFilterTrailersStatus(uint64_t result)
{
  if (wasm()->isNextIterationStopped() || result > static_cast<uint64_t>(FilterTrailersStatus::StopIteration)) {
...
FilterMetadataStatus
ContextBase::convertVmCallResultToFilterMetadataStatus(uint64_t result)
{
