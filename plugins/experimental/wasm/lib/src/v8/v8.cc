...
#undef _GET_MODULE_FUNCTION
  ...
  static std::string
  printValue(const wasm::Val &value)
  {
    switch (value.kind()) {
    case wasm::I32:
  ...
  static std::string
  printValues(const wasm::Val values[], size_t size)
  {
    if (size == 0) {
      return "";
    }
  ...
  static const char *
  printValKind(wasm::ValKind kind)
  {
    switch (kind) {
    case wasm::I32:
      return "i32";
  ...
  static std::string
  printValTypes(const wasm::ownvec<wasm::ValType> &types)
  {
    if (types.size() == 0) {
      return "void";
    }
  ...
  static bool
  equalValTypes(const wasm::ownvec<wasm::ValType> &left, const wasm::ownvec<wasm::ValType> &right)
  {
  ...
  makeVal(Word t)
  {
    return wasm::Val::make(static_cast<uint32_t>(t.u64_));
  }

  template <typename T> constexpr auto convertArgToValKind();
  template <>
  constexpr auto
  convertArgToValKind<Word>()
  {
    return wasm::I32;
  };
  template <>
  constexpr auto
  convertArgToValKind<uint32_t>()
  {
    return wasm::I32;
  };
  template <>
  constexpr auto
  convertArgToValKind<int64_t>()
  {
    return wasm::I64;
  };
  template <>
  constexpr auto
  convertArgToValKind<uint64_t>()
  {
    return wasm::I64;
  };
  template <>
  constexpr auto
  convertArgToValKind<double>()
  {
    return wasm::F64;
  };

  template <typename T, std::size_t... I> constexpr auto convertArgsTupleToValTypesImpl(std::index_sequence<I...> /*comptime*/)
  {
  ...
  constexpr T
  convertValTypesToArgsTupleImpl(const U &arr, std::index_sequence<I...> /*comptime*/)
  {
  ...
  constexpr T
  convertValTypesToArgsTuple(const U &arr)
  {
    return convertValTypesToArgsTupleImpl<T>(arr, std::make_index_sequence<std::tuple_size<T>::value>());
  }

  // V8 implementation.

  bool
  V8::load(std::string_view bytecode, std::string_view precompiled, const std::unordered_map<uint32_t, std::string> &function_names)
  {
...
#define WEE8_PLATFORM ""
  ...
  std::string_view
  V8::getPrecompiledSectionName()
  {
    static const auto name =
      sizeof(WEE8_PLATFORM) - 1 > 0 ?
        ("precompiled_wee8_v" + std::to_string(V8_MAJOR_VERSION) + "." + std::to_string(V8_MINOR_VERSION) + "." +
         std::to_string(V8_BUILD_NUMBER) + "." + std::to_string(V8_PATCH_LEVEL) + "_" + WEE8_PLATFORM) :
        "";
    return name;
  }
  ...
  bool V8::link(std::string_view /*debug_name*/)
  {
    ...
    for (size_t i = 0; i < import_types.size(); i++) {
      ...
      case wasm::EXTERN_FUNC: {
        ...
        if (it == host_functions_.end()) {
          fail(FailState::UnableToInitializeCode,
               std::string("Failed to load Wasm module due to a missing import: ") + std::string(module) + "." + std::string(name));
          return false;
        }
        ...
        if (!equalValTypes(import_type->func()->params(), func->type()->params()) ||
          ...
          fail(FailState::UnableToInitializeCode,
               std::string("Failed to load Wasm module due to an import type mismatch: ") + std::string(module) + "." +
                 std::string(name) + ", want: " + printValTypes(import_type->func()->params()) + " -> " +
                 printValTypes(import_type->func()->results()) + ", but host exports: " + printValTypes(func->type()->params()) +
      ...
      case wasm::EXTERN_GLOBAL: {
        ...
        fail(FailState::UnableToInitializeCode,
             "Failed to load Wasm module due to a missing import: " + std::string(module) + "." + std::string(name));
        return false;
      } break;
      ...
      case wasm::EXTERN_TABLE: {
        ...
        auto type =
          wasm::TableType::make(wasm::ValType::make(import_type->table()->element()->kind()), import_type->table()->limits());
        if (type == nullptr) {
          return false;
        }
  ...
  V8::getMemory(uint64_t pointer, uint64_t size)
  {
    assert(memory_ != nullptr);
  ...
  bool
  V8::setMemory(uint64_t pointer, uint64_t size, const void *data)
  {
    assert(memory_ != nullptr);
    if (pointer + size > memory_->data_size()) {
      return false;
  ...
  bool
  V8::getWord(uint64_t pointer, Word *word)
  {
  ...
  bool
  V8::setWord(uint64_t pointer, Word word)
  {
    constexpr auto size = sizeof(uint32_t);
    if (pointer + size > memory_->data_size()) {
      return false;
  ...
  template <typename... Args>
  void
  V8::registerHostFunctionImpl(std::string_view module_name, std::string_view function_name, void (*function)(Args...))
  {
    auto data = std::make_unique<FuncData>(std::string(module_name) + "." + std::string(function_name));
    auto type = wasm::FuncType::make(convertArgsTupleToValTypes<std::tuple<Args...>>(), convertArgsTupleToValTypes<std::tuple<>>());
    auto func = wasm::Func::make(
      ...
      [](void *data, const wasm::Val params[], wasm::Val /*results*/[]) -> wasm::own<wasm::Trap> {
        ...
        if (log) {
          func_data->vm_->integration()->trace("[vm->host] " + func_data->name_ + "(" + printValues(params, sizeof...(Args)) + ")");
        }
  ...
  template <typename R, typename... Args>
  void
  V8::registerHostFunctionImpl(std::string_view module_name, std::string_view function_name, R (*function)(Args...))
  {
    auto data = std::make_unique<FuncData>(std::string(module_name) + "." + std::string(function_name));
    auto type =
      wasm::FuncType::make(convertArgsTupleToValTypes<std::tuple<Args...>>(), convertArgsTupleToValTypes<std::tuple<R>>());
    auto func = wasm::Func::make(
      store_.get(), type.get(),
      [](void *data, const wasm::Val params[], wasm::Val results[]) -> wasm::own<wasm::Trap> {
        auto *func_data = reinterpret_cast<FuncData *>(data);
        ...
        if (log) {
          func_data->vm_->integration()->trace("[vm<-host] " + func_data->name_ + " return: " + std::to_string(rvalue));
        }
        return nullptr;
      },
      data.get());
  ...
  template <typename... Args>
  void
  V8::getModuleFunctionImpl(std::string_view function_name, std::function<void(ContextBase *, Args...)> *function)
  {
    ...
    if (!equalValTypes(func->type()->params(), arg_valtypes) || !equalValTypes(func->type()->results(), result_valtypes)) {
  ...
  template <typename R, typename... Args>
  void
  V8::getModuleFunctionImpl(std::string_view function_name, std::function<R(ContextBase *, Args...)> *function)
  {
    auto it = module_functions_.find(std::string(function_name));
    if (it == module_functions_.end()) {
      *function = nullptr;
    ...
    *function = [func, function_name, this](ContextBase *context, Args... args) -> R {
      ...
      if (log) {
        integration()->trace("[host<-vm] " + std::string(function_name) + " return: " + std::to_string(rvalue));
      }
      return rvalue;
    };
  }
  ...
  std::string
  V8::getFailMessage(std::string_view function_name, wasm::own<wasm::Trap> trap)
  {
