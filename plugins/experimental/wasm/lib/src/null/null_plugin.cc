...
{
void
NullPlugin::getFunction(std::string_view function_name, WasmCallVoid<0> *f)
{
...
void
NullPlugin::getFunction(std::string_view function_name, WasmCallVoid<1> *f)
{
  auto *plugin = this;
...
void
NullPlugin::getFunction(std::string_view function_name, WasmCallVoid<2> *f)
{
  auto *plugin = this;
...
void
NullPlugin::getFunction(std::string_view function_name, WasmCallVoid<3> *f)
{
  auto *plugin = this;
...
void
NullPlugin::getFunction(std::string_view function_name, WasmCallVoid<5> *f)
{
  auto *plugin = this;
  if (function_name == "proxy_on_http_call_response") {
    *f = [plugin](ContextBase *context, Word context_id, Word token, Word headers, Word body_size, Word trailers) {
      SaveRestoreContext saved_context(context);
...
void
NullPlugin::getFunction(std::string_view function_name, WasmCallWord<1> *f)
{
  auto *plugin = this;
  if (function_name == "malloc") {
    *f = [](ContextBase * /*context*/, Word size) -> Word { return Word(reinterpret_cast<uint64_t>(::malloc(size))); };
...
void
NullPlugin::getFunction(std::string_view function_name, WasmCallWord<2> *f)
{
  auto *plugin = this;
  if (function_name == "main") {
    ...
    *f = [plugin](ContextBase *context, Word context_id, Word configuration_size) {
      SaveRestoreContext saved_context(context);
      return Word(static_cast<uint64_t>(plugin->validateConfiguration(context_id, configuration_size)));
    };
...
void
NullPlugin::getFunction(std::string_view function_name, WasmCallWord<3> *f)
{
  auto *plugin = this;
  if (function_name == "proxy_on_downstream_data") {
    *f = [plugin](ContextBase *context, Word context_id, Word body_buffer_length, Word end_of_stream) -> Word {
      SaveRestoreContext saved_context(context);
...
null_plugin::Context *
NullPlugin::ensureContext(uint64_t context_id, uint64_t root_context_id)
{
...
NullPlugin::ensureRootContext(uint64_t context_id)
{
  ...
  if (factory) {
    ...
    // Default handlers.
    auto context             = std::make_unique<null_plugin::RootContext>(static_cast<uint32_t>(context_id), root_id->view());
    root_context             = context->asRoot();
    context_map_[context_id] = std::move(context);
  }
  return root_context;
...
null_plugin::ContextBase *
NullPlugin::getContextBase(uint64_t context_id)
{
  auto it = context_map_.find(context_id);
  if (it == context_map_.end() || !(it->second->asContext() != nullptr || it->second->asRoot() != nullptr)) {
...
null_plugin::Context *
NullPlugin::getContext(uint64_t context_id)
{
  auto it = context_map_.find(context_id);
...
null_plugin::RootContext *
NullPlugin::getRootContext(uint64_t context_id)
{
  auto it = context_map_.find(context_id);
...
null_plugin::RootContext *
NullPlugin::getRoot(std::string_view root_id)
{
...
bool
NullPlugin::validateConfiguration(uint64_t root_context_id, uint64_t configuration_size)
{
  return getRootContext(root_context_id)->validateConfiguration(configuration_size);
}

bool
NullPlugin::onStart(uint64_t root_context_id, uint64_t vm_configuration_size)
{
...
bool
NullPlugin::onConfigure(uint64_t root_context_id, uint64_t plugin_configuration_size)
{
...
void
NullPlugin::onTick(uint64_t root_context_id)
{
...
void
NullPlugin::onCreate(uint64_t context_id, uint64_t parent_context_id)
{
...
uint64_t
NullPlugin::onNewConnection(uint64_t context_id)
{
  return static_cast<uint64_t>(getContext(context_id)->onNewConnection());
}

uint64_t
NullPlugin::onDownstreamData(uint64_t context_id, uint64_t data_length, uint64_t end_of_stream)
{
  return static_cast<uint64_t>(getContext(context_id)->onDownstreamData(static_cast<size_t>(data_length), end_of_stream != 0));
}

uint64_t
NullPlugin::onUpstreamData(uint64_t context_id, uint64_t data_length, uint64_t end_of_stream)
{
  return static_cast<uint64_t>(getContext(context_id)->onUpstreamData(static_cast<size_t>(data_length), end_of_stream != 0));
}

void
NullPlugin::onDownstreamConnectionClose(uint64_t context_id, uint64_t close_type)
{
  getContext(context_id)->onDownstreamConnectionClose(static_cast<CloseType>(close_type));
}

void
NullPlugin::onUpstreamConnectionClose(uint64_t context_id, uint64_t close_type)
{
  getContext(context_id)->onUpstreamConnectionClose(static_cast<CloseType>(close_type));
}

uint64_t
NullPlugin::onRequestHeaders(uint64_t context_id, uint64_t headers, uint64_t end_of_stream)
{
  return static_cast<uint64_t>(getContext(context_id)->onRequestHeaders(headers, end_of_stream != 0));
}

uint64_t
NullPlugin::onRequestBody(uint64_t context_id, uint64_t body_buffer_length, uint64_t end_of_stream)
{
  return static_cast<uint64_t>(getContext(context_id)->onRequestBody(static_cast<size_t>(body_buffer_length), end_of_stream != 0));
}

uint64_t
NullPlugin::onRequestTrailers(uint64_t context_id, uint64_t trailers)
{
  return static_cast<uint64_t>(getContext(context_id)->onRequestTrailers(trailers));
}

uint64_t
NullPlugin::onRequestMetadata(uint64_t context_id, uint64_t elements)
{
  return static_cast<uint64_t>(getContext(context_id)->onRequestMetadata(elements));
}

uint64_t
NullPlugin::onResponseHeaders(uint64_t context_id, uint64_t headers, uint64_t end_of_stream)
{
  return static_cast<uint64_t>(getContext(context_id)->onResponseHeaders(headers, end_of_stream != 0));
}

uint64_t
NullPlugin::onResponseBody(uint64_t context_id, uint64_t body_buffer_length, uint64_t end_of_stream)
{
  return static_cast<uint64_t>(getContext(context_id)->onResponseBody(static_cast<size_t>(body_buffer_length), end_of_stream != 0));
}

uint64_t
NullPlugin::onResponseTrailers(uint64_t context_id, uint64_t trailers)
{
  return static_cast<uint64_t>(getContext(context_id)->onResponseTrailers(trailers));
}

uint64_t
NullPlugin::onResponseMetadata(uint64_t context_id, uint64_t elements)
{
  return static_cast<uint64_t>(getContext(context_id)->onResponseMetadata(elements));
}

void
NullPlugin::onHttpCallResponse(uint64_t context_id, uint64_t token, uint64_t headers, uint64_t body_size, uint64_t trailers)
{
  getRootContext(context_id)->onHttpCallResponse(token, headers, body_size, trailers);
}

void
NullPlugin::onGrpcReceive(uint64_t context_id, uint64_t token, size_t body_size)
{
  getRootContext(context_id)->onGrpcReceive(token, body_size);
}

void
NullPlugin::onGrpcClose(uint64_t context_id, uint64_t token, uint64_t status_code)
{
  getRootContext(context_id)->onGrpcClose(token, static_cast<GrpcStatus>(status_code));
}

void
NullPlugin::onGrpcReceiveInitialMetadata(uint64_t context_id, uint64_t token, uint64_t headers)
{
  getRootContext(context_id)->onGrpcReceiveInitialMetadata(token, headers);
}

void
NullPlugin::onGrpcReceiveTrailingMetadata(uint64_t context_id, uint64_t token, uint64_t trailers)
{
  getRootContext(context_id)->onGrpcReceiveTrailingMetadata(token, trailers);
}

void
NullPlugin::onQueueReady(uint64_t context_id, uint64_t token)
{
  getRootContext(context_id)->onQueueReady(token);
}

void
NullPlugin::onForeignFunction(uint64_t context_id, uint64_t foreign_function_id, uint64_t data_size)
{
...
void
NullPlugin::onLog(uint64_t context_id)
{
...
uint64_t
NullPlugin::onDone(uint64_t context_id)
{
...
void
NullPlugin::onDelete(uint64_t context_id)
{
...
{
  RootContext *
  nullVmGetRoot(std::string_view root_id)
  {
  ...
  Context *
  nullVmGetContext(uint32_t context_id)
  {
    auto *null_vm = dynamic_cast<NullVm *>(current_context_->wasmVm());
  ...
  RootContext *
  getRoot(std::string_view root_id)
  {
    return nullVmGetRoot(root_id);
  }

  Context *
  getContext(uint32_t context_id)
  {
    return nullVmGetContext(context_id);
  }
