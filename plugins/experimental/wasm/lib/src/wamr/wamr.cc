...
#undef _GET_MODULE_FUNCTION
  ...
  bool
  Wamr::load(std::string_view bytecode, std::string_view /*precompiled*/,
  ...
  Wamr::getMemory(uint64_t pointer, uint64_t size)
  {
    assert(memory_ != nullptr);
  ...
  bool
  Wamr::setMemory(uint64_t pointer, uint64_t size, const void *data)
  {
    assert(memory_ != nullptr);
    if (pointer + size > wasm_memory_data_size(memory_.get())) {
      return false;
  ...
  bool
  Wamr::getWord(uint64_t pointer, Word *word)
  {
    assert(memory_ != nullptr);
  ...
  bool
  Wamr::setWord(uint64_t pointer, Word word)
  {
    constexpr auto size = sizeof(uint32_t);
    if (pointer + size > wasm_memory_data_size(memory_.get())) {
      return false;
  ...
  template <typename... Args>
  void
  Wamr::registerHostFunctionImpl(std::string_view module_name, std::string_view function_name, void (*function)(Args...))
  {
  ...
  template <typename R, typename... Args>
  void
  Wamr::registerHostFunctionImpl(std::string_view module_name, std::string_view function_name, R (*function)(Args...))
  {
    auto data            = std::make_unique<HostFuncData>(std::string(module_name) + "." + std::string(function_name));
  ...
  template <typename... Args>
  void
  Wamr::getModuleFunctionImpl(std::string_view function_name, std::function<void(ContextBase *, Args...)> *function)
  {
  ...
  template <typename R, typename... Args>
  void
  Wamr::getModuleFunctionImpl(std::string_view function_name, std::function<R(ContextBase *, Args...)> *function)
  {
    auto it = module_functions_.find(std::string(function_name));
    if (it == module_functions_.end()) {
      *function = nullptr;
...
createWamrVm()
{
  return std::make_unique<wamr::Wamr>();
}
