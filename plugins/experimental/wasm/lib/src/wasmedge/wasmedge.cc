...
namespace WasmEdge
{
  ...
  makeVal(double t)
  {
    return WasmEdge_ValueGenF64(t);
  }

  // Helper function to print values.
  std::string
  printValue(const WasmEdge_Value &value)
  {
    switch (value.Type) {
  ...
  std::string
  printValues(const WasmEdge_Value *values, size_t size)
  {
    if (size == 0) {
      return "";
    }
  ...
  const char *
  printValType(WasmEdge_ValType kind)
  {
    switch (kind) {
    case WasmEdge_ValType_I32:
      return "i32";
  ...
  std::string
  printValTypes(const WasmEdge_ValType *types, size_t size)
  {
    if (size == 0) {
      return "void";
    }
  ...
  convArgToValType<Word>() {
    return WasmEdge_ValType_I32;
  }
  template <>
  enum WasmEdge_ValType
  convArgToValType<uint32_t>() {
    return WasmEdge_ValType_I32;
  }
  template <>
  enum WasmEdge_ValType
  convArgToValType<int64_t>() {
    return WasmEdge_ValType_I64;
  }
  template <>
  enum WasmEdge_ValType
  convArgToValType<uint64_t>() {
    return WasmEdge_ValType_I64;
  }
  template <>
  enum WasmEdge_ValType
  convArgToValType<double>() {
  ...
  template <>
  uint32_t
  convValTypeToArg<uint32_t>(WasmEdge_Value val)
  {
    return static_cast<uint32_t>(WasmEdge_ValueGetI32(val));
  }
  template <>
  Word
  convValTypeToArg<Word>(WasmEdge_Value val)
  {
    return WasmEdge_ValueGetI32(val);
  }
  template <>
  int64_t
  convValTypeToArg<int64_t>(WasmEdge_Value val)
  {
    return WasmEdge_ValueGetI64(val);
  }
  template <>
  uint64_t
  convValTypeToArg<uint64_t>(WasmEdge_Value val)
  {
    return static_cast<uint64_t>(WasmEdge_ValueGetI64(val));
  }
  template <>
  double
  convValTypeToArg<double>(WasmEdge_Value val)
  {
  ...
  constexpr T
  convValTypesToArgsTupleImpl(const WasmEdge_Value *arr, std::index_sequence<I...> /*comptime*/)
  {
    return std::make_tuple(convValTypeToArg<typename ConvertWordType<std::tuple_element_t<I, T>>::type>(arr[I])...);
  }

  template <typename T, typename Is = std::make_index_sequence<std::tuple_size<T>::value>>
  constexpr T
  convValTypesToArgsTuple(const WasmEdge_Value *arr)
  {
  ...
  template <typename T, std::size_t... I>
  uint32_t
  convArgsTupleToValTypesImpl(std::vector<enum WasmEdge_ValType> &types, std::index_sequence<I...> /*comptime*/)
  {
  ...
  template <typename T, typename Is = std::make_index_sequence<std::tuple_size<T>::value>>
  uint32_t
  convArgsTupleToValTypes(std::vector<enum WasmEdge_ValType> &types)
  {
  ...
  public:
    ...
    std::string_view
    getEngineName() override
    {
      return "wasmedge";
    }
    std::string_view
    getPrecompiledSectionName() override
    {
      return "";
    }
...
#define _GET_MODULE_FUNCTION(T) \
  void getFunction(std::string_view function_name, T *f) override { getModuleFunctionImpl(function_name, f); };
    FOR_ALL_WASM_VM_EXPORTS(_GET_MODULE_FUNCTION)
#undef _GET_MODULE_FUNCTION
  private:
    template <typename... Args>
    void registerHostFunctionImpl(std::string_view module_name, std::string_view function_name, void (*function)(Args...));

    template <typename R, typename... Args>
    void registerHostFunctionImpl(std::string_view module_name, std::string_view function_name, R (*function)(Args...));

    template <typename... Args>
    void getModuleFunctionImpl(std::string_view function_name, std::function<void(ContextBase *, Args...)> *function);

    template <typename R, typename... Args>
    void getModuleFunctionImpl(std::string_view function_name, std::function<R(ContextBase *, Args...)> *function);
  ...
  bool
  WasmEdge::load(std::string_view bytecode, std::string_view /*precompiled*/,
                 const std::unordered_map<uint32_t, std::string> & /*function_names*/)
  {
    WasmEdge_ASTModuleContext *mod = nullptr;
    WasmEdge_Result res =
      WasmEdge_LoaderParseFromBuffer(loader_.get(), &mod, reinterpret_cast<const uint8_t *>(bytecode.data()), bytecode.size());
  ...
  bool WasmEdge::link(std::string_view /*debug_name*/)
  {
    ...
    for (auto &&it : host_modules_) {
      ...
      if (!WasmEdge_ResultOK(res)) {
        fail(FailState::UnableToInitializeCode, std::string("Failed to link Wasm module due to import: ") + it.first);
        return false;
      }
    }
    // Instantiate module.
  ...
  WasmEdge::getMemory(uint64_t pointer, uint64_t size)
  {
  ...
  bool
  WasmEdge::setMemory(uint64_t pointer, uint64_t size, const void *data)
  {
    auto res = WasmEdge_MemoryInstanceSetData(memory_, reinterpret_cast<const uint8_t *>(data), pointer, size);
    return WasmEdge_ResultOK(res);
  }

  bool
  WasmEdge::getWord(uint64_t pointer, Word *word)
  {
    constexpr auto size = sizeof(uint32_t);
    uint32_t word32;
    auto res = WasmEdge_MemoryInstanceGetData(memory_, reinterpret_cast<uint8_t *>(&word32), pointer, size);
    if (WasmEdge_ResultOK(res)) {
      word->u64_ = word32;
      return true;
    }
  ...
  bool
  WasmEdge::setWord(uint64_t pointer, Word word)
  {
    constexpr auto size = sizeof(uint32_t);
    uint32_t word32     = word.u32();
    auto res            = WasmEdge_MemoryInstanceSetData(memory_, reinterpret_cast<uint8_t *>(&word32), pointer, size);
    return WasmEdge_ResultOK(res);
  }

  template <typename... Args>
  void
  WasmEdge::registerHostFunctionImpl(std::string_view module_name, std::string_view function_name, void (*function)(Args...))
  {
    ...
    data->callback_ = [](void *data, WasmEdge_MemoryInstanceContext * /*MemCxt*/, const WasmEdge_Value *Params, WasmEdge_Value *
                         /*Returns*/) -> WasmEdge_Result {
      auto *func_data = reinterpret_cast<HostFuncData *>(data);
      const bool log  = func_data->vm_->cmpLogLevel(LogLevel::trace);
      if (log) {
        func_data->vm_->integration()->trace("[vm->host] " + func_data->modname_ + "." + func_data->name_ + "(" +
      ...
      std::apply(fn, args);
      if (log) {
        func_data->vm_->integration()->trace("[vm<-host] " + func_data->modname_ + "." + func_data->name_ + " return: void");
      }
    ...
    WasmEdge_ModuleInstanceAddFunction(it->second->cxt_, WasmEdge_StringWrap(function_name.data(), function_name.length()),
                                       hostfunc_cxt);
    host_functions_.insert_or_assign(std::string(module_name) + "." + std::string(function_name), std::move(data));
  }

  template <typename R, typename... Args>
  void
  WasmEdge::registerHostFunctionImpl(std::string_view module_name, std::string_view function_name, R (*function)(Args...))
  {
    auto it = host_modules_.find(std::string(module_name));
    if (it == host_modules_.end()) {
      host_modules_.emplace(module_name, std::make_unique<HostModuleData>(module_name));
    ...
    data->vm_       = this;
    data->raw_func_ = reinterpret_cast<void *>(function);
    data->callback_ = [](void *data, WasmEdge_MemoryInstanceContext * /*MemCxt*/, const WasmEdge_Value *Params,
  ...
  template <typename... Args>
  void
  WasmEdge::getModuleFunctionImpl(std::string_view function_name, std::function<void(ContextBase *, Args...)> *function)
  {
    auto *func_cxt =
      WasmEdge_ModuleInstanceFindFunction(module_.get(), WasmEdge_StringWrap(function_name.data(), function_name.length()));
    if (!func_cxt) {
      *function = nullptr;
      return;
    }
    ...
    std::vector<enum WasmEdge_ValType> act_args(WasmEdge_FunctionTypeGetParametersLength(functype_cxt));
    std::vector<enum WasmEdge_ValType> act_returns(WasmEdge_FunctionTypeGetReturnsLength(functype_cxt));
    ...
    if (exp_args != act_args || exp_returns != act_returns) {
      fail(FailState::UnableToInitializeCode, "Bad function signature for: " + std::string(function_name) +
    ...
    *function = [function_name, func_cxt, this](ContextBase *context, Args... args) -> void {
      ...
      if (log) {
        integration()->trace("[host->vm] " + std::string(function_name) + "(" + printValues(params, sizeof...(Args)) + ")");
      }
      SaveRestoreContext saved_context(context);
      WasmEdge_Result res = WasmEdge_ExecutorInvoke(executor_.get(), func_cxt, params, sizeof...(Args), nullptr, 0);
      if (!WasmEdge_ResultOK(res)) {
  ...
  template <typename R, typename... Args>
  void
  WasmEdge::getModuleFunctionImpl(std::string_view function_name, std::function<R(ContextBase *, Args...)> *function)
  {
    auto *func_cxt =
      WasmEdge_ModuleInstanceFindFunction(module_.get(), WasmEdge_StringWrap(function_name.data(), function_name.length()));
    if (!func_cxt) {
    ...
    *function = [function_name, func_cxt, this](ContextBase *context, Args... args) -> R {
      ...
      SaveRestoreContext saved_context(context);
      WasmEdge_Result res = WasmEdge_ExecutorInvoke(executor_.get(), func_cxt, params, sizeof...(Args), results, 1);
      if (!WasmEdge_ResultOK(res)) {
        fail(FailState::RuntimeError, "Function: " + std::string(function_name) + " failed:\n" + WasmEdge_ResultGetMessage(res));
        return R{};
      }
      ...
      if (log) {
        integration()->trace("[host<-vm] " + std::string(function_name) + " return: " + std::to_string(ret));
      }
      return ret;
    };
  }
