...
#undef _GET_MODULE_FUNCTION
  ...
  bool
  Wasmtime::load(std::string_view bytecode, std::string_view /*precompiled*/,
  ...
  static bool
  equalValTypes(const wasm_valtype_vec_t *left, const wasm_valtype_vec_t *right)
  {
  ...
  static std::string
  printValue(const wasm_val_t &value)
  {
    switch (value.kind) {
    case WASM_I32:
  ...
  static std::string
  printValues(const wasm_val_vec_t *values)
  {
    if (values->size == 0) {
      return "";
    }
  ...
  static const char *
  printValKind(wasm_valkind_t kind)
  {
    switch (kind) {
    case WASM_I32:
      return "i32";
  ...
  static std::string
  printValTypes(const wasm_valtype_vec_t *types)
  {
    if (types->size == 0) {
      return "void";
    }
  ...
  bool Wasmtime::link(std::string_view /*debug_name*/)
  {
    ...
    for (size_t i = 0; i < import_types.get()->size; i++) {
      ...
      case WASM_EXTERN_FUNC: {
        ...
        if (it == host_functions_.end()) {
          fail(FailState::UnableToInitializeCode, std::string("Failed to load Wasm module due to a missing import: ") +
        ...
        if (!equalValTypes(wasm_functype_params(exp_type), wasm_functype_params(actual_type.get())) ||
            !equalValTypes(wasm_functype_results(exp_type), wasm_functype_results(actual_type.get()))) {
          fail(FailState::UnableToInitializeCode,
               std::string("Failed to load Wasm module due to an import type mismatch for function ") + std::string(module_name) +
                 "." + std::string(name) + ", want: " + printValTypes(wasm_functype_params(exp_type)) + " -> " +
      ...
      case WASM_EXTERN_GLOBAL: {
        ...
        fail(FailState::UnableToInitializeCode,
             "Failed to load Wasm module due to a missing import: " + std::string(module_name) + "." + std::string(name));
        return false;
      } break;
      case WASM_EXTERN_MEMORY: {
        assert(memory_ == nullptr);
        const wasm_memorytype_t *memory_type = wasm_externtype_as_memorytype_const(extern_type); // owned by `extern_type`
      ...
      case WASM_EXTERN_TABLE: {
        ...
        const wasm_tabletype_t *table_type = wasm_externtype_as_tabletype_const(extern_type); // owned by `extern_type`
    ...
    for (size_t i = 0; i < export_types.get()->size; i++) {
      ...
      case WASM_EXTERN_FUNC: {
        ...
        module_functions_.insert_or_assign(std::string(name_ptr->data, name_ptr->size), std::move(func));
      } break;
      case WASM_EXTERN_GLOBAL: {
        // TODO(mathetake): add support when/if needed.
      } break;
  ...
  uint64_t
  Wasmtime::getMemorySize()
  {
    return wasm_memory_data_size(memory_.get());
  }

  std::optional<std::string_view>
  Wasmtime::getMemory(uint64_t pointer, uint64_t size)
  {
    assert(memory_ != nullptr);
  ...
  bool
  Wasmtime::setMemory(uint64_t pointer, uint64_t size, const void *data)
  {
    assert(memory_ != nullptr);
    if (pointer + size > wasm_memory_data_size(memory_.get())) {
      return false;
  ...
  bool
  Wasmtime::getWord(uint64_t pointer, Word *word)
  {
    assert(memory_ != nullptr);
  ...
  bool
  Wasmtime::setWord(uint64_t pointer, Word word)
  {
    constexpr auto size = sizeof(uint32_t);
    if (pointer + size > wasm_memory_data_size(memory_.get())) {
      return false;
  ...
  template <>
  void
  assignVal<Word>(Word t, wasm_val_t &val)
  {
    val.kind   = WASM_I32;
  ...
  template <>
  void
  assignVal(uint32_t t, wasm_val_t &val)
  {
    val.kind   = WASM_I32;
  ...
  template <>
  void
  assignVal(uint64_t t, wasm_val_t &val)
  {
    val.kind   = WASM_I64;
  ...
  template <>
  void
  assignVal(double t, wasm_val_t &val)
  {
    val.kind   = WASM_F64;
    val.of.f64 = t;
  }
  ...
  template <>
  auto
  convertArgToValTypePtr<Word>()
  {
    return wasm_valtype_new_i32();
  };
  template <>
  auto
  convertArgToValTypePtr<uint32_t>()
  {
    return wasm_valtype_new_i32();
  };
  template <>
  auto
  convertArgToValTypePtr<int64_t>()
  {
    return wasm_valtype_new_i64();
  };
  template <>
  auto
  convertArgToValTypePtr<uint64_t>()
  {
    return wasm_valtype_new_i64();
  };
  template <>
  auto
  convertArgToValTypePtr<double>()
  {
  ...
  template <>
  uint32_t
  convertValueTypeToArg<uint32_t>(wasm_val_t val)
  {
    return static_cast<uint32_t>(val.of.i32);
  }
  template <>
  Word
  convertValueTypeToArg<Word>(wasm_val_t val)
  {
    return val.of.i32;
  }
  template <>
  int64_t
  convertValueTypeToArg<int64_t>(wasm_val_t val)
  {
    return val.of.i64;
  }
  template <>
  uint64_t
  convertValueTypeToArg<uint64_t>(wasm_val_t val)
  {
    return static_cast<uint64_t>(val.of.i64);
  }
  template <>
  double
  convertValueTypeToArg<double>(wasm_val_t val)
  {
    return val.of.f64;
  }
  ...
  constexpr T
  convertValTypesToArgsTuple(const U &vec, std::index_sequence<I...> /*comptime*/)
  {
    return std::make_tuple(convertValueTypeToArg<typename ConvertWordType<std::tuple_element_t<I, T>>::type>(vec->data[I])...);
  }

  template <typename T, std::size_t... I>
  void
  convertArgsTupleToValTypesImpl(wasm_valtype_vec_t *types, std::index_sequence<I...> /*comptime*/)
  {
    auto size = std::tuple_size<T>::value;
    auto ps =
      std::array<wasm_valtype_t *, std::tuple_size<T>::value>{convertArgToValTypePtr<typename std::tuple_element<I, T>::type>()...};
    wasm_valtype_vec_new(types, size, ps.data());
  }

  template <typename T, typename Is = std::make_index_sequence<std::tuple_size<T>::value>>
  void
  convertArgsTupleToValTypes(wasm_valtype_vec_t *types)
  {
  ...
  template <typename... Args>
  void
  Wasmtime::registerHostFunctionImpl(std::string_view module_name, std::string_view function_name, void (*function)(Args...))
  {
    auto data = std::make_unique<HostFuncData>(std::string(module_name) + "." + std::string(function_name));
    ...
    WasmFuncPtr func     = wasm_func_new_with_env(
      ...
      [](void *data, const wasm_val_vec_t *params, wasm_val_vec_t * /*results*/) -> wasm_trap_t * {
        ...
        if (log) {
          func_data->vm_->integration()->trace("[vm->host] " + func_data->name_ + "(" + printValues(params) + ")");
        }
        auto args = convertValTypesToArgsTuple<std::tuple<Args...>>(params, std::make_index_sequence<sizeof...(Args)>{});
  ...
  template <typename R, typename... Args>
  void
  Wasmtime::registerHostFunctionImpl(std::string_view module_name, std::string_view function_name, R (*function)(Args...))
  {
    auto data            = std::make_unique<HostFuncData>(std::string(module_name) + "." + std::string(function_name));
    ...
    WasmFuncPtr func     = wasm_func_new_with_env(
      ...
      [](void *data, const wasm_val_vec_t *params, wasm_val_vec_t *results) -> wasm_trap_t * {
        ...
        if (log) {
          func_data->vm_->integration()->trace("[vm<-host] " + func_data->name_ + " return: " + std::to_string(res));
        }
        return nullptr;
      },
      data.get(), nullptr);
  ...
  template <typename... Args>
  void
  Wasmtime::getModuleFunctionImpl(std::string_view function_name, std::function<void(ContextBase *, Args...)> *function)
  {
    ...
    if (!equalValTypes(wasm_functype_params(func_type.get()), exp_args.get()) ||
      ...
      fail(FailState::UnableToInitializeCode,
           "Bad function signature for: " + std::string(function_name) + ", want: " + printValTypes(exp_args.get()) + " -> " +
             printValTypes(exp_returns.get()) + ", but the module exports: " +
             printValTypes(wasm_functype_params(func_type.get())) + " -> " + printValTypes(wasm_functype_results(func_type.get())));
      return;
    }
  ...
  template <typename R, typename... Args>
  void
  Wasmtime::getModuleFunctionImpl(std::string_view function_name, std::function<R(ContextBase *, Args...)> *function)
  {
    auto it = module_functions_.find(std::string(function_name));
    if (it == module_functions_.end()) {
      *function = nullptr;
...
createWasmtimeVm()
{
  return std::make_unique<wasmtime::Wasmtime>();
}
