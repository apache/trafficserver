...
std::unordered_map<std::string, WasmForeignFunction> &
foreignFunctions()
{
...
WasmForeignFunction
getForeignFunction(std::string_view function_name)
{
...
namespace exports
{
  ...
  {
    Pairs
    toPairs(std::string_view buffer)
    {
      Pairs result;
    ...
    bool
    getPairs(ContextBase *context, const Pairs &result, uint64_t ptr_ptr, uint64_t size_ptr)
    {
      if (result.empty()) {
  ...
  Word
  set_property(Word key_ptr, Word key_size, Word value_ptr, Word value_size)
  {
  ...
  // Generic selector
  Word
  get_property(Word path_ptr, Word path_size, Word value_ptr_ptr, Word value_size_ptr)
  {
    auto *context = contextOrEffectiveContext();
  ...
  Word
  get_configuration(Word value_ptr_ptr, Word value_size_ptr)
  {
    auto *context = contextOrEffectiveContext();
  ...
  Word
  get_status(Word code_ptr, Word value_ptr_ptr, Word value_size_ptr)
  {
  ...
  Word
  send_local_response(Word response_code, Word response_code_details_ptr, Word response_code_details_size, Word body_ptr,
                      Word body_size, Word additional_response_header_pairs_ptr, Word additional_response_header_pairs_size,
                      Word grpc_status)
  {
    auto *context = contextOrEffectiveContext();
    auto details  = context->wasmVm()->getMemory(response_code_details_ptr, response_code_details_size);
    auto body     = context->wasmVm()->getMemory(body_ptr, body_size);
    auto additional_response_header_pairs =
      context->wasmVm()->getMemory(additional_response_header_pairs_ptr, additional_response_header_pairs_size);
  ...
  Word
  set_effective_context(Word context_id)
  {
    auto *context = contextOrEffectiveContext();
  ...
  Word
  call_foreign_function(Word function_name, Word function_name_size, Word arguments, Word arguments_size, Word results,
                        Word results_size)
  {
    auto *context = contextOrEffectiveContext();
  ...
  // SharedData
  Word
  get_shared_data(Word key_ptr, Word key_size, Word value_ptr_ptr, Word value_size_ptr, Word cas_ptr)
  {
    auto *context = contextOrEffectiveContext();
    auto key      = context->wasmVm()->getMemory(key_ptr, key_size);
    if (!key) {
  ...
  Word
  set_shared_data(Word key_ptr, Word key_size, Word value_ptr, Word value_size, Word cas)
  {
    auto *context = contextOrEffectiveContext();
    auto key      = context->wasmVm()->getMemory(key_ptr, key_size);
    auto value    = context->wasmVm()->getMemory(value_ptr, value_size);
  ...
  Word
  register_shared_queue(Word queue_name_ptr, Word queue_name_size, Word token_ptr)
  {
    auto *context   = contextOrEffectiveContext();
  ...
  Word
  dequeue_shared_queue(Word token, Word data_ptr_ptr, Word data_size_ptr)
  {
    auto *context = contextOrEffectiveContext();
    std::string data;
  ...
  Word
  resolve_shared_queue(Word vm_id_ptr, Word vm_id_size, Word queue_name_ptr, Word queue_name_size, Word token_ptr)
  {
    auto *context   = contextOrEffectiveContext();
  ...
  Word
  enqueue_shared_queue(Word token, Word data_ptr, Word data_size)
  {
    auto *context = contextOrEffectiveContext();
  ...
  Word
  add_header_map_value(Word type, Word key_ptr, Word key_size, Word value_ptr, Word value_size)
  {
    ...
    return context->addHeaderMapValue(static_cast<WasmHeaderMapType>(type.u64_), key.value(), value.value());
  }

  Word
  get_header_map_value(Word type, Word key_ptr, Word key_size, Word value_ptr_ptr, Word value_size_ptr)
  {
    if (type > static_cast<uint64_t>(WasmHeaderMapType::MAX)) {
      return WasmResult::BadArgument;
    }
    ...
    auto result = context->getHeaderMapValue(static_cast<WasmHeaderMapType>(type.u64_), key.value(), &value);
    if (result != WasmResult::Ok) {
      return result;
    }
    if (!context->wasm()->copyToPointerSize(value, value_ptr_ptr, value_size_ptr)) {
  ...
  Word
  replace_header_map_value(Word type, Word key_ptr, Word key_size, Word value_ptr, Word value_size)
  {
    if (type > static_cast<uint64_t>(WasmHeaderMapType::MAX)) {
      return WasmResult::BadArgument;
    }
    ...
    return context->replaceHeaderMapValue(static_cast<WasmHeaderMapType>(type.u64_), key.value(), value.value());
  }

  Word
  remove_header_map_value(Word type, Word key_ptr, Word key_size)
  {
    if (type > static_cast<uint64_t>(WasmHeaderMapType::MAX)) {
      return WasmResult::BadArgument;
    }
  ...
  Word
  get_header_map_pairs(Word type, Word ptr_ptr, Word size_ptr)
  {
    if (type > static_cast<uint64_t>(WasmHeaderMapType::MAX)) {
      return WasmResult::BadArgument;
    }
  ...
  Word
  set_header_map_pairs(Word type, Word ptr, Word size)
  {
    if (type > static_cast<uint64_t>(WasmHeaderMapType::MAX)) {
      return WasmResult::BadArgument;
    }
    ...
    return context->setHeaderMapPairs(static_cast<WasmHeaderMapType>(type.u64_), toPairs(data.value()));
  }

  Word
  get_header_map_size(Word type, Word result_ptr)
  {
    if (type > static_cast<uint64_t>(WasmHeaderMapType::MAX)) {
      return WasmResult::BadArgument;
    }
  ...
  // Buffer
  Word
  get_buffer_bytes(Word type, Word start, Word length, Word ptr_ptr, Word size_ptr)
  {
  ...
  Word
  get_buffer_status(Word type, Word length_ptr, Word flags_ptr)
  {
    if (type > static_cast<uint64_t>(WasmBufferType::MAX)) {
      return WasmResult::BadArgument;
    }
  ...
  Word
  set_buffer_bytes(Word type, Word start, Word length, Word data_ptr, Word data_size)
  {
    if (type > static_cast<uint64_t>(WasmBufferType::MAX)) {
      return WasmResult::BadArgument;
    }
  ...
  Word
  http_call(Word uri_ptr, Word uri_size, Word header_pairs_ptr, Word header_pairs_size, Word body_ptr, Word body_size,
            Word trailer_pairs_ptr, Word trailer_pairs_size, Word timeout_milliseconds, Word token_ptr)
  {
    auto *context      = contextOrEffectiveContext()->root_context();
    ...
    auto result = context->httpCall(uri.value(), headers, body.value(), trailers, timeout_milliseconds, &token);
    context->wasm()->setDatatype(token_ptr, token);
    return result;
  }

  Word
  define_metric(Word metric_type, Word name_ptr, Word name_size, Word metric_id_ptr)
  {
    auto *context = contextOrEffectiveContext();
    ...
    auto result        = context->defineMetric(static_cast<uint32_t>(metric_type.u64_), name.value(), &metric_id);
    if (result != WasmResult::Ok) {
      return result;
    }
  ...
  Word
  increment_metric(Word metric_id, int64_t offset)
  {
    auto *context = contextOrEffectiveContext();
  ...
  Word
  record_metric(Word metric_id, uint64_t value)
  {
    auto *context = contextOrEffectiveContext();
  ...
  Word
  get_metric(Word metric_id, Word result_uint64_ptr)
  {
    auto *context  = contextOrEffectiveContext();
    uint64_t value = 0;
  ...
  Word
  grpc_call(Word service_ptr, Word service_size, Word service_name_ptr, Word service_name_size, Word method_name_ptr,
            Word method_name_size, Word initial_metadata_ptr, Word initial_metadata_size, Word request_ptr, Word request_size,
            Word timeout_milliseconds, Word token_ptr)
  {
    ...
    auto initial_metadata_pairs = context->wasmVm()->getMemory(initial_metadata_ptr, initial_metadata_size);
    ...
    auto result = context->grpcCall(service.value(), service_name.value(), method_name.value(), initial_metadata, request.value(),
  ...
  Word
  grpc_stream(Word service_ptr, Word service_size, Word service_name_ptr, Word service_name_size, Word method_name_ptr,
              Word method_name_size, Word initial_metadata_ptr, Word initial_metadata_size, Word token_ptr)
  {
    auto *context               = contextOrEffectiveContext()->root_context();
    auto service                = context->wasmVm()->getMemory(service_ptr, service_size);
    auto service_name           = context->wasmVm()->getMemory(service_name_ptr, service_name_size);
    ...
    uint32_t token        = 0;
    auto initial_metadata = toPairs(initial_metadata_pairs.value());
    auto result = context->grpcStream(service.value(), service_name.value(), method_name.value(), initial_metadata, &token);
    if (result != WasmResult::Ok) {
      return result;
    }
    if (!context->wasm()->setDatatype(token_ptr, token)) {
  ...
  Word
  grpc_send(Word token, Word message_ptr, Word message_size, Word end_stream)
  {
    auto *context = contextOrEffectiveContext()->root_context();
  ...
  // logs.
  Word
  writevImpl(Word fd, Word iovs, Word iovs_len, Word *nwritten_ptr)
  {
    auto *context = contextOrEffectiveContext();
    ...
    for (size_t i = 0; i < iovs_len; i++) {
      auto memslice = context->wasmVm()->getMemory(iovs + i * 2 * sizeof(uint32_t), 2 * sizeof(uint32_t));
      if (!memslice) {
        return 21; // __WASI_EFAULT
      }
      const auto *iovec = reinterpret_cast<const uint32_t *>(memslice.value().data());
      if (iovec[1] != 0U /* buf_len */) {
        memslice = context->wasmVm()->getMemory(wasmtoh(iovec[0]) /* buf */, wasmtoh(iovec[1]) /* buf_len */);
        if (!memslice) {
          return 21; // __WASI_EFAULT
        }
  ...
  Word
  wasi_unstable_fd_write(Word fd, Word iovs, Word iovs_len, Word nwritten_ptr)
  {
    auto *context = contextOrEffectiveContext();
  ...
  Word wasi_unstable_fd_seek(Word /*fd*/, int64_t /*offset*/, Word /*whence*/, Word /*newoffset_ptr*/)
  {
    auto *context = contextOrEffectiveContext();
  ...
  Word
  wasi_unstable_fd_fdstat_get(Word fd, Word statOut)
  {
  ...
  Word
  wasi_unstable_environ_get(Word environ_array_ptr, Word environ_buf)
  {
    auto *context    = contextOrEffectiveContext();
  ...
  // *environ_buf_size);
  Word
  wasi_unstable_environ_sizes_get(Word count_ptr, Word buf_size_ptr)
  {
    auto *context    = contextOrEffectiveContext();
    const auto &envs = context->wasm()->envs();
  ...
  Word
  wasi_unstable_args_sizes_get(Word argc_ptr, Word argv_buf_size_ptr)
  {
    auto *context = contextOrEffectiveContext();
  ...
  Word
  wasi_unstable_clock_time_get(Word clock_id, uint64_t /*precision*/, Word result_time_uint64_ptr)
  {
    uint64_t result = 0;
    auto *context   = contextOrEffectiveContext();
    switch (clock_id) {
  ...
  Word
  wasi_unstable_random_get(Word result_buf_ptr, Word buf_len)
  {
  ...
  Word
  pthread_equal(Word left, Word right)
  {
  ...
  Word
  set_tick_period_milliseconds(Word period_milliseconds)
  {
    TimerToken token = 0;
    return contextOrEffectiveContext()->setTimerPeriod(std::chrono::milliseconds(period_milliseconds), &token);
  }

  Word
  get_current_time_nanoseconds(Word result_uint64_ptr)
  {
    auto *context   = contextOrEffectiveContext();
  ...
  Word
  log(Word level, Word address, Word size)
  {
  ...
  Word
  get_log_level(Word result_level_uint32_ptr)
  {
    auto *context  = contextOrEffectiveContext();
