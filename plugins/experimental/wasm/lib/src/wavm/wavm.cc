...
template <>
constexpr ValueType
inferValueType<proxy_wasm::Word>()
{
...
void getFunctionWavm(WasmVm *vm, std::string_view function_name, std::function<void(ContextBase *, Args...)> *function);
template <typename R, typename... Args>
void getFunctionWavm(WasmVm *vm, std::string_view function_name, std::function<R(ContextBase *, Args...)> *function);
template <typename R, typename... Args>
void registerCallbackWavm(WasmVm *vm, std::string_view module_name, std::string_view function_name, R (*function)(Args...));
...
#define CALL_WITH_CONTEXT(_x, _context, _wavm)                                                                                \
  do {                                                                                                                        \
    ...
    std::string
    getFailMessage(std::string_view function_name, WAVM::Runtime::Exception *exception)
    {
      std::string message = "Function: " + std::string(function_name) +
                            " failed: " + WAVM::Runtime::describeExceptionType(exception->type) +
                            "\nProxy-Wasm plugin in-VM backtrace:\n";
      std::vector<std::string> callstack_descriptions = WAVM::Runtime::describeCallStack(exception->callStack);
    ...
    public:
              ...
              WAVM::Runtime::Object *&out_object) override
      {
          ...
          if (out_object != nullptr) {
            if (!isA(out_object, type)) {
              vm_->fail(FailState::UnableToInitializeCode,
                        "Failed to load WASM module due to a type mismatch in an import: " + std::string(module_name) + "." +
        ...
        vm_->fail(FailState::MissingFunction, "Failed to load Wasm module due to a missing import: " + std::string(module_name) +
                                                "." + std::string(export_name) + " " + asString(type));
        return false;
      }

      HashMap<std::string, WAVM::Runtime::Instance *> &
...
#undef _REGISTER_CALLBACK
    ...
    std::map<std::string, WAVM::Runtime::GCPointer<WAVM::Runtime::Instance>> intrinsic_module_instances_{};
  ...
  bool
  Wavm::load(std::string_view bytecode, std::string_view precompiled,
             const std::unordered_map<uint32_t, std::string> & /*function_names*/)
  {
    ...
    if (!WASM::loadBinaryModule(reinterpret_cast<const unsigned char *>(bytecode.data()), bytecode.size(), ir_module_)) {
      return false;
    }

    if (!precompiled.empty()) {
      module_ = WAVM::Runtime::loadPrecompiledModule(ir_module_, {precompiled.data(), precompiled.data() + precompiled.size()});
      if (module_ == nullptr) {
        return false;
      }
  ...
  bool
  Wavm::link(std::string_view debug_name)
  {
  ...
  std::string_view
  Wavm::getPrecompiledSectionName()
  {
    return "wavm.precompiled_object";
  }
...
createWavmVm()
{
  return std::make_unique<proxy_wasm::Wavm::Wavm>();
}

template <typename R, typename... Args> IR::FunctionType inferHostFunctionType(R (*/*func*/)(Args...))
{
  return IR::FunctionType(IR::inferResultType<R>(), IR::TypeTuple({IR::inferValueType<Args>()...}), IR::CallingConvention::c);
}
...
registerCallbackWavm(WasmVm *vm, std::string_view module_name, std::string_view function_name, R (*f)(Args...))
{
  ...
  wavm->host_functions_.emplace_back(new Intrinsics::Function(&wavm->intrinsic_modules_[std::string(module_name)],
                                                              function_name.data(), reinterpret_cast<void *>(f),
                                                              inferHostFunctionType(f)));
}

template <typename R, typename... Args>
IR::FunctionType
inferStdFunctionType(std::function<R(ContextBase *, Args...)> * /*func*/)
{
...
template <typename R, typename... Args>
void
getFunctionWavm(WasmVm *vm, std::string_view function_name, std::function<R(ContextBase *, Args...)> *function)
{
  auto *wavm = dynamic_cast<proxy_wasm::Wavm::Wavm *>(vm);
  ...
  if (!checkFunctionType(f, inferStdFunctionType(function))) {
    *function = nullptr;
    wavm->fail(FailState::UnableToInitializeCode, "Bad function signature for: " + std::string(function_name));
    return;
  }
  *function = [wavm, f, function_name](ContextBase *context, Args... args) -> R {
    WasmUntaggedValue values[] = {args...};
    WasmUntaggedValue return_value;
    CALL_WITH_CONTEXT(invokeFunction(wavm->context_, f, getFunctionType(f), &values[0], &return_value), context, wavm);
    if (wavm->isFailed()) {
      return 0;
    }
...
template <typename... Args>
void
getFunctionWavm(WasmVm *vm, std::string_view function_name, std::function<void(ContextBase *, Args...)> *function)
{
  auto *wavm = dynamic_cast<proxy_wasm::Wavm::Wavm *>(vm);
  auto *f    = asFunctionNullable(getInstanceExport(wavm->module_instance_, std::string(function_name)));
  if (!f) {
  ...
  *function = [wavm, f, function_name](ContextBase *context, Args... args) {
    WasmUntaggedValue values[] = {args...};
    CALL_WITH_CONTEXT(invokeFunction(wavm->context_, f, getFunctionType(f), &values[0]), context, wavm);
  };
}
