...
using WasmForeignFunction =
  std::function<WasmResult(WasmBase &wasm, std::string_view argument, std::function<void *(size_t size)> alloc_result)>;
...
namespace exports
{
  ...
  Word send_local_response(Word response_code, Word response_code_details_ptr, Word response_code_details_size, Word body_ptr,
  ...
  Word get_shared_data(Word key_ptr, Word key_size, Word value_ptr_ptr, Word value_size_ptr, Word cas_ptr);
  ...
  Word resolve_shared_queue(Word vm_id_ptr, Word vm_id_size, Word queue_name_ptr, Word queue_name_size, Word token_ptr);
  ...
  Word get_header_map_value(Word type, Word key_ptr, Word key_size, Word value_ptr_ptr, Word value_size_ptr);
  Word replace_header_map_value(Word type, Word key_ptr, Word key_size, Word value_ptr, Word value_size);
  ...
  Word http_call(Word uri_ptr, Word uri_size, Word header_pairs_ptr, Word header_pairs_size, Word body_ptr, Word body_size,
  ...
  Word grpc_call(Word service_ptr, Word service_size, Word service_name_ptr, Word service_name_size, Word method_name_ptr,
  ...
  Word grpc_stream(Word service_ptr, Word service_size, Word service_name_ptr, Word service_name_size, Word method_name_ptr,
  ...
  Word done();
  Word call_foreign_function(Word function_name, Word function_name_size, Word arguments, Word warguments_size, Word results,
                             Word results_size);

  // Runtime environment functions exported from envoy to wasm.

  Word wasi_unstable_path_open(Word fd, Word dir_flags, Word path, Word path_len, Word oflags, int64_t fs_rights_base,
                               int64_t fg_rights_inheriting, Word fd_flags, Word nwritten_ptr);
...
#define FOR_ALL_HOST_FUNCTIONS(_f)                                                                                                \
  _f(log) _f(get_status) _f(set_property) _f(get_property) _f(send_local_response) _f(get_shared_data) _f(set_shared_data)        \
    _f(register_shared_queue) _f(resolve_shared_queue) _f(dequeue_shared_queue) _f(enqueue_shared_queue) _f(get_header_map_value) \
      _f(add_header_map_value) _f(replace_header_map_value) _f(remove_header_map_value) _f(get_header_map_pairs)                  \
        _f(set_header_map_pairs) _f(get_header_map_size) _f(get_buffer_status) _f(get_buffer_bytes) _f(set_buffer_bytes)          \
          _f(http_call) _f(grpc_call) _f(grpc_stream) _f(grpc_close) _f(grpc_cancel) _f(grpc_send)                                \
            _f(set_tick_period_milliseconds) _f(get_current_time_nanoseconds) _f(define_metric) _f(increment_metric)              \
              _f(record_metric) _f(get_metric) _f(set_effective_context) _f(done) _f(call_foreign_function)

#define FOR_ALL_HOST_FUNCTIONS_ABI_SPECIFIC(_f)                                                                               \
  _f(get_configuration) _f(continue_request) _f(continue_response) _f(clear_route_cache) _f(continue_stream) _f(close_stream) \
    _f(get_log_level)

#define FOR_ALL_WASI_FUNCTIONS(_f)                                                                                       \
  _f(fd_write) _f(fd_read) _f(fd_seek) _f(fd_close) _f(fd_fdstat_get) _f(environ_get) _f(environ_sizes_get) _f(args_get) \
    _f(args_sizes_get) _f(clock_time_get) _f(random_get) _f(proc_exit) _f(path_open) _f(fd_prestat_get) _f(fd_prestat_dir_name)
...
#define _CREATE_PROXY_WASM_STUB(_fn)                                                                              \
  ...
  template <typename... Args> struct _fn##Stub<Word(Args...)> {                                                   \
    ...
    {                                                                                                             \
      ...
      context->wasmVm()->integration()->error("Attempted call to restricted proxy-wasm capability: proxy_" #_fn); \
...
#define _CREATE_WASI_STUB(_fn)                                                                        \
  ...
  template <typename... Args> struct _fn##Stub<Word(Args...)> {                                       \
    ...
    {                                                                                                 \
      auto context = contextOrEffectiveContext();                                                     \
      context->wasmVm()->integration()->error("Attempted call to restricted WASI capability: " #_fn); \
