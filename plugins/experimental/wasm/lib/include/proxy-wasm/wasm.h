...
{
public:
  WasmBase(std::unique_ptr<WasmVm> wasm_vm, std::string_view vm_id, std::string_view vm_configuration, std::string_view vm_key,
  ...
  const std::string &
  moduleBytecode() const
  {
    return module_bytecode_;
  }
  const std::string &
  modulePrecompiled() const
  {
    return module_precompiled_;
  }
  ...
  virtual CallOnThreadFunction
  callOnThreadFunction()
  {
    unimplemented();
    return nullptr;
  }
  ...
  bool
  capabilityAllowed(std::string capability_name)
  {
    return allowed_capabilities_.empty() || allowed_capabilities_.find(capability_name) != allowed_capabilities_.end();
  }
  ...
  virtual ContextBase *
  createRootContext(const std::shared_ptr<PluginBase> &plugin)
  {
    return new ContextBase(this, plugin);
  }
  virtual ContextBase *
  createContext(const std::shared_ptr<PluginBase> &plugin)
  {
    return new ContextBase(this, plugin);
  }
  virtual void
  setTimerPeriod(uint32_t root_context_id, std::chrono::milliseconds period)
  {
  ...
  void
  stopNextIteration(bool stop)
  {
    stop_iteration_ = stop;
  };
  bool
  isNextIterationStopped()
  {
    return stop_iteration_;
  };

  void
  addAfterVmCallAction(std::function<void()> f)
  {
    after_vm_call_actions_.push_back(f);
  }
  void
...
protected:
  ...
  std::unordered_map<std::string, std::string> envs_; // environment variables passed through wasi.environ_get
...
#define FOR_ALL_MODULE_FUNCTIONS(_f)                                                                                           \
  _f(validate_configuration) _f(on_vm_start) _f(on_configure) _f(on_tick) _f(on_context_create) _f(on_new_connection)          \
    _f(on_downstream_data) _f(on_upstream_data) _f(on_downstream_connection_close) _f(on_upstream_connection_close)            \
      _f(on_request_body) _f(on_request_trailers) _f(on_request_metadata) _f(on_response_body) _f(on_response_trailers)        \
        _f(on_response_metadata) _f(on_http_call_response) _f(on_grpc_receive) _f(on_grpc_close)                               \
          _f(on_grpc_receive_initial_metadata) _f(on_grpc_receive_trailing_metadata) _f(on_queue_ready) _f(on_done) _f(on_log) \
            _f(on_delete)
...
using WasmHandleCloneFactory = std::function<std::shared_ptr<WasmHandleBase>(std::shared_ptr<WasmHandleBase> wasm)>;
...
std::shared_ptr<WasmHandleBase> createWasm(const std::string &vm_key, const std::string &code,
                                           const std::shared_ptr<PluginBase> &plugin, const WasmHandleFactory &factory,
                                           const WasmHandleCloneFactory &clone_factory, bool allow_precompiled);
...
{
public:
  explicit PluginHandleBase(std::shared_ptr<WasmHandleBase> wasm_handle, std::shared_ptr<PluginBase> plugin)
...
using PluginHandleFactory =
  std::function<std::shared_ptr<PluginHandleBase>(std::shared_ptr<WasmHandleBase> base_wasm, std::shared_ptr<PluginBase> plugin)>;

// Get an existing ThreadLocal VM matching 'vm_id' or create one using 'base_wavm' by cloning or by
// using it it as a template.
std::shared_ptr<PluginHandleBase> getOrCreateThreadLocalPlugin(const std::shared_ptr<WasmHandleBase> &base_handle,
...
WasmBase::vm_configuration() const
{
...
inline void *
WasmBase::allocMemory(uint64_t size, uint64_t *address)
{
  if (!malloc_) {
    return nullptr;
  }
...
inline uint64_t
WasmBase::copyString(std::string_view s)
{
  if (s.empty()) {
    return 0; // nullptr
  }
...
inline bool
WasmBase::copyToPointerSize(std::string_view s, uint64_t ptr_ptr, uint64_t size_ptr)
{
  uint64_t pointer = 0;
...
template <typename T>
inline bool
WasmBase::setDatatype(uint64_t ptr, const T &t)
{
