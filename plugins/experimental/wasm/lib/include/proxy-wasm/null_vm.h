...
struct NullVm : public WasmVm {
  ...
  cloneable() override
  {
    return Cloneable::InstantiatedModule;
  };
...
#define _FORWARD_GET_FUNCTION(_T) \
  void getFunction(std::string_view function_name, _T *f) override { plugin_->getFunction(function_name, f); }
...
#define _REGISTER_CALLBACK(_T) \
  void registerCallback(std::string_view, std::string_view, _T, typename ConvertFunctionTypeWordToUint32<_T>::type) override{};
