...
TEST_CASE("JAx fingerprint registry rejects misaligned entries", "[jax_fingerprint][registry]")
{
  using namespace jax_fingerprint;
  alignas(RegistryEntryV1) unsigned char storage[sizeof(RegistryEntryV1) + alignof(RegistryEntryV1)]{};
  RegistryV1                             registry{REGISTRY_MAGIC,
                      REGISTRY_ABI_VERSION,
                      sizeof(RegistryV1),
                      sizeof(RegistryEntryV1),
                      0,
                      1,
                      reinterpret_cast<const RegistryEntryV1 *>(storage + 1)};

  CHECK_FALSE(is_valid(&registry));
  CHECK(entry_at(&registry, 0) == nullptr);
  CHECK(find(&registry, "JA3").empty());
  registry.entry_count = 0;
  registry.entries     = nullptr;
  CHECK(is_valid(&registry));
}
