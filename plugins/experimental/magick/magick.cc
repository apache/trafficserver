...
struct ThreadPool {
  ...
  ThreadPool(const size_t s) : size_(s), running_(true), pool_(new TSThread[s]) {
    ...
    assert(nullptr != pool_);
    for (size_t i = 0; i < size_; ++i) {
      pool_[i] = TSThreadCreate([] (void * d) -> void * {
        assert(nullptr != d);
        ...
        while (self->running_) {
          Callback callback;
          {
            ...
            self->semaphore_.wait(lock, [self] {
                return ! self->queue_.empty() || ! self->running_;
            });
...
struct EVPContext {
  ...
  EVPContext(void) : context(EVP_MD_CTX_create()) {
    assert(nullptr != context);
  }
};
...
struct EVPKey {
  ...
  EVPKey(void) : key(EVP_PKEY_new()) {
    assert(nullptr != key);
  }

  bool assign(const char * const k) const {
    assert(nullptr != k);
...
bool verify(const byte * const msg, const size_t mlen, const byte * const sig,
    const size_t slen, EVP_PKEY * const pkey) {
...
bool QueryParameterToCharVector(CharVector & v) {
  {
    std::size_t s = 0;
    const TSReturnCode rc = TSStringPercentDecode(
        v.data(), v.size(), v.data(), v.size(), &s);
  ...
  {
    std::size_t s = 0;
    const TSReturnCode rc = TSBase64Decode(v.data(), v.size(),
        reinterpret_cast< unsigned char * >(v.data()), v.size(), &s);
    assert(TS_SUCCESS == rc);
    v.resize(s);
  }
...
struct ImageTransform : TransformationPlugin {
  ...
  ImageTransform(Transaction & t, CharVector && a, CharPointerVector && m, ThreadPool & p) :
    ...
    arguments_(std::move(a)), argumentMap_(std::move(m)), threadPool_(p) {
  ...
  void handleInputComplete(void) override {
    ...
    threadPool_.emplace_back([this](void) {
      ...
      const bool result = MagickCommandGenesis(image.info, ConvertImageCommand,
        this->argumentMap_.size(), this->argumentMap_.data(), nullptr,
        exception.info) == MagickTrue;
      ...
      TSDebug(PLUGIN_TAG, "Background transformation is done, resuming continuation (%p)",
          this);
...
struct GlobalHookPlugin : GlobalPlugin {
  ...
  void handleSendRequestHeaders(Transaction & t) override {
  ...
  void handleReadResponseHeaders(Transaction & t) override {
    ...
    std::transform(contentType.cbegin(), contentType.cend(),
        contentType.begin(), ::tolower);

    const bool compatibleContentType = "image/bmp" == contentType
      || "image/gif" == contentType
      || "image/jpeg" == contentType
      || "image/jpg" == contentType
      || "image/png" == contentType
      || "image/tiff" == contentType
      || "image/webp" == contentType
      || "image/svg+xml" == contentType
      || "application/pdf" == contentType
      || "application/postscript" == contentType;
    ...
    if (compatibleContentType) {
      ...
      if ( ! magickQueryParameter.empty()) {
        ...
        if (verified) {
          ...
          t.addPlugin(new ImageTransform(t, std::move(magick),
                std::move(argumentMap), threadPool_));
        } else {
