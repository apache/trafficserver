...
ConditionTransactCount::eval(const Resources& res)
{
  ...
  append_value(s, res);
  bool rval = static_cast<const Matchers<std::string>*>(_matcher)->test(s);
  TSDebug(PLUGIN_NAME, "Evaluating TRANSACT-COUNT(): %s: rval: %d", s.c_str(), rval);
  return rval;
}
...
ConditionTransactCount::append_value(std::string& s, const Resources& res)
{
  ...
  if (ssn) {
    ...
    if (length > 0) {
      TSDebug(PLUGIN_NAME, "Appending TRANSACT-COUNT(%s) to evaluation value -> %.*s", _qualifier.c_str(), length, value);
