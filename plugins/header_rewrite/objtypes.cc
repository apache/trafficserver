...
create_operator(const OperatorSpec &spec)
{
  ...
  case OperatorType::SET_STATE_FLAG:
    op = new OperatorSetStateFlag(spec.session_scope ? TS_USER_ARGS_SSN : TS_USER_ARGS_TXN);
    break;

  case OperatorType::SET_STATE_INT8:
    op = new OperatorSetStateInt8(spec.session_scope ? TS_USER_ARGS_SSN : TS_USER_ARGS_TXN);
    break;

  case OperatorType::SET_STATE_INT16:
    op = new OperatorSetStateInt16(spec.session_scope ? TS_USER_ARGS_SSN : TS_USER_ARGS_TXN);
    break;
...
parse_operator_string(const std::string &op_str, const std::string &arg, const std::string &val)
{
  ...
  } else if (op_str == "set-session-flag") {
    spec.type          = OperatorType::SET_STATE_FLAG;
    spec.session_scope = true;
  } else if (op_str == "set-session-int8") {
    spec.type          = OperatorType::SET_STATE_INT8;
    spec.session_scope = true;
  } else if (op_str == "set-session-int16") {
    spec.type          = OperatorType::SET_STATE_INT16;
    spec.session_scope = true;
