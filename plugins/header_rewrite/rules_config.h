...
public:
  ...
  bool parse_config(const std::string &fname, TSHttpHookID default_hook, const char *from_url = nullptr,
                    const char *to_url = nullptr, bool force_hrw4u = false);

  void
  add_procedures_path(const std::string &path)
  {
    _procedures_paths.push_back(path);
  }
...
private:
  ...
  std::vector<std::string> _procedures_paths;
};
