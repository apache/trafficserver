...
namespace
{
  ...
  public:
    ...
    create_condition(const hrw4u::ParserContext &ctx)
    {
      ...
      if (cond_spec.starts_with("%{") && cond_spec.ends_with("}")) {
    ...
    {
      Operator *op = ctx.op.empty() ? operator_factory(ctx.op_type) : operator_factory(ctx.op);
    ...
    static void
    destroy(void *ptr, hrw4u::ObjKind kind)
    {
      if (!ptr) {
        return;
      }
      ...
      switch (kind) {
      case hrw4u::ObjKind::CONDITION:
        delete static_cast<Condition *>(ptr);
        break;
      case hrw4u::ObjKind::OPERATOR:
      case hrw4u::ObjKind::OPERATOR_IF:
        delete static_cast<Operator *>(ptr);
        break;
      case hrw4u::ObjKind::RULESET:
...
parse_hrw4u_content(std::string_view content, const HRW4UConfig &config)
{
  ...
  parser_config.procedures_paths = config.procedures_paths;
