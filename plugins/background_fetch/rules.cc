...
check_client_ip_configured(TSHttpTxn txnp, const char *cfg_ip)
{
  ...
  TSDbg(dbg_ctl, "cfg_ip %s, client_ip %s", cfg_ip, ip_buf);

  if ((strlen(cfg_ip) == strlen(ip_buf)) && !strcmp(cfg_ip, ip_buf)) {
    TSDbg(dbg_ctl, "bg fetch for ip %s, configured ip %s", ip_buf, cfg_ip);
    return true;
  }
...
BgFetchRule::check_field_configured(TSHttpTxn txnp) const
{
  ...
  if (!strcmp(_field, "Client-IP")) {
    if (!strcmp(_value, "*")) {
      TSDbg(dbg_ctl, "Found client_ip wild card");
      return true;
    }
    if (check_client_ip_configured(txnp, _value)) {
      TSDbg(dbg_ctl, "Found client_ip match");
      return true;
    }
  }
    ...
    if (TS_SUCCESS == TSHttpTxnServerRespGet(txnp, &hdr_bufp, &hdr_loc)) {
      ...
      if (TS_NULL_MLOC != loc) {
        ...
        if (check_content_length(content_len, _value)) {
          TSDbg(dbg_ctl, "Found content-length match");
          hdr_found = true;
        }
        TSHandleMLocRelease(hdr_bufp, hdr_loc, loc);
      } else {
        TSDbg(dbg_ctl, "No content-length field in resp");
      }
  ...
  if (TS_SUCCESS == TSHttpTxnClientReqGet(txnp, &hdr_bufp, &hdr_loc)) {
    ...
    if (TS_NULL_MLOC != loc) {
      if (!strcmp(_value, "*")) {
        TSDbg(dbg_ctl, "Found %s wild card", _field);
        hdr_found = true;
      } else {
        int val_len         = 0;
        ...
        if (!val_str || val_len <= 0) {
          TSDbg(dbg_ctl, "invalid field");
        } else {
          TSDbg(dbg_ctl, "comparing with %s", _value);
      ...
      TSDbg(dbg_ctl, "no field %s in request header", _field);
    }
    TSHandleMLocRelease(hdr_bufp, TS_NULL_MLOC, hdr_loc);
  } else {
    TSError("[%s] Failed to get resp headers", PLUGIN_NAME);
