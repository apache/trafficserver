...
PromotionPolicy::doSample() const
{
  if (_sample > 0) {
    ...
    if (_sample > r) {
      TSDbg(dbg_ctl, "checking sampling, is %f > %f? Yes!", _sample, r);
    } else {
      TSDbg(dbg_ctl, "checking sampling, is %f > %f? No!", _sample, r);
      return false;
    }
  }
  return true;
...
PromotionPolicy::create_stat(std::string_view name, std::string_view remap_identifier)
{
  ...
  if (TS_ERROR == TSStatFindName(stat_name.data(), &stat_id)) {
    ...
    if (TS_ERROR == stat_id) {
      TSDbg(dbg_ctl, "error creating stat_name: %s", stat_name.data());
    } else {
      TSDbg(dbg_ctl, "created stat_name: %s, stat_id: %d", stat_name.data(), stat_id);
    }
  }
