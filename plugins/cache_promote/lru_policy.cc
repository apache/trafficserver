...
#endif
        TSDbg(dbg_ctl, "LRUHash::initFromUrl(%.*s%s)", url_len > 100 ? 100 : url_len, url, url_len > 100 ? "..." : "");
        TSfree(url);
        ret = true;
      }
    }
...
LRUPolicy::parseOption(int opt, char *optarg)
{
  ...
  case 'b':
    ...
    if (_buckets < MINIMUM_BUCKET_SIZE) {
      ...
      TSDbg(dbg_ctl, "enforcing minimum bucket size of %d", MINIMUM_BUCKET_SIZE);
...
LRUPolicy::doPromote(TSHttpTxn txnp)
{
  ...
  if (_map.end() != map_it) {
    ...
    if (TS_SUCCESS == TSHttpTxnClientReqGet(txnp, &request, &req_hdr)) {
      ...
      TSDbg(dbg_ctl, "The request is %s", cacheable ? "cacheable" : "not cacheable");
    ...
    if (cacheable && (val_hits >= _hits || (_bytes > 0 && val_bytes > _bytes))) {
      ...
      TSDbg(dbg_ctl, "saving the LRUEntry to the freelist");
      ...
      TSDbg(dbg_ctl, "still not promoted, got %d hits so far and %" PRId64 " bytes", val_hits, val_bytes);
    ...
    if (_list_size >= _buckets) {
      TSDbg(dbg_ctl, "repurposing last LRUHash entry");
      ...
      TSDbg(dbg_ctl, "reusing LRUEntry from freelist");
      ...
      TSDbg(dbg_ctl, "creating new LRUEntry");
    ...
    if (_map.end() != map_it) {
      ...
      if (TS_SUCCESS == TSHttpTxnServerRespGet(txnp, &resp, &resp_hdr)) {
        ...
        if (field_loc) {
          ...
          val_bytes += cl;
          TSDbg(dbg_ctl, "Added %" PRId64 " bytes for LRU entry", cl);
