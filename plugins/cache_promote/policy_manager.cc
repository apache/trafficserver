...
PolicyManager::coalescePolicy(PromotionPolicy *policy)
{
  ...
  if (tag.size() != 0) {
    ...
    TSDbg(dbg_ctl, "looking up policy by tag: %s", tag.c_str());

    if (res != _policies.end()) {
      TSDbg(dbg_ctl, "repurposing policy for tag: %s", tag.c_str());
      ...
      TSDbg(dbg_ctl, "inserting policy for tag: %s", tag.c_str());
  ...
  if (tag.size() != 0) { // this is always the case for instances of LRUPolicy
    ...
    if (res != _policies.end()) {
      if (0 == --res->second.second) {
        TSDbg(dbg_ctl, "releasing unused PromotionPolicy");
      ...
      TSDbg(dbg_ctl, "Tried to release a policy which was not properly initialized nor acquired via PolicyManager");
    }
  }
