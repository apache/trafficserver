...
PromotionConfig::factory(int argc, char *argv[])
{
  while (true) {
    ...
    if (opt == -1) {
      ...
      if (_policy) {
        TSDbg(dbg_ctl, "created remap with cache promotion policy = %s", _policy->policyName());
      }
    } else if (opt == 'e') {
      if (optarg == nullptr) {
        ...
        if (_policy && _policy->stats_add(optarg)) {
          ...
          TSDbg(dbg_ctl, "stats collection is enabled");
        }
      }
    } else {
      if (_policy) {
        ...
        if (opt == 's') {
          ...
          TSDbg(dbg_ctl, "internal_enabled set to true");
        } else {
