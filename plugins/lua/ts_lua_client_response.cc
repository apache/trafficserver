...
ts_lua_client_response_set_status(lua_State *L)
{
  ...
  TSHttpHdrStatusSet(http_ctx->client_response_bufp, http_ctx->client_response_hdrp, TSHttpStatus(status), "lua");
...
ts_lua_client_response_set_error_resp(lua_State *L)
{
  ...
  TSHttpTxnErrorBodySet(http_ctx->txnp, resp_buf, resp_len, nullptr);
