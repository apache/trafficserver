...
ts_lua_remap_plugin_init(void *ih, TSHttpTxn rh, TSRemapRequestInfo *rri)
{
  ...
  if (main_ctx == NULL || (int)(main_ctx - ts_lua_main_ctx_array) >= instance_conf->states) {
  ...
  http_ctx->txnp       = rh;
  http_ctx->has_hook   = 0;
  http_ctx->from_remap = (rri != NULL) ? 1 : 0;
  http_ctx->rri        = rri;
  if (rri != NULL) {
...
vconnHookHandler(TSCont contp, TSEvent event, void *edata)
{
  ...
  if (main_ctx == NULL || (int)(main_ctx - ts_lua_g_main_ctx_array) >= conf->states) {
