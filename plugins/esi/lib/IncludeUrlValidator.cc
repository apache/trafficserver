...
IncludeUrlValidator::setHostAllowRegex(const std::string &pattern)
{
  std::string error;
  int         erroffset = -1;
  if (!_allow_regex.compile(pattern, error, erroffset, RE_CASE_INSENSITIVE)) {
...
IncludeUrlValidator::validate(string_view url) const
{
  ...
  if (_has_allow_regex) {
    ...
    std::string h(bare);
    // RE_FULL_MATCH requires the pattern to span the entire host (the
    // std::regex_match semantics this replaced). exec() returns the capture
    // count on success, 0 if it matched but the match buffer was too small for
    // all capture groups, and a negative value for no match / error / exceeded
