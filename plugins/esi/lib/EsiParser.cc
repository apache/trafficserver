...
EsiParser::EsiParser(const char *debug_tag, ComponentBase::Error error_func)
  : ComponentBase(debug_tag, error_func), _parse_start_pos(-1)
{
                  ...
                  int &data_len) const
{
  ...
  if (!data_ptr || !data_len) {
    TSDbg(_dbg_ctl, "[%s] Returning true for empty data", __FUNCTION__);
  } else {
    if (data_len == -1) {
                          ...
                          const char *data_ptr /* = 0 */, int data_len /* = -1 */) const
{
  ...
  if (!data.size()) {
    TSDbg(_dbg_ctl, "[%s] No data to parse!", __FUNCTION__);
    return true;
  }
...
EsiParser::_searchData(const string &data, size_t start_pos, const char *str, int str_len, size_t &pos) const
{
  ...
  if (i_str == str_len) {
    ...
    TSDbg(_dbg_ctl, "[%s] Found full match of %.*s in [%.5s...] at position %d", __FUNCTION__, str_len, str, data_ptr, int(pos));
    return COMPLETE_MATCH;
  } else if (i_str) {
    pos = start_pos + i_data - i_str;
    TSDbg(_dbg_ctl, "[%s] Found partial match of %.*s in [%.5s...] at position %d", __FUNCTION__, str_len, str, data_ptr, int(pos));
    return PARTIAL_MATCH;
  } else {
    TSDbg(_dbg_ctl, "[%s] Found no match of %.*s in [%.5s...]", __FUNCTION__, str_len, str, data_ptr);
    return NO_MATCH;
  }
}
...
EsiParser::_compareData(const string &data, size_t pos, const char *str, int str_len) const
{
  ...
  for (; i_data < data.size(); ++i_data) {
    if (data[i_data] == str[i_str]) {
      ++i_str;
      if (i_str == str_len) {
        TSDbg(_dbg_ctl, "[%s] string [%.*s] is equal to data at position %d", __FUNCTION__, str_len, str, int(pos));
        return COMPLETE_MATCH;
      }
    } else {
      return NO_MATCH;
  ...
  TSDbg(_dbg_ctl, "[%s] string [%.*s] is partially equal to data at position %d", __FUNCTION__, str_len, str, int(pos));
  return PARTIAL_MATCH;
}
...
EsiParser::_parse(const string &data, int &parse_start_pos, DocNodeList &node_list, bool last_chunk /* = false */) const
{
  ...
  while (parse_start_pos < static_cast<int>(data_size)) {
    ...
    if ((curr_pos - parse_start_pos) > 0) {
      ...
      TSDbg(_dbg_ctl, "[%s], Adding data of size %d before (newly found) ESI tag as PRE node", __FUNCTION__,
    ...
    if (is_html_comment_node) {
      TSDbg(_dbg_ctl, "[%s] Found html comment tag at position %d", __FUNCTION__, int(curr_pos));
      ...
      for (node_info = ESI_NODES; node_info->type != DocNode::TYPE_UNKNOWN; ++node_info) {
        ...
        if (search_result == COMPLETE_MATCH) {
          if (node_info->tag_suffix[node_info->tag_suffix_len - 1] == '>') {
            TSDbg(_dbg_ctl, "[%s] Found [%s] tag at position %d", __FUNCTION__, DocNode::type_names_[node_info->type],
    ...
    case DocNode::TYPE_EXCEPT:
      TSDbg(_dbg_ctl, "[%s] Handling %s tag...", __FUNCTION__, DocNode::type_names_[node_info->type]);
    ...
    case DocNode::TYPE_HTML_COMMENT:
      TSDbg(_dbg_ctl, "[%s] added string of size %d starting with [%.5s] for node %s", __FUNCTION__, int(end_pos - curr_pos),
    ...
    case DocNode::TYPE_SPECIAL_INCLUDE:
      TSDbg(_dbg_ctl, "[%s] Handling special include tag...", __FUNCTION__);
  ...
  lPartialMatch:
    if (last_chunk) {
      TSDbg(_dbg_ctl, "[%s] Found a partial ESI tag - will be treated as PRE text", __FUNCTION__);
    } else {
      TSDbg(_dbg_ctl, "[%s] Deferring to next chunk to find complete tag", __FUNCTION__);
    }
    break;
  }
  if (last_chunk && (parse_start_pos < static_cast<int>(data_size))) {
    TSDbg(_dbg_ctl, "[%s] Adding trailing text of size %d starting at [%.5s] as a PRE node", __FUNCTION__,
  ...
  TSDbg(_dbg_ctl, "[%s] Added %d node(s) during parse", __FUNCTION__, int(node_list.size() - orig_list_size));
  return true;
...
EsiParser::_processIncludeTag(const string &data, size_t curr_pos, size_t end_pos, DocNodeList &node_list) const
{
  ...
  TSDbg(_dbg_ctl, "[%s] Added include tag with url [%.*s]", __FUNCTION__, src_info.value_len, src_info.value);
  return true;
}
...
EsiParser::_processSpecialIncludeTag(const string &data, size_t curr_pos, size_t end_pos, DocNodeList &node_list) const
{
  ...
  TSDbg(_dbg_ctl, "[%s] Added special include tag with handler [%.*s] and data [%.*s]", __FUNCTION__, handler_info.value_len,
...
EsiParser::_processWhenTag(const string &data, size_t curr_pos, size_t end_pos, DocNodeList &node_list) const
{
  ...
  TSDbg(_dbg_ctl, "[%s] Added when tag with expression [%.*s] and data starting with [%.5s]", __FUNCTION__, test_expr.value_len,
...
EsiParser::_processTryTag(const string &data, size_t curr_pos, size_t end_pos, DocNodeList &node_list) const
{
  ...
  while (iter != end_node) {
    if (iter->type == DocNode::TYPE_ATTEMPT) {
      ...
      TSDbg(_dbg_ctl, "[%s] Ignoring top-level whitespace raw text", __FUNCTION__);
      temp_iter = iter;
      ++temp_iter;
  ...
  TSDbg(_dbg_ctl, "[%s] Added try node successfully", __FUNCTION__);
  return true;
}
