...
atscppapi::TSDbgCtlUniqPtr dbg_ctl_guard{TSDbgCtlCreate("plugin_esi_intercept")};
...
struct SContData {
  ...
  ~SContData()
  {
    TSDbg(dbg_ctl, "[%s] Destroying continuation data", __FUNCTION__);
...
SContData::init(TSVConn vconn)
{
  ...
  initialized = true;
  TSDbg(dbg_ctl, "[%s] SContData initialized!", __FUNCTION__);
  return true;
}
...
handleRead(SContData *cont_data, bool &read_complete)
{
  ...
  TSDbg(dbg_ctl, "[%s] Parsed header, avail: %d", __FUNCTION__, avail);
    ...
    while (block != nullptr) {
      ...
      if (!cont_data->req_hdr_parsed) {
        ...
        if (TSHttpHdrParseReq(cont_data->http_parser, cont_data->req_hdr_bufp, cont_data->req_hdr_loc, &data, endptr) ==
            TS_PARSE_DONE) {
          TSDbg(dbg_ctl, "[%s] Parsed header", __FUNCTION__);
          ...
          TSDbg(dbg_ctl, "[%s] Got content length as %d", __FUNCTION__, cont_data->req_content_len);
          if (cont_data->req_content_len <= 0) {
            TSError("[server_intercept][%s] Invalid content length [%d]", __FUNCTION__, cont_data->req_content_len);
            return false;
          }
          if (endptr - data) {
            TSDbg(dbg_ctl, "[%s] Appending %ld bytes to body", __FUNCTION__, static_cast<long int>(endptr - data));
            cont_data->body.append(data, endptr - data);
          }
          cont_data->req_hdr_parsed = true;
        }
      } else {
        TSDbg(dbg_ctl, "[%s] Appending %" PRId64 " bytes to body", __FUNCTION__, data_len);
  ...
  TSDbg(dbg_ctl, "[%s] Consumed %d bytes from input vio, avail: %d", __FUNCTION__, consumed, avail);
  ...
  if (static_cast<int>(cont_data->body.size()) == cont_data->req_content_len) {
    TSDbg(dbg_ctl, "[%s] Completely read body of size %d", __FUNCTION__, cont_data->req_content_len);
    read_complete = true;
  } else {
    read_complete = false;
    TSDbg(dbg_ctl, "[%s] Reenabling input vio as %ld bytes still need to be read", __FUNCTION__,
...
processRequest(SContData *cont_data)
{
    ...
    if (name) {
      ...
      if (echo_header) {
        ...
        for (int i = 0; i < n_field_values; ++i) {
          ...
          if (!value_len) {
            TSDbg(dbg_ctl, "[%s] Error while getting value #%d of header [%.*s]", __FUNCTION__, i, name_len, name);
          } else {
  ...
  TSDbg(dbg_ctl, "[%s] Wrote reply of size %d", __FUNCTION__, total_bytes_written);
...
{
  TSDbg(dbg_ctl, "[%s] Received event: %d", __FUNCTION__, static_cast<int>(event));
  ...
  case TS_EVENT_NET_ACCEPT:
    TSDbg(dbg_ctl, "[%s] Received net accept event", __FUNCTION__);
  ...
  case TS_EVENT_VCONN_READ_READY:
    TSDbg(dbg_ctl, "[%s] Received read ready event", __FUNCTION__);
  ...
  case TS_EVENT_VCONN_EOS:
    ...
    TSDbg(dbg_ctl, "[%s] Received read complete/eos event %d", __FUNCTION__, event);
    read_complete = true;
    break;
  case TS_EVENT_VCONN_WRITE_READY:
    TSDbg(dbg_ctl, "[%s] Received write ready event", __FUNCTION__);
    break;
  case TS_EVENT_VCONN_WRITE_COMPLETE:
    TSDbg(dbg_ctl, "[%s] Received write complete event", __FUNCTION__);
    shutdown = true;
    break;
  case TS_EVENT_ERROR:
  ...
  if (read_complete) {
    if (!processRequest(cont_data)) {
      ...
      TSDbg(dbg_ctl, "[%s] Processed request successfully", __FUNCTION__);
    }
  }

  if (shutdown) {
    TSDbg(dbg_ctl, "[%s] Completed request processing. Shutting down...", __FUNCTION__);
    if (cont_data->net_vc) {
...
setupServerIntercept(TSHttpTxn txnp)
{
  ...
  TSDbg(dbg_ctl, "[%s] Setup server intercept successfully", __FUNCTION__);
  return true;
}
