  ...
  : _contp(contp), _n_pending_requests(0), _curr_event_id_base(FETCH_EVENT_ID_BASE), _headers_str("")
{
  ...
  _dbg_ctl     = TSDbgCtlCreate(debug_tag);
...
HttpDataFetcherImpl::addFetchRequest(const string &url, FetchedDataProcessor *callback_obj /* = 0 */)
{
  ...
  if (!insert_result.second) {
    TSDbg(_dbg_ctl, "[%s] Fetch request for url [%s] already added", __FUNCTION__, url.data());
    return true;
  }
  ...
  TSDbg(_dbg_ctl, "[%s] Successfully added fetch request for URL [%s]", __FUNCTION__, url.data());
...
HttpDataFetcherImpl::_isFetchEvent(TSEvent event, int &base_event_id) const
{
  ...
  if ((base_event_id < 0) || (base_event_id >= static_cast<int>(_page_entry_lookup.size()))) {
    TSDbg(_dbg_ctl, "[%s] Event id %d not within fetch event id range [%d, %ld)", __FUNCTION__, event, FETCH_EVENT_ID_BASE,
...
HttpDataFetcherImpl::handleFetchEvent(TSEvent event, void *edata)
{
  ...
  if (TSHttpHdrParseResp(_http_parser, req_data.bufp, req_data.hdr_loc, &startptr, endptr) == TS_PARSE_DONE) {
    ...
    if (req_data.resp_status == TS_HTTP_STATUS_OK) {
      ...
      TSDbg(_dbg_ctl, "[%s] Inserted page data of size %d starting with [%.6s] for request [%s]", __FUNCTION__, req_data.body_len,
      ...
      TSDbg(_dbg_ctl, "[%s] Received non-OK status %d for request [%s]", __FUNCTION__, req_data.resp_status, req_str.data());
    ...
    TSDbg(_dbg_ctl, "[%s] Could not parse response for request [%s]", __FUNCTION__, req_str.data());
  }
  ...
  if (exp_value && exp_value_len) {
    ...
    for (int i = 0; i < n_values; ++i) {
      ...
      if (nullptr != value && value_len) {
        ...
        TSDbg(_dbg_ctl, "[%s] Error while getting value # %d of header [%.*s]", __FUNCTION__, i, name_len, name);
      }
      if (retval) {
        break;
      }
...
HttpDataFetcherImpl::getData(const string &url, ResponseData &resp_data) const
{
  ...
  TSDbg(_dbg_ctl, "[%s] Found data for URL [%s] of size %d starting with [%.5s]", __FUNCTION__, url.data(), req_data.body_len,
        req_data.body);
  return true;
}
