...
class ReplayTest:
    ...
    def run(self) -> None:
        ...
        reason = self.runtime.requirement_failure(self.spec.autest.get("requires", {}))
        if reason:
        ...
        if client is not None:
            self._validate_process_output(client, self.spec.autest["client"], "Violation|Invalid status")
        if server is not None:
            self._validate_process_output(server, self.spec.autest["server"], "Violation")
        if ats is not None:
            self._validate_ats_logs()

    def _start_dns(self) -> ManagedProcess | None:
        config = self.spec.autest.get("dns")
        if config is None:
            return None
    ...
    def _start_verifier_server(self) -> ManagedProcess:
        config = self.spec.autest["server"]
    ...
    def _run_verifier_client(self) -> ManagedProcess:
        config = self.spec.autest["client"]
        process_config = dict(config.get("process_config", {}))
        name = str(config.get("name", "client"))
        directory = self.sandbox / name
        directory.mkdir()
        ats_process_config = self.spec.autest["ats"].get("process_config", {})
    ...
    def _prepare_ats_tree(self, root: Path) -> dict[str, Path]:
        ...
        min_config = self.runtime.repository_root / "tests" / "gold_tests" / "autest-site" / "min_cfg"
    ...
    def _check_metrics(self) -> None:
        checks = self.spec.autest["ats"].get("metric_checks", [])
        for check in checks:
