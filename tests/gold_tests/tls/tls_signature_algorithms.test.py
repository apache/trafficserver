...
class TestTLSSignatureAlgorithms:
    ...
    _paths = (
        '/tls12-full', '/tls12-resumed', '/tls13-full', '/tls13-resumed', '/tls13-ecdsa', '/tls12-ecdsa-ticket-full',
        '/tls12-ecdsa-ticket-resumed', '/tls13-ecdsa-disambiguation', '/tls13-rsa-disambiguation')

    def __init__(self) -> None:
        '''Configure the test processes and handshake runs.'''
        version = subprocess.check_output(['openssl', 'version'], text=True)
        match = re.search(r'OpenSSL (\d+)\.(\d+)', version)
        self._has_brainpool_sigalg = match is not None and tuple(map(int, match.groups())) >= (3, 2)
        ...
        session = os.path.join(Test.RunDirectory, 'tls12-ecdsa-ticket.session')
        for phase, session_option in (('full', '-sess_out'), ('resumed', '-sess_in')):
            self._add_handshake_run(
                f'Log an ECDSA TLS 1.2 {phase} handshake using a session ticket', f'/tls12-ecdsa-ticket-{phase}', '-tls1_2',
                'ecdsa_secp256r1_sha256', f'-cipher ECDHE-ECDSA-AES128-GCM-SHA256 {session_option} {session}')
        # The OpenSSL CLI added TLS 1.3 brainpool signature schemes in 3.2.
        if self._has_brainpool_sigalg:
            self._add_handshake_run(
                'Disambiguate TLS 1.3 ECDSA schemes using the certificate curve', '/tls13-ecdsa-disambiguation', '-tls1_3',
                'ecdsa_secp256r1_sha256:ecdsa_brainpoolP256r1tls13_sha256', '')
        self._add_handshake_run(
            'Disambiguate TLS 1.3 RSA-PSS schemes using the certificate key type', '/tls13-rsa-disambiguation', '-tls1_3',
...
logging:
  formats:
    - name: tls_signature_algorithms
      format: '%<cqup> %<cqssv> %<cqssig> %<cqssin> %<cqssr> %<cqssrt>'
  logs:
...
'''.split('\n'))
        ...
        gold = 'tls_signature_algorithms' if self._has_brainpool_sigalg else 'tls_signature_algorithms_no_brainpool'
        Test.Disk.File(log_path, exists=True, content=f'gold/{gold}.gold')
        return ts
    ...
    def _add_handshake_run(
            ...
            sigalgs: str,
            session_option: str,
            start_processes: bool = False) -> 'TestRun':
    ...
    def _add_log_flush_run(self) -> None:
        ...
        tr = Test.AddAwaitFileContainsTestRun('Await TLS signature algorithm log entries', log_path, r'^tls', self._handshake_count)
