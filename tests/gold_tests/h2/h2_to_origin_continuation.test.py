...
stop_file = f"{tr.RunDirectory}/stop-origin"
lifecycle_script = f"{tr.RunDirectory}/origin_lifecycle.py"

server = Test.Processes.Process("h2-continuation-origin")
...
server.Command = (
    f"{sys.executable} {tr.RunDirectory}/continuation_origin.py "
    f"{server_port} server.pem server.key 3 {stop_file}")
...
tr = Test.AddTestRun("Verify both large response fields reach the client intact")
tr.Setup.Copy("verify_continuation_headers.py")
tr.Processes.Default.Command = f"{sys.executable} {tr.RunDirectory}/verify_continuation_headers.py {ts.Variables.port}"
tr.Processes.Default.ReturnCode = 0
tr.StillRunningAfter = ts

tr.StillRunningAfter = server

tr = Test.AddTestRun("Stop origin after verifying complete response delivery")
tr.Processes.Default.Command = f"{sys.executable} {lifecycle_script} {stop_file}"
tr.Processes.Default.ReturnCode = 0
tr.TimeOut = 15
