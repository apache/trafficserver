...
tr.MakeCurlCommand(curl_and_args + ' http://exclude/slice.txt', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
...
ps = tr.Processes.Default
tr.MakeCurlCommand(curl_and_args + ' http://exclude/slice.mp4', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
...
ps = tr.Processes.Default
tr.MakeCurlCommand(curl_and_args + ' http://include/slice.txt', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
ps.Streams.stdout.Content = Testers.ContainsExpression("X-Info: notsliced", "expected not sliced header")
tr.StillRunningAfter = ts
...
ps = tr.Processes.Default
tr.MakeCurlCommand(curl_and_args + ' http://include/slice.mp4', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
ps.Streams.stdout.Content = Testers.ContainsExpression("X-Info: sliced", "expected sliced header")
tr.StillRunningAfter = ts
tr.StillRunningAfter = ts
