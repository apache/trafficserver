...
tr.MakeCurlCommand(
    '-o /dev/null -H "Host: one"' + ' http://127.0.0.1:{}/argh'.format(ts.Variables.port), uds_path=ts.Variables.uds_path)
...
tr.MakeCurlCommand(
    '-o /dev/null -H "Host: two"' + ' http://127.0.0.1:{}/badpath'.format(ts.Variables.port), uds_path=ts.Variables.uds_path)
tr.Processes.Default.ReturnCode = 0
tr.StillRunningAfter = ts
tr.StillRunningAfter = server
