...
class HeaderRewriteSetBodyFromTest:
    ...
    def test_setBodyFromFails_remap(self):
        ...
        tr = Test.AddTestRun()
        tr.MakeCurlCommand(
            '-s -v --proxy 127.0.0.1:{0} "http://www.example.com/remap_fail"'.format(self.ts.Variables.port),
    ...
    def test_setBodyFromSucceeds_remap(self):
        ...
        tr = Test.AddTestRun()
        tr.MakeCurlCommand(
            '-s -v --proxy 127.0.0.1:{0} "http://www.example.com/remap_success"'.format(self.ts.Variables.port),
            uds_path=self.ts.Variables.uds_path)
        tr.Processes.Default.ReturnCode = 0
    ...
    def test_setBodyFromSucceeds_plugin(self):
        ...
        tr = Test.AddTestRun()
        tr.MakeCurlCommand(
            '-s -v --proxy 127.0.0.1:{0} "http://www.example.com/plugin_success"'.format(self.ts.Variables.port),
            uds_path=self.ts.Variables.uds_path)
        tr.Processes.Default.ReturnCode = 0
        tr.Processes.Default.Streams.stdout = "gold/header_rewrite-set_body_from_success.gold"
        tr.Processes.Default.Streams.stderr.Content = Testers.ContainsExpression("404 Not Found", "Expected 404 response")
    ...
    def test_setBodyFromFails_plugin(self):
        ...
        tr = Test.AddTestRun()
        tr.MakeCurlCommand(
            '-s -v --proxy 127.0.0.1:{0} "http://www.example.com/plugin_fail"'.format(self.ts.Variables.port),
            uds_path=self.ts.Variables.uds_path)
        tr.Processes.Default.ReturnCode = 0
    ...
    def test_setBodyFromSucceeds_200(self):
        ...
        tr = Test.AddTestRun()
        tr.MakeCurlCommand(
            '-s -v --proxy 127.0.0.1:{0} "http://www.example.com/200"'.format(self.ts.Variables.port),
            uds_path=self.ts.Variables.uds_path)
        tr.Processes.Default.ReturnCode = 0
