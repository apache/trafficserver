...
tr.MakeCurlCommand(curl_and_args + ' http://ims/path -r 0-', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
...
ps = tr.Processes.Default
tr.MakeCurlCommand(curl_and_args + ' http://ims/path -r 0- -H "X-CRR-IMS: {}"'.format(futurestr), uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
...
ps = tr.Processes.Default
tr.MakeCurlCommand(curl_and_args + ' http://imsheader/path -r 0- -H "CrrIms: {}"'.format(futurestr), uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
ps.Streams.stdout.Content = Testers.ContainsExpression("X-Cache: hit-stale", "expected cache hit-stale")
tr.StillRunningAfter = ts
