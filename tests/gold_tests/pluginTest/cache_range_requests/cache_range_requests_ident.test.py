...
tr.MakeCurlCommand(curl_and_args + ' http://ident/path -r 0-', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
...
tr = Test.AddTestRun("0- range cache load")
ps = tr.Processes.Default
tr.MakeCurlCommand(curl_and_args + ' http://identheader/pathheader -r 0-', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
ps.Streams.stdout.Content = Testers.ContainsExpression("X-Cache: miss", "expected cache miss for load")
tr.StillRunningAfter = ts
...
tr.DelayStart = 2
ps = tr.Processes.Default
tr.MakeCurlCommand(curl_and_args + f" http://ident/path -r 0- -H 'X-Crr-Ident: Etag: {etag}'", uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
...
ps = tr.Processes.Default
tr.MakeCurlCommand(
    curl_and_args + f" http://identheader/pathheader -r 0- -H 'CrrIdent: Etag: {etag_custom}'", uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
ps.Streams.stdout.Content = Testers.ContainsExpression("X-Cache: hit-fresh", "expected cache hit-fresh")
tr.StillRunningAfter = ts
...
ps = tr.Processes.Default
tr.MakeCurlCommand(
    curl_and_args + f' http://ident/path -r 0- -H "X-Crr-Ident: Last-Modified: {last_modified}"', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
ps.Streams.stdout.Content = Testers.ContainsExpression("X-Cache: hit-fresh", "expected cache hit-fresh")
tr.StillRunningAfter = ts
...
ps = tr.Processes.Default
tr.MakeCurlCommand(curl_and_args + f' http://ident/path -r 0- -H "X-Crr-Ident: Last-Modified: foo"', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
