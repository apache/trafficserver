...
ps = tr.Processes.Default
tr.MakeCurlCommand(curl_and_args + ' http://www.example.com/path -r 0- -H "uuid: range_full"', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
ps.Streams.stdout.Content = Testers.ContainsExpression("X-Cache: miss", "expected cache miss for load")
tr.StillRunningAfter = ts
...
ps = tr.Processes.Default
tr.MakeCurlCommand(curl_and_args + ' http://www.fail.com/path -r 0- -H "uuid: range_fail"', uds_path=ts.Variables.uds_path)
ps.ReturnCode = 0
tr.StillRunningAfter = ts
