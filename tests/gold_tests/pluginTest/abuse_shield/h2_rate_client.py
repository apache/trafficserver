...
def get_socket(host: str, port: int, timeout: float = 30.0) -> socket.socket:
    ...
    tls_socket = socket.create_connection((host, port), timeout=timeout)
...
class H2RateClient:
    ...
    def send_request(self, path: str, reset_stream: bool = False) -> int:
        """Send a single HTTP/2 request.

        :param path: The request path.
        :param reset_stream: Whether to cancel the request stream after its headers.
        ...
        try:
            self.h2_conn.send_headers(stream_id, headers, end_stream=not reset_stream)
            if reset_stream:
                self.h2_conn.reset_stream(stream_id, error_code=0x8)
            self.socket.sendall(self.h2_conn.data_to_send())
            if not reset_stream:
    ...
    def run(self, num_requests: int, rate: float, path: str, reset_streams: bool = False) -> Tuple[int, int, int]:
        """Send requests at the specified rate.

        :param num_requests: Total number of requests to send.
        :param rate: Requests per second.
        :param path: Request path.
        :param reset_streams: Whether to cancel every request stream.
        ...
        try:
            for i in range(num_requests):
                ...
                stream_id = self.send_request(path, reset_streams)
                if stream_id > 0:
                    requests_sent += 1
        ...
        failed = (len(self.responses) - successful) + len(self.pending_streams)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--reset-streams', action='store_true', help='Reset each stream with CANCEL after sending its headers')
...
def main() -> int:
    ...
    requests_sent, successful, failed = client.run(args.num_requests, args.rate, args.path, args.reset_streams)
