...
class CompressCacheUntransformedTest:
    ...
    def run(self):
        ...
        tr.Setup.CopyAs("compress_100_continue_client.py")
        tr.Setup.Copy("etc/cache-true-untransformed.config")
        ...
        origin.Command = (
            f'{sys.executable} compress_100_continue_origin.py'
            f' --port {origin_port}')
        ...
        self.ts.Disk.records_config.update(
            {
                ...
                # Enable POST caching so that the 200 OK response to the
                # POST request is cached, triggering the cache write path
                # where the stale client_response_hdr_bytes causes the crash.
        ...
        self.ts.Disk.remap_config.AddLine(
            ...
            f' @pparam={Test.RunDirectory}/cache-true-untransformed.config')

        # Client sends POST with Expect: 100-continue and Accept-Encoding: gzip.
        client = tr.Processes.Default
        client.Command = (
            f'{sys.executable} compress_100_continue_client.py'
            f' 127.0.0.1 {self.ts.Variables.port}')
        client.ReturnCode = 0
