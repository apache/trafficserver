...
/*
Tests to run when "ports ready" and "task threads ready" lifecycle hooks have both fired.
...
//
#define TEST(TEST_FUNC) CoreReadyHook::ATest t(TEST_FUNC);
...
{
  TSReleaseAssert((TS_EVENT_LIFECYCLE_PORTS_READY == event) || (TS_EVENT_LIFECYCLE_TASK_THREADS_READY == event));

  ++lifecycle_event_count;

  TSReleaseAssert(lifecycle_event_count <= 2);
...
init(char const *rm_pathspec_on_completion_of_all_tests)
{
  ...
  TSLifecycleHookAdd(TS_LIFECYCLE_TASK_THREADS_READY_HOOK, TSContCreate(contFunc, nullptr));
}
