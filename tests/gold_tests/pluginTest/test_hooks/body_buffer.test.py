...
class BodyBufferTest:
    ...
    def setupTS(self):
        ...
        self._ts.Streams.stderr = Testers.ContainsExpression(
            f"request_buffer_plugin gets the request body with length [{self.content_length_size}]",
            "Verify that the plugin parsed the content-length request body data.")
        self._ts.Streams.stderr += Testers.ContainsExpression(
            f"request_buffer_plugin gets the request body with length [{self.encoded_chunked_size}]",
