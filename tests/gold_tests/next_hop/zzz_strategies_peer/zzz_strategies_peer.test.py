...
for i in range(num_object):
    tr = Test.AddTestRun()
    tr.MakeCurlCommand(f'--verbose --proxy 127.0.0.1:{ts_peer[i % num_peer].Variables.port} http://dummy.com/obj{i}')
...
for i in range(num_object):
    ...
    tr.MakeCurlCommand(f'--verbose --proxy 127.0.0.1:{ts_peer[(i * 3) % num_peer].Variables.port} http://dummy.com/obj{i}')
    tr.Processes.Default.Streams.stdout = "body.gold"
    tr.Processes.Default.ReturnCode = 0
