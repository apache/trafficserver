'''
Verify remap.yaml acl behavior.
'''
...
class Test_remap_acl:
    ...
    def __init__(
            ...
            acl_configuration: List[str], named_acls: List[Tuple[str,
                                                                 List[str]]], expected_responses: List[int], proxy_protocol: bool):
        """Initialize the test.
    ...
    def _configure_traffic_server(self) -> int:
        ...
        remap_yaml_lines = ['remap:']
        if self._deactivate_ip_allow:
            remap_yaml_lines.append('- deactivate_filter: ip_allow')

        # First, define the name ACLs (filters).
        defined_filters = []
        for name, definition in self._named_acls:
            if len(definition) > 0:
                remap_yaml_lines.append('- define_filter:')
                remap_yaml_lines.append(f'    {name}:')
                for d in definition:
                    remap_yaml_lines.append(f'        {d}')
                defined_filters.append(name)
        # Now activate them.
        for name in defined_filters:
            remap_yaml_lines.append(f'- activate_filter: {name}')

        remap_yaml_lines.append('- type: map')
        remap_yaml_lines.append('  from:')
        remap_yaml_lines.append('     url: /')
        remap_yaml_lines.append('  to:')
        remap_yaml_lines.append(f'     url: http://127.0.0.1:{server_port}')

        if len(self._acl_configuration) > 0:
            remap_yaml_lines.append('  acl_filter:')
        for f in self._acl_configuration:
            remap_yaml_lines.append(f'     {f}')
        ...
        if call_reload:
            ...
            remap_yaml_path = os.path.join(ts.Variables.CONFIGDIR, 'remap.yaml')
            ...
            ts.Disk.remap_yaml.AddLines(remap_yaml_lines)
...
class Test_old_action:
    ...
    def __init__(self, name: str, acl_filter: List[str], ip_allow_content: str) -> None:
    ...
    def _configure_traffic_server(self, tr: 'TestRun', acl_filter: List[str], ip_allow_content: str) -> 'Process':
...
remap:
  - type: map
    ...
    acl_filter: '''.split("\n"))
        ...
        for f in acl_filter:
            ts.Disk.remap_yaml.AddLine(f'      {f}')
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    acl_configuration=['action: set_allow', 'src_ip: 127.0.0.1', 'method: [GET, POST]'],
    named_acls=[],
...
test_ip_allow_optional_methods_pp = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_allow', 'src_ip: 1.2.3.4', 'method: [GET, POST]'],
    named_acls=[],
    expected_responses=[200, 200, 403, 403, 403],
    proxy_protocol=True)
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: add_allow', 'src_ip: 127.0.0.1', 'method: [GET, POST]'],
    named_acls=[],
    expected_responses=[200, 200, 403, 403, 403],
    proxy_protocol=False)
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_allow', 'src_ip: all', 'method: [GET, POST]'],
    named_acls=[],
    expected_responses=[200, 200, 403, 403, 403],
    proxy_protocol=False)
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_allow', 'src_ip_category: ACME_LOCAL', 'method: [GET, POST]'],
    named_acls=[],
    expected_responses=[200, 200, 403, 403, 403],
    proxy_protocol=False)
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_allow', 'method: [GET, POST]'],
    named_acls=[],
    expected_responses=[200, 200, 403, 403, 403],
    proxy_protocol=False)
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_deny', 'src_ip: 127.0.0.1', 'method: [GET, POST]'],
    named_acls=[],
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_allow', 'src_ip_invert: 127.0.0.1', 'method: [GET, POST]'],
    named_acls=[('deny', ['action: set_deny'])],
    expected_responses=[403, 403, 403, 403, 403],
    proxy_protocol=False)
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_allow', 'src_ip_invert: 3.4.5.6', 'method: [GET, POST]'],
    named_acls=[('deny', ['action: set_deny'])],
    expected_responses=[200, 200, 403, 403, 403],
    proxy_protocol=False)
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_allow', 'src_ip_category_invert: ACME_LOCAL', 'method: [GET, POST]'],
    named_acls=[('deny', ['action: set_deny'])],
    expected_responses=[403, 403, 403, 403, 403],
    proxy_protocol=False)
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_allow', 'src_ip_category_invert: ACME_EXTERNAL', 'method: [GET, POST]'],
    named_acls=[('deny', ['action: set_deny'])],
    expected_responses=[200, 200, 403, 403, 403],
    proxy_protocol=False)
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    acl_configuration=['action: set_allow', 'src_ip: 127.0.0.1', 'src_ip_category: ACME_EXTERNAL', 'method: [GET, POST]'],
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_allow', 'in_ip: 127.0.0.1', 'method: [GET, POST]'],
    named_acls=[],
    expected_responses=[200, 200, 403, 403, 403],
    proxy_protocol=False)
...
test_ip_allow_optional_methods = Test_remap_acl(
    ...
    deactivate_ip_allow=False,
    acl_behavior_policy=1,
    acl_configuration=['action: set_allow', 'in_ip: 3.4.5.6', 'method: [GET, POST]'],
    named_acls=[],
    expected_responses=[200, 403, 403, 403, 403],
    proxy_protocol=False)
...
test_named_acl_deny = Test_remap_acl(
    ...
    acl_behavior_policy=1,
    acl_configuration=[],
    named_acls=[('deny', ['action: set_deny', 'method: [HEAD, POST]'])],
...
from deactivate_ip_allow_yaml import all_deactivate_ip_allow_tests_yaml
from all_acl_combinations_yaml import all_acl_combination_tests_yaml
"""
Test all acl combinations
"""
for idx, test in enumerate(all_acl_combination_tests_yaml):
...
Test all ACL combinations
"""
for idx, test in enumerate(all_deactivate_ip_allow_tests_yaml):
    try:
