...
cont_func(TSCont, TSEvent event, void *event_data)
{
  if (TS_EVENT_SSL_SECRET == event) {
    ...
    Dbg(dbg_ctl, "Lifecycle SSL Secret hook invocation number %d", secret_hook_invocation_num);
    return TS_SUCCESS;
  }
  ...
  ++txn_num;
  Dbg(dbg_ctl, "HTTP request number number %d", txn_num);
...
TSPluginInit(int n_arg, char const *arg[])
{
  ...
  Dbg(dbg_ctl, "TSPluginInit() completed.");
