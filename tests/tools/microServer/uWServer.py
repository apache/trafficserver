...
class SSLServer(ThreadingMixIn, HTTPServer):
    ...
    def __init__(self, server_address, HandlerClass, options):
        ...
        if options.clientverify:
            ...
            self.socket = ssl.wrap_socket(
                socket.socket(self.address_family, self.socket_type), keyfile=keys, certfile=certs, server_side=True)
                ...
                for header in headers[1:]:  # skip first one b/c it's response code
                    if header == '':
                        ...
                        if test_mode_enabled:  # the length of the body is given priority in test mode rather than the value in Content-Length. But in replay mode Content-Length gets the priority
                            if not (resp and resp.getBody()
                                   ):  # Don't attach content-length yet if body is present in the response specified by tester
...
def _bool(arg):
    ...
    if tmp in opt_true_values:
        ...
        msg = 'Invalid value Boolean value : "{0}"\n Valid options are {0}'.format(arg, opt_true_values | opt_false_values)
        raise argparse.ArgumentTypeError(msg)
...
def main():
    ...
    parser.add_argument("--data-dir", "-d", type=lambda x: _path(True, x), required=True, help="Directory with data file")

    parser.add_argument(
        "--public", "-P", type=_bool, default=False, help="Bind server to public IP 0.0.0.0 vs private IP of 127.0.0.1")

    parser.add_argument("--ip_address", "-ip", type=str, default='', help="IP address of the interface to serve on")

    parser.add_argument("--port", "-p", type=int, default=SERVER_PORT, help="Port to use")

    parser.add_argument("--timeout", "-t", type=float, default=None, help="socket time out in seconds")

    parser.add_argument('-V', '--version', action='version', version='%(prog)s {0}'.format(__version__))

    parser.add_argument("--mode", "-m", type=str, default="test", help="Mode of operation")
    parser.add_argument("--ssl", "-ssl", type=str, default="False", help="SSL port")
    parser.add_argument("--key", "-k", type=str, default="ssl/server.pem", help="key for ssl connnection")
    parser.add_argument("--cert", "-cert", type=str, default="ssl/server.crt", help="certificate")
    parser.add_argument("--clientverify", "-cverify", type=bool, default=False, help="verify client cert")
    parser.add_argument("--load", dest='load', type=str, default='', help="A file which will install observers on hooks")
