...
def test_records_yaml(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr = urtest.AddTestRun("Load unregistered records")
    ...
    # 1
    tr = urtest.AddTestRun("Query unregistered records.")
    ...
    # 2
    tctl = urtest.AddTestRun("Test field with multiplier")
    ...
    tr = urtest.AddTestRun("Start a new ATS instance.")
    tr.Processes.Default.Command = 'echo 1'
    tr.Processes.Default.ReturnCode = 0
    tr.Processes.Default.StartBefore(ts2)
    tr.Processes.StillRunningAfter = ts2

    tr2 = urtest.AddTestRun("Test multiple docs from the same file")
    ...
    tr3 = urtest.AddTestRun("test null string")
    ...
    tr3.Processes.Default.Streams.stdout += Testers.ContainsExpression(
        'proxy.config.dns.resolv_conf: null', 'should be set to null')
    tr3.Processes.Default.Streams.stdout += Testers.ContainsExpression(
        'proxy.config.dns.nameservers: null', 'should be set to null')
    tr3.Processes.Default.Streams.stdout += Testers.ContainsExpression(
    ...
    tr4 = urtest.AddTestRun("test parsing multiple docs with invalid fields.")
