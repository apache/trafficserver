...
def test_records_config_to_yaml(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Testing ATS records to yaml script.'

    file_suffix = 1

    # case 1 TLS with no data
    tr = urtest.AddTestRun("Test records to yaml convert script - full file")

    tr.Setup.Copy(os.path.join(urtest.Variables.RepoDir, "tools/records/convert2yaml.py"))
    ...
    tr = urtest.AddTestRun("Test records to yaml convert script -only renamed records.")
    tr.Setup.Copy(os.path.join(urtest.Variables.RepoDir, "tools/records/convert2yaml.py"))
    ...
    tr = urtest.AddTestRun("Test errors when trying to override values ")
    tr.Setup.Copy(os.path.join(urtest.Variables.RepoDir, "tools/records/convert2yaml.py"))
    ...
    tr = urtest.AddTestRun("Test errors when trying to override maps")
    tr.Setup.Copy(os.path.join(urtest.Variables.RepoDir, "tools/records/convert2yaml.py"))
    ...
    tr = urtest.AddTestRun("Test config file without trailing newline")
    tr.Setup.Copy(os.path.join(urtest.Variables.RepoDir, "tools/records/convert2yaml.py"))
