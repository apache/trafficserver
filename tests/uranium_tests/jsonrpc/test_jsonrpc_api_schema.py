...
def test_jsonrpc_api_schema(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.Summary = 'Test jsonrpc admin API'

    # set the schema folder.
    schema_folder = os.path.join(urtest.TestDirectory, '..', '..', '..', "src", "mgmt", "rpc", "schema")
    ...
    def add_testrun_for_jsonrpc_request(
        ...
        '''
        tr = urtest.AddTestRun(test_description)
    ...
    urtest.testName = 'Basic JSONRPC API test'
    ...
    storage_path = os.path.join(urtest.RunDirectory, "ts", "storage")
    ts.Disk.storage_yaml.AddLine(
        f'''
    cache:
       spans:
         - name: disk-1
