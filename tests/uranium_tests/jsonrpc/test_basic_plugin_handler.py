...
def test_basic_plugin_handler(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Test JSONRPC Handler inside a plugin'
    ...
    urtest.testName = 'Basic plugin handler test'
    ...
    urtest.PrepareTestPlugin(
        os.path.join(urtest.Variables.AtsBuildUraniumTestsDir, 'jsonrpc', 'plugins', '.libs', 'jsonrpc_plugin_handler_test.so'), ts)
    ...
    tr = urtest.AddTestRun("Test registered API")
    ...
    tr = urtest.AddTestRun("Test registered API - using AddJsonRPCShowRegisterHandlerRequest")
    ...
    tr = urtest.AddTestRun("Test JSONRPC test_join_hosts_method")
    ...
    tr = urtest.AddTestRun("Test test_io_on_et_task")
    newHosts = [
        {
            'name': 'brbzull',
            'status': 'up'
    ...
    # 5
    tr = urtest.AddTestRun("Test test_io_on_et_task - update, yahoo up")
    ...
    # 6
    tr = urtest.AddTestRun("Test privileged field from plugin handlers")
