...
def test_verify_global_plugin(urtest: UraniumTest) -> None:
    '''
    ...
    def create_ts_process():
        ...
        ts = urtest.MakeATSProcess("ts{counter}".format(counter=process_counter), log_data=no_log_data)
    ...
    """
    tr = urtest.AddTestRun("Verify the requirement of an argument")
    ...
    object file.
    """
    tr = urtest.AddTestRun("Verify the requirement of a file")
    ts = create_ts_process()
    tr.Processes.Default.Env = ts.Env
    ...
    """
    tr = urtest.AddTestRun("Verify the requirement of our Plugin API.")
    ts = create_ts_process()
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'missing_ts_plugin_init.so'), ts)
    tr.Processes.Default.Env = ts.Env
    tr.Processes.Default.Command = \
        "traffic_server -C 'verify_global_plugin {filename}'".format(
    ...
    """
    tr = urtest.AddTestRun("Verify a properly formed plugin works as expected.")
    ts = create_ts_process()
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'conf_remap_stripped.so'), ts)
    tr.Processes.Default.Env = ts.Env
    tr.Processes.Default.Command = \
        "traffic_server -C 'verify_global_plugin {filename}'".format(
    ...
    tr = urtest.AddTestRun("Verify a properly formed plugin works as expected.")
    ts = create_ts_process()
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'ssl_hook_test.so'), ts)
    tr.Processes.Default.Env = ts.Env
    tr.Processes.Default.Command = \
        "traffic_server -C 'verify_global_plugin {filename}'".format(
    ...
    and C++ files.
    """
    tr = urtest.AddTestRun("Regression test for an undefined, mangled C++ symbol.")
    ts = create_ts_process()
    plugin_filename = 'missing_mangled_definition.so'
    built_plugin_path = os.path.join(urtest.Variables.AtsTestPluginsDir, plugin_filename)
