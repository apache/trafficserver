...
def test_all_headers(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    if Condition.CurlUsingUnixDomainSocket():
        urtest.Disk.File(sanitized_log_path, exists=True, content='gold/test_all_headers_uds.gold')
    else:
        urtest.Disk.File(sanitized_log_path, exists=True, content='gold/test_all_headers.gold')
    ...
    if Condition.CurlUsingUnixDomainSocket():
        tr.MakeCurlCommand(
            '"http://127.0.0.1:{0}" --user-agent "007" -H "Host: 127.0.0.1:{0}" --verbose '.format(ts.Variables.port) +
            reallyLong(),
            ts=ts)
    else:
    ...
    sanitizer_python_script = os.path.join(urtest.TestDirectory, 'all_headers_sanitizer.py')
    sanitizer_shell_script = os.path.join(urtest.TestDirectory, 'all_headers_sanitizer.sh')
