...
def test_sigusr2(urtest: UraniumTest) -> None:
    '''
    ...
    class Sigusr2Test:
        ...
        def _configure_server(self):
            server = urtest.MakeOriginServer(self._next_server_name())
        ...
        def add_system_log_test(self):
            tr = urtest.AddTestRun('Verify system logs can be rotated')
        ...
        def add_configured_log_test(self):
            tr = urtest.AddTestRun('Verify yaml.log logs can be rotated')
            ts, ts_name = self._configure_traffic_server(tr)
    ...
    urtest.Setup.Copy(TS_PID_SCRIPT)
    urtest.Setup.Copy(ROTATE_DIAGS_SCRIPT)
    urtest.Setup.Copy(ROTATE_CUSTOM_LOG_SCRIPT)
