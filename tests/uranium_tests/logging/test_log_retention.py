...
def test_log_retention(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipIf(Condition.true("This test is sensitive to timing issues which makes it flaky."))
    ...
    class TestLogRetention:
        ...
        def __init__(self, records_config, run_description, command="traffic_server"):
            ...
            self.tr = urtest.AddTestRun(run_description)
        ...
        def __initialize_processes(self):
            ...
            """
            tr = urtest.AddTestRun("Initialize processes for ts{}".format(TestLogRetention.__ts_counter - 1))
        ...
        def __create_ts(self, records_config, command="traffic_server"):
            ...
            self.ts = urtest.MakeATSProcess(ts_name, command=command)
    ...
    test = TestLogRetention(
        twelve_meg_log_space, "Verify log rotation and deletion of the configured log file with a min_count of 1.")
    ...
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'test_log_interface.so'), test.ts)
    ...
    tr = urtest.AddConfigReload(test.ts, description="Perform a config reload")
    tr.StillRunningAfter = test.server

    tr = urtest.AddTestRun("Get the log to rotate.")
