...
def test_binary_log_v3(urtest: UraniumTest) -> None:
    '''
    Uranium test of the self-describing v3 binary log format.
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    ...
    class BinaryLogV3Test:
        ...
        def __configure_traffic_server(self):
            ...
            ts.Disk.remap_config.AddLine(
                f'map http://127.0.0.1:{ts.Variables.port}/ http://127.0.0.1:{self.httpbin.Variables.Port}/')
        ...
        def __configure_traffic_test_run(self):
            ...
            for i in range(BinaryLogV3Test.num_requests):
                tr = urtest.AddTestRun(f'Generate binary log traffic #{i + 1}')
                if i == 0:
        ...
        def __configure_decode_test_runs(self):
            ...
            urtest.AddAwaitFileContainsTestRun('Await the logs to flush.', self.ascii_log, r'/get', BinaryLogV3Test.num_requests)

            # The binary_log_version:2 log decodes to the same ASCII as v3.
            tr = urtest.AddTestRun('Decode the v2 binary log to ASCII')
            ...
            tr = urtest.AddTestRun('Decode the v3 binary log to ASCII')
            tr.Processes.Default.Command = f'{logcat} {self.v3_blog}'
            tr.Processes.Default.ReturnCode = 0
            tr.Processes.Default.Streams.stdout = 'gold/binary_log_v3_ascii.gold'

            # traffic_logcat -j decodes the v3 .blog to the expected JSON.
            tr = urtest.AddTestRun('Decode the v3 binary log to JSON')
            ...
            tr = urtest.AddTestRun('Print the v3 binary log header')
            ...
            # field-type schema.
            tr = urtest.AddTestRun('Print the v2 binary log header')
