...
def test_log_milestone_fields(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.Summary = 'Verify msdms milestone logging fields for cache miss and cache hit paths'
    ...
    class MilestoneFieldsTest:
        ...
        def __init__(self):
            ...
            self._nameserver = urtest.MakeDNServer("dns", default='127.0.0.1')
    ...
    '''.split("\n"))
        ...
        def _validate_script(self) -> str:
            return os.path.join(urtest.TestDirectory, 'verify_milestone_fields.py')
        ...
        def _sendCacheMiss(self):
            tr = urtest.AddTestRun('Cache miss request')
        ...
        def _waitForCacheIO(self):
            tr = urtest.AddTestRun('Wait for cache write to complete')
        ...
        def _sendCacheHit(self):
            tr = urtest.AddTestRun('Cache hit request')
            tr.MakeCurlCommand(
                f'--verbose --header "Host: example.com" '
                f'http://127.0.0.1:{self._ts.Variables.port}/cacheable', ts=self._ts)
            tr.Processes.Default.ReturnCode = 0
        ...
        def _validateLog(self):
            tr = urtest.AddTestRun('Validate milestone fields in log')
