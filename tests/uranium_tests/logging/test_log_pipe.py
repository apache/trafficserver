...
def test_log_pipe(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    '''
    urtest.SkipUnless(Condition.HasATSFeature('TS_HAS_PIPE_BUFFER_SIZE_CONFIG'))
    ...
    def get_ts(logging_config, disable_log_checks=False):
        ...
        """
        nonlocal ts_counter
        ts = urtest.MakeATSProcess("ts{}".format(ts_counter), disable_log_checks=disable_log_checks)
        ts_counter += 1
    ...
    ts.Disk.diags_log.Content += Testers.ExcludesExpression(
        r"ERROR:(?! Set pipe size failed for pipe .*: Operation not permitted)",
        "The diagnostics should contain no unexpected errors.")
