...
def test_qlog_quiche(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    class quiche_qlog_Test:
        ...
        def _configure_traffic_server(self, tr: 'TestRun'):
            ...
            """
            ts = urtest.MakeATSProcess(f"ts-{quiche_qlog_Test.ts_counter}", enable_quic=True, enable_tls=True)
            self._ts = ts
    ...
    """.split("\n"))
        ...
        def test(self):
            ...
            test_run = urtest.AddTestRun(f"{self._name} : check qlog files")
            qlog_base_name = "test_qlog"
            rename_script = os.path.join(urtest.TestDirectory, 'rename_qlog.sh')
            ...
            # Basic valdation
            if self._generate_qlog:
                tr = urtest.AddTestRun("Check qlog content")
