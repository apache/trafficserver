...
def test_parent_config_reload(urtest: UraniumTest) -> None:
    '''
    ...
    tr = urtest.AddTestRun("Touch parent.config")
    tr.Processes.Default.StartBefore(ts)
    tr.Processes.Default.Command = f"sleep 3 && touch {os.path.join(config_dir, 'parent.config')} && sleep 1"
    tr.Processes.Default.ReturnCode = 0
    tr.StillRunningAfter = ts

    tr = urtest.AddConfigReload(ts, expect_tasks=["parent.config"], description="Reload after parent.config touch")
    tr.StillRunningAfter = ts
    ...
    tr = urtest.AddTestRun("Change parent retry_time record value")
