...
def test_config_destroy_thread(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipIf(Condition.true("Test takes over 60 seconds to run."))
    ...
    tr = urtest.AddTestRun("Mark parent.config for reload")
    tr.Processes.Default.StartBefore(ts)
    tr.Processes.Default.Command = f"sleep 3 && touch {os.path.join(config_dir, 'parent.config')} && sleep 1"
    tr.Processes.Default.ReturnCode = 0
    tr.StillRunningAfter = ts

    urtest.AddConfigReload(ts, expect="any", token="config_destroy_thread")

    tr = urtest.AddTestRun("Replace the HTTP config from a network thread")
    tr.DelayStart = 3
    ...
    tr = urtest.AddTestRun("Wait for the release timeout to expire")
    tr.DelayStart = 3
