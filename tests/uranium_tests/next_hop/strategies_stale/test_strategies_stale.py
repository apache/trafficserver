...
def test_strategies_stale(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    #
    ts_nh = urtest.MakeATSProcess(f"ts_nh0", use_traffic_out=False, command=f"traffic_server 2>nh_trace.log")
    ...
    tr = urtest.AddTestRun()
    tr.MakeCurlCommand(f'--verbose --proxy 127.0.0.1:{ts.Variables.port} http://dummy.com/obj0', ts=ts)
    ...
    tr.Processes.Default.Streams.stdout.Content = Testers.ContainsExpression(
        "Stale in cache", "Expected stale content to be evaluated")
    tr.Processes.Default.ReturnCode = 0
    urtest.execute()
