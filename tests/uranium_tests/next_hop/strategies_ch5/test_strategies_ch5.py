...
def test_strategies_ch5(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    for ii in range(num_hosts):
        ts = urtest.MakeATSProcess(f"ts_nh{ii}")
    ...
    ts = urtest.MakeATSProcess("ts", use_traffic_out=False, command="traffic_server 2> trace.log")
