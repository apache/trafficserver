...
def test_zzz_strategies_peer2(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    for i in range(num_upstream):
        ts = urtest.MakeATSProcess(f"ts_upstream{i}")
    ...
    for i in range(num_peer):
        ts = urtest.MakeATSProcess(f"ts_peer{i}", use_traffic_out=False, command=f"traffic_server 2> trace_peer{i}.log")
