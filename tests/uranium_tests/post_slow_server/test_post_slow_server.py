...
def test_post_slow_server(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipIf(Condition.true("Test takes too long to run it in CI."))
    ...
    ts.Disk.remap_config.AddLine('map https://localhost http://localhost:{}'.format(urtest.Variables.server_port))
    ...
    #
    tr = urtest.AddTestRun()
    tr.Processes.Default.Command = "bash -c '" + urtest.TestDirectory + "/check.sh'"
    tr.Processes.Default.ReturnCode = 0
    urtest.execute()
