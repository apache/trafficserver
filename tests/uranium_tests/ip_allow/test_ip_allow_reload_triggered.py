...
def test_ip_allow_reload_triggered(urtest: UraniumTest) -> None:
    '''
    ...
    # --- Setup: origin server ---
    server = urtest.MakeOriginServer("server", ssl=False)
    ...
    categories_allow = os.path.join(urtest.RunDirectory, 'categories_allow.yaml')
    ...
    categories_deny = os.path.join(urtest.RunDirectory, 'categories_deny.yaml')
    with open(categories_deny, 'w') as f:
        f.write('ip_categories:\n  - name: INTERNAL\n    ip_addrs: 1.2.3.4\n')

    # Version C: 127.0.0.1 back in INTERNAL (for record value change test)
    categories_restore = os.path.join(urtest.RunDirectory, 'categories_restore.yaml')
    with open(categories_restore, 'w') as f:
        f.write('ip_categories:\n  - name: INTERNAL\n    ip_addrs: 127.0.0.1\n')

    # Active ip_categories file that the record points to (start with allow version)
    categories_file = os.path.join(urtest.RunDirectory, 'ip_categories.yaml')
    ...
    tr = urtest.AddTestRun("Touch ip_allow.yaml")
    ...
    reload_counter += 1
    tr = urtest.AddConfigReload(ts, expect_tasks=["ip_allow.yaml"], description="Reload after ip_allow.yaml touch")
    tr.StillRunningAfter = ts
    ...
    tr = urtest.AddTestRun("Touch ip_categories")
    tr.Processes.Default.Command = f"touch {categories_file} && sleep 1"
    tr.Processes.Default.ReturnCode = 0
    tr.StillRunningAfter = ts

    reload_counter += 1
    tr = urtest.AddConfigReload(ts, expect_tasks=["ip_allow.yaml"], description="Reload after ip_categories touch")
    tr.StillRunningAfter = ts
    ...
    tr = urtest.AddTestRun("Touch hosting.config before reload")
    tr.Processes.Default.Command = f"touch {os.path.join(config_dir, 'hosting.config')}"
    tr.Processes.Default.ReturnCode = 0
    tr.StillRunningAfter = ts

    tr = urtest.AddConfigReload(ts, expect_absent_tasks=["ip_allow.yaml"], description="Reload (should NOT trigger ip_allow)")
    ...
    tr = urtest.AddTestRun("GET should succeed (127.0.0.1 in INTERNAL category)")
    ...
    tr = urtest.AddTestRun("Change ip_categories to deny 127.0.0.1")
    ...
    reload_counter += 1
    tr = urtest.AddConfigReload(ts, expect_tasks=["ip_allow.yaml"], description="Reload after ip_categories content change")
    tr.StillRunningAfter = ts

    # 4d: GET should now be denied (falls to catch-all: only HEAD allowed)
    tr = urtest.AddTestRun("GET should be denied (127.0.0.1 NOT in INTERNAL)")
    tr.Processes.Default.Command = (f"curl -s -o /dev/null -w '%{{http_code}}' "
                                    f"http://127.0.0.1:{ts.Variables.port}/test")
    tr.Processes.Default.ReturnCode = 0
    tr.Processes.Default.Streams.stdout = Testers.ContainsExpression(
        "403", "Should get 403 when 127.0.0.1 is not in INTERNAL category")
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server
    ...
    reload_counter += 1
    tr = urtest.AddTestRun("Change ip_categories record value to new file")
    ...
    tr = urtest.AddTestRun("GET should succeed after record value change")
    tr.Processes.Default.Command = (f"curl -s -o /dev/null -w '%{{http_code}}' "
                                    f"http://127.0.0.1:{ts.Variables.port}/test")
    tr.Processes.Default.ReturnCode = 0
    tr.Processes.Default.Streams.stdout = Testers.ContainsExpression(
