...
def test_ip_category(urtest: UraniumTest) -> None:
    '''
    ...
    class CategoryFile:
        ...
        parent_directory: str = urtest.RunDirectory
        _index: int = 0
    ...
    class Test_ip_category:
        ...
        _categories_filename: str = f'{urtest.RunDirectory}/categories.yaml'
        ...
        def _update_remap_with_acl(self) -> None:
            ...
            if Test_ip_category._ts:
                if self._acl_configuration:
                    tr = urtest.AddTestRun(f"remap.config file update with acl: {self._acl_configuration}")
                    ...
                    tr = urtest.AddTestRun(f"remap.config file update with no acl")
                    p = tr.Processes.Default
                    destination = os.path.join(Test_ip_category._ts.Variables.CONFIGDIR, 'remap.config')
                    common = f'map / http://127.0.0.1:{Test_ip_category._server.Variables.http_port} '
        ...
        def _update_categories_file(self, category_content: 'CategoryFile') -> None:
            ...
            """
            tr = urtest.AddTestRun(f"Categories file update: {category_content.get_path()}")
            p = tr.Processes.Default
        ...
        def _configure_server(self) -> None:
            ...
            if Test_ip_category._server:
                ...
                return
            server = urtest.MakeVerifierServerProcess(f"server", self._server_replay)
        ...
        def _configure_traffic_server(self) -> None:
            ...
            if Test_ip_category._ts:
                ...
                tr = urtest.AddConfigReload(ts, expect_tasks=["ip_allow.yaml"], description="Reload the configuration")
