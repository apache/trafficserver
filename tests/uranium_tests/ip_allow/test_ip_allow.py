...
def test_ip_allow(urtest: UraniumTest) -> None:
    '''
    ...
    #
    tr = urtest.AddTestRun('Allowed GET request')
    ...
    tr.MakeCurlCommand(
        '--verbose -H "Host: www.example.com" http://localhost:{ts_port}/get'.format(ts_port=ts.Variables.port), ts=ts)
    ...
    #
    tr = urtest.AddTestRun('Denied CONNECT request')
    tr.MakeCurlCommand(
    ...
    #
    tr = urtest.AddTestRun('Denied PUSH request over HTTP/2')
    tr.MakeCurlCommand(
        '--http2 --verbose -k -X PUSH -H "Host: localhost" https://localhost:{ts_port}/h2_push'.format(
    ...
    tr = urtest.AddTestRun('Await and verify the transaction log file')
    squid_log = os.path.join(ts.Variables.LOGDIR, 'squid.log')
    tr.Processes.Default.Command = (
        os.path.join(urtest.Variables.AtsTestToolsDir, 'stdout_wait') + ' 60 "{} {}" {}'.format(
            os.path.join(urtest.TestDirectory, 'run_sed.sh'), squid_log, os.path.join(urtest.TestDirectory, 'gold/log.gold')))
    tr.Processes.Default.ReturnCode = 0
