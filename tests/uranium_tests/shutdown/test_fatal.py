...
def test_fatal(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Test TSFatal API'
    urtest.ContinueOnFail = True

    # Define default ATS
    ts = urtest.MakeATSProcess('ts')

    urtest.testName = 'Fatal Shutdown Test'
    ...
    # Load plugin
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'fatal_shutdown.so'), ts)
