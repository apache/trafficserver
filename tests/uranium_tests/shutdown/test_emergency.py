...
def test_emergency(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Test TSEmergency API'
    urtest.ContinueOnFail = True

    # Define default ATS
    ts = urtest.MakeATSProcess('ts')

    urtest.testName = 'Emergency Shutdown Test'
    ...
    # Load plugin
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'emergency_shutdown.so'), ts)
    ...
    ts.Disk.diags_log.Content = Testers.IncludesExpression(
        'testing emergency shutdown', 'should contain "testing emergency shutdown"')
    urtest.execute()
