...
def test_tls_check_cert_selection_reload(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    server3 = urtest.MakeOriginServer("server3", ssl=True)
    ...
    tr = urtest.AddTestRun("bar.com cert signer1")
    ...
    tr = urtest.AddTestRun("bar.com cert signer2")
    tr.MakeCurlCommand(
    ...
    tr = urtest.AddTestRun("Pause a little to ensure mtime will be different")
    tr.Processes.Default.Command = "sleep 2"
    tr.Processes.Default.ReturnCode = 0

    tr = urtest.AddTestRun("Update the bar cert to the signed 2 version")
    ...
    tr = urtest.AddConfigReload(ts, expect_tasks=["ssl_multicert.yaml"], description="Reload config")
    tr.StillRunningAfter = server

    tr = urtest.AddTestRun("Try with signer 1 again")
    ...
    tr = urtest.AddTestRun("Try with signer 2 again")
    tr.MakeCurlCommand(
        "-v --cacert ./signer2.pem  --resolve 'bar.com:{0}:127.0.0.1' https://bar.com:{0}/random".format(ts.Variables.ssl_port),
        ts=ts)
    tr.ReturnCode = 0
