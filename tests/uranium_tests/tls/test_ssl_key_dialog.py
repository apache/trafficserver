...
def test_ssl_key_dialog(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr = urtest.AddTestRun("use a key with passphrase")
    ...
    tr2reload = urtest.AddConfigReload(ts, expect_tasks=["ssl_multicert.yaml"], description="Reload config")
    tr2reload.StillRunningAfter = server

    tr3 = urtest.AddTestRun("use a key with passphrase")
