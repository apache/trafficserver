...
def test_tls_hooks_client_verify(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.PrepareTestPlugin(
        os.path.join(urtest.Variables.AtsTestPluginsDir, 'ssl_client_verify_test.so'), ts, '-count=2 -good=foo.com')
    ...
    tr3 = urtest.AddTestRun("request badly signed cert")
    tr3.Setup.Copy("ssl/server.pem")
    tr3.Setup.Copy("ssl/server.key")
    tr3.StillRunningAfter = ts
    tr3.StillRunningAfter = server
    tr3.MakeCurlCommand(
        "--tls-max 1.2 -k --cert ./server.pem --key ./server.key --resolve 'foo.com:{0}:127.0.0.1' https://foo.com:{0}/case1"
        .format(ts.Variables.ssl_port),
        ts=ts)
