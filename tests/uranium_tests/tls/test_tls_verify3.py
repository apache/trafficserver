...
def test_tls_verify3(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr = urtest.AddTestRun("foo.com Permissive-Test")
    ...
    tr = urtest.AddTestRun("my.random.com Permissive-Test log failure")
    tr.MakeCurlCommand(
        "-v -k --resolve 'my.random.com:{0}:127.0.0.1' https://my.random.com:{0}".format(ts.Variables.ssl_port), ts=ts)
    tr.ReturnCode = 0
    tr.StillRunningAfter = server
    tr.StillRunningAfter = ts
    tr.Processes.Default.Streams.stdout = Testers.ExcludesExpression("Could Not Connect", "Curl attempt should have succeeded")

    tr2 = urtest.AddTestRun("bob.bar.com Override-enforcing-Test")
    ...
    tr3 = urtest.AddTestRun("bob.foo.com override-enforcing-name-test")
    ...
    tr3 = urtest.AddTestRun("random.bar.com override-no-test")
    tr3.MakeCurlCommand(
