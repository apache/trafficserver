...
def test_tls_secret_update_default(urtest: UraniumTest) -> None:
    '''
    ...
    class TestDefaultSecretUpdate:
        ...
        def _configure_traffic_server(self) -> 'Process':
            ...
            urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, self.plugin), ts)
        ...
        def _add_start_run(self) -> 'TestRun':
            ...
            tr = urtest.AddTestRun('Start ATS')
        ...
        def _add_config_reload_run(self) -> 'TestRun':
            ...
            tr = urtest.AddTestRun('Reload the multicert configuration serially')
        ...
        def _add_config_touch_run(self) -> 'TestRun':
            ...
            tr = urtest.AddTestRun('Mark the multicert configuration for reload')
        ...
        def _add_config_reload_wait_run(self) -> 'TestRun':
            ...
            tr = urtest.AddTestRun('Wait for the multicert configuration reload')
        ...
        def _add_mtime_delay_run(self) -> 'TestRun':
            ...
            tr = urtest.AddTestRun('Make the cert mtime differ')
        ...
        def _add_shadowed_certificate_update_run(self) -> 'TestRun':
            ...
            tr = urtest.AddTestRun('Update the shadowed wildcard certificate')
        ...
        def _add_unchanged_default_certificate_run(self) -> 'TestRun':
            ...
            tr.Processes.Default.Streams.All += Testers.ContainsExpression(
                r'subject:.*CN=bar\.com', 'Default cert is still bar.com.')
        ...
        def _add_default_certificate_update_run(self) -> 'TestRun':
            ...
            tr = urtest.AddTestRun('Update the default certificate')
        ...
        def _add_secret_update_wait_run(self, secret_name: str) -> 'TestRun':
            ...
            tr = urtest.AddTestRun(f'Wait for the {secret_name} secret update')
        ...
        def _add_curl_run(
            ...
            tr.Processes.Default.ReturnCode = 0
            tr.Processes.Default.Streams.All = Testers.ContainsExpression(
                f'issuer:.*CN={re.escape(expected_cn)}', expected_description)
            tr.Processes.Default.Streams.All += Testers.ExcludesExpression(
