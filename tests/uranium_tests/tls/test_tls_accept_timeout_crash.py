...
def test_tls_accept_timeout_crash(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    urtest.Summary = 'TLS accept timeout must not crash HttpSessionAccept when SSN_START hook defers.'
    ...
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'hook_add_plugin.so'), ts, '-delay-ms=3000')
    ...
    tr = urtest.AddTestRun("TLS request through delayed SSN_START hook")
