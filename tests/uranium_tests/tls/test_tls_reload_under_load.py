...
def test_tls_reload_under_load(urtest: UraniumTest) -> None:
    '''
    ...
    class TestTlsReloadUnderLoad:
        ...
        def _configure_trafficserver(self) -> 'Process':
            ...
            '''
            ts = urtest.MakeATSProcess(f'ts-{TestTlsReloadUnderLoad._ts_counter}', enable_tls=True, enable_cache=False)
    ...
    """.split("\n"))
        ...
        def run(self) -> None:
            ...
            tr = urtest.AddTestRun("reload certs while TLS handshakes are in flight")
            tr.Processes.Default.StartBefore(self._ts)
            tr.Processes.Default.Command = (
                f'{sys.executable} {os.path.join(urtest.TestDirectory, "tls_reload_under_load_client.py")} '
