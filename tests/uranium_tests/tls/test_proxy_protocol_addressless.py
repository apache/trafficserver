...
def test_proxy_protocol_addressless(urtest: UraniumTest) -> None:
    '''
    ...
    def add_addressless_proxy_protocol_run(protocol_version: int, description: str, use_tls: bool) -> None:
        ...
        ts = urtest.MakeATSProcess(
            f'ts_{mode}_v{protocol_version}', enable_tls=use_tls, enable_cache=False, enable_proxy_protocol=True)
        server = urtest.MakeOriginServer(f'server_{mode}_v{protocol_version}')
        server.ReturnCode = 0
    ...
    """.split("\n"))
        ...
        tr.Processes.Default.Streams.All += Testers.ContainsExpression(
            "HTTP/1.1 200 OK", "Verify a successful response is received")
