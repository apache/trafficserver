...
def test_tls_session_key_logging(urtest: UraniumTest) -> None:
    '''
    ...
    class TlsKeyloggingTest:
        ...
        def setupOriginServer(self):
            ...
            self.server = urtest.MakeVerifierServerProcess(server_name, TlsKeyloggingTest.replay_file)

        def setupTS(self, enable_secrets_logging):
            ts_name = f"ts_{TlsKeyloggingTest.ts_counter}"
            TlsKeyloggingTest.ts_counter += 1
            self.ts = urtest.MakeATSProcess(ts_name, enable_tls=True, enable_cache=False)
