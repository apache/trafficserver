...
def test_tls_cert_comp_cache(urtest: UraniumTest) -> None:
    '''
    ...
    """.split("\n"))
        ...
        def run(self) -> None:
            ...
            tr = urtest.AddTestRun(f'Send 2 requests with cache {cache_label}')
            ...
            tr = urtest.AddTestRun(f'Verify compression count with cache {cache_label}')
            ...
            tr = urtest.AddTestRun(f'Verify decompression count with cache {cache_label}')
            tr.Processes.Default.Command = (f'traffic_ctl metric get'
            ...
            tr = urtest.AddTestRun(f'Verify no compression failures with cache {cache_label}')
            ...
            if self._cache_enabled:
                tr = urtest.AddTestRun(f'Verify cache_hit is 1 with cache {cache_label}')
                ...
                tr = urtest.AddTestRun(f'Verify cache_hit is 0 with cache {cache_label}')
                tr.Processes.Default.Command = 'traffic_ctl metric get proxy.process.ssl.cert_compress.cache_hit'
                tr.Processes.Default.Env = self._ts_mid.Env
                tr.Processes.Default.ReturnCode = 0
                tr.Processes.Default.Streams.All = Testers.ContainsExpression(
