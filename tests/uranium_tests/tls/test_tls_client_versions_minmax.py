...
def test_tls_client_versions_minmax(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.SkipUnless(Condition.HasLegacyTLSSupport())
    ...
    tr = urtest.AddTestRun("foo.com TLSv1_2")
    ...
    if has_curl_tlsv1:
        tr = urtest.AddTestRun("foo.com TLSv1")
        tr.MakeCurlCommand(
            "-v --ciphers DEFAULT@SECLEVEL=0 --tls-max 1.0 --tlsv1 --resolve 'foo.com:{0}:127.0.0.1' -k  https://foo.com:{0}"
    ...
    if has_curl_tlsv1_1:
        tr = urtest.AddTestRun("foo.com TLSv1_1")
        tr.MakeCurlCommand(
            "-v --ciphers DEFAULT@SECLEVEL=0 --tls-max 1.1 --tlsv1.1 --resolve 'foo.com:{0}:127.0.0.1' -k  https://foo.com:{0}"
            .format(ts.Variables.ssl_port),
            ts=ts)
        tr.ReturnCode = 0
        tr.StillRunningAfter = ts
    ...
    if has_curl_tlsv1:
        tr = urtest.AddTestRun("bar.com TLSv1")
        tr.MakeCurlCommand(
            "-v --ciphers DEFAULT@SECLEVEL=0 --tls-max 1.0 --tlsv1 --resolve 'bar.com:{0}:127.0.0.1' -k  https://bar.com:{0}"
            .format(ts.Variables.ssl_port),
            ts=ts)
        tr.ReturnCode = 35
        tr.StillRunningAfter = ts
    ...
    tr = urtest.AddTestRun("bar.com TLSv1_2")
    tr.MakeCurlCommand(
