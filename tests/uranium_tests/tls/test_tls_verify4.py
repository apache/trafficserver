...
def test_tls_verify4(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr = urtest.AddTestRun("default-enforce-bad-sig")
    ...
    trreload = urtest.AddConfigReload(ts, description="Reload config")
    trreload.StillRunningAfter = server

    tragain = urtest.AddTestRun("permissive-after-update")
    ...
    tr2 = urtest.AddTestRun("Update config files to enforced")
    recordspath = ts.Disk.records_config.AbsPath
    # recreate the records.config with the cert filename changed
    tr2.Disk.File(recordspath, id="records_config", typename="ats:config:records"),
    tr2.Disk.records_config.update(
    ...
    trreload = urtest.AddConfigReload(ts, description="Reload config to ENFORCED")
    trreload.StillRunningAfter = server

    tragain = urtest.AddTestRun("enforced-after-update")
