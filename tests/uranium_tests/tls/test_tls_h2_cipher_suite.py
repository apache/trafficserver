...
def test_tls_h2_cipher_suite(urtest: UraniumTest) -> None:
    ...
    """.split("\n"))
        ...
        def _configure_allowed_cipher_test(self) -> None:
            ...
            tr = urtest.AddTestRun("Allow HTTP/2 with an ephemeral AEAD cipher")
        ...
        def _configure_prohibited_cipher_test(self, description: str, cipher: str) -> None:
            ...
            """
            tr = urtest.AddTestRun(f"Fall back to HTTP/1.1 with a prohibited {description} cipher")
            tr.Processes.Default.Command = (
                "printf 'GET / HTTP/1.1\\r\\nHost: example.com\\r\\nConnection: close\\r\\n\\r\\n' | "
            ...
            tr.Processes.Default.Streams.All += Testers.IncludesExpression(
                "ALPN protocol: http/1.1", "HTTP/1.1 should be negotiated")
            tr.Processes.Default.Streams.All += Testers.IncludesExpression(
                "HTTP/1.1 404", "The HTTP/1.1 request should be processed")
            tr.ReturnCode = 0
            tr.StillRunningAfter = self._ts
