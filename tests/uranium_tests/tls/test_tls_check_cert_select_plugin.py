...
def test_tls_check_cert_select_plugin(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr = urtest.AddTestRun("bar.com cert")
    ...
    tr2 = urtest.AddTestRun("foo.com cert")
    tr2.MakeCurlCommand(
    ...
    tr2 = urtest.AddTestRun("random.server.com cert")
    tr2.MakeCurlCommand(
    ...
    # differs
    trupdate = urtest.AddTestRun("Update server bar cert file in place")
    ...
    # should fail
    tr = urtest.AddTestRun("Test new version of bar cert with good CA")
    tr.DelayStart = 4
    tr.MakeCurlCommandMulti(
        "date; {{curl}} -v --cacert ./signer.pem  --resolve 'bar.com:{0}:127.0.0.1' https://bar.com:{0}".format(
    ...
    tr = urtest.AddTestRun("Test new version of bar cert with bad CA")
    tr.MakeCurlCommand(
        "-v --cacert ./signer2.pem  --resolve 'bar.com:{0}:127.0.0.1' https://bar.com:{0}".format(ts.Variables.ssl_port), ts=ts)
    tr.ReturnCode = 60
    tr.StillRunningAfter = server
    ...
    tr.Processes.Default.Streams.All = Testers.ContainsExpression(
        r"curl: \(60\) SSL certificate", "Failed certificate verification")
