...
def test_tls_client_cert_override_plugin(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    tr3bar = urtest.AddTestRun("Make request with other foo.  badcase1 should now work")
    ...
    trupdate = urtest.AddTestRun("Update signed-foo cert file in place")
    trupdate.StillRunningAfter = ts
    trupdate.StillRunningAfter = server
    ...
    tr = urtest.AddTestRun("Retest case1")
    tr.DelayStart = 4
    tr.ReturnCode = 0
    tr.StillRunningAfter = server
    tr.StillRunningAfter = server2
    ...
    tr = urtest.AddTestRun("Retest badcase1")
    tr.ReturnCode = 0
    tr.StillRunningAfter = server
    tr.StillRunningAfter = server2
    tr.StillRunningAfter = ts
