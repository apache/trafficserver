...
def test_tls_client_cert_plugin(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'ssl_secret_load_test.so'), ts)
    ...
    # Should succeed
    tr = urtest.AddTestRun("Connect with first client cert to first server")
    ...
    # Should fail
    trfail = urtest.AddTestRun("Connect with first client cert to second server")
    ...
    # Should succeed
    trbar = urtest.AddTestRun("Connect with signed2 bar to second server")
    ...
    # Should fail
    trbarfail = urtest.AddTestRun("Connect with signed2 bar cert to first server")
    ...
    tr2reload = urtest.AddConfigReload(ts, expect_tasks=["sni.yaml"], description="Reload config")
    tr2reload.StillRunningAfter = server
    tr2reload.StillRunningAfter = server2

    # Should succeed
    tr3bar = urtest.AddTestRun("Make request with other bar cert to first server")
    ...
    # Should fail
    tr3barfail = urtest.AddTestRun("Make request with other bar cert to second server")
    ...
    # Should succeed
    tr3 = urtest.AddTestRun("Make request with other cert to second server")
    ...
    # Should fail
    tr3fail = urtest.AddTestRun("Make request with other cert to first server")
    ...
    trupdate = urtest.AddTestRun("Update client cert file in place")
    ...
    trreload = urtest.AddConfigReload(ts, expect_tasks=["sni.yaml"], description="Reload config after renaming certs")
    trreload.StillRunningAfter = server
    trreload.StillRunningAfter = server2

    # Should succeed
    tr4bar = urtest.AddTestRun("Make request with renamed bar cert to second server")
    ...
    # Should fail
    tr4barfail = urtest.AddTestRun("Make request with renamed bar cert to first server")
    ...
    # Should succeed
    tr4 = urtest.AddTestRun("Make request with renamed foo cert to first server")
    ...
    # Should fail
    tr4fail = urtest.AddTestRun("Make request with renamed foo cert to second server")
