...
def test_tls_verify_override(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr2 = urtest.AddTestRun("default-permissive-fail")
    ...
    tr2 = urtest.AddTestRun("default-permissive-fail2")
    ...
    tr3 = urtest.AddTestRun("default-foo-to-bar")
    ...
    tr3 = urtest.AddTestRun("override-foo")
    ...
    tr4 = urtest.AddTestRun("override-bar-disabled")
    ...
    tr4.Processes.Default.Streams.All = Testers.ExcludesExpression("Could Not Connect", "Curl attempt should have succeeded")
    tr4.Processes.Default.Streams.All += Testers.ContainsExpression("200 OK", "Curl attempt should have succeeded")

    tr5 = urtest.AddTestRun("override-bar-signature-enforced")
    ...
    tr5a = urtest.AddTestRun("override-bar-none-permissive")
    ...
    tr6 = urtest.AddTestRun("override-bar-enforced")
    ...
    # Should succeed
    tr = urtest.AddTestRun("foo-to-bar-sni-policy-remap")
    tr.MakeCurlCommand("-v -k -H \"host: foo.com\"  http://127.0.0.1:{0}/snipolicybarremap".format(ts.Variables.port), ts=ts)
    tr.ReturnCode = 0
    tr.StillRunningAfter = server
    tr.StillRunningAfter = ts
    tr.Processes.Default.Streams.All = Testers.ExcludesExpression("Could not connect", "Curl attempt should succeed")
    tr.Processes.Default.Streams.All += Testers.ContainsExpression("200 OK", "Curl attempt should have succeeded")

    # Should fail
    tr = urtest.AddTestRun("foo-to-bar-sni-policy-host")
    tr.MakeCurlCommand("-v -k -H \"host: foo.com\"  http://127.0.0.1:{0}/snipolicybarhost".format(ts.Variables.port), ts=ts)
    tr.ReturnCode = 0
    tr.StillRunningAfter = server
    tr.StillRunningAfter = ts
    tr.Processes.Default.Streams.All = Testers.ContainsExpression("Could not connect", "Curl attempt should fail")

    # Should fail
    tr = urtest.AddTestRun("bar-to-foo-sni-policy-remap")
    ...
    # Should succeed
    tr = urtest.AddTestRun("bar-to-foo-sni-policy-host")
