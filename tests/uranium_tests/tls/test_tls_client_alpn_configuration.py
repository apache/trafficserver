...
def test_tls_client_alpn_configuration(urtest: UraniumTest) -> None:
    ...
    class TestAlpnFunctionality:
        ...
        def _configure_server(self, expected_alpn: Optional[str] = None):
            ...
            """
            server = urtest.MakeVerifierServerProcess(f'server-{TestAlpnFunctionality._server_counter}', self._replay_file)
        ...
        def _configure_trafficserver(
            ...
            """
            ts = urtest.MakeATSProcess(f'ts-{TestAlpnFunctionality._ts_counter}', enable_tls=True, enable_cache=False)
    ...
    """.split("\n"))
        ...
        def run(self):
            ...
            tr = urtest.AddTestRun(f'ATS ALPN configuration: {description}')
