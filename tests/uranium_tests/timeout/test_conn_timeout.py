...
def test_conn_timeout(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    ts.Disk.remap_config.AddLine('map /blocked http://10.1.1.1:{0}'.format(urtest.Variables.blocked_upstream_port))
    ts.Disk.remap_config.AddLine('map /not-blocked http://10.1.1.1:{0}'.format(urtest.Variables.upstream_port))
    ...
    tr = urtest.AddTestRun("tr-ns-setup")
    ...
    tr.Processes.Default.Command = 'echo start; sudo sh -x ./setupnetns.sh {0} {1}'.format(
        urtest.Variables.blocked_upstream_port, urtest.Variables.upstream_port)
    ...
    tr = urtest.AddTestRun("tr-blocking")
    ...
    tr = urtest.AddTestRun("tr-delayed")
    ...
    tr = urtest.AddTestRun("tr-cleanup")
