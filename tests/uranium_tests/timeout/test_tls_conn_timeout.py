...
def test_tls_conn_timeout(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Testing ATS TLS handshake timeout'

    ts = urtest.MakeATSProcess("ts")

    urtest.Setup.Copy(os.path.join(urtest.Variables.AtsBuildUraniumTestsDir, 'timeout', 'ssl-delay-server'))

    urtest.ContinueOnFail = True
    urtest.GetTcpPort("block_connect_port")
    urtest.GetTcpPort("block_ttfb_port")
    urtest.GetTcpPort("get_block_connect_port")
    urtest.GetTcpPort("get_block_ttfb_port")

    delay_post_connect = urtest.Processes.Process(
        "delay post connect", './ssl-delay-server {0} 3 0 server.pem'.format(urtest.Variables.block_connect_port))
    delay_post_ttfb = urtest.Processes.Process(
        "delay post ttfb", './ssl-delay-server {0} 0 6 server.pem'.format(urtest.Variables.block_ttfb_port))

    delay_get_connect = urtest.Processes.Process(
        "delay get connect", './ssl-delay-server {0} 3 0 server.pem'.format(urtest.Variables.get_block_connect_port))
    delay_get_ttfb = urtest.Processes.Process(
        "delay get ttfb", './ssl-delay-server {0} 0 6 server.pem'.format(urtest.Variables.get_block_ttfb_port))
    ...
    ts.Disk.remap_config.AddLine('map /connect_blocked https://127.0.0.1:{0}'.format(urtest.Variables.block_connect_port))
    ts.Disk.remap_config.AddLine('map /ttfb_blocked https://127.0.0.1:{0}'.format(urtest.Variables.block_ttfb_port))
    ts.Disk.remap_config.AddLine('map /get_connect_blocked https://127.0.0.1:{0}'.format(urtest.Variables.get_block_connect_port))
    ts.Disk.remap_config.AddLine('map /get_ttfb_blocked https://127.0.0.1:{0}'.format(urtest.Variables.get_block_ttfb_port))
    ...
    # Should retry once
    tr = urtest.AddTestRun("tr-blocking-post")
    tr.Setup.Copy(os.path.join(urtest.Variables.AtsTestToolsDir, "ssl", "server.pem"))
    tr.Processes.Default.StartBefore(urtest.Processes.ts)
    tr.Processes.Default.StartBefore(delay_post_connect, ready=When.PortOpen(urtest.Variables.block_connect_port))
    tr.MakeCurlCommand(
    ...
    tr.StillRunningAfter = urtest.Processes.ts
    ...
    tr = urtest.AddTestRun("tr-delayed-post")
    tr.Processes.Default.StartBefore(delay_post_ttfb, ready=When.PortOpen(urtest.Variables.block_ttfb_port))
    tr.MakeCurlCommand(
        '-H"Connection:close" -d "bob" -i http://127.0.0.1:{0}/ttfb_blocked --tlsv1.2'.format(ts.Variables.port), ts=ts)
    ...
    # Request the port that should timeout in the handshake
    # Should retry once
    tr = urtest.AddTestRun("tr-blocking-get")
    tr.Processes.Default.StartBefore(delay_get_connect, ready=When.PortOpen(urtest.Variables.get_block_connect_port))
    tr.MakeCurlCommand(
        '-H"Connection:close" -i http://127.0.0.1:{0}/get_connect_blocked --tlsv1.2'.format(ts.Variables.port), ts=ts)
    tr.Processes.Default.Streams.All = Testers.ContainsExpression("HTTP/1.1 502 Connection timed out", "Connect failed")
    tr.Processes.Default.ReturnCode = 0
    ...
    tr = urtest.AddTestRun("tr-delayed-get")
    tr.Processes.Default.StartBefore(delay_get_ttfb, ready=When.PortOpen(urtest.Variables.get_block_ttfb_port))
