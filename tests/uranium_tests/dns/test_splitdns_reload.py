...
def test_splitdns_reload(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.Summary = 'Test splitdns.config reload via ConfigRegistry.'
    ...
    class SplitDNSReloadTest:
        ...
        def run(self):
            ...
            tr = urtest.AddTestRun("Verify SplitDNS works at startup")
            ...
            tr = urtest.AddTestRun("Touch splitdns.config")
            tr.Processes.Default.Command = (f"touch {os.path.join(config_dir, 'splitdns.config')} && sleep 1")
            tr.Processes.Default.ReturnCode = 0
            tr.StillRunningAfter = self.ts

            tr = urtest.AddConfigReload(self.ts, expect_tasks=["splitdns.config"], description="Reload after splitdns.config touch")
            tr.StillRunningAfter = self.ts
