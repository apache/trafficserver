...
def test_cache_write_lock_contention(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    class ContentionTest:
        ...
        def _setup(self):
            ...
            self.server = urtest.MakeOriginServer(f"server_{self.name}", delay=origin_delay)
            ...
            self.ts = urtest.MakeATSProcess(f"ts_{self.name}", enable_cache=True)
        ...
        def run(self):
            ...
            if self.stale_scenario:
                tr = urtest.AddTestRun(f"{self.description} - Prime cache")
                ...
                tr = urtest.AddTestRun(f"{self.description} - Wait for stale")
                tr.Processes.Default.Command = "sleep 3"
                tr.Processes.Default.ReturnCode = 0
                tr.StillRunningAfter = self.ts
                tr.StillRunningAfter = self.server

            tr = urtest.AddTestRun(self.description)
            ...
            tr = urtest.AddTestRun(f"{self.description} - Verify cache")
            tr.MakeCurlCommand(f'-s -D - "{url}" -H "Host: example.com"', ts=self.ts)
            tr.Processes.Default.ReturnCode = 0
