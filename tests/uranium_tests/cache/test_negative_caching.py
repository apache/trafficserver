...
def test_negative_caching(urtest: UraniumTest) -> None:
    '''
    ...
    #
    ts = urtest.MakeATSProcess("ts-lifetime")
    ...
    tr = urtest.AddTestRun("Add a 404 response to the cache")
    ...
    tr = urtest.AddTestRun("Wait for cached object to be stale.")
    ...
    tr = urtest.AddTestRun("Make sure object is stale")
    ...
    #
    ts = urtest.MakeATSProcess("ts-lifetime-2")
    ts.Disk.records_config.update(
        {
            'proxy.config.diags.debug.enabled': 1,
            'proxy.config.diags.debug.tags': 'http',
    ...
    tr = urtest.AddTestRun("Add a 404 response with explicit max-age=300 to the cache")
    ...
    tr = urtest.AddTestRun("Wait for cached object to be stale if lifetime is incorrectly used.")
    tr.Processes.Default.Command = "sleep 4"
    tr.StillRunningAfter = ts
    ...
    tr = urtest.AddTestRun("Make sure object is fresh")
    ...
    tr = urtest.AddTestRun("Verify negative_caching_lifetime and ttl-in-cache interaction.")
