...
def test_cache_shm_purge_on_disable(urtest: UraniumTest) -> None:
    '''
    ...
    class CacheShmPurgeOnDisableTest:
        ...
        DISK_SIZE_BYTES = 256 * 1024 * 1024  # 256 MiB; matches the other shm Uranium tests.
        ...
        def _ensure_disk(self, disk_name):
            storage_dir = os.path.join(urtest.RunDirectory, 'storage')
        ...
        def _cleanup(self):
            tr = urtest.AddTestRun('Unlink any remaining test shm segments')
        ...
        def run(self):
            ...
            self._shm_status(
                'PURGE: precondition -- clean leftover segment is present',
                self.seed_purge,
                self._prefix_purge,
            ...
            # NOOP (no leftover)
            tr = urtest.AddTestRun('NOOP: start disabled+purge=1 against an unused prefix; nothing to do')
