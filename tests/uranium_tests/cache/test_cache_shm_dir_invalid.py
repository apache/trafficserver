...
def test_cache_shm_dir_invalid(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition(lambda: platform.system() == 'Linux', "shm byte-poke gates need Linux /dev/shm"))
    ...
    class CacheShmDirInvalidTest:
        ...
        def __init__(self):
            ...
            self.server = urtest.MakeVerifierServerProcess('shmd-origin', self.REPLAY_FILE)
        ...
        def _setup_shared_state(self):
            urtest.Setup.Copy(os.path.join(urtest.TestDirectory, '..', 'logging', self.TS_PID_SCRIPT))
            urtest.Setup.Copy(os.path.join(urtest.TestDirectory, self.POKE_SCRIPT))

            shared_storage_dir = os.path.join(urtest.RunDirectory, 'shared-storage')
        ...
        def _fill(self):
            tr = urtest.AddTestRun('Cold-start ts1 and cache an object')
            tr.AddVerifierClientProcess(
                'shmd-fill-client',
                self.REPLAY_FILE,
                http_ports=[self.ts1.Variables.port],
                keys='fill',
        ...
        def _clean_shutdown(self, ts, name):
            tr = urtest.AddTestRun(f'Drain and clean-shutdown {name}')
        ...
        def _cleanup_shm(self):
            tr = urtest.AddTestRun('Unlink the test shm segments')
        ...
        def run(self):
            ...
            self._verify_reject(
                self.ts3, 'hit_freelist', 'Start ts3; an out-of-range freelist head is rejected and rebuilt from disk')
