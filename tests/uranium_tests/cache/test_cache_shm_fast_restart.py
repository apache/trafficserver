...
def test_cache_shm_fast_restart(urtest: UraniumTest) -> None:
    '''
    ...
    class CacheShmFastRestartTest:
        ...
        # identifier match. Reused from uranium_tests/logging.
        ...
        def __init__(self):
            ...
            self.server = urtest.MakeVerifierServerProcess('shm-origin', self.REPLAY_FILE)
        ...
        def _add_diags_log_assertions(self):
            ...
            self.ts2.Disk.diags_log.Content += Testers.ContainsExpression(
                ...
                'ts2 should attach at least one shm-backed stripe segment')
            self.ts2.Disk.diags_log.Content += Testers.ContainsExpression(
                r"attaching cached directory from shm for '.+' \(fast restart",
                'ts2 should reuse the per-stripe directory from shm')
        ...
        def _start_ts1(self):
            ...
            tr = urtest.AddTestRun('Start ts1, then cache contents (fill)')
        ...
        def _start_ts2(self):
            ...
            tr = urtest.AddTestRun('Start ts2; verify shm fast-attach and cache HIT')
            tr.AddVerifierClientProcess(
