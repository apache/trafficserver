...
def test_cache_shm_unclean_shutdown(urtest: UraniumTest) -> None:
    '''
    ...
    class CacheShmUncleanShutdownTest:
        ...
        def _add_diags_log_assertions(self):
            ...
            self.ts2.Disk.diags_log.Content += Testers.ContainsExpression(
                ...
                'ts2 must reject the dirty segment left by the crash')
            self.ts2.Disk.diags_log.Content += Testers.ContainsExpression(
        ...
        def _kill_ts1(self):
            ...
            tr = urtest.AddTestRun('SIGKILL ts1 (unclean shutdown)')
        ...
        def _verify_dirty_drop(self):
            tr = urtest.AddTestRun('Start ts2; verify the dirty segment is dropped and rebuilt from disk')
