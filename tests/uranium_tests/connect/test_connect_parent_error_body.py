...
def test_connect_parent_error_body(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Verify CONNECT error response bodies from parent proxies'
    ...
    class ConnectParentErrorBodyTest:
        ...
        def _setupParentProxy(self):
            self.parent = urtest.MakeVerifierServerProcess('parent-proxy', self.replay_file)
        ...
        def run(self):
            tr = urtest.AddTestRun('CONNECT error response body from parent proxy')
