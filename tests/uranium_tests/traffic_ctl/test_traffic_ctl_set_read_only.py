...
def test_traffic_ctl_set_read_only(urtest: UraniumTest) -> None:
    '''
    ...
    # write path.
    tr = urtest.AddTestRun("Confirm record is RECA_READ_ONLY and at default")
    ...
    tr = urtest.AddTestRun("Attempt to set the RECA_READ_ONLY record (must be refused)")
    ...
    tr = urtest.AddTestRun("Confirm record was not modified")
    tr.AddJsonRPCClientRequest(ts, lookup_request())
    tr.Processes.Default.Streams.stdout = Testers.CustomJSONRPCResponse(assert_record_at_default)
    urtest.execute()
