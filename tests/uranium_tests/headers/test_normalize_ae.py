...
def test_normalize_ae(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition.HasATSFeature('TS_HAS_BROTLI'))

    urtest.ContinueOnFail = True

    server = urtest.MakeOriginServer("server", options={'--load': os.path.join(urtest.TestDirectory, 'normalize_ae_observer.py')})
    ...
    #
    normalize_ae_log_id = urtest.Disk.File("normalize_ae.log")
