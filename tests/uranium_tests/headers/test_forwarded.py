...
def test_forwarded(urtest: UraniumTest) -> None:
    '''
    ...
    server = urtest.MakeOriginServer("server", options={'--load': os.path.join(urtest.TestDirectory, 'forwarded-observer.py')})
    ...
    #
    forwarded_log_id = urtest.Disk.File("forwarded.log")
    ...
    tr = urtest.AddTestRun()
    tr.MakeCurlCommand(
        '--verbose --ipv4 --http1.1 --insecure --header "Host: www.no-oride.com" https://localhost:{}'.format(
    ...
    tr = urtest.AddTestRun()
    tr.MakeCurlCommand(
        '--verbose --ipv6 --http1.1 --proxy localhost:{} http://www.no-oride.com'.format(ts2.Variables.portv6), ts=ts2)
    tr.Processes.Default.ReturnCode = 0

    tr = urtest.AddTestRun()
    tr.MakeCurlCommand(
        '--verbose --ipv6 --http1.1 --insecure --header "Host: www.no-oride.com" https://localhost:{}'.format(
