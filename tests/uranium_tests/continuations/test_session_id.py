...
def test_session_id(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.PrepareTestPlugin(
        os.path.join(urtest.Variables.AtsBuildUraniumTestsDir, 'continuations', 'plugins', '.libs', 'session_id_verify.so'), ts)
    ...
    #
    tr = urtest.AddTestRun("Perform HTTP/1 transactions")
    ...
    if not Condition.CurlUsingUnixDomainSocket():
        tr = urtest.AddTestRun("Perform HTTP/2 transactions")
