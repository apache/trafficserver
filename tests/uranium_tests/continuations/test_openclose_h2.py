...
def test_openclose_h2(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'ssntxnorder_verify.so'), ts)
    ...
    ts.Disk.remap_config.AddLine(
        'map https://oc.test:{0} http://127.0.0.1:{1}'.format(ts.Variables.ssl_port, server.Variables.Port))
    ...
    tr = urtest.AddTestRun("Check Ssn order errors")
    ...
    # number of sessions/transactions opened and closed are equal
    tr = urtest.AddTestRun("Check for ssn open/close")
    ...
    tr = urtest.AddTestRun("Check for txn/open/close")
