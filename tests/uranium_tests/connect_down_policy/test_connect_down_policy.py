...
def test_connect_down_policy(urtest: UraniumTest) -> None:
    '''
    ...
    class ConnectDownPolicy3Test:
        ...
        def __init__(self, policy, expect_mark_down):
            ...
            self._server = urtest.MakeVerifierServerProcess(f"server-{self._name}", REPLAY_FILE)
            self._configure_trafficserver()

        def _configure_trafficserver(self):
            self._ts = urtest.MakeATSProcess(f"ts-{self._name}", enable_cache=False)
        ...
        def _test_inactive_timeout(self):
            tr = urtest.AddTestRun(f"policy={self._policy}: inactive timeout triggers 504")
        ...
        def _test_mark_down(self):
            if self._expect_mark_down:
                ...
                tr = urtest.AddTestRun(f"policy={self._policy}: verify no mark-down in error.log")
