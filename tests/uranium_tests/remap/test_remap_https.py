...
def test_remap_https(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr = urtest.AddTestRun()
    tr.MakeCurlCommand(
        '--http1.1 -k https://127.0.0.1:{0} -H "Host: www.example.com" --verbose'.format(ts.Variables.ssl_port), ts=ts)
    tr.Processes.Default.ReturnCode = 0
    ...
    tr = urtest.AddTestRun()
    tr.MakeCurlCommand(
        '--http1.1 -k https://127.0.0.1:{0} -H "Host: www.example3.com" --verbose'.format(ts.Variables.ssl_port), ts=ts)
    tr.Processes.Default.ReturnCode = 0
