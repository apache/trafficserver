...
def test_basic_conf_remap_yaml(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    class conf_remap_yaml_load_test:
        ...
        def _configure_server(self, tr: 'TestRun'):
            ...
            """
            server = urtest.MakeOriginServer(f"server-{conf_remap_yaml_load_test.ts_counter}", lookup_key="{%Host}{PATH}")
            request_header2 = {
        ...
        def _configure_traffic_server(self, tr: 'TestRun'):
            ...
            """
            ts = urtest.MakeATSProcess(f"ts-{conf_remap_yaml_load_test.ts_counter}")
