...
def test_remap_reload(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    tr = urtest.AddTestRun("Change remap.config to have only two lines")
    ...
    tr = urtest.AddConfigReload(
        tm, expect="fail", expect_tasks=["remap.config"], delay_start=2, description="remap_config reload, fails")

    tr = urtest.AddTestRun("after first reload")
    tr.AddVerifierClientProcess("client_2", replay_file_2, http_ports=[tm.Variables.port])

    tr = urtest.AddTestRun("Change remap.config to have more than three lines")
    p = tr.Processes.Default
    p.Env = tm.Env
