...
def test_thread_affinity(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Test TSContThreadAffinity APIs'
    urtest.ContinueOnFail = True

    # Define default ATS
    ts = urtest.MakeATSProcess('ts')

    urtest.testName = 'Test TSContThreadAffinity APIs'
    ...
    # Load plugin
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'cont_schedule.so'), ts, 'affinity')
