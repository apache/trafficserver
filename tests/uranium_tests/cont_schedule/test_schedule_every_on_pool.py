...
def test_schedule_every_on_pool(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Test TSContScheduleEveryOnPool API'
    urtest.ContinueOnFail = True

    # Define default ATS
    ts = urtest.MakeATSProcess('ts')

    urtest.testName = 'Test TSContScheduleEveryOnPool API'
    ...
    # Load plugin
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'cont_schedule.so'), ts, 'every_pool')

    # www.example.com Host
    tr = urtest.AddTestRun()
    tr.Processes.Default.Command = 'printf "Test TSContScheduleEveryOnPool API"'
    ...
    tr.Processes.Default.ReturnCode = 0
    ts.Disk.traffic_out.Content = "gold/schedule_every_on_pool.gold"
