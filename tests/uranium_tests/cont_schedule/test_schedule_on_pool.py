...
def test_schedule_on_pool(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Test TSContScheduleOnPool API'
    urtest.ContinueOnFail = True

    # Define default ATS
    ts = urtest.MakeATSProcess('ts')

    urtest.testName = 'Test TSContScheduleOnPool API'
    ...
    # Load plugin
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'cont_schedule.so'), ts, 'pool')
