...
def test_deny0(urtest: UraniumTest) -> None:
    '''
    ...
    redirect_serv = urtest.MakeOriginServer("redirect_serv", ip='0.0.0.0')
    ...
    def buildMetaTest(testName, requestString):
        tr = urtest.AddTestRun(testName)
        nonlocal isFirstTest
        if isFirstTest:
            isFirstTest = False
