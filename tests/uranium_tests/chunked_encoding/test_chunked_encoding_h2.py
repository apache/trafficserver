...
def test_chunked_encoding_h2(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    origin_server = os.path.join(urtest.TestDirectory, "chunked_encoding_h2_server.py")
    delay_server = urtest.Processes.Process(
        "delay-server", f'{sys.executable} "{origin_server}" 127.0.0.1 {urtest.Variables.upstream_port} outserver1 delayed-chunked')
    server2 = urtest.Processes.Process(
        "server2", f'{sys.executable} "{origin_server}" 127.0.0.1 {urtest.Variables.upstream_port2} outserver2 content-length')
    server3 = urtest.Processes.Process(
        "server3", f'{sys.executable} "{origin_server}" 127.0.0.1 {urtest.Variables.upstream_port3} outserver3 chunked')
    ...
    ts.Disk.remap_config.AddLine('map /delay-chunked-response http://127.0.0.1:{0}'.format(urtest.Variables.upstream_port))
    ts.Disk.remap_config.AddLine('map /post-full http://127.0.0.1:{0}'.format(urtest.Variables.upstream_port2))
    ts.Disk.remap_config.AddLine('map /post-chunked http://127.0.0.1:{0}'.format(urtest.Variables.upstream_port3))
    ...
    server1_out = urtest.Disk.File("outserver1")
    ...
    tr.Processes.Default.StartBefore(delay_server, ready=When.PortOpen(urtest.Variables.upstream_port))
    ...
    server2_out = urtest.Disk.File("outserver2")
    tr = urtest.AddTestRun()
    tr.Processes.Default.StartBefore(server2, ready=When.PortOpen(urtest.Variables.upstream_port2))
    tr.MakeCurlCommand(
    ...
    server3_out = urtest.Disk.File("outserver3")
    tr = urtest.AddTestRun()
    tr.Processes.Default.StartBefore(server3, ready=When.PortOpen(urtest.Variables.upstream_port3))
    tr.MakeCurlCommand(
