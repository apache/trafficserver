...
def test_runroot_error(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr = urtest.AddTestRun("using existing runroot")
    tr.Processes.Default.Command = "$ATS_BIN/traffic_layout init --path " + path
    ...
    tr = urtest.AddTestRun("create runroot inside runroot")
    ...
    path_invalid = os.path.join(urtest.RunDirectory, "tmp")
    tr = urtest.AddTestRun("remove invalid runroot")
    ...
    tr = urtest.AddTestRun("verify invalid runroot")
