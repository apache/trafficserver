...
def test_runroot_remove(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr = urtest.AddTestRun("Test traffic_layout remove #1")
    ...
    tr = urtest.AddTestRun("Test traffic_layout remove #2")
    tr.Processes.Default.Command = "cd " + urtest.RunDirectory + ";$ATS_BIN/traffic_layout remove --path runroot2"
    tr.Processes.Default.ReturnCode = 0
    f = tr.Disk.File(os.path.join(path2, "runroot.yaml"))
    f.Exists = False
    ...
    # remove cwd
    tr = urtest.AddTestRun("Test traffic_layout remove #3")
