...
def test_runroot_init(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.SkipUnless(
        urtest.Variables.BINDIR.startswith(urtest.Variables.PREFIX), "need to guarantee bin path starts with prefix for runroot")

    # init from pass in path
    path1 = os.path.join(urtest.RunDirectory, "runroot1")
    tr = urtest.AddTestRun("Test traffic_layout init #1")
    ...
    path2 = os.path.join(urtest.RunDirectory, "runroot2")
    tr = urtest.AddTestRun("Test traffic_layout init #2")
    tr.Processes.Default.Command = "cd " + urtest.RunDirectory + ";$ATS_BIN/traffic_layout init --path runroot2"
    tr.Processes.Default.ReturnCode = 0
    f = tr.Disk.File(os.path.join(path2, "runroot.yaml"))
    f.Exists = True

    # init to cwd
    path3 = os.path.join(urtest.RunDirectory, "runroot3")
    tr = urtest.AddTestRun("Test traffic_layout init #3")
    ...
    path4 = os.path.join(urtest.RunDirectory, "runroot4")
    tr = urtest.AddTestRun("Test traffic_layout init #4")
    ...
    bin_path = urtest.Variables.BINDIR[urtest.Variables.BINDIR.find(urtest.Variables.PREFIX) + len(urtest.Variables.PREFIX) + 1:]
    path5 = os.path.join(urtest.RunDirectory, "runroot5")
    exe_path = os.path.join(bin_path, "traffic_layout")
    junk_file = os.path.join(bin_path, "junk")
    junk = os.path.join(path1, junk_file)

    tr = urtest.AddTestRun("Test traffic_layout init #5")
