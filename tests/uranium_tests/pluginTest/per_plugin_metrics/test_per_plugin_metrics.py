...
def test_per_plugin_metrics(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(
        Condition.PluginExists('header_rewrite.so'), Condition.PluginExists('conf_remap.so'),
        Condition.PluginExists('generator.so'))
    ...
    class TestPerPluginWorkloadCounters:
        ...
        def setUpTS(self):
            ...
            self.ts.Setup.CopyAs('rules/global.conf', urtest.RunDirectory)
            self.ts.Disk.plugin_config.AddLine(f'header_rewrite.so {urtest.RunDirectory}/global.conf')
        ...
        def driveTraffic(self):
            ...
            tr = urtest.AddTestRun("Drive traffic through global + remap plugins")
            tr.Processes.Default.StartBefore(self.server)
            tr.Processes.Default.StartBefore(self.ts)
            tr.MakeCurlCommand(
                f'-s -D - -o /dev/null --proxy 127.0.0.1:{self.ts.Variables.port} "http://test.example/"', ts=self.ts)
            ...
            tr = urtest.AddTestRun("Drive traffic through a PluginVC intercept plugin")
            tr.MakeCurlCommand(
