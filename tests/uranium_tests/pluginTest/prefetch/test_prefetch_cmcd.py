...
def test_prefetch_cmcd(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    ts1 = urtest.MakeATSProcess("ts1", enable_uds=False)
    ...
    urtest.AddAwaitFileContainsTestRun('Await ts transactions to finish logging.', ts0log, 'crr.txt')

    ts1log = os.path.join(ts1.Variables.LOGDIR, 'transaction.log')
    urtest.AddAwaitFileContainsTestRun('Await ts1 transactions to finish logging.', ts1log, 'crr.txt')
