...
def test_jax_fingerprint(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition.PluginExists('jax_fingerprint.so'))
        ...
        def _configure_server(self, tr: 'TestRun') -> None:
            ...
            if self._servernames:
                ...
                self._server.Streams.All += Testers.ContainsExpression(
                    uuid_key, 'Verify the allowed-SNI request reached the server.')
                self._server.Streams.All += Testers.ContainsExpression(
                ...
                self._server.Streams.All += Testers.ContainsExpression(
                    uuid_key, 'Verify the matched-route request reached the server.')
                self._server.Streams.All += Testers.ContainsExpression(
    ...
    """.split("\n"))
            ...
            if self._setup == 'global':
                ...
                if self._servernames:
                    ...
                    self._ts.Disk.remap_config.AddLine(
                        f'map https://jax-no-plugin.server.test https://jax.backend.test:{server_port}')
