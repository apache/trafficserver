...
def test_escalate(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition.PluginExists('escalate.so'))
    ...
    class EscalateTest:
        ...
        def _setup_client(self, tr: 'Process') -> None:
            ...
            client.Streams.All += Testers.ExcludesExpression(
                '500 Internal', 'Verify none of the 500 responses make it to the client.')
    ...
    class EscalateNonGetMethodsTest:
        ...
        def __init__(self):
            ...
            tr = urtest.AddTestRun('Test escalate plugin with --escalate-non-get-methods option.')
            self._setup_dns(tr)
        ...
        def _setup_client(self, tr: 'TestRun') -> None:
            ...
            client.Streams.All += Testers.ContainsExpression(
                'x-response: fourth', 'Verify fourth GET response received (escalated).')
            client.Streams.All += Testers.ContainsExpression(
                'x-response: post_success', 'Verify successful POST response received.')
            client.Streams.All += Testers.ContainsExpression(
