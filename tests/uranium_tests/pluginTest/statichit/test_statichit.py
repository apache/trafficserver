...
def test_statichit(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition.PluginExists('statichit.so'),)
    ...
    class RemapData:
        ...
        def setup():
            ...
            urtest.Setup.Copy('story_16.json', urtest.RunDirectory)
            RemapData.json_body_path = os.path.join(urtest.RunDirectory, 'story_16.json')

            urtest.Setup.Copy('empty.txt', urtest.RunDirectory)
            RemapData.empty_body_path = os.path.join(urtest.RunDirectory, 'empty.txt')

            # This directory will be created, and a file copied to it, by the 1st (dummy) test run.
            RemapData.dir_path = os.path.join(urtest.RunDirectory, 'body_dir')
    ...
    for d in remap_data:
        ...
        ts.Disk.remap_config.AddLine(
            f'map {d.remap_from} http://127.0.0.1/ @plugin=statichit.so @pparam=--file-path={d.body}{arg_str}')

    # Dummy test run for copying a file.
    tr = urtest.AddTestRun()
    p = tr.Processes.Default
    p.Command = (
        f'mkdir {urtest.RunDirectory}/body_dir ; ' + f'cp {urtest.TestDirectory}/small_body.txt {urtest.RunDirectory}/body_dir/.')

    # Dummy test run for copying a file.
    tr = urtest.AddTestRun()
    p = tr.Processes.Default
    p.Command = (
        f'mkdir {ts.Variables.CONFIGDIR}/statichit ; ' +
        f'cp {urtest.TestDirectory}/small_body.txt {ts.Variables.CONFIGDIR}/statichit/.')
    p.StartBefore(ts)
