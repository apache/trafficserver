...
def test_packet_mark(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    class PacketMarkTest:
        ...
        def _make_server(self):
            server = urtest.MakeOriginServer(f"server{self._num}")
        ...
        def _make_ats(self):
            return urtest.MakeATSProcess(f"ts{self._num}", enable_cache=False)
    ...
    class ClientPacketMarkTest(PacketMarkTest):
        ...
        def _configure(self, ts):
            ...
            urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, f'client_packet_mark.so'), ts)
    ...
    class ServerPacketMarkTest(PacketMarkTest):
        ...
        def _configure(self, ts):
            ...
            urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, f'server_packet_mark.so'), ts)
