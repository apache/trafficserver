...
def test_webp_transform_decode_limit(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.Summary = 'An over-dimension image is rejected by the decode limits and reverts to the original'

    urtest.SkipUnless(Condition.PluginExists('webp_transform.so'))
    ...
    tr = urtest.AddTestRun("over-dimension image reverts to original instead of crashing")
    tr.MakeCurlCommand(
