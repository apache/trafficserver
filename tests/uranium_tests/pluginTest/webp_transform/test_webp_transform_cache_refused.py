...
def test_webp_transform_cache_refused(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.Summary = 'An over-cap refused (502) response must not poison the cache'

    urtest.SkipUnless(Condition.PluginExists('webp_transform.so'))

    urtest.ContinueOnFail = True

    server = urtest.MakeVerifierServerProcess("server", "replay/webp_chunked_cap.replay.yaml")
    ...
    tr = urtest.AddTestRun("first request: over-cap chunked body refused with 502")
    tr.MakeCurlCommand(
    ...
    tr2 = urtest.AddTestRun("second request: must not be served a cached poisoned 200")
    tr2.MakeCurlCommand(
        '-sS -D - -o /dev/null -w "size_download=%{{size_download}}" -x 127.0.0.1:{0} '
        '-H "Accept: image/webp" -H "uuid: chunked-huge" http://127.0.0.1:{1}/chunked-huge.jpg'.format(
            ts.Variables.port, server.Variables.http_port),
