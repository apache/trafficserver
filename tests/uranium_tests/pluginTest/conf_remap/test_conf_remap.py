...
def test_conf_remap(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition.PluginExists('conf_remap.so'))

    class ConfRemapPluginTest:

        def __init__(self):
            self._tr = urtest.AddTestRun("conf_remap")
        ...
        def __setupTS(self):
            self._ts = urtest.MakeATSProcess("ts", enable_cache=True)
    ...
    map /custom_negative_caching_list/ \
        ...
        @plugin=conf_remap.so @pparam={urtest.RunDirectory}/negative_caching_list.yaml
        """
                })
