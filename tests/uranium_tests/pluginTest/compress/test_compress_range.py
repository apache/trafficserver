...
def test_compress_range(urtest: UraniumTest) -> None:
    '''
    ...
    class CompressPluginTest:
        ...
        def setupOriginServer(self):
            self.server = urtest.MakeVerifierServerProcess("verifier-server", self.replayFile)
    ...
    map /cache-true-remove-range/ http://127.0.0.1:{self.server.Variables.http_port}/ \
        @plugin=compress.so \
        @pparam={urtest.RunDirectory}/cache-true-remove-range.config
    map /cache-true-remove-accept-encoding/ http://127.0.0.1:{self.server.Variables.http_port}/ \
        @plugin=compress.so \
        @pparam={urtest.RunDirectory}/cache-true-remove-accept-encoding.config
    map /cache-true-no-compression/ http://127.0.0.1:{self.server.Variables.http_port}/ \
        @plugin=compress.so \
        @pparam={urtest.RunDirectory}/cache-true-no-compression.config
    """
                })
