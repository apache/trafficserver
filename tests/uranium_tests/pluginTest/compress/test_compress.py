...
def test_compress(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    server = urtest.MakeOriginServer("server", options={'--load': f'{urtest.TestDirectory}/compress_observer.py'})

    # Need a fairly big body, otherwise the plugin will refuse to compress
    line = "lets go surfin now everybodys learnin how"
    body = f'{line}\n' * 24 + line
    orig_path = f'{urtest.RunDirectory}/orig.txt'
    ...
    ts.Disk.remap_config.AddLine(
        ...
        f' @plugin=compress.so @pparam={urtest.RunDirectory}/compress.config')
    ts.Disk.remap_config.AddLine(
    ...
    ts.Disk.remap_config.AddLine(
        ...
        f' @plugin=compress.so @pparam={urtest.RunDirectory}/compress2.config')
    ts.Disk.remap_config.AddLine(
    ...
    ts.Disk.remap_config.AddLine(
        ...
        f' @plugin=compress.so @pparam={urtest.RunDirectory}/compress3.config')
    ts.Disk.remap_config.AddLine(
    ...
    def get_out_path():
        nonlocal out_path_counter
        out_path = f'{urtest.RunDirectory}/curl_out_{out_path_counter}'
        out_path_counter += 1
        return out_path

    deflate_path = f'{urtest.RunDirectory}/deflate.txt'

    def get_verify_command(out_path, decrompressor):
        return f"{decrompressor} {out_path} > {deflate_path} && diff {deflate_path} {orig_path}"

    for i in range(6):

        tr = urtest.AddTestRun(f'gzip, deflate, sdch, br, zstd: {i}')
        if (waitForTs):
        ...
        tr.MakeCurlCommand(curl(ts, i, 'gzip, deflate, sdch, br, zstd', out_path), ts=ts)
        tr = urtest.AddTestRun(f'verify gzip, deflate, sdch, br, zstd: {i}')
        tr.ReturnCode = 0
        if i == 0:
            ...
            tr.Processes.Default.Command = get_verify_command(out_path, "zstd -d -c")
        elif i == 5:
            tr.Processes.Default.Command = get_verify_command(out_path, "zstd -d -c")

        tr = urtest.AddTestRun(f'gzip: {i}')
        tr.Processes.Default.ReturnCode = 0
        out_path = get_out_path()
        tr.MakeCurlCommand(curl(ts, i, "gzip", out_path), ts=ts)
        tr = urtest.AddTestRun(f'verify gzip: {i}')
        tr.ReturnCode = 0
        tr.Processes.Default.Command = get_verify_command(out_path, "gunzip -k -c")

        tr = urtest.AddTestRun(f'br: {i}')
        tr.Processes.Default.ReturnCode = 0
        out_path = get_out_path()
        tr.MakeCurlCommand(curl(ts, i, "br", out_path), ts=ts)
        tr = urtest.AddTestRun(f'verify br: {i}')
        tr.ReturnCode = 0
        if i == 1:
        ...
        tr = urtest.AddTestRun(f'deflate: {i}')
        tr.Processes.Default.ReturnCode = 0
        out_path = get_out_path()
        tr.MakeCurlCommand(curl(ts, i, "deflate", out_path), ts=ts)
        tr = urtest.AddTestRun(f'verify deflate: {i}')
        tr.ReturnCode = 0
        tr.Processes.Default.Command = f"diff {out_path} {orig_path}"

        tr = urtest.AddTestRun(f'zstd: {i}')
        tr.Processes.Default.ReturnCode = 0
        out_path = get_out_path()
        tr.MakeCurlCommand(curl(ts, i, "zstd", out_path))
        tr = urtest.AddTestRun(f'verify zstd: {i}')
        tr.ReturnCode = 0
        if i == 4 or i == 5:
            tr.Processes.Default.Command = get_verify_command(out_path, "zstd -d -c")
        else:
    ...
    tr = urtest.AddTestRun(f'verify gzip;q=0.666')
    tr.ReturnCode = 0
    tr.Processes.Default.Command = get_verify_command(out_path, "gunzip -k -c")
    ...
    tr = urtest.AddTestRun(f'verify gzip;q=0.666x')
    tr.ReturnCode = 0
    tr.Processes.Default.Command = get_verify_command(out_path, "gunzip -k -c")
    ...
    tr.Processes.Default.ReturnCode = 0
    out_path = get_out_path()
    tr.MakeCurlCommand(curl(ts, 0, "gzip;q=0.0", out_path), ts=ts)
    tr = urtest.AddTestRun(f'verify gzip;q=0.0')
    tr.ReturnCode = 0
    tr.Processes.Default.Command = f"diff {out_path} {orig_path}"
    ...
    tr = urtest.AddTestRun(f'verify gzip;q=-0.1')
    tr.ReturnCode = 0
    tr.Processes.Default.Command = get_verify_command(out_path, "gunzip -k -c")
    ...
    tr = urtest.AddTestRun(f'verify aaa, gzip;q=0.666, bbb')
    tr.ReturnCode = 0
    tr.Processes.Default.Command = get_verify_command(out_path, "gunzip -k -c")
    ...
    tr = urtest.AddTestRun(f'verify br ; q=0.666, bbb')
    tr.ReturnCode = 0
    tr.Processes.Default.Command = get_verify_command(out_path, "brotli -d -c")
    ...
    tr = urtest.AddTestRun(f'verify aaa, gzip;q=0.666 , ')
    tr.ReturnCode = 0
    tr.Processes.Default.Command = get_verify_command(out_path, "gunzip -k -c")
    ...
    tr = urtest.AddTestRun(f'verify gzip post')
    tr.ReturnCode = 0
    tr.Processes.Default.Command = get_verify_command(out_path, "gunzip -k -c")

    # Test Vary header: compressible content without Accept-Encoding should get Vary: Accept-Encoding
    tr = urtest.AddTestRun('vary header test: no accept-encoding')
    tr.Processes.Default.ReturnCode = 0
    out_path = get_out_path()
    tr.MakeCurlCommand(
        f"-o {out_path} --verbose --proxy http://127.0.0.1:{ts.Variables.port}"
    ...
    tr = urtest.AddTestRun('vary header test: unsupported accept-encoding')
    tr.Processes.Default.ReturnCode = 0
    out_path = get_out_path()
    ...
    tr = urtest.AddTestRun('verify vary headers')
    tr.Processes.Default.ReturnCode = 0
