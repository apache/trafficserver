...
def test_healthchecks(urtest: UraniumTest) -> None:
    """
    ...
    urtest.Summary = '''Verify healthchecks plugin behavior.'''

    urtest.SkipUnless(Condition.PluginExists('healthchecks.so'))

    CONFIG = f'''
    /acme {urtest.RunDirectory}/acme text/plain 200 404
    /acme-ssl {urtest.RunDirectory}/acme-ssl text/plain 200 404
    '''
    ...
    class TestFileChangeBehavior:
        ...
        def _configure_global_ts(self) -> None:
            ...
            config.AddLine(f'/acme {urtest.RunDirectory}/acme text/plain 200 404')
            config.AddLine(f'/acme-ssl {urtest.RunDirectory}/acme-ssl text/plain 200 404')
            ts.Setup.Copy('acme')
    ...
    """.split("\n"))
        ...
        def _expect_positive_healthchecks(self) -> None:
            ...
            tr = urtest.AddTestRun(f'Positive acme healthchecks: {counter}')
            ...
            if not Condition.CurlUsingUnixDomainSocket():
                # Repeat for acme-ssl
                tr2 = urtest.AddTestRun(f'Positive acme-ssl healthchecks: {counter}')
        ...
        def _remove_acme_ssl(self) -> None:
            ...
            '''
            if not Condition.CurlUsingUnixDomainSocket():
                tr = urtest.AddTestRun('Remove acme-ssl file')
                p = tr.Processes.Default
                p.Command = f'rm {urtest.RunDirectory}/acme-ssl && sleep 1'
        ...
        def _expect_acme_ssl_404(self) -> None:
            ...
            :return: None
            '''
            tr = urtest.AddTestRun('Expect 200 for acme after acme-ssl removal')
            tr.MakeCurlCommand(f'-v http://127.0.0.1:{self._ts.Variables.port}/acme', ts=self._ts)
            curl_acme = tr.Processes.Default
            if not self._ts_started:
                curl_acme.StartBefore(self._ts)
                self._ts_started = True
            curl_acme.Streams.All += Testers.ContainsExpression(
                'HTTP/1.1 200', 'Verify 200 response for /acme after acme-ssl removal')

            if not Condition.CurlUsingUnixDomainSocket():
                tr2 = urtest.AddTestRun('Expect 404 for acme-ssl after removal')
                tr2.MakeCurlCommand(f'-kv https://127.0.0.1:{self._ts.Variables.ssl_port}/acme-ssl', ts=self._ts)
                curl_acme_ssl = tr2.Processes.Default
                if not self._ts_started:
                    curl_acme_ssl.StartBefore(self._ts)
                    self._ts_started = True
                curl_acme_ssl.Streams.All += Testers.ContainsExpression(
                    'HTTP/2 404', 'Verify 404 response for /acme-ssl after removal')
        ...
        def _re_add_acme_ssl(self) -> None:
            ...
            '''
            if not Condition.CurlUsingUnixDomainSocket():
                tr = urtest.AddTestRun('Re-add acme-ssl file')
                tr.Setup.Copy('acme-ssl', urtest.RunDirectory)
                p = tr.Processes.Default
                p.Command = 'sleep 1'
                p.ReturnCode = 0
        ...
        def _rewrite_acme_while_serving(self) -> None:
            ...
            :return: None
            '''
            tr = urtest.AddTestRun('Rewrite acme while healthchecks are being served')
            acme_file = os.path.join(urtest.RunDirectory, 'acme')
        ...
        def _expect_full_buffer_acme_body(self) -> None:
            ...
            :return: None
            '''
            tr = urtest.AddTestRun('Expect a full-sized healthcheck response body')
            acme_file = os.path.join(urtest.RunDirectory, 'acme')
            url = f'http://127.0.0.1:{self._ts.Variables.port}/acme'
            command = (
                f'dd if=/dev/zero of={acme_file} bs=16384 count=1 2>/dev/null && sleep 1 && ' + '{curl} -s ' + url + ' | wc -c')
        ...
        def _expect_rewritten_acme_body(self) -> None:
            ...
            :return: None
            '''
            tr = urtest.AddTestRun('Expect the last written acme content in the response body')
            acme_file = os.path.join(urtest.RunDirectory, 'acme')
            url = f'http://127.0.0.1:{self._ts.Variables.port}/acme'
