...
def test_lua_states_stats(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.testName = "Lua states and stats"

    urtest.Setup.Copy("hello.lua")
    urtest.Setup.Copy("global.lua")
    urtest.Setup.Copy("metrics.sh")
    urtest.Setup.Copy("lifecycle_stats.sh")
    ...
    ts.Disk.remap_config.AddLines(
        {
            ...
            ' @plugin=tslua.so @pparam={}/hello.lua'.format(urtest.RunDirectory)
        })

    ts.Disk.plugin_config.AddLine('tslua.so {}/global.lua'.format(urtest.RunDirectory))
    ...
    tr = urtest.AddTestRun("Lua states")
    ...
    tr = urtest.AddTestRun("Lua hello")
    ps = tr.Processes.Default  # alias
    ...
    tr = urtest.AddTestRun("Check for metrics")
    ps = tr.Processes.Default  # alias
    ...
    tr = urtest.AddTestRun("Check for lifecycle stats")
    ps = tr.Processes.Default  # alias
