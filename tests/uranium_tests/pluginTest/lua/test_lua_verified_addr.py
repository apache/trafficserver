...
def test_lua_verified_addr(urtest: UraniumTest) -> None:
    '''
    ...
    tr = urtest.AddTestRun("Lua Verified Address - IPv4")
    ...
    tr = urtest.AddTestRun("Lua Verified Address - IPv6")
    ps = tr.Processes.Default
    ...
    tr = urtest.AddTestRun("Lua Verified Address - Invalid IP")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(f"-s -H 'X-Invalid-IP: not.a.valid.ip' http://127.0.0.1:{ts.Variables.port}", ts=ts)
    ps.Env = ts.Env
    ps.ReturnCode = 0
    ps.Streams.stdout.Content = Testers.ContainsExpression("invalid:rejected;", "Invalid IP address should be rejected")
    tr.StillRunningAfter = ts

    # Test 4: Both IPv4 and IPv6 in sequence
    tr = urtest.AddTestRun("Lua Verified Address - IPv4 then IPv6")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(
        f"-s -H 'X-Real-IP: 203.0.113.42' -H 'X-Real-IP-V6: 2001:db8::42' http://127.0.0.1:{ts.Variables.port}", ts=ts)
    ps.Env = ts.Env
    ps.ReturnCode = 0
    ps.Streams.stdout.Content = Testers.ContainsExpression(
