...
def test_slice_prefetch(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    server = urtest.MakeOriginServer("server", lookup_key="{%Range}")
    ...
    curl_and_args = '{{curl}} -s -D /dev/stdout -o /dev/stderr -x http://127.0.0.1:{} -H "x-debug: x-cache"'.format(
        ts.Variables.port)
    ...
    tr = urtest.AddTestRun("Full object slice: first block is miss, only block 14-20 prefetched")
    ...
    tr = urtest.AddTestRun("Full object slice: first block is hit-fresh, no blocks prefetched")
    ps = tr.Processes.Default
    ...
    tr = urtest.AddTestRun("Full object slice: first block is hit-stale, only block 14-20 prefetched")
    ps = tr.Processes.Default
    ...
    tr = urtest.AddTestRun("Full object slice: first block is hit-fresh with range 0-, no blocks prefetched")
    ps = tr.Processes.Default
    ...
    tr = urtest.AddTestRun("Client range request")
    ps = tr.Processes.Default
