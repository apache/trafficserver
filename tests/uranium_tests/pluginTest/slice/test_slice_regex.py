...
def test_slice_regex(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr = urtest.AddTestRun("Exclude - asset passed through")
    ps = tr.Processes.Default
    ps.StartBefore(server, ready=When.PortOpen(server.Variables.Port))
    ps.StartBefore(urtest.Processes.ts)
    tr.MakeCurlCommand(curl_and_args + ' http://exclude/slice.txt', ts=ts)
    ps.ReturnCode = 0
    ...
    tr = urtest.AddTestRun("Exclude - asset is sliced")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://exclude/slice.mp4', ts=ts)
    ps.ReturnCode = 0
    ...
    tr = urtest.AddTestRun("Include - asset passed through")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://include/slice.txt', ts=ts)
    ps.ReturnCode = 0
    ps.Streams.stdout.Content = Testers.ContainsExpression("X-Info: notsliced", "expected not sliced header")
    tr.StillRunningAfter = ts

    # 3 Test - Exclude mp4 gets sliced
    tr = urtest.AddTestRun("Include - asset is sliced")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://include/slice.mp4', ts=ts)
    ps.ReturnCode = 0
    ps.Streams.stdout.Content = Testers.ContainsExpression("X-Info: sliced", "expected sliced header")
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = ts
