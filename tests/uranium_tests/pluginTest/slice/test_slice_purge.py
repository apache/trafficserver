...
def test_slice_purge(urtest: UraniumTest) -> None:
    '''
    ...
    class SlicePurgeRequestTest:
        ...
        def __init__(self, ref_block, num):
            ...
            self._num = num
            self._server = urtest.MakeVerifierServerProcess(f"server_{self._num}", SlicePurgeRequestTest.replay_file)
            self._configure_trafficserver()

        def _configure_trafficserver(self):
            """Configure Traffic Server."""
            self._ts = urtest.MakeATSProcess(f"ts_{self._num}", enable_cache=True)
        ...
        def slice_purge(self):
            ...
            tr.AddVerifierClientProcess(
                f"client_{self._num}", SlicePurgeRequestTest.replay_file, http_ports=[self._ts.Variables.port])
        ...
        def slice_purge_no_ref(self):
            ...
            tr.AddVerifierClientProcess(
                "client_no_ref", SlicePurgeRequestTest.replay_no_ref_file, http_ports=[self._ts.Variables.port])
