...
def test_slice_stale_generation(urtest: UraniumTest) -> None:
    ...
    class SliceHierarchyTest:
        ...
        def _configure_dns(self) -> None:
            ...
            self._dns = urtest.MakeDNServer(f'dns-{self._name}', default='127.0.0.1')
        ...
        def _configure_origin(self) -> None:
            ...
            """
            self._origin = urtest.MakeVerifierServerProcess(
        ...
        def _configure_parent(self) -> None:
            ...
            self._parent = urtest.MakeATSProcess(f'ts-parent-{self._name}')
        ...
        def _make_child(self, label: str) -> 'Process':
            ...
            ts = urtest.MakeATSProcess(f'ts-{label}-{self._name}')
    ...
    class SliceStaleGenerationTest(SliceHierarchyTest):
        ...
        def _fill_cache(self) -> None:
            ...
            tr = urtest.AddTestRun('Cache the object while the origin holds the first generation')
        ...
        def _verify_clipped_range(self) -> None:
            ...
            tr = urtest.AddTestRun('A range inside the current object is clipped to the stale length')
        ...
        def _verify_unsatisfiable_range(self) -> None:
            ...
            tr = urtest.AddTestRun('A range past the stale length is refused with a 416')
        ...
        def _verify_uncached_object(self) -> None:
            ...
            tr = urtest.AddTestRun('An object first fetched after the replacement is served correctly')
    ...
    class SliceMixedGenerationTest(SliceHierarchyTest):
        ...
        def _fill_interior_block(self) -> None:
            ...
            tr = urtest.AddTestRun('Cache an interior block at the first generation')
            self._start_hierarchy(tr)
        ...
        def _verify_aborted_response(self) -> None:
            ...
            tr = urtest.AddTestRun('A mixed generation object cannot be delivered and the request fails')
        ...
        def _verify_mix_is_on_the_parent(self) -> None:
            ...
            tr = urtest.AddTestRun('A second child with a cold cache inherits the mix from the parent')
            tr.Processes.Default.StartBefore(self._cold_child)
            client = self._replay_phase(tr, 'cold-child', ts=self._cold_child)
            client.ReturnCode = 1
            client.Streams.stdout += Testers.ContainsExpression(
                'Failed HTTP/1 transaction with key: cold-child',
                'A node that never saw the old generation should fail the same way.')
