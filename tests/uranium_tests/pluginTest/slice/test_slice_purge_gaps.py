...
def test_slice_purge_gaps(urtest: UraniumTest) -> None:
    ...
    class SlicePurgeGapsTest:
        ...
        def _configure_origin(self) -> None:
            ...
            self._origin = urtest.MakeVerifierServerProcess('origin', self._server_replay, other_args=self._origin_key_format)
        ...
        def _make_ts(self, label: str, miss_bound: int = None, fail_rules: bool = False) -> 'Process':
            ...
            """
            ts = urtest.MakeATSProcess(label, enable_cache=True)

            rules = ''
            if fail_rules:
                ts.Setup.CopyAs(f'rules/{self._fail_rules}', urtest.RunDirectory)
                rules = f' @plugin=header_rewrite.so @pparam={urtest.RunDirectory}/{self._fail_rules}'
        ...
        def _miss_bound_and_override(self) -> None:
            ...
            self._survived(
                'sparse-check-alive', 'sparsebytes=40-49', 'A walk that gave up before block 4 should not have purged it.')
