...
def test_slice_crr_ident(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    tr = urtest.AddTestRun("Preload plain")
    ...
    tr = urtest.AddTestRun("Request 2nd slice (expect slice1 to be fresh)")
    ...
    tr = urtest.AddTestRun("Request again, asset replaced")
    tr.DelayStart = 2  # ensure its really stale
    ps = tr.Processes.Default
    ...
    tr = urtest.AddTestRun("Request again, asset replaced but hit")
    tr.DelayStart = 2  # ensure its really stale
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://slice/plain -H "UID: chg"', ts=ts)
    ps.ReturnCode = 0
    ...
    tr = urtest.AddTestRun("Fetch 404.txt asset")
    ps = tr.Processes.Default
