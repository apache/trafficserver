...
def test_transaction_data_sink(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.SkipUnless(Condition.PluginExists('txn_data_sink.so'),)
    ...
    class TransactionDataSyncTest:
        ...
        def _setupOriginServer(self):
            self.server = urtest.MakeVerifierServerProcess("server", self.replay_file)

        def _setupNameserver(self):
            self.nameserver = urtest.MakeDNServer("dns", default='127.0.0.1')

        def _setupTS(self):
            self.ts = urtest.MakeATSProcess("ts", enable_cache=False, enable_tls=True)
    ...
    """.split("\n"))
        ...
        def run(self):
            ...
            tr = urtest.AddTestRun("Wait for transaction data sink output")
            timeout = 30
