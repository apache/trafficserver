...
def test_esi_ssrf_validate(urtest: UraniumTest) -> None:
    '''
    ...
    class EsiSsrfTest:
        ...
        def __init__(self) -> None:
            tr = urtest.AddTestRun("ESI include URLs are validated for SSRF")
        ...
        def _create_server(self, tr: 'TestRun') -> None:
            ...
            server.Streams.All += Testers.ContainsExpression(
                'GET /allowed.php', 'Verify the allowed top-level request reached origin.')
            server.Streams.All += Testers.ContainsExpression(
        ...
        def _create_ats(self, tr: 'TestRun') -> None:
            ...
            ts.Disk.diags_log.Content += Testers.ContainsExpression(
                r'Rejecting include URL.*127\.0\.0\.1/admin.*private-host',
                'Loopback IPv4 literal must be rejected as private-host.')
            ts.Disk.diags_log.Content += Testers.ContainsExpression(
                r'Rejecting include URL.*gopher://internal\.svc/x.*bad-scheme',
                'Non-http(s) scheme must be rejected as bad-scheme.')
            ts.Disk.diags_log.Content += Testers.ContainsExpression(
