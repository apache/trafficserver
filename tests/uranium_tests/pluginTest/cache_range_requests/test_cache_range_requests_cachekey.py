...
def test_cache_range_requests_cachekey(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.testName = "cache_range_requests_cachekey"
    ...
    tr = urtest.AddTestRun("full asset fetch")
    ...
    tr = urtest.AddTestRun("0- asset fetch")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://www.example.com/path -r 0- -H "uuid: range_full"', ts=ts)
    ps.ReturnCode = 0
    ps.Streams.stdout.Content = Testers.ContainsExpression("X-Cache: miss", "expected cache miss for load")
    tr.StillRunningAfter = ts

    # 2 Test - Ensure assert happens instead of possible cache poisoning.
    tr = urtest.AddTestRun("Attempt poisoning")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://www.fail.com/path -r 0- -H "uuid: range_fail"', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
