...
def test_rate_limit_sni_expiry(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition.PluginExists('rate_limit.so'))
    ...
    class RateLimitSniExpiryTest:
        ...
        def __init__(self) -> None:
            tr = urtest.AddTestRun('rate_limit SNI queue max_age expiry')
        ...
        def _configure_client(self, tr: 'TestRun') -> None:
            ts = self._ts
            client = os.path.join(urtest.TestDirectory, 'rate_limit_sni_expiry_client.sh')
            tr.Processes.Default.Command = f'bash {client} 127.0.0.1 {ts.Variables.ssl_port} rate.limited.com'
            tr.Processes.Default.ReturnCode = 0
            tr.Processes.Default.StartBefore(ts)
            tr.Processes.Default.Streams.stdout = Testers.ContainsExpression(
                'rate_limit-expiry-done', 'the client ran to completion')
