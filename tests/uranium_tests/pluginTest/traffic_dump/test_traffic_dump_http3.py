...
def test_traffic_dump_http3(urtest: UraniumTest) -> None:
    """
    ...
    urtest.SkipIf(Condition.true("Skip this test until the TS_EVENT_HTTP_SSN are supported for QUIC connections."),)

    schema_path = os.path.join(urtest.Variables.AtsTestToolsDir, 'lib', 'replay_schema.json')

    # Configure the origin server.
    replay_file = "replay/http3.yaml"
    server = urtest.MakeVerifierServerProcess("server", replay_file, ssl_cert="ssl/server_combined.pem", ca_cert="ssl/signer.pem")
