...
def test_traffic_dump(urtest: UraniumTest) -> None:
    """
    ...
    ts.Disk.traffic_out.Content += Testers.ContainsExpression(
        "Dumping body bytes: false", "Verify that dumping body bytes is enabled.")
    ...
    tr = urtest.AddTestRun("Verify the json content of the second session")
    tr.Setup.CopyAs(verify_replay, urtest.RunDirectory)
    tr.Processes.Default.Command = \
    ...
    tr = urtest.AddTestRun("Verify the replay file has the explicit target.")
    tr.Setup.CopyAs(verify_replay, urtest.RunDirectory)
    ...
    tr = urtest.AddTestRun("Verify the client-request size node for a request with a body.")
    tr.Setup.CopyAs(verify_replay, urtest.RunDirectory)
    ...
    #
    tr = urtest.AddTestRun("Verify that the cached response's replay file looks appropriate.")
    tr.Setup.CopyAs(verify_replay, urtest.RunDirectory)
    tr.Processes.Default.Command = \
    ...
    #
    tr = urtest.AddTestRun("Verify the dump file of two transactions in a session.")
    tr.Setup.CopyAs(verify_replay, urtest.RunDirectory)
    tr.Processes.Default.Command = \
    ...
    #
    tr = urtest.AddTestRun("Verify the client protocol stack of a TLS session.")
    ...
    tr = urtest.AddTestRun("Verify the server TLS protocol stack.")
    ...
    #
    tr = urtest.AddTestRun("Verify the client HTTP/2 protocol stack.")
    ...
    tr = urtest.AddTestRun("Verify the server HTTP/2 protocol stack.")
    tr.Setup.CopyAs(verify_replay, urtest.RunDirectory)
    tr.Processes.Default.Command = \
        (f'{sys.executable} {verify_replay} {schema_path} {replay_file_session_9} '
    ...
    #
    tr = urtest.AddTestRun("Verify the client TLS protocol stack.")
    tr.Setup.CopyAs(verify_replay, urtest.RunDirectory)
    tr.Processes.Default.Command = \
    ...
    tr = urtest.AddTestRun("Verify the server HTTP protocol stack.")
    tr.Setup.CopyAs(verify_replay, urtest.RunDirectory)
    ...
    tr = urtest.AddTestRun("Verify handling of a CONNECT request.")
    tr.Setup.CopyAs(verify_replay, urtest.RunDirectory)
    ...
    tr = urtest.AddTestRun("Run some more test traffic with the restricted disk limit.")
    tr.AddVerifierClientProcess(
        "client-2",
        replay_file,
        http_ports=[ts.Variables.port],
    ...
    tr = urtest.AddTestRun("Verify no new traffic was dumped")
    ...
    #
    tr = urtest.AddTestRun("Removing the disk limit via traffic_ctl.")
    ...
    tr = urtest.AddTestRun("Run some more test traffic with no disk limit.")
    tr.AddVerifierClientProcess(
        "client-3",
        replay_file,
        http_ports=[ts.Variables.port],
    ...
    # Since the limit is zero, we should not see any new replay file created.
    tr = urtest.AddTestRun("Verify the new traffic was dumped")
    # Sleep 2 seconds to give the replay plugin plenty of time to write the file.
    tr.Processes.Default.Command = "sleep 2"
    tr.Processes.Default.ReturnCode = 0
    ...
    #
    tr = urtest.AddTestRun("Verify re-adding --limit via traffic_ctl.")
    tr.Processes.Default.Command = "traffic_ctl plugin msg traffic_dump.limit 0"
    tr.Processes.Default.ReturnCode = 0
    tr.Processes.Default.Env = ts.Env
    tr.StillRunningAfter = ts
    ...
    tr = urtest.AddTestRun("Run test traffic with newly restricted disk limit.")
    tr.AddVerifierClientProcess(
        "client-4",
        replay_file,
        http_ports=[ts.Variables.port],
