...
def test_cache_promote(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    urtest.SkipUnless(Condition.PluginExists('cache_promote.so'))

    class CachePromotePluginTest:

        def __init__(self):
            self._tr = urtest.AddTestRun("cache_promote")
        ...
        def __setupOriginServer(self):
            self._httpbin = urtest.MakeHttpBinServer("httpbin")
