...
def test_txn_box_ramp(urtest: UraniumTest) -> None:
    # @file
    #
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    server_ramp = urtest.MakeVerifierServerProcess('pv-server-staging', replay_file)
    # Make it so that server-ramp is used in the txn_box config.
    orig_replay_file = os.path.join(urtest.TestDirectory, replay_file)
    ...
    temp_replay = tempfile.NamedTemporaryFile(dir=urtest.RunDirectory, delete=False)
