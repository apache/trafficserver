...
def test_test_hooks(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'test_hooks.so'), ts)
    ...
    #
    tr = urtest.AddTestRun()
    tr.Processes.Default.Command = "cd " + urtest.RunDirectory + " ; . " + urtest.TestDirectory + "/clean.sh"
    tr.Processes.Default.ReturnCode = 0
