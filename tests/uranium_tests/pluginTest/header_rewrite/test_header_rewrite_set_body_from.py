...
def test_header_rewrite_set_body_from(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    class HeaderRewriteSetBodyFromTest:
        ...
        def setUpTS(self):
            ...
            self.ts.Setup.CopyAs('rules/rule_set_body_from_remap.conf', urtest.RunDirectory)
            ...
            self.ts.Disk.remap_config.AddLine(
                """\
                 ...
                 """.format(self.server.Variables.Port, urtest.RunDirectory, urtest.GetTcpPort("bad_port")))
            self.ts.Disk.plugin_config.AddLine('header_rewrite.so {0}/rule_set_body_from_plugin.conf'.format(urtest.RunDirectory))
        ...
        def test_setBodyFromSucceeds_200(self):
            ...
            tr = urtest.AddTestRun()
            tr.MakeCurlCommand(
                '-s -v --proxy 127.0.0.1:{0} "http://www.example.com/200"'.format(self.ts.Variables.port), ts=self.ts)
            tr.Processes.Default.ReturnCode = 0
        ...
        def test_setBodyFromBinary(self):
            ...
            '''
            body_path = f"{urtest.RunDirectory}/binary_body.out"
