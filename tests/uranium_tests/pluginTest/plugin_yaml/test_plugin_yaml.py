...
def test_plugin_yaml(urtest: UraniumTest) -> None:
    '''
    ...
    tr = urtest.AddTestRun("Verify inline config sets response header")
    ...
    tr = urtest.AddTestRun("Verify disabled plugin is not loaded")
    tr.MakeCurlCommand(
        '-s -D- -o /dev/null -H "Host: example.com" -H "X-Debug: x-cache" http://127.0.0.1:{0}/test'.format(ts.Variables.port),
        ts=ts)
    tr.Processes.Default.ReturnCode = 0
    ...
    tr = urtest.AddTestRun("Verify plugin.yaml loading logged")
    tr.Processes.Default.Command = "echo check diags.log"
    tr.Processes.Default.ReturnCode = 0
    ts.Disk.diags_log.Content += Testers.ContainsExpression(
        "plugin.yaml loading", "diags.log should indicate plugin.yaml was loaded")
