...
def test_tsapi(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    urtest.Env["OUTPUT_FILE"] = log_file_name
    ...
    rp = os.path.join(urtest.Variables.AtsBuildUraniumTestsDir, 'pluginTest', 'tsapi', '.libs', f'{plugin_name}.so')
    ...
    ts.Disk.remap_config.AddLine(
        "map https://myhost.test:123 http://127.0.0.1:{0} @plugin={1} @plugin={1}".format(
            server.Variables.Port, f"{plugin_name}.so"))
    ...
    #
    tr.MakeCurlCommand(
        '--verbose {0} --header "Host: mYhOsT.teSt" hTtP://loCalhOst:{1}/'.format(ipv4flag, ts.Variables.port), ts=ts)
    tr.Processes.Default.ReturnCode = 0
