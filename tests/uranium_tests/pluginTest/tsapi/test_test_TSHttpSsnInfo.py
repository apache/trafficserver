...
def test_test_TSHttpSsnInfo(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.PrepareTestPlugin(
        os.path.join(urtest.Variables.AtsBuildUraniumTestsDir, 'pluginTest', 'tsapi', '.libs', 'test_TSHttpSsnInfo.so'), ts)
    ...
    tr = urtest.AddTestRun('HTTP/2 Test')
    tr.TimeOut = 10
    ...
    # H3
    if Condition.HasATSFeature('TS_USE_QUIC') and Condition.HasCurlFeature('http3'):
        tr = urtest.AddTestRun('HTTP/3 Test')
        tr.TimeOut = 10
    ...
    tr = urtest.AddTestRun('Check the log output')
