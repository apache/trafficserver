...
def test_test_TSHttpTxnServerAddrSet(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    class TestTSHttpTxnServerAddrSet:
        ...
        def _configure_server(self, tr: 'TestRun') -> 'Process':
            ...
            self._server.Streams.stdout += Testers.ContainsExpression(
                "redirect-succeeded", "Verify that the server sent the response.")
            return self._server
        ...
        def run(self):
            ...
            tr = urtest.AddTestRun("Verify TSHttpTxnServerAddrSet() works as expected.")
