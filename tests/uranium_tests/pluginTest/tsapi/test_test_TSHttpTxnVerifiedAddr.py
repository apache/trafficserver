...
def test_test_TSHttpTxnVerifiedAddr(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.PrepareTestPlugin(
        os.path.join(urtest.Variables.AtsBuildUraniumTestsDir, 'pluginTest', 'tsapi', '.libs', 'test_TSHttpTxnVerifiedAddr.so'), ts)

    ts.Setup.CopyAs('hrw_verified_addr.conf', urtest.RunDirectory)
    ts.Disk.plugin_config.AddLine(f'header_rewrite.so --inbound-ip-source=PLUGIN {urtest.RunDirectory}/hrw_verified_addr.conf')
    ...
    tr = urtest.AddTestRun("Test that a verified address is set by a plugin and read by another plugin")
