...
def test_money_trace(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    urtest.SkipUnless(Condition.PluginExists('money_trace.so'),)
    urtest.ContinueOnFail = False
    urtest.testName = "money_trace remap"
    ...
    urtest.Disk.File(os.path.join(ts.Variables.LOGDIR, 'remap.log'), exists=True, content='gold/remap-log.gold')

    curl_and_args = f"-s -D /dev/stdout -o /dev/stderr -x 127.0.0.1:{ts.Variables.port}"

    # 0 Test
    tr = urtest.AddTestRun("no plugin test")
    ps = tr.Processes.Default
    ps.StartBefore(server)
    ps.StartBefore(urtest.Processes.ts)
    tr.MakeCurlCommand(curl_and_args + " http://none/path", ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 1 Test
    tr = urtest.AddTestRun("basic config, no money trace client header")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + " http://basic/path", ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server
    ...
    # 2 Test
    tr = urtest.AddTestRun("basic config, money trace client header")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://basic/path -H "X-MoneyTrace: ' + maketrace("basic") + '"', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 3 Test
    tr = urtest.AddTestRun("header config, mt client header")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://header/path -H "mt: ' + maketrace("header") + '"', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 4 Test
    tr = urtest.AddTestRun("pregen config, but no header passed in")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://pregen/path', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 5 Test
    tr = urtest.AddTestRun("pregen config, money trace client header")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://pregen/path -H "X-MoneyTrace: ' + maketrace("pregen") + '"', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 6 Test
    tr = urtest.AddTestRun("pregen config, mt client header")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://pgh/path -H "mt: ' + maketrace("pgh") + '"', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 7 Test
    tr = urtest.AddTestRun("create config, money trace client header")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://create/path', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 8 Test
    tr = urtest.AddTestRun("create config, mt client header")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://cheader/path', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 9 Test
    tr = urtest.AddTestRun("create config, pregen client header")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://cpregen/path', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 10 Test
    tr = urtest.AddTestRun("passthru mode")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://passthru/path -H "X-MoneyTrace: ' + maketrace("passthru") + '"', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server
    ...
    for trace in trace_strings:
        tr = urtest.AddTestRun(trace)
        ps = tr.Processes.Default
        tr.MakeCurlCommand(curl_and_args + ' http://pregen/path -H "X-MoneyTrace: ' + trace + '"', ts=ts)
        ps.ReturnCode = 0
        tr.StillRunningAfter = ts
        tr.StillRunningAfter = server
    ...
    for trace in trace_strings:
        tr = urtest.AddTestRun(trace)
        ps = tr.Processes.Default
        tr.MakeCurlCommand(curl_and_args + ' http://cpregen/path -H "X-MoneyTrace: ' + trace + '"', ts=ts)
        ps.ReturnCode = 0
        tr.StillRunningAfter = ts
        tr.StillRunningAfter = server
