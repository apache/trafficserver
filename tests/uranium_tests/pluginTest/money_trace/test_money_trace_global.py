...
def test_money_trace_global(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    urtest.testName = "money_trace global"
    ...
    urtest.Disk.File(os.path.join(ts.Variables.LOGDIR, 'global.log'), exists=True, content='gold/global-log.gold')
    ...
    # 0 Test
    tr = urtest.AddTestRun("normal header test")
    ps = tr.Processes.Default
    ps.StartBefore(server, ready=When.PortOpen(server.Variables.Port))
    ps.StartBefore(urtest.Processes.ts)
    tr.MakeCurlCommand(curl_and_args + ' http://ats/path -H "MoneyTrace: ' + clientvalue + '"', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 1 Test
    tr = urtest.AddTestRun("skip plugin test - pregen will still be set")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://ats/path -H "Skip-Global-MoneyTrace: true"', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server

    # 2 Test
    tr = urtest.AddTestRun("create header test")
    ps = tr.Processes.Default
    tr.MakeCurlCommand(curl_and_args + ' http://ats/path', ts=ts)
    ps.ReturnCode = 0
    tr.StillRunningAfter = ts
    tr.StillRunningAfter = server
