...
def test_ja4_fingerprint(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    urtest.Summary = 'Tests the ja4_fingerprint plugin.'
    ...
    urtest.SkipUnless(Condition.PluginExists('ja4_fingerprint.so'))
    ...
    class TestJA4Fingerprint:
        ...
        def _configure_traffic_server(self, server_one: 'Process'):
            ...
            '''
            ts = urtest.MakeATSProcess(f'ts-{TestJA4Fingerprint.ts_counter + 1}', enable_tls=True)
    ...
    """.split("\n"))
        ...
        def _configure_wait_for_log(self, server_one: 'Process'):
            waiting_tr = urtest.AddAwaitFileContainsTestRun(self.name, self._ts.Disk.log_file.Name, 'JA4')
            return waiting_tr
    ...
    def test_preserve(params: TestParams) -> None:
        ...
        tr2 = urtest.AddTestRun('Verify preserve skips adding JA4 headers when they exist.')
        tr2.MakeCurlCommand(
        ...
        tr3 = urtest.AddTestRun('Verify preserve triggers when only x-ja4-via exists.')
        tr3.MakeCurlCommand(
