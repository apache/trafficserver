...
def test_stale_response(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition.PluginExists('stale_response.so'),)
    ...
    class TestStaleResponse:
        ...
        def __init__(self, option_type: OptionType, is_global: bool) -> None:
            ...
            tr = urtest.AddTestRun(f"stale_response.so Options: {option_description}")
        ...
        def verify_plugin_log(self) -> None:
            ...
            def expect_log_entry(pattern: str, description: str) -> None:
                tr = urtest.AddAwaitFileContainsTestRun(f"Await {description}", self._ts.Disk.stale_responses_log.AbsPath, pattern)
