...
def test_polite_hook_wait(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    # server.
    ts = urtest.MakeATSProcess("ts", enable_cache=False, block_for_debug=False)
    ...
    rp = os.path.join(urtest.Variables.AtsBuildUraniumTestsDir, 'pluginTest', 'polite_hook_wait', '.libs', f'{plugin_name}.so')
