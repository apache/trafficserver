...
def test_x_probe_full_json_slow_origin(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition.HasProgram("nc", "nc (netcat) is required for custom slow server"))
    ...
    ts.Disk.remap_config.AddLine(f"map / http://127.0.0.1:{urtest.Variables.server_port}/")

    # Start the custom slow-body server
    server = urtest.Processes.Process(
        "server",
        f"bash -c '{urtest.TestDirectory}/slow-body-server.sh {urtest.Variables.server_port} {urtest.RunDirectory}/server_request.txt'"
    )

    # Test with probe-full-json=nobody (which triggers the bug most easily)
    tr = urtest.AddTestRun("Verify probe-full-json with slow body delivery")
    ...
    tr2 = urtest.AddTestRun("Verify no infinite loop in transform")
    tr2.Processes.Default.Command = f"bash {urtest.TestDirectory}/verify_no_loop.sh {ts.Variables.LOGDIR}/traffic.out"
