...
def test_x_probe_full_json(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.Summary = 'Test xdebug plugin probe-full-json functionality'
    urtest.ContinueOnFail = True
    urtest.SkipUnless(Condition.PluginExists('xdebug.so'))
    urtest.SkipUnless(Condition.HasProgram("jq", "jq is required to validate JSON output"))
    ...
    class XDebugProbeFullJsonTest:
        ...
        def _setupClient(self) -> None:
            ...
            """
            tr = urtest.AddTestRun("Verify probe-full-json functionality")
        ...
        def _setupJqValidation(self) -> None:
            ...
            """
            tr = urtest.AddTestRun("Escaped text/html content.")
            self._startServersIfNeeded(tr)
            tr.MakeCurlCommand(
            ...
            tr = urtest.AddTestRun("Hex-encoded application/octet-stream content.")
            self._startServersIfNeeded(tr)
            tr.MakeCurlCommand(
            ...
            tr = urtest.AddTestRun("CSP header with single quotes (valid JSON)")
            self._startServersIfNeeded(tr)
            tr.MakeCurlCommand(
