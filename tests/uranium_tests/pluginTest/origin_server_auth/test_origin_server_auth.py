...
def test_origin_server_auth(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.testName = "origin_server_auth: config parsing"
    urtest.Summary = '''
    ...
    class OriginServerAuthTest:
        ...
        def setUpTS(self):
            ...
            self.ts.Setup.CopyAs('rules/v4-parse-test.test_input', urtest.RunDirectory)
            ...
            self.ts.Disk.remap_config.AddLine(
                f'map http://www.example.com/s3-bucket http://127.0.0.1:{self.server.Variables.Port}/s3-bucket \
                    @plugin=origin_server_auth.so \
                        @pparam=--config @pparam={urtest.RunDirectory}/v4-parse-test.test_input')
        ...
        def test_s3_origin_remap(self):
            ...
            tr.Processes.Default.Streams.stderr.Content += Testers.ContainsExpression(
                "Content-Length: 8", "expected content-length 8")
