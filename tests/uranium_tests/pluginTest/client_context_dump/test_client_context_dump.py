...
def test_client_context_dump(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition.PluginExists('client_context_dump.so'))
    ...
    # Set up plugin
    urtest.PrepareInstalledPlugin('client_context_dump.so', ts)

    # custom log comparison.  Verify the two certs we have loaded are dumped
    log = urtest.Disk.File(ts.Variables.LOGDIR + '/client_context_dump.log', exists=True)
    ...
    # traffic server test
    t = urtest.AddTestRun("Test traffic server started properly")
    t.StillRunningAfter = urtest.Processes.ts
