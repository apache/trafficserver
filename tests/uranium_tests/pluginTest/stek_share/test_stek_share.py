...
def test_stek_share(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Test the STEK Share plugin'
    urtest.testName = "stek_share"

    urtest.SkipUnless(Condition.PluginExists('stek_share.so'))
    ...
    ts5 = urtest.MakeATSProcess("ts5", enable_tls=True)

    urtest.Setup.Copy('ssl/self_signed.crt')
    urtest.Setup.Copy('ssl/self_signed.key')
    urtest.Setup.Copy('server_list.yaml')

    cert_path = os.path.join(urtest.RunDirectory, 'self_signed.crt')
    key_path = os.path.join(urtest.RunDirectory, 'self_signed.key')
    server_list_path = os.path.join(urtest.RunDirectory, 'server_list.yaml')
    ...
    ts1.Disk.records_config.update(
        {
            ...
            'proxy.config.ssl.server.cert.path': '{0}'.format(urtest.RunDirectory),
            'proxy.config.ssl.server.private_key.path': '{0}'.format(urtest.RunDirectory),
    ...
    tr1 = urtest.AddTestRun('Basic Curl test, and give it enough time for all ATS to start up and sync STEK')
    ...
    tr2 = urtest.AddTestRun("TLSv1.2 Session Ticket")
    tr2.Command = \
        ...
        .format(
            ...
            os.path.join(urtest.RunDirectory, 'sess.dat')
        )
    tr2.ReturnCode = 0
