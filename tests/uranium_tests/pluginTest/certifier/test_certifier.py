...
def test_certifier(urtest: UraniumTest) -> None:
    '''
    ...
    urtest.SkipUnless(Condition.PluginExists('certifier.so'))
    ...
    class DynamicCertTest:
        ...
        certPathSrc = os.path.join(urtest.TestDirectory, "certs")
        host = "www.tls.com"
        certPathDest = ""
        ...
        def setupOriginServer(self):
            self.server = urtest.MakeVerifierServerProcess("verifier-server1", self.httpsReplayFile)

        def setupTS(self):
            self.ts = urtest.MakeATSProcess("ts1", enable_tls=True)
    ...
    """.split("\n"))
        ...
        def runHTTPSTraffic(self):
            tr = urtest.AddTestRun("Test dynamic generation of certs")
        ...
        def verifyCert(self, certPath):
            tr = urtest.AddTestRun("Verify the content of the generated cert")
        ...
        def verifyCertNotExist(self, certPath):
            tr = urtest.AddTestRun("Verify the cert doesn't exist in the store")
    ...
    class ReuseExistingCertTest:
        ...
        def setupOriginServer(self):
            self.server = urtest.MakeVerifierServerProcess("verifier-server2", self.httpsReplayFile)

        def setupTS(self):
            self.ts = urtest.MakeATSProcess("ts2", enable_tls=True)
            self.ts.addDefaultSSLFiles()
            # copy over the cert store in which the certs will be generated/stored
            self.certPathDest = os.path.join(self.ts.Variables.CONFIGDIR, "certifier-certs")
            prepare_certifier_storage(self.certPathSrc, self.certPathDest)
    ...
    class UnsafeSniTest:
        ...
        def setupOriginServer(self):
            self.server = urtest.MakeVerifierServerProcess("verifier-server3", self.httpsReplayFile)

        def setupTS(self):
            self.ts = urtest.MakeATSProcess("ts3", enable_tls=True)
            self.ts.addDefaultSSLFiles()
            self.certPathDest = os.path.join(self.ts.Variables.CONFIGDIR, "certifier-certs")
            prepare_certifier_storage(self.certPathSrc, self.certPathDest)
            self.ts.Disk.records_config.update(
    ...
    """.split("\n"))
            ...
            self.ts.Disk.traffic_out.Content += Testers.ContainsExpression(
                "rejecting unsafe SNI for certificate storage",
                "Should reject SNI values that cannot be used safely as file names.")

        def runHTTPSTraffic(self):
            tr = urtest.AddTestRun("Test unsafe SNI is rejected")
            tr.AddVerifierClientProcess(
        ...
        def verifyCertNotExist(self, certPath):
            tr = urtest.AddTestRun("Verify unsafe SNI did not escape the store")
            tr.Processes.Default.Command = "echo verify"
    ...
    class NoSniTest:
        ...
        def setupOriginServer(self):
            self.server = urtest.MakeVerifierServerProcess("verifier-server4", self.httpsReplayFile)

        def setupTS(self):
            self.ts = urtest.MakeATSProcess("ts4", enable_tls=True)
            self.ts.addDefaultSSLFiles()
            self.certPathDest = os.path.join(self.ts.Variables.CONFIGDIR, "certifier-certs")
            prepare_certifier_storage(self.certPathSrc, self.certPathDest)
            self.ts.Disk.records_config.update(
    ...
    """.split("\n"))
        ...
        def runHTTPSTraffic(self):
            tr = urtest.AddTestRun("Test missing SNI falls back to default cert")
            tr.AddVerifierClientProcess(
