...
def test_substitute(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    '''
    urtest.SkipUnless(Condition.PluginExists('cookie_remap.so'))
    urtest.ContinueOnFail = True
    urtest.testName = "cookie_remap: Substitute variables"

    # Define default ATS
    ts = urtest.MakeATSProcess("ts")

    server = urtest.MakeOriginServer("server", ip='127.0.0.10')
    ...
    config_path = os.path.join(urtest.TestDirectory, "configs/substituteconfig.txt")
    ...
    tr = urtest.AddTestRun("Substitute $path in the dest query")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    ...
    tr = urtest.AddTestRun("Substitute $unmatched_path in the dest query")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    ...
    tr = urtest.AddTestRun("Substitute $cr_req_url using $cr_urlencode")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    "http://www.example.com/magic" \
    ...
    tr = urtest.AddTestRun("Substitute $path as is in outgoing path")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
