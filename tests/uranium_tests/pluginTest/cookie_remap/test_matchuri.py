...
def test_matchuri(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    config_path = os.path.join(urtest.TestDirectory, "configs/matchuriconfig.txt")
    ...
    tr = urtest.AddTestRun("URI value matches")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    ...
    tr = urtest.AddTestRun("URI value doesn't match")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
