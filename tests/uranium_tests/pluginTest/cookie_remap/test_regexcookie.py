...
def test_regexcookie(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.testName = "cookie_remap: cookie regex match and substitution"
    ...
    server2 = urtest.MakeOriginServer("server2", ip='127.0.0.11')
    request_header2 = {
    ...
    config_path = os.path.join(urtest.TestDirectory, "configs/regexconfig.txt")
    ...
    tr = urtest.AddTestRun("cookie regex matches")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    ...
    tr = urtest.AddTestRun("cookie regex doesn't match")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    "http://www.example.com/magic" \
    -H"Cookie: fpbeta=etc" \
