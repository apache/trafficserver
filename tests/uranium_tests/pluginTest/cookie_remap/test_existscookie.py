...
def test_existscookie(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.testName = "cookie_remap: cookie exists"
    ...
    config_path = os.path.join(urtest.TestDirectory, "configs/existsconfig.txt")
    ...
    tr = urtest.AddTestRun("cookie fpbeta exists")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    ...
    tr = urtest.AddTestRun("cooke fpbeta doesnt exist")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    "http://www.example.com/magic" \
