...
def test_setstatus(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.testName = "cookie_remap: set HTTP status of the response"

    # Define default ATS
    ts = urtest.MakeATSProcess("ts")

    # Setup the remap configuration
    config_path = os.path.join(urtest.TestDirectory, "configs/statusconfig.txt")
    ...
    tr = urtest.AddTestRun("Sets the status to 205")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    ...
    tr = urtest.AddTestRun("Sets the else status to 400")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    "http://www.example.com/magic" \
    -H "Proxy-Connection: keep-alive" \
