...
def test_disable_pristine_host_hdr(urtest: UraniumTest) -> None:
    '''
    ...
    class TestUpdateHostHeader:
        ...
        def _setupDns(self):
            ...
            self._dns = urtest.MakeDNServer(f"dns_{self.test_id}", default='127.0.0.1')
        ...
        def _setupServers(self):
            ...
            """
            self._server_canary = urtest.MakeVerifierServerProcess(f"server_canary_{self.test_id}", self.canary_replay_file)
            ...
            self._server_stable = urtest.MakeVerifierServerProcess(f"server_stable_{self.test_id}", self.stable_replay_file)
        ...
        def _setupTS(self, disable_pristine_host_hdr):
            ...
            """
            ts = urtest.MakeATSProcess(f"ts_{self.test_id}", enable_cache=False)
            self._ts = ts
            ...
            config_path = os.path.join(urtest.TestDirectory, f"configs/{config_filename}")
        ...
        def _setupClient(self):
            ...
            tr = urtest.AddTestRun(f'Test cookie bucket routing with disable_pristine_host_hdr {enabled_str}')
