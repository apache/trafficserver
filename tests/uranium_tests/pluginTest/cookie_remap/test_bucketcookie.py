...
def test_bucketcookie(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.testName = "cookie_remap: cookie in bucket or not"
    ...
    config_path = os.path.join(urtest.TestDirectory, "configs/bucketconfig.txt")
    ...
    tr = urtest.AddTestRun("cookie value in bucket")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    ...
    tr = urtest.AddTestRun("cooke value not in bucket")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    "http://www.example.com/magic" \
    -H"Cookie: fpbeta=etc" \
