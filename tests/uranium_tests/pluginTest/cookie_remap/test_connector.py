...
def test_connector(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.testName = "cookie_remap: test connector"
    ...
    config_path = os.path.join(urtest.TestDirectory, "configs/connectorconfig.txt")
    ...
    tr = urtest.AddTestRun("cookie value doesn't match")
    tr.MakeCurlCommand(
        ''' --proxy 127.0.0.1:{0} \
    "http://www.example.com/magic" \
