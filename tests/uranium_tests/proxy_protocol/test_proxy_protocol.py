...
def test_proxy_protocol(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    urtest.Summary = 'Test PROXY Protocol'
    ...
    class ProxyProtocolInTest:
        ...
        def setupOriginServer(self, name):
            self.server = urtest.MakeVerifierServerProcess(f"pp-in-server-{name}", self.replay_file)

        def setupTS(self, name, enable_cp):
            self.ts = urtest.MakeATSProcess(
                f"ts_in_{name}",
                enable_tls=True,
                enable_cache=False,
    ...
    '''.split("\n"))
        ...
        def runTraffic(self):
            tr = urtest.AddTestRun(f"Verify correct handling of incoming PROXY header. {self.name}")
        ...
        def checkAccessLog(self):
            ...
            tr = urtest.AddTestRun(f'Verify PROXY protocol access log lines. {self.name}')
    ...
    class ProxyProtocolAllowlistTest:
        ...
        def setupOriginServer(self):
            self.server = urtest.MakeVerifierServerProcess("pp-allowlist-server", self.replay_file)

        def setupTS(self):
            self.ts = urtest.MakeATSProcess("ts_pp_allowlist", enable_tls=True, enable_cache=False, enable_proxy_protocol=True)
    ...
    """.split("\n"))
        ...
        def setLogExpectations(self, tr: 'TestRun') -> None:
            ...
            tr.Processes.Default.Streams.All += Testers.ContainsExpression(
                "HTTP/1.1 200 OK", "Verify the client got a 200 response.")
