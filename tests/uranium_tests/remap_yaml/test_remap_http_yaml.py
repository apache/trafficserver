...
def test_remap_http_yaml(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    server2 = urtest.MakeOriginServer("server2", lookup_key="{%Host}{PATH}")
    ...
    if not Condition.CurlUsingUnixDomainSocket():
        tr = urtest.AddTestRun()
        tr.MakeCurlCommand(
            '--proxy 127.0.0.1:{0} "http://www.example2.com"  -H "Proxy-Connection: keep-alive" --verbose'.format(
                ts.Variables.port),
            ts=ts)
        tr.Processes.Default.ReturnCode = 0
    ...
    tr = urtest.AddTestRun()
    tr.MakeCurlCommand(
        ' --proxy 127.0.0.1:{0} "http://www.example.com:80/"  -H "Proxy-Connection: keep-alive" --verbose'.format(
            ts.Variables.port),
        ts=ts)
    tr.Processes.Default.ReturnCode = 0
    tr.Processes.Default.Streams.stderr = "gold/remap-200.gold"
    ...
    tr = urtest.AddTestRun()
    tr.MakeCurlCommand(
        ' --proxy 127.0.0.1:{0} "http://www.example.com:8080"  -H "Proxy-Connection: keep-alive" --verbose'.format(
            ts.Variables.port),
        ts=ts)
    tr.Processes.Default.ReturnCode = 0
    tr.Processes.Default.Streams.stderr = "gold/remap-200.gold"
    ...
    tr = urtest.AddTestRun()
    tr.MakeCurlCommand(
        ' --proxy 127.0.0.1:{0} "http://www.test.com/"  -H "Proxy-Connection: keep-alive" --verbose'.format(ts.Variables.port),
        ts=ts)
    tr.Processes.Default.ReturnCode = 0
