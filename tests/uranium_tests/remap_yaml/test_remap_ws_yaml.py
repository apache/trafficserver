...
def test_remap_ws_yaml(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    remap:
      - type: map
        from:
          url: ws://www.example.com:{ts.Variables.port}
        to:
    ...
    if not Condition.CurlUsingUnixDomainSocket():
        ...
        tr.Processes.Default.StartBefore(urtest.Processes.ts, ready=1)
        tr.MakeCurlCommand(
    ...
    tr.Processes.Default.Command = (
        ...
        f" {urtest.TestDirectory}/gold/{metrics_gold_file}")
