...
def test_remap_ip_resolve_yaml(urtest: UraniumTest) -> None:
    '''
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    ...
    server_v6 = urtest.MakeOriginServer("server_v6", None, None, '::1', 0)
    ...
    remap:
      - type: map
        ...
        to:
          url: http://test.ipv4.only.com:{server.Variables.Port}
        plugins:
          - name: conf_remap.so
            params:
              - proxy.config.hostdb.ip_resolve=ipv6;ipv4;client
      - type: map
        from:
          url: http://testDNS2.com
        to:
          url: http://test.ipv6.only.com:{server_v6.Variables.Port}
        plugins:
          - name: conf_remap.so
            params:
    ...
    tr = urtest.AddTestRun()
    tr.MakeCurlCommand(' --proxy 127.0.0.1:{0} "http://testDNS2.com" --verbose'.format(ts.Variables.port), ts=ts)
    tr.Processes.Default.ReturnCode = 0
