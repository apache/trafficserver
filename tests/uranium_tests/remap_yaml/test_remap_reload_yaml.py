...
def test_remap_reload_yaml(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    tm = urtest.MakeATSProcess("ts", disable_log_checks=True)
    remap_cfg_path = os.path.join(tm.Variables.CONFIGDIR, 'remap.yaml')

    pv = urtest.MakeVerifierServerProcess("pv", "reload_server.replay.yaml")
    ...
    tr = urtest.AddTestRun("verify load")
    ...
    tr = urtest.AddTestRun("Change remap.yaml to have only two remap rules")
    ...
    tr = urtest.AddConfigReload(
        tm, expect="fail", expect_tasks=["remap.yaml"], delay_start=2, description="remap_yaml reload, fails")

    tr = urtest.AddTestRun("after first reload")
    tr.AddVerifierClientProcess("client_2", replay_file_2, http_ports=[tm.Variables.port])

    tr = urtest.AddTestRun("Change remap.yaml to have more than three remap rules")
    p = tr.Processes.Default
    p.Env = tm.Env
