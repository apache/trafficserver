...
def test_redirect(urtest: UraniumTest) -> None:
    '''
    Test redirection
    '''
    #  Licensed to the Apache Software Foundation (ASF) under one
    ...
    redirect_serv = urtest.MakeOriginServer("re_server")
    ...
    tr = urtest.AddTestRun("FollowsRedirectWithAbsoluteLocationURI")
    ...
    tr = urtest.AddTestRun("FollowsRedirectWithRelativeLocationURI")
    command_path = os.path.join(data_path, tr.Name)
    with open(command_path, 'w') as f:
        f.write(
    ...
    tr = urtest.AddTestRun("FollowsRedirectWithRelativeLocationURIMissingLeadingSlash")
    command_path = os.path.join(data_path, tr.Name)
    with open(command_path, 'w') as f:
        f.write(
    ...
    for status, phrase in sorted({
        ...
        tr = urtest.AddTestRun("FollowsRedirect{0}".format(status))
        command_path = os.path.join(data_path, tr.Name)
        with open(command_path, 'w') as f:
            f.write(
    ...
    urtest.Setup.Copy('wait_for_log.sh')

    tr = urtest.AddTestRun("wait_for_log")
