...
def test_session_match(urtest: UraniumTest) -> None:
    '''
    ...
    class SessionMatchTest:
        ...
        def __init__(self, TestSummary, sharingMatchValue):
            ...
            urtest.Summary = TestSummary
            self._tr = urtest.AddTestRun()
            self._sharingMatchValue = sharingMatchValue
            self.setupOriginServer()
            self.setupTS()

        def setupOriginServer(self):
            self._server = urtest.MakeOriginServer("server{counter}".format(counter=self._MyTestCount))
            request_header = {
        ...
        def setupTS(self):
            self._ts = urtest.MakeATSProcess("ts{counter}".format(counter=self._MyTestCount))
