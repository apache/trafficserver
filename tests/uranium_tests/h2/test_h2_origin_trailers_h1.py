...
def test_h2_origin_trailers_h1(urtest: UraniumTest) -> None:
    '''
    ...
    class TestOriginTrailers:
        ...
        def _configure_server(self, name):
            return urtest.MakeVerifierServerProcess(name, self._replay_file)
    ...
    """.split("\n"))
        ...
        def _configure_h1_client(self):
            urtest.Setup.CopyAs('h1_trailer_client.py', urtest.RunDirectory)

            tr = urtest.AddTestRun('HTTP/2 origin trailers are dropped for HTTP/1 clients')
        ...
        def _configure_h2_client(self):
            tr = urtest.AddTestRun('HTTP/2 origin trailers are forwarded to HTTP/2 clients')
