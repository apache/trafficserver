...
def test_http2_flow_control(urtest: UraniumTest) -> None:
    ...
    """.split("\n"))
        ...
        def _configure_inbound_http1_to_origin_test_run(self) -> None:
            ...
            tr = urtest.AddTestRun(f'{self._description} - inbound, '
        ...
        def _configure_inbound_http2_to_origin_test_run(self) -> None:
            """Configure the TestRun for inbound stream configuration."""
            tr = urtest.AddTestRun(f'{self._description} - inbound, HTTP/2 origin')
        ...
        def _configure_outbound_test_run(self) -> None:
            ...
            tr = urtest.AddTestRun(f'{self._description} - outbound, HTTP/2 origin')
    ...
    class Http2DynamicWindowSettingsCapTest:
        ...
        def run(self) -> None:
            ...
            tr = urtest.AddTestRun('Dynamic stream windows cap unacknowledged SETTINGS')
            ...
            ts.addDefaultSSLFiles()
            ts.Setup.CopyAs('clients/h2_settings_ack_stall.py', urtest.RunDirectory)
            ts.Disk.records_config.update(
                {
