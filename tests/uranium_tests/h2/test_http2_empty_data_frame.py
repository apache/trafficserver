...
def test_http2_empty_data_frame(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    class Http2EmptyDataFrameTest:
        ...
        def __setupOriginServer(self):
            self._server = urtest.MakeHttpBinServer("httpbin")

        def __setupTS(self):
            self._ts = urtest.MakeATSProcess(f"ts", enable_tls=True, enable_cache=True)
    ...
    """.split("\n"))
        ...
        def __setupClient(self):
            self._ts.Setup.CopyAs("clients/h2empty_data_frame.py", urtest.RunDirectory)

        def run(self):
            tr = urtest.AddTestRun("warm-up cache")
            ...
            tr = urtest.AddTestRun("open 20 streams")
