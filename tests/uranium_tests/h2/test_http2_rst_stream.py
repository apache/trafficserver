...
def test_http2_rst_stream(urtest: UraniumTest) -> None:
    '''
    ...
    #
    ts = urtest.MakeATSProcess("ts0", enable_tls=True)
    replay_file = "replay_rst_stream/http2_rst_stream_client_after_data.yaml"
    server = urtest.MakeVerifierServerProcess("server0", replay_file)
    ...
    tr = urtest.AddTestRun('Client sends RST_STREAM after DATA frame')
    ...
    server = urtest.MakeVerifierServerProcess("server1", replay_file)
    ts.addDefaultSSLFiles()
    ts.Disk.records_config.update(
        {
            'proxy.config.ssl.server.cert.path': f'{ts.Variables.SSLDir}',
    ...
    tr = urtest.AddTestRun('Client sends RST_STREAM after HEADERS frame')
    tr.Processes.Default.StartBefore(server)
    tr.Processes.Default.StartBefore(ts)
    ...
    server = urtest.MakeVerifierServerProcess("server2", replay_file)
    ts.addDefaultSSLFiles()
    ts.Disk.records_config.update(
        {
            'proxy.config.ssl.server.cert.path': f'{ts.Variables.SSLDir}',
    ...
    tr = urtest.AddTestRun('Server sends RST_STREAM after HEADERS frame')
    tr.Processes.Default.StartBefore(server)
    tr.Processes.Default.StartBefore(ts)
