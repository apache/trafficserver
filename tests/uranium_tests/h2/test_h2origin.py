...
def test_h2origin(urtest: UraniumTest) -> None:
    '''
    ...
    server = urtest.MakeVerifierServerProcess("h2-origin", replay_file)
    server_expect = urtest.MakeVerifierServerProcess("server-expect", "expect_100_continue.yaml")
    ...
    tr = urtest.AddTestRun("Test traffic to origin using HTTP/2")
    ...
    tr = urtest.AddTestRun("Test an empty body POST request with an Expect: 100-continue header")
    ...
    tr = urtest.AddTestRun("Wait for the squid.log to be written")
    timeout = 30
    ...
    tr = urtest.AddTestRun("Test HTTP method Metrics")
    tr.Processes.Default.Command = (
        f"{urtest.Variables.AtsTestToolsDir}/stdout_wait" + " 'traffic_ctl metric get" + " proxy.process.http.get_requests" +
        " proxy.process.http.post_requests" + " proxy.process.http.put_requests'" +
        f" {urtest.TestDirectory}/gold/http-request-method-metrics.gold")
