...
def test_http2_close_connection(urtest: UraniumTest) -> None:
    #  Licensed to the Apache Software Foundation (ASF) under one
    #  or more contributor license agreements.  See the NOTICE file
    #  distributed with this work for additional information
    #  regarding copyright ownership.  The ASF licenses this file
    ...
    urtest.SkipUnless(Condition.HasProxyVerifierVersion('2.8.0'))

    pv_server = urtest.MakeVerifierServerProcess("pv_server", "http2_close_connection.yaml")

    ts = urtest.MakeATSProcess('ts', select_ports=True, enable_tls=True)
    ...
    urtest.PrepareTestPlugin(os.path.join(urtest.Variables.AtsTestPluginsDir, 'http2_close_connection.so'), ts)
