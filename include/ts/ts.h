...
/** Set an identifying label for the entity that last set the HTTP status.
 *
 * This function allows tracking which component  was responsible for
 * setting the current HTTP status code. The setter information is used
 * for logging purposes and can be retrieved using the 'plss' log field.
 *
 * @param[in] txnp The transaction for which the status was set.
 * @param[in] setter Identifying label for the entity setting the status
 *   (e.g., plugin name). If empty, clears the current setter information.
 */
void TSHttpStatusSetterSet(TSHttpTxn txnp, std::string_view setter);

/** Get the identifying label for the entity that last set the HTTP status.
 *
 * @param[in] txnp The transaction handle.
 * @return The setter label, or an empty string if no setter has been
 *   recorded. The returned string_view is valid for the transaction
 *   lifetime.
 */
std::string_view TSHttpStatusSetterGet(TSHttpTxn txnp);
