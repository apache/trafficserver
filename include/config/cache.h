...
public:
   ...
   * @param[out] format Detected input format, if requested.
   * @return The parsed rules and any diagnostics.
   */
  ConfigResult<CacheConfig> parse(std::string const &filename, Format *format = nullptr) const;
   ...
   * @return The parsed rules and any diagnostics.
   */
  ConfigResult<CacheConfig> parse_content(std::string_view content, std::string_view filename, Format *format = nullptr) const;
