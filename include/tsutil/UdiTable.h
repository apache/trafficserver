...
public:
  ...
  using data_format_fn = std::function<std::string(Key const &, uint32_t, const_data_ptr const &)>;
   ...
   * Called under the table mutex: any predicate locks must follow that lock.
   * For abuse_shield the order is table mutex, then RuleBuckets mutex.
   */
   ...
   * The returned shared_ptr remains valid even if the slot is later evicted.
   */
  data_ptr process_event(Key const &key, uint32_t score_delta = 1, ProcessStatus *status = nullptr);
  ...
  /// Snapshot occupied data for maintenance without holding the table lock.
  std::vector<data_ptr>
  data_snapshot() const
  {
    std::lock_guard       lock(mutex_);
    std::vector<data_ptr> result;
    result.reserve(lookup_.size());
    for (const auto &[key, index] : lookup_) {
      result.push_back(slots_[index].data);
    }
    return result;
  }

  /// Clear entries; outstanding shared pointers remain valid.
  void
  clear()
  {
    std::lock_guard lock(mutex_);
...
private:
  ...
  /// Monotonic timestamp for elapsed-time metrics.
  std::chrono::steady_clock::time_point last_reset_steady_{std::chrono::steady_clock::now()};
};
...
UdiTable<Key, Data, Hash, CanEvict>::process_event(Key const &key, uint32_t score_delta, ProcessStatus *status)
{
  ...
  if (it != lookup_.end()) {
    ...
    slot.score = score_delta > UINT32_MAX - slot.score ? UINT32_MAX : slot.score + score_delta;
    return slot.data;
  }
...
UdiTable<Key, Data, Hash, CanEvict>::reset_metrics()
{
  ...
  last_reset_steady_ = std::chrono::steady_clock::now();
}
...
{
  std::lock_guard<std::mutex> lock(mutex_);
  auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::steady_clock::now() - last_reset_steady_);
...
UdiTable<Key, Data, Hash, CanEvict>::dump(data_format_fn format_data) const
{
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    snapshot.reserve(lookup_.size());
    for (auto const &[key, slot_idx] : lookup_) {
      Slot const &slot = slots_[slot_idx];
      snapshot.push_back({slot.key, slot.score, slot.data, slot_idx});
    }
  }

  std::string result;
  result.reserve(snapshot.size() * 64);

  for (auto const &entry : snapshot) {
    if (format_data) {
      result += format_data(entry.key, entry.score, entry.data);
    } else {
      result += "slot[" + std::to_string(entry.slot_idx) + "] score=" + std::to_string(entry.score) + "\n";
    }
  }
  return result;
}
...
UdiTable<Key, Data, Hash, CanEvict>::contest(Key const &key, uint32_t incoming_score, ProcessStatus *status)
{
  ...
  if (slot.is_empty() || incoming_score > current_score) {
    ...
    if (status) {
      *status = ProcessStatus::CONTEST_LOST;
    }
    return nullptr;
  }
}
