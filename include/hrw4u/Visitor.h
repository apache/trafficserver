...
public:
  ...
  const char              *from_url  = nullptr;
  const char              *to_url    = nullptr;
...
enum class ObjKind : uint8_t { CONDITION, OPERATOR, OPERATOR_IF, RULESET };
...
using DestroyCallback             = std::function<void(void *ptr, ObjKind kind)>;
...
struct ParserConfig {
  ...
  const char              *from_url = nullptr;
  const char              *to_url   = nullptr;
  std::vector<std::string> procedures_paths;
};
