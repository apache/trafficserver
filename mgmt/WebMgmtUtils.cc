...
varSetFromStr(const char *varName, const char *value)
{
  ...
  case RECD_INT:
    if (sscanf(value, "%" PRId64 "", &data.rec_int) == 1) {
      RecSetRecordInt((char *)varName, data.rec_int, REC_SRC_API);
    } else {
      found = false;
    }
    break;
  case RECD_COUNTER:
    if (sscanf(value, "%" PRId64 "", &data.rec_counter) == 1) {
      RecSetRecordCounter((char *)varName, data.rec_counter, REC_SRC_API);
    } else {
      found = false;
    }
    break;
  case RECD_FLOAT:
    ...
    if (sscanf(value, "%f", &data.rec_float) == 1) {
      RecSetRecordFloat((char *)varName, data.rec_float, REC_SRC_API);
    } else {
      found = false;
    }
    break;
  case RECD_STRING:
    if (*value == '\0') {
      RecSetRecordString((char *)varName, NULL, REC_SRC_API);
    } else {
      RecSetRecordString((char *)varName, (char *)value, REC_SRC_API);
    }
    break;
  case RECD_NULL:
  default:
...
varSetFloat(const char *varName, RecFloat value, bool convert)
{
  ...
  switch (varDataType) {
  case RECD_FLOAT:
    RecSetRecordFloat((char *)varName, (RecFloat)value, REC_SRC_API);
    break;
  case RECD_INT:
    if (convert) {
      value += 0.5; // rounding up
      RecSetRecordInt((char *)varName, (RecInt)value, REC_SRC_API);
      break;
    }
  case RECD_COUNTER:
    if (convert) {
      RecSetRecordCounter((char *)varName, (RecCounter)value, REC_SRC_API);
      break;
    }
  case RECD_STRING:
  case RECD_NULL:
...
varSetData(RecDataT varType, const char *varName, RecData value)
{
  ...
  switch (varType) {
  case RECD_INT:
    err = RecSetRecordInt((char *)varName, value.rec_int, REC_SRC_API);
    break;
  case RECD_COUNTER:
    err = RecSetRecordCounter((char *)varName, value.rec_counter, REC_SRC_API);
    break;
  case RECD_FLOAT:
    err = RecSetRecordFloat((char *)varName, value.rec_float, REC_SRC_API);
    break;
  default:
