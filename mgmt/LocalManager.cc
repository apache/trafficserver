...
LocalManager::LocalManager(bool proxy_on) : BaseManager(), run_proxy(proxy_on), configFiles(NULL)
{
  ...
  proxy_running = 0;
  RecSetRecordInt("proxy.node.proxy_running", 0, REC_SRC_DEFAULT);
...
LocalManager::initMgmtProcessServer()
{
  ...
  umask(oldmask);
  RecSetRecordInt("proxy.node.restarts.manager.start_time", manager_started_at, REC_SRC_DEFAULT);
}
...
LocalManager::handleMgmtMsgFromProcesses(MgmtMessageHdr *mh)
{
  ...
  case MGMT_SIGNAL_PID:
    ...
    proxy_launch_pid = -1;
    proxy_launch_outstanding = false;
    RecSetRecordInt("proxy.node.proxy_running", 1, REC_SRC_DEFAULT);
    break;
  ...
  case MGMT_SIGNAL_PLUGIN_SET_CONFIG: {
    ...
    switch (stype) {
    case MGMT_INT:
      RecSetRecordInt(var_name, ink_atoi64(var_value), REC_SRC_API);
      break;
    case MGMT_COUNTER:
    case MGMT_FLOAT:
    case MGMT_STRING:
...
LocalManager::startProxy()
{
...
#endif
  {
    ...
    ++proxy_launch_count;
    RecSetRecordInt("proxy.node.restarts.proxy.start_time", proxy_started_at, REC_SRC_DEFAULT);
    RecSetRecordInt("proxy.node.restarts.proxy.restart_count", proxy_launch_count, REC_SRC_DEFAULT);
  } else {
    int res, i = 0;
