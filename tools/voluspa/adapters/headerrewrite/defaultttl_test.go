...
func TestDefaultTTL(t *testing.T) {
	...
	plugin := &DefaultTTLAdapter{}

	if plugin.Type() != "default_ttl" {
		t.Errorf("Type() returned unexpected %q", plugin.Type())
	}

	if fmt.Sprintf("%v", plugin.ConfigParameters()) != "[default_ttl]" {
	...
	for i, tt := range []struct {
		...
		{
			param: nil,
			err:   errors.New(`option "default_ttl": value for "default_ttl" is not a string: <nil>`),
		},
		{
			param: true,
			err:   errors.New(`option "default_ttl": value for "default_ttl" is not a string: true`),
		},
		{
			param: 42,
			err:   errors.New(`option "default_ttl": value for "default_ttl" is not a string: 42`),
		},
		{
			param: "5d",
			expected: `
...
cond %{STATUS} <400
    set-header Cache-Control "no-cache, no-store, must-revalidate, stale-while-revalidate=30"`,
		...
		t.Run(fmt.Sprintf("%d-%v", i, tt.param), func(t *testing.T) {
			...
			(*ro)[DefaultTTLAdapterParameter] = tt.param
