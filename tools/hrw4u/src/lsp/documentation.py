...
URL_COMPONENTS: Final[dict[str, FieldInfo]] = {
    ...
    'query': FieldInfo(
        'Query String', 'The query string parameters (everything after ?)', usage='Used for query parameter analysis and filtering.'
    ),
...
ID_FIELDS: Final[dict[str, FieldInfo]] = {
    ...
    'PROCESS': FieldInfo(
        'Process ID', 'ATS process identifier', '%{ID:PROCESS}', 'Used for process-level debugging and monitoring.'
    ),
    'THREAD': FieldInfo(
    ...
    'SSN': FieldInfo(
        'Session ID (alias)',
        'Alias for SESSION - client session identifier',
        '%{ID:SSN}',
...
TIME_FIELDS: Final[dict[str, FieldInfo]] = {
    ...
    'HOUR': FieldInfo(
        'Current Hour', 'Hour in 24-hour format (0-23)', '%{NOW:HOUR}', 'Used for time-of-day based routing and load balancing.'
    ),
    ...
    'WEEKDAY': FieldInfo(
        'Day of Week', 'Day of week (0=Sunday, 6=Saturday)', '%{NOW:WEEKDAY}', 'Used for weekday-based routing and scheduling.'
    ),
...
GEO_FIELDS: Final[dict[str, FieldInfo]] = {
    ...
    'ASN': FieldInfo(
        ...
        'ASN of the client IP address (e.g., 15169 for Google)',
        '%{GEO:ASN}',
        'Used for network-based routing and traffic analysis.',
    ),
    'ASN-NAME': FieldInfo(
        'AS Organization',
        'Name of the Autonomous System organization (e.g., "Google LLC")',
        '%{GEO:ASN-NAME}',
...
IP_PATTERNS: Final[dict[str, IPPatternInfo]] = {
    'inbound.ip': IPPatternInfo(
        'Client IP Address',
        'IP address of the connecting client',
        '%{IP:CLIENT}',
        'Used for access control, logging, and geographic routing.',
    ),
    'outbound.ip': IPPatternInfo(
        'Server IP Address',
        'IP address of the destination server',
        '%{IP:SERVER}',
        'Used for server selection and routing decisions.',
    ),
    'inbound.server': IPPatternInfo(
        'Inbound Server IP',
        'IP address of the ATS inbound interface',
        '%{IP:INBOUND}',
        'Used for multi-homed server configurations.',
    ),
    'outbound.server': IPPatternInfo(
        'Outbound Server IP',
        'IP address of the ATS outbound interface',
        '%{IP:OUTBOUND}',
...
CONN_FIELDS: Final[dict[str, FieldInfo]] = {
    'DSCP': FieldInfo(
        ...
        'Traffic classification and QoS marking (0-63)',
        'set-conn-dscp',
        'Used for Quality of Service (QoS) traffic classification and prioritization.',
    ),
    'MARK': FieldInfo(
        'Connection Mark',
        'Netfilter connection mark for traffic shaping (32-bit integer)',
        'set-conn-mark',
    ...
    def parse_capture_expression(expression: str, is_interpolation: bool = False) -> dict[str, str] | None:
        ...
        try:
            ...
            return {
                ...
                "usage": "Used to insert captured substrings from regex matches into string values. The capture group must be defined in a preceding regex condition (e.g., using `//` matches).",
...
LSP_COOKIE_DOCUMENTATION: Final[dict[str, DocumentationInfo]] = {
    "inbound.cookie": DocumentationInfo(
        ...
        examples=[
            ...
            "inbound.cookie.role = \"guest\"; // Set default role",
            "inbound.cookie.tracking = \"\"; // Delete tracking cookie",
            "if inbound.cookie.consent == \"accepted\" {\n    // User accepted cookies\n}",
        ],
    ),
    "outbound.cookie": DocumentationInfo(
        ...
        while i < len(line):
            if line[i] == '/':
                ...
                if (
                    ...
                    or ' ~ ' in before_context
                    or '(' in before_context
                    or before_context.endswith('(')
                    or 'if ' in before_context
                ):
        ...
        if brief:
            ...
            doc_parts.extend(
                [
                    ...
                    "**Usage Examples:**",
                    "```hrw4u",
                    "if inbound.req.User-Agent ~ /(?i)bot|crawler/ {",
                    "    // Match bot user agents case-insensitively",
                    "}",
                    "",
                    "if inbound.url.path ~ /^/api/v\\d+/ {",
...
LSP_HTTP_CONTROL_DOCUMENTATION: Final[dict[str, DocumentationInfo]] = {
    ...
    "INTERCEPT_RETRY": DocumentationInfo(
    ...
    "RESP_CACHEABLE": DocumentationInfo(
    ...
    "REQ_CACHEABLE": DocumentationInfo(
    ...
    "SERVER_NO_STORE": DocumentationInfo(
...
LSP_FUNCTION_DOCUMENTATION: Final[dict[str, FunctionDoc]] = {
    ...
    "set-config": FunctionDoc(
        ...
        examples=["set-config(\"proxy.config.http.cache.http\", 0);", "set-config(\"proxy.config.http.allow_multi_range\", 1);"],
    ),
    "no-op": FunctionDoc(
    ...
    "set-body": FunctionDoc(
        ...
        examples=["set-body(\"Not found\", \"text/plain\");", 'set-body("{{"error": "not found"}}", "application/problem+json");'],
    ),
    "set-body-from": FunctionDoc(
        ...
        examples=[
            "set-body-from(\"https://errors.example.com/500.html\");",
            "set-body-from(\"http://content.example.com/body.txt\");",
        ],
    ),
    "set-body-from-file": FunctionDoc(
...
LSP_FIELD_DOCUMENTATION: Final[dict[str, DocumentationInfo]] = {
    "inbound.resp.body": DocumentationInfo(
        ...
        examples=["inbound.resp.body = \"Custom error message\";", 'set-body("{{"error": "bad request"}}", "application/json");'],
    ),
    "outbound.resp.body": DocumentationInfo(
    ...
    "http.status.reason": DocumentationInfo(
...
class ModifierPattern:
    ...
    def create_modifier_hover(modifier_data: dict[str, str]) -> str:
        ...
        if current_modifier in LSP_MODIFIER_DOCUMENTATION:
            ...
            sections = [
                f"**{current_modifier}** - {mod_doc.name}",
                "",
                f"**Context:** {mod_doc.context}",
                "",
                f"**Description:** {mod_doc.description}",
                "",
                f"**Usage:** {mod_doc.usage}",
            ]
...
LSP_NAMESPACE_DOCUMENTATION: Final[dict[str, DocumentationInfo]] = {
    ...
    "inbound": DocumentationInfo(
        ...
        examples=["inbound.method == \"POST\"", "inbound.req.User-Agent ~ /mobile/", "inbound.url.host == \"api.example.com\""],
    ),
    "outbound": DocumentationInfo(
        ...
        examples=["outbound.req.Authorization = \"Bearer {token}\"", "if outbound.status >= 500 {\n    // Handle server errors\n}"],
    ),
    "client": DocumentationInfo(
...
LSP_SUB_NAMESPACE_DOCUMENTATION: Final[dict[str, DocumentationInfo]] = {
    "inbound.conn": DocumentationInfo(
    ...
    "outbound.conn": DocumentationInfo(
        ...
        usage="Used for upstream server routing, protocol negotiation, load balancing decisions, and server connection analysis.",
        examples=[
    ...
    "inbound.resp": DocumentationInfo(
        ...
        examples=[
            "inbound.resp.X-Powered-By = \"Apache Traffic Server\"",
            "inbound.resp.Access-Control-Allow-Origin = \"*\"",
            "inbound.resp.Cache-Control = \"max-age=3600\"",
        ],
    ),
    "outbound.req": DocumentationInfo(
        ...
        examples=[
            ...
            "outbound.req.X-Forwarded-For = \"{inbound.conn.REMOTE-ADDR}\"",
            "outbound.req.User-Agent = \"ATS-Proxy/9.0\"",
        ],
    ),
    "outbound.resp": DocumentationInfo(
    ...
    "inbound.url": DocumentationInfo(
        ...
        examples=[
            ...
            "inbound.url.path ~ /^\\/api\\/v[0-9]+\\//",
            "inbound.url.query ~ /debug=true/",
        ],
    ),
    "outbound.url": DocumentationInfo(
        ...
        examples=[
            "outbound.url.host = \"backend.internal.com\"",
            "outbound.url.path = \"/v2\" + inbound.url.path",
    ...
    "outbound.cookie": DocumentationInfo(
        ...
        examples=[
            "outbound.cookie.sessionid = \"{id.UNIQUE}\"",
            "outbound.cookie.theme = \"dark\"",
