...
class ValidatorChain:
    ...
    def _wrap_argwise(self, fn: Callable[[str], None]) -> Callable[[List[str]], None]:
    ...
    def _add(self, fn: Callable[[List[str]], None]) -> 'ValidatorChain':
        self._validators.append(fn)
        return self

    def arg_at(self, index: int, fn: Callable[[str], None]) -> 'ValidatorChain':
    ...
    def min_args(self, n: int) -> 'ValidatorChain':
        return self._add(Validator.min_args(n))
...
class Validator:
    ...
    @staticmethod
    def arg_count(*counts: int) -> 'ValidatorChain':

        def validator(args: List[str]) -> None:
            if len(args) not in counts:
                allowed = ", ".join(str(c) for c in counts)
                raise SymbolResolutionError(str(args), f"Invalid number of arguments (expected {allowed}, got {len(args)})")

        return ValidatorChain([validator])

    @staticmethod
    def min_args(n: int) -> ValidatorChain:

        def validator(args: List[str]) -> None:
            if len(args) < n:
                raise SymbolResolutionError(str(args), f"At least {n} argument(s) required (got {len(args)})")
