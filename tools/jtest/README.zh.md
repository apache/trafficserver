...
#### 参数格式：
参数格式是 短参数_长参数_类型_默认值_参数说明，多数参数能做到自我说明比较详细，更多说明参见下面的说明以及后续高级用法示例。
...
* -k -K -j，控制客户端和服务器的长连接。
* -i，用来控制jtest汇报间隔时间。
* -L，用来控制jtest生成的随机url的大小，默认-1表示完全随机，没有控制。
...
#### 测试的流程处理：
jtest测试，是可以进行复杂的情况，比如对一个网站进行深度抓取测试，对反向、正向、透明模式测试
...
### screen的并行jtest管理
首先，我们必须有一个并发测试的机制，单机多进程、多机并行，并且可以统计各个进程返回的结果。当初ATS有一个测试框架，可以执行多个机器的并行测试，并能够汇总多个jtest的返回结果。我们这里采用简单的模式，screen脚本：
...
我们采用screen的`-X quit`命令，来停止jtest压测。同时，为了表示各个目标的机器，我们可以给各个screen打上标识，以方便区分一个机器上的多个screen。

### N:N的测试机器
为了测试，我们假定有10台测试机器192.168.0.{10..19}，10台服务器192.168.0.{100..109}（组成cluster或其他），这里是我用来生成测试的一些小脚本：
...
我们就可以用来做一些测试的screen脚本，我们在每个测试机器上，生成一些针对每个福尔起机器的脚本：
...
### 测试的定义
通过上面的screen脚本的定义，我们可以随时修改其中的重要设置，如-c -z等等，甚至增加一些客户端仿真的随机因素等等，具体情况不再赘述。
