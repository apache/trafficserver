...
namespace {
  ...
  bool Validate_Stripe_Meta(ts::CacheStripeMeta const& stripe)
  {
    return ts::CacheStripeMeta::MAGIC == stripe.magic &&
      stripe.version.ink_major <= ts::CACHE_DB_MAJOR_VERSION &&
  ...
  ProbeResult Probe_For_Stripe(ts::StringView& mem)
  {
  ...
  void Calc_Stripe_Data(ts::CacheStripeMeta const& header, ts::CacheStripeMeta const& footer, off_t delta, ts::StripeData& data)
  {
    ...
    std::cout << "Stripe is " << data.segments << " segments with " << data.buckets << " buckets per segment for " << data.buckets * data.segments * 4 << " total directory entries taking " << data.buckets * data.segments * 40 << " out of " << space << " bytes." << std::endl;
  }

  void Open_Stripe(ats_scoped_fd const& fd, ts::CacheStripeDescriptor const& block)
  {
    int found;
    ts::StringView data;
    ...
    if (found > 0) {
      if (stripe_mem.ptr() != buff) {
        ...
        if (stripe_pos[1]) {
          if (stripe_meta[0].sync_serial == stripe_meta[1].sync_serial &&
              (0 == stripe_pos[3] || stripe_meta[2].sync_serial != stripe_meta[3].sync_serial || stripe_meta[0].sync_serial > stripe_meta[2].sync_serial)) {
            ts::StripeData sdata;
  ...
  {
    if (!path.is_readable()) throw(std::system_error(errno, std::system_category(), static_cast<char const*>(path)));
    else if (path.is_regular_file()) this->loadConfig(path);
    else if (path.is_char_device() || path.is_block_device()) this->loadDevice(path);
    else printf("Not a valid file type: '%s'\n", static_cast<char const*>(path));
  }
...
# if 0
        printf("Span: %s\n    : Magic = 0x%x (%s) volumes=%d used=%d free=%d vol_blocks=%d total blocks=%" PRIu64 " \n"
               , static_cast<char const*>(path)
               , span_hdr.magic, (span_hdr.magic == ts::SpanHeader::MAGIC ? "match" : "fail")
               , span_hdr.num_volumes, span_hdr.num_used, span_hdr.num_free
               , span_hdr.num_diskvol_blks, span_hdr.num_blocks
          );
# endif
        // See if it looks valid
        if (span_hdr.magic == ts::SpanHeader::MAGIC &&
            span_hdr.num_diskvol_blks == span_hdr.num_used + span_hdr.num_free) {
...
# if 0
            std::cout << "    : SpanBlock " << i << " @ " << stripe.offset.units()
                      << " blocks=" << stripe.len.units() << " vol=" << stripe.vol_idx
                      << " type=" << stripe.type << " " << (stripe.free ? "free" : "in-use") << std::endl;
# endif
            if (stripe.free == 0) {
              // Add to volume.
              _volumes[stripe.vol_idx]._stripes.push_back(Volume::StripeRef { span, i });
//              if (Examine_Stripes_P) Open_Stripe(fd, stripe);
            }
          }
        }
      } else {
  ...
  Cache::dumpSpans(SpanDumpDepth depth)
  {
    if (depth >= SpanDumpDepth::SPAN) {
      for (auto span : _spans) {
        std::cout << "Span: " << span->_path << " "
                  << span->_header->num_volumes << " Volumes "
                  << span->_header->num_used << " in use "
                  << span->_header->num_free << " free "
                  << span->_header->num_diskvol_blks << " stripes "
                  << span->_header->num_blocks << " blocks"
                  << std::endl;
  ...
  Cache::dumpVolumes()
  {
    for ( auto const& elt : _volumes ) {
      ...
      std::cout << "Volume " << elt.first << " has " << elt.second._stripes.size() << " stripes and "
                << size << " bytes"
                << std::endl;
    }
  }
  ...
  Span::clearPermanently()
  {
    ...
    if (n == sizeof(zero)) std::cout << "done";
      ...
      std::cout << "failed";
      if (n >= 0) std::cout << " - " << n << " of " << sizeof(zero) << " bytes written";
      std::cout << " - " << text;
    }
    std::cout << std::endl;
  }
...
int main(int argc, char* argv[])
{
  ...
  Commands.add(std::string("list"), std::string("List elements of the cache"), [] (int argc, char* argv[]) { return List_Stripes(Cache::SpanDumpDepth::SPAN, argc, argv); } )
