...
void     rules_config_add_procedures_path(RulesConfig *conf, const char *path);
}
...
compare_pair(const char *hrw_file, const char *hrw4u_file, bool debug, bool quiet, bool profile,
             const char *procedures_path = nullptr)
{
  ...
  if (procedures_path) {
    rules_config_add_procedures_path(hrw_config, procedures_path);
  }
  ...
  if (procedures_path) {
    rules_config_add_procedures_path(hrw4u_config, procedures_path);
  }
...
main(int argc, char *argv[])
{
  ...
  static const struct option longopts[] = {
    ...
    {"procedures-path", required_argument, nullptr, 'p'},
  ...
  while ((opt = getopt_long(argc, argv, "dqbPp:h", longopts, nullptr)) != -1) {
    switch (opt) {
    case 'd':
      debug = true;
      break;
  ...
  if (batch) {
    ...
    while (std::getline(std::cin, line)) {
      ...
      CompareResult result = compare_pair(hrw_file.c_str(), hrw4u_file.c_str(), debug, quiet, profile, procedures_path);
  ...
  return compare_pair(files[0], files[1], debug, quiet, profile, procedures_path);
}
