...
run_serial_mode() {
  ...
  process_directory() {
    ...
    while IFS= read -r orig_file; do
      ...
      local start_ms=$(get_time_ms)
      if "$CONFCMP" $procs_arg "$abs_orig" "$abs_hrw4u" >/dev/null 2>&1; then
        local end_ms=$(get_time_ms)
        local elapsed=$((end_ms - start_ms))
        ...
        failed_tests+=("${dir_name}/${test_name}|$CONFCMP $procs_arg \"$abs_orig\" \"$abs_hrw4u\"")
        ...
        if [[ "$DEBUG_MODE" == true ]]; then
          ...
          echo "  $CONFCMP $procs_arg \"$abs_orig\" \"$abs_hrw4u\""
          echo ""
          ...
          echo "────────────────────────────────────────────────────────────"
          $CONFCMP --debug $procs_arg "$abs_orig" "$abs_hrw4u" 2>&1
          echo ""
          exit 1
        fi
      fi
    ...
    while IFS= read -r input_file; do
      ...
      local start_ms=$(get_time_ms)
      if "$CONFCMP" $procs_arg "$abs_output" "$abs_input" >/dev/null 2>&1; then
        local end_ms=$(get_time_ms)
        local elapsed=$((end_ms - start_ms))
        total_time_ms=$((total_time_ms + elapsed))
        printf "  ✓ %-30s %4d ms\n" "${dir_name}/${test_name}:" "$elapsed"
        ...
        printf "  ✗ %-30s %4d ms (FAILED)\n" "${dir_name}/${test_name}:" "$elapsed"
        dir_failed=$((dir_failed + 1))
        failed_tests+=("${dir_name}/${test_name}|$CONFCMP $procs_arg \"$abs_output\" \"$abs_input\"")
        ...
        if [[ "$DEBUG_MODE" == true ]]; then
          ...
          echo ""
          echo "To re-run this test:"
          echo "  $CONFCMP $procs_arg \"$abs_output\" \"$abs_input\""
          echo ""
          ...
          echo "Comparison Details (--debug):"
          echo "────────────────────────────────────────────────────────────"
          $CONFCMP --debug $procs_arg "$abs_output" "$abs_input" 2>&1
          echo ""
          exit 1
        fi
      fi
