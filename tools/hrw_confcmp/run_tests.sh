...
run_batch_mode() {
  ...
  # Auto-detect procedures path from test directories
  local procs_arg=""
  for test_dir in "${TEST_DIRS[@]}"; do
    local abs_dir="$(cd "$test_dir" 2>/dev/null && pwd)"
    if [[ -d "${abs_dir}/procs" ]]; then
      procs_arg="--procedures-path ${abs_dir}/procs"
      break
    fi
  done
  ...
  local result
  set +e
  result=$(echo "$pairs" | "$CONFCMP" --batch $procs_arg 2>&1)
  local exit_code=$?
  set -e
...
run_serial_mode() {
  ...
  # Run a single test: time it, print pass/fail, handle debug mode
  run_single_test() {
    local dir_name="$1"
    local test_name="$2"
    ...
    local start_ms=$(get_time_ms)
    if "$CONFCMP" $procs_arg "$expected_file" "$input_file" >/dev/null 2>&1; then
      local end_ms=$(get_time_ms)
      local elapsed=$((end_ms - start_ms))
      ...
      failed_tests+=("${dir_name}/${test_name}|$CONFCMP $procs_arg \"$expected_file\" \"$input_file\"")
      ...
      if [[ "$DEBUG_MODE" == true ]]; then
        ...
        echo "  $CONFCMP $procs_arg \"$expected_file\" \"$input_file\""
        echo ""
        echo "${expected_label}:"
        echo "────────────────────────────────────────────────────────────"
        cat "$expected_file" 2>/dev/null || echo "Error: Could not read $expected_file"
        echo ""
        echo "${input_label}:"
        echo "────────────────────────────────────────────────────────────"
        ...
        echo "────────────────────────────────────────────────────────────"
        $CONFCMP --debug $procs_arg "$expected_file" "$input_file" 2>&1
        echo ""
        exit 1
      fi
    fi
  ...
  process_directory() {
    ...
    # Auto-detect procedures path
    local procs_arg=""
    if [[ -d "${abs_test_dir}/procs" ]]; then
      procs_arg="--procedures-path ${abs_test_dir}/procs"
    fi
    ...
    while IFS= read -r orig_file; do
      ...
      run_single_test "$dir_name" "$test_name" "$abs_orig" "$abs_hrw4u" "$procs_arg" \
        "Original Config File (authority)" "Generated HRW4U File"
    done < <(find "$test_dir" -maxdepth 1 \( -name "*.conf" -o -name "*.config" -o -name "*.hrw" \) | sort)
    ...
    while IFS= read -r input_file; do
      ...
      run_single_test "$dir_name" "$test_name" "$abs_output" "$abs_input" "$procs_arg" \
        "Expected Output (.output.txt)" "HRW4U Input (.input.txt)"
    done < <(find "$test_dir" -maxdepth 1 -name "*.input.txt" | sort)
