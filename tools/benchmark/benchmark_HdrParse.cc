...
struct UrlCase {
  std::string label;
  std::string data; // a bare request target
};

struct Corpus {
  std::vector<HeaderCase>  cases;
  std::vector<std::string> urls;      // bare request targets for the url target
  std::vector<UrlCase>     file_urls; // request targets of the file-loaded requests
...
// The request-target of a request block's "METHOD SP target SP version" start
// line, or empty if the start line doesn't have exactly that shape.
std::string_view
request_target(std::string_view block)
{
  std::string_view const line  = block.substr(0, block.find("\r\n"));
  ...
  if (second == std::string_view::npos || second == first + 1 || line.find(' ', second + 1) != std::string_view::npos) {
    return {};
  }
...
build_corpus(const std::vector<std::filesystem::path> &files, const std::vector<std::filesystem::path> &dirs)
{
  ...
  // File-loaded.
  size_t const builtin_count = corp.cases.size();
  ...
  for (size_t i = builtin_count; i < corp.cases.size(); ++i) {
    const auto &c = corp.cases[i];
    if (c.is_response) {
      continue;
    }
    if (auto const target = request_target(c.data); !target.empty()) {
      corp.file_urls.push_back({c.label, std::string(target)});
    }
  }
...
run_profile(Target target, uint64_t iters)
{
  ...
  case Target::Url:
    ...
    for (const auto &u : g_corpus.file_urls) {
      add_input(u.data);
    }
    break;
  case Target::Wks:
...
TEST_CASE("hdr parse: request", "[bench][request]")
{
  ...
  if (!loaded.empty()) {
    BENCHMARK("request: corpus (" + std::to_string(loaded.size()) + " blocks)")
    {
      uint64_t sink = 0;
      for (auto block : loaded) {
        sink += drive_request(block).second;
      }
      return sink;
    };
  }
...
TEST_CASE("hdr parse: response", "[bench][response]")
{
  ...
  if (!loaded.empty()) {
    BENCHMARK("response: corpus (" + std::to_string(loaded.size()) + " blocks)")
    {
      uint64_t sink = 0;
      for (auto block : loaded) {
        sink += drive_response(block).second;
      }
      return sink;
    };
  }
...
TEST_CASE("hdr parse: url only", "[bench][url]")
{
  ...
  for (const auto &u : g_corpus.file_urls) {
    CAPTURE(u.label, u.data);
    REQUIRE(drive_url(u.data).first != ParseResult::ERROR);
  }
  ...
  if (!g_corpus.file_urls.empty()) {
    BENCHMARK("url: corpus (" + std::to_string(g_corpus.file_urls.size()) + " targets)")
    {
      uint64_t sink = 0;
      for (const auto &u : g_corpus.file_urls) {
        sink += drive_url(u.data).second;
      }
      return sink;
    };
  }
