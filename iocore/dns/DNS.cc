...
{
  int r = ink_res_mkquery(res, QUERY, qname, C_IN, qtype, nullptr, 0, nullptr, reinterpret_cast<unsigned char *>(buffer),
                          MAX_DNS_PACKET_LEN);
  return r;
}
    ...
    *hap                 = nullptr;
    ...
    while (ancount-- > 0 && cp < eom && !error) {
      ...
      if (type == T_PTR) {
        ...
        // expand the name
        n =
          ink_dn_expand(reinterpret_cast<u_char *>(h), eom, srv_off + SRV_SERVER, reinterpret_cast<u_char *>(srv->host), MAXDNAME);
        if (n < 0) {
          ++error;
          break;
        }
