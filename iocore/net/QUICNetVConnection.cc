...
QUICNetVConnection::startEvent(int event, Event *e)
{
  ...
  if (!action_.cancelled) {
    ...
    this->free_from_thread(e->ethread);
  }
...
void
QUICNetVConnection::free_from_thread(EThread *t)
{
...
{
  this->free_from_thread(this_ethread());
}
...
QUICNetVConnection::state_connection_closed(int event, Event *data)
{
  ...
  case QUIC_EVENT_SHUTDOWN: {
    ...
    if (this->nh) {
      ...
      this->free_from_thread(this->mutex->thread_holding);
    }
    break;
  }
  case QUIC_EVENT_PACKET_WRITE_READY: {
