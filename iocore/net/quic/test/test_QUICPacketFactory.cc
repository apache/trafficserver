...
TEST_CASE("QUICPacketFactory_Create_VersionNegotiationPacket", "[quic]")
{
  ...
  uint8_t expected[] = {
    ...
    0xff, 0x00, 0x00, 0x0f,                         // Supported Version
  };
  uint8_t buf[1024] = {0};
  size_t buf_len;
...
TEST_CASE("QUICPacketFactory_Create_Handshake", "[quic]")
{
  ...
  QUICPacketUPtr packet = factory.create_handshake_packet(
    ...
    QUICConnectionId(reinterpret_cast<const uint8_t *>("\x11\x12\x13\x14"), 4), 0, std::move(payload), sizeof(raw), true, false, dummy_frames);
