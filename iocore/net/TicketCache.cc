...
void
TicketCache::clear(SessionTicket *s)
{
  ink_zero(*s);
}

void
TicketCache::save(SessionTicket *s, const char *hostname, time_t expTime, const unsigned char *ticket, unsigned int ticketLength)
{
  ...
  Debug("ssl.ticket", "save ticket for %s: expire hint time: %d, currentTime: %d, new expTime: %d\n", hostname, (int)expTime,
        (int)time(NULL), (int)s->expTime);
}

void
TicketCache::enableCache(bool enable)
{
  enabled = enable;
}
...
int
TicketCache::lookup(const char *hostname, unsigned char *ticketBuff, unsigned int ticketBuffSize)
{
  ...
  pthread_mutex_lock(&ticket_cache_mutex);
  s = cache[hostname];
  pthread_mutex_unlock(&ticket_cache_mutex);
  if (NULL == s) {
  ...
  if (s->expTime < time(NULL)) {
    ...
    Debug("ssl.ticket", "though ticket expired, %s==%s: expTime is %d, time is %d, s->ticketLength=%d\n", hostname, s->hostname,
          (int)s->expTime, (int)time(NULL), s->ticketLength);
    clear(s);
    return 0; // expired
  }
...
void
TicketCache::store(const char *hostname, uint64_t expireHint, const unsigned char *ticket, unsigned int ticketLength)
{
  ...
  Debug("ssl.ticket", "Storing session ticket for host \"%s\", length=%d bytes, expireHint=%d", hostname, ticketLength,
        (int)expireHint);
  ...
  if (ticketLength > ST_SESSION_TICKET_MAX_LENGTH) {
    ...
    Debug("ssl.ticket", "Not caching oversized session ticket (%d bytes).  Max we store is %d bytes", ticketLength,
          ST_SESSION_TICKET_MAX_LENGTH);
    return;
  }
...
TicketCache::TicketCache(bool enable)
{
  ...
  pthread_mutex_init(&ticket_cache_mutex, NULL);
...
TicketCache::~TicketCache()
{
  ...
  cache.clear();
  pthread_mutex_destroy(&ticket_cache_mutex);
}
