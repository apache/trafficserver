...
send_throttle_message(NetAccept *na)
{
  ...
  int n = 0;
  while (check_net_throttle(ACCEPT, Thread::ink_get_hrtime()) && n < THROTTLE_AT_ONCE - 1 && (socketManager.poll(&afd, 1, 0) > 0)) {
    int res = 0;
...
NetAccept::acceptFastEvent(int event, void *ep)
{
  ...
  do {
    if (!backdoor && check_net_throttle(ACCEPT, Thread::ink_get_hrtime())) {
      ifd = -1;
      return EVENT_CONT;
    }
