...
test_RamCache(RegressionTest *t, RamCache *cache, const char *name, int64_t cache_size)
{
  ...
  for (int l = 0; l < 10; l++) {
    for (int i = 0; i < 200; i++) {
      ...
      md5.u64[0] = (static_cast<uint64_t>(i) << 32) + i;
      md5.u64[1] = (static_cast<uint64_t>(i) << 32) + i;
      ...
      for (int j = 0; j <= i && j < 10; j++) {
        ...
        md5.u64[0] = (static_cast<uint64_t>(j) << 32) + j;
        md5.u64[1] = (static_cast<uint64_t>(j) << 32) + j;
        cache->get(&md5, &data);
      }
    }
  }
  ...
  for (int i = 0; i < sample_size; i++) {
    INK_MD5 md5;
    md5.u64[0] = (static_cast<uint64_t>(r[i]) << 32) + r[i];
    md5.u64[1] = (static_cast<uint64_t>(r[i]) << 32) + r[i];
