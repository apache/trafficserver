...
CacheVC::scanObject(int /* event ATS_UNUSED */, Event * /* e ATS_UNUSED */)
{
  ...
  while (static_cast<off_t>(reinterpret_cast<char *>(doc) - buf->data()) + next_object_len <
    ...
    while (true) {
      ...
      if (!dir_agg_valid(vol, &dir) || !dir_head(&dir) ||
          (vol_offset(vol, &dir) != io.aiocb.aio_offset + (reinterpret_cast<char *>(doc) - buf->data()))) {
        continue;
      }
      break;
    }
  ...
  // fix it.
  if (might_need_overlap_read && (static_cast<off_t>(reinterpret_cast<char *>(doc) - buf->data()) + next_object_len >
                                  static_cast<off_t>(io.aiocb.aio_nbytes)) &&
      next_object_len > 0) {
