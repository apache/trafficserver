...
/*
   ...
   - pin_in_cache. Doc's pinned value is set to this + Thread::ink_get_hrtime().
...
Vol::evacuateDocReadDone(int event, Event *e)
{
  ...
  if (!b)
    goto Ldone;
  if ((b->f.pinned && !b->readers) && doc->pinned < (uint32_t)(Thread::ink_get_based_hrtime() / HRTIME_SECOND))
    goto Ldone;
...
agg_copy(char *p, CacheVC *vc)
{
  ...
  if (!vc->f.evacuator) {
    ...
    if (vc->pin_in_cache) {
      ...
      doc->pinned = (uint32_t)(Thread::ink_get_based_hrtime() / HRTIME_SECOND) + vc->pin_in_cache;
    } else {
