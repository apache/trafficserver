  ...
  if (n_init == gndisks - 1) {
    ...
    if (bad_disks != 0) {
      ...
      if ((gndisks - bad_disks) > 0) {
        p_good_disks = static_cast<CacheDisk **>(ats_malloc((gndisks - bad_disks) * sizeof(CacheDisk *)));
      } else {
      ...
      if (cache_config_ram_cache_size == AUTO_SIZE_RAM_CACHE) {
        ...
        for (i = 0; i < gnvol; i++) {
          ...
          if (gvol[i]->cache == theCache) {
            ...
            factor = static_cast<double>(static_cast<int64_t>(gvol[i]->len >> STORE_BLOCK_SHIFT)) / theStreamCache->cache_size;
            Debug("cache_init", "CacheProcessor::cacheInitialized - factor = %f", factor);
            gvol[i]->ram_cache->init(static_cast<int64_t>(stream_ram_cache_size * factor), vol);
            ram_cache_bytes += static_cast<int64_t>(stream_ram_cache_size * factor);
...
Vol::init(char *s, off_t blocks, off_t dir_skip, bool clear)
{
  ...
  if (ats_hugepage_enabled()) {
    raw_dir = static_cast<char *>(ats_alloc_hugepage(vol_dirlen(this)));
  }
  if (raw_dir == nullptr) {
    raw_dir = static_cast<char *>(ats_memalign(ats_pagesize(), vol_dirlen(this)));
  }

  dir    = reinterpret_cast<Dir *>(raw_dir + vol_headerlen(this));
  header = reinterpret_cast<VolHeaderFooter *>(raw_dir);
  footer = reinterpret_cast<VolHeaderFooter *>(raw_dir + vol_dirlen(this) - ROUND_TO_STORE_BLOCK(sizeof(VolHeaderFooter)));
