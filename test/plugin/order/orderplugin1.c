...
static int
plugin1(TSCont contp, TSEvent event, void *edata)
{
  ...
  if ((field_loc = TSMimeHdrFieldFind(bufp, hdr_loc, FIELD_NAME, -1)) != 0) {
    ...
    TSMimeHdrFieldValueIntGet(bufp, hrd_loc, field_loc, 0, &count);
    if (value != ++count) {
      TSError("Incorrect sequence of calling...orderplugin1\n");
    }
...
TSPluginInit(int argc, const char *argv[])
{
  ...
  contp = TSContCreate(plugin1, lock1);
