...
/* orderplugin1.c: one of the set of 5 plugins that help determine if
 ...
 *
 *   Usage:
 *   (NT):orderplugin1.dll valuei
 *   (Solaris):orderplugin1.so valuei
 *
...
static int
plugin1(INKCont contp, INKEvent event, void *edata)
{
  ...
  if ((field_loc = INKMimeHdrFieldFind(bufp, hdr_loc, FIELD_NAME, -1)) != 0) {
    ...
    INKMimeHdrFieldValueIntGet(bufp, hrd_loc, field_loc, 0, &count);
    if (value != ++count) {
      INKError("Incorrect sequence of calling...orderplugin1\n");
    }
...
INKPluginInit(int argc, const char *argv[])
{
  ...
  if (argc != 2) {
    printf("Usage: orderplugin1.so <valuei>\n");
    return;
  }
  ...
  contp = INKContCreate(plugin1, lock1);
