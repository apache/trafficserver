...
/* orderplugin3.c: one of the set of 54 plugins that help determine if  
 ...
 *
 *   Usage: 
 *   (NT):orderplugin3.dll valuei
 *   (Solaris):orderplugin3.so valuei
 *  
...
static int
plugin3(INKCont contp, INKEvent event, void *edata)
{
  ...
  if ((field_loc = INKMimeHdrFieldFind(bufp, hdr_loc, FIELD_NAME, -1)) != 0) {
    ...
    if (value != ++count) {
      INKError("Incorrect sequence of calling...orderplugin3\n");
    }
...
INKPluginInit(int argc, const char *argv[])
{
  ...
  if (argc != 2) {
    printf("Usage: orderplugin3.so <valuei>\n");
    return;
  }
  ...
  contp = INKContCreate(plugin3, lock1);
