 ...
 *
 *
 *   Added Options in InkBench.config -
 *
...
typedef struct
{
  ...
  /* for cachability rate */
  double cachability_ratio;

  /* for reporting stats */
  long requests;
  long cachable_requests;
...
INKOptionsProcess(char *option, char *value)
{
  ...
  } else if (strcmp(option, "cachability") == 0) {
                 ...
                 void **req_id /* return */ )
{
  ...
  if (my_plugin.direct) {
    ...
    if (cachable) {
      ...
      sprintf(request_buf,
              "GET http://%s:%s/%d/length%d HTTP/1.1\r\nAccept: */*\r\nCache-Control: no-cache\r\n\r\n",
              my_plugin.origin_host[origin_server_num],
              my_plugin.origin_port[origin_server_num], serial_number, doc_size);
    }
  }
