...
int getUrl(FILE * urlFile, char *urlLine);
...
INKOptionsProcess(char *option, char *value)
{
  ...
  } else if (strcmp(option, "credentials") == 0) {      /* get the credentials here */
    my_plugin.credentials = strdup(value);
  } else if (strcmp(option, "url_file") == 0) { /* get the live URL file here */
                 ...
                 void **req_id /* return */ )
{
  ...
  /* SDKtest in "Proxy" mode */
    ...
    printf("url_line = %s\n", url_line);
    sprintf(request_buf,
            "GET %s HTTP/1.1\r\nAccept: */*\r\nProxy-Authorization: Basic %s\r\n\r\n", url_line, my_plugin.credentials);
  }
...
int
getUrl(FILE * urlFile, char *urlLine)
{
  if (fscanf(urlFile, "%s", urlLine)) {
    return 1;
  }
  return 0;
}
