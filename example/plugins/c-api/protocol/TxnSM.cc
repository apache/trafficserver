...
main_handler(TSCont contp, TSEvent event, void *data)
{
  ...
  if (q_current_handler != reinterpret_cast<TxnSMHandler>(&state_interface_with_server)) {
    if (event == TS_EVENT_VCONN_EOS) {
      return prepare_to_die(contp);
    }
  }

  Dbg(dbg_ctl, "current_handler (%p)", q_current_handler);
...
TxnSMCreate(TSMutex pmutex, TSVConn client_vc, int server_port)
{
  ...
  txn_sm->q_server_vc = nullptr;

  txn_sm->q_client_read_vio               = nullptr;
  txn_sm->q_client_write_vio              = nullptr;
  txn_sm->q_client_request_buffer         = nullptr;
  txn_sm->q_client_response_buffer        = nullptr;
  txn_sm->q_client_request_buffer_reader  = nullptr;
  txn_sm->q_client_response_buffer_reader = nullptr;

  txn_sm->q_server_read_vio              = nullptr;
  txn_sm->q_server_write_vio             = nullptr;
  txn_sm->q_server_request_buffer        = nullptr;
  txn_sm->q_server_response_buffer       = nullptr;
  txn_sm->q_server_request_buffer_reader = nullptr;

  /* Char buffers to store client request and server response. */
  txn_sm->q_client_request = static_cast<char *>(TSmalloc(sizeof(char) * (MAX_REQUEST_LENGTH + 1)));
  ...
  txn_sm->q_cache_vc                 = nullptr;
  txn_sm->q_cache_response_length    = 0;
  txn_sm->q_cache_read_buffer        = nullptr;
  txn_sm->q_cache_read_buffer_reader = nullptr;

  txn_sm->q_server_name = static_cast<char *>(TSmalloc(sizeof(char) * (MAX_SERVER_NAME_LENGTH + 1)));
  txn_sm->q_file_name   = static_cast<char *>(TSmalloc(sizeof(char) * (MAX_FILE_NAME_LENGTH + 1)));
  ...
  case TS_EVENT_VCONN_READ_READY:
    ...
    if (bytes_read > 0) {
      ...
      if (strstr(txn_sm->q_client_request, "\r\n\r\n") != nullptr) {
        temp_buf = static_cast<char *>(TSmalloc(sizeof(char) * (strlen(txn_sm->q_client_request) + 1)));
        ...
        Dbg(dbg_ctl, "Key material: file name is %s*****", txn_sm->q_file_name);
...
state_handle_cache_lookup(TSCont contp, TSEvent event, TSVConn vc)
{
  ...
  case TS_EVENT_CACHE_OPEN_READ_FAILED:
    ...
    Dbg(dbg_ctl, "cache miss or error!!!");
    /* Write log */
...
load_buffer_cache_data(TxnSM *txn_sm)
{
  ...
  Dbg(dbg_ctl, "entering buffer_cache_data");
  Dbg(dbg_ctl, "loading %d bytes to buffer reader", rdr_avail);
...
state_handle_cache_read_response(TSCont contp, TSEvent event, TSVIO vio ATS_UNUSED)
{
  ...
  case TS_EVENT_VCONN_READ_COMPLETE:
    ...
    txn_sm->q_cache_vc        = nullptr;
    txn_sm->q_cache_read_vio  = nullptr;
    txn_sm->q_cache_write_vio = nullptr;
  ...
  default:
    ...
    set_handler(txn_sm->q_current_handler, (TxnSMHandler)&state_handle_cache_prepare_for_write);
    TSAssert(txn_sm->q_pending_action == nullptr);
    txn_sm->q_pending_action = TSCacheWrite(contp, txn_sm->q_key);
    break;
  }
  return TS_SUCCESS;
...
state_handle_cache_prepare_for_write(TSCont contp, TSEvent event, TSVConn vc)
{
  ...
  return state_build_and_send_request(contp, TS_EVENT_NONE, nullptr);
}
...
state_build_and_send_request(TSCont contp, TSEvent event ATS_UNUSED, void *data ATS_UNUSED)
{
  ...
  Dbg(dbg_ctl, "initiating host lookup");
...
state_interface_with_server(TSCont contp, TSEvent event, TSVIO vio)
{
  ...
  case TS_EVENT_VCONN_EOS:
    ...
    if (txn_sm->q_server_response_length == 0) {
      ...
      txn_sm->q_cache_write_vio = nullptr;
      return state_done(contp, TS_EVENT_NONE, nullptr);
    }
    ...
    if (txn_sm->q_cache_response_length >= txn_sm->q_server_response_length) {
      ...
      Dbg(dbg_ctl, "cache_response_length is %d, server response length is %d", txn_sm->q_cache_response_length,
...
state_read_response_from_server(TSCont contp, TSEvent event ATS_UNUSED, TSVIO vio ATS_UNUSED)
{
  ...
  Dbg(dbg_ctl, "bytes read is %d, total response length is %d", bytes_read, txn_sm->q_server_response_length);
...
state_write_to_cache(TSCont contp, TSEvent event, TSVIO vio)
{
  ...
  case TS_EVENT_VCONN_WRITE_COMPLETE:
    Dbg(dbg_ctl, "nbytes %" PRId64 ", ndone %" PRId64, TSVIONBytesGet(vio), TSVIONDoneGet(vio));
    ...
    if (txn_sm->q_server_vc != nullptr) {
      Dbg(dbg_ctl, "re-enable server_read_vio");
    ...
    if (txn_sm->q_cache_response_length >= txn_sm->q_server_response_length) {
      /* Write is complete, close the cache_vc. */
      Dbg(dbg_ctl, "close cache_vc, cache_response_length is %d, server_response_length is %d", txn_sm->q_cache_response_length,
          txn_sm->q_server_response_length);
      TSVConnClose(txn_sm->q_cache_vc);
      txn_sm->q_cache_vc        = nullptr;
      txn_sm->q_cache_write_vio = nullptr;
      ...
      Dbg(dbg_ctl, "re-enable cache_write_vio");
      TSVIOReenable(txn_sm->q_cache_write_vio);
    }
    return TS_SUCCESS;
  default:
...
state_send_response_to_client(TSCont contp, TSEvent event, TSVIO vio)
{
  ...
  switch (event) {
  case TS_EVENT_VCONN_WRITE_READY:
    Dbg(dbg_ctl, " . wr ready");
    Dbg(dbg_ctl, "write_ready: nbytes %" PRId64 ", ndone %" PRId64, TSVIONBytesGet(vio), TSVIONDoneGet(vio));
    TSVIOReenable(txn_sm->q_client_write_vio);
    break;

  case TS_EVENT_VCONN_WRITE_COMPLETE:
    Dbg(dbg_ctl, " . wr complete");
    Dbg(dbg_ctl, "write_complete: nbytes %" PRId64 ", ndone %" PRId64, TSVIONBytesGet(vio), TSVIONDoneGet(vio));
  ...
  default:
    Dbg(dbg_ctl, " . default handler");
    return prepare_to_die(contp);
  }

  Dbg(dbg_ctl, "leaving send_response_to_client");
...
state_done(TSCont contp, TSEvent event ATS_UNUSED, TSVIO vio ATS_UNUSED)
{
  ...
  if (txn_sm->q_pending_action && !TSActionDone(txn_sm->q_pending_action)) {
    Dbg(dbg_ctl, "cancelling pending action %p", txn_sm->q_pending_action);
    TSActionCancel(txn_sm->q_pending_action);
  } else if (txn_sm->q_pending_action) {
    Dbg(dbg_ctl, "action is done %p", txn_sm->q_pending_action);
  }

  txn_sm->q_pending_action = nullptr;
  txn_sm->q_mutex          = nullptr;
  ...
  if (txn_sm->q_client_request) {
    ...
    txn_sm->q_client_request = nullptr;
  }
