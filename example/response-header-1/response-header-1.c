...
modify_header(TSHttpTxn txnp)
{
  ...
  if (TSHttpTxnServerRespGet(txnp, &resp_bufp, &resp_loc) != TS_SUCCESS) {
    TSError("[response_header_1] Couldn't retrieve server response header");
    return; /* caller reenables */
  }
  ...
  if (TS_HTTP_STATUS_OK == resp_status) {
    ...
    if (TSHttpTxnCachedRespGet(txnp, &cached_bufp, &cached_loc) != TS_SUCCESS) {
      TSError("[response_header_1] STATUS 304, TSHttpTxnCachedRespGet():");
      TSError("[response_header_1] Couldn't retrieve cached response header");
      TSHandleMLocRelease(resp_bufp, TS_NULL_MLOC, resp_loc);
      return; /* Caller reenables */
    }
    ...
    if (TS_NULL_MLOC == cached_field_loc) {
      TSError("[response_header_1] Can't find header %s in cached document", mimehdr1_name);
      TSHandleMLocRelease(resp_bufp, TS_NULL_MLOC, resp_loc);
    ...
    if (NULL == chkptr || !chklength) {
      TSError("[response_header_1] Could not find value for cached MIME field name %s", mimehdr1_name);
      TSHandleMLocRelease(resp_bufp, TS_NULL_MLOC, resp_loc);
      TSHandleMLocRelease(cached_bufp, TS_NULL_MLOC, cached_loc);
...
TSPluginInit(int argc, const char *argv[])
{
  ...
  if (TSPluginRegister(TS_SDK_VERSION_3_0, &info) != TS_SUCCESS) {
    TSError("[response_header_1] Plugin registration failed.");
  }

  init_buffer_status = 0;
  if (argc > 1) {
    TSError("[response_header_1] usage: %s", argv[0]);
    TSError("[response_header_1] warning: too many args %d", argc);
    TSError("[response_header_1] warning: ignoring unused arguments beginning with %s", argv[1]);
  }
  ...
  if (TS_NULL_MLOC == (chk_field_loc = TSMimeHdrFieldGet(hdr_bufp, hdr_loc, 0))) {
    TSError("[response_header_1] Couldn't retrieve header field from init buffer");
    TSError("[response_header_1] Marking init buffer as corrupt; no more plugin processing");
    init_buffer_status = 0;
    /* bail out here and reenable transaction */
  } else {
    if (field_loc != chk_field_loc)
      TSError("[response_header_1] Retrieved buffer field loc is %p when it should be %p", chk_field_loc, field_loc);
  }
}
