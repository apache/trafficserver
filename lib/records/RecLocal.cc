  ...
  while (1) {
    ...
    if (RecSyncConfigToTB(tb, &inc_version) == REC_ERR_OKAY) {
      ...
      if (configFiles->isConfigStale()) {
        RecSetRecordInt("proxy.node.config.reconfigure_required", 1, REC_SRC_DEFAULT);
      }
    }
...
config_update_thr(void * /* data */)
{
  while (true) {
    ...
    case RECU_RESTART_TS:
      RecSetRecordInt("proxy.node.config.restart_required.proxy", 1, REC_SRC_DEFAULT);
      break;
    case RECU_RESTART_TM:
      RecSetRecordInt("proxy.node.config.restart_required.proxy", 1, REC_SRC_DEFAULT);
      RecSetRecordInt("proxy.node.config.restart_required.manager", 1, REC_SRC_DEFAULT);
      break;
    case RECU_RESTART_TC:
      RecSetRecordInt("proxy.node.config.restart_required.proxy", 1, REC_SRC_DEFAULT);
      RecSetRecordInt("proxy.node.config.restart_required.manager", 1, REC_SRC_DEFAULT);
      RecSetRecordInt("proxy.node.config.restart_required.cop", 1, REC_SRC_DEFAULT);
      break;
    case RECU_NULL:
    case RECU_DYNAMIC:
      break;
