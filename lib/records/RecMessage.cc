...
{
  RecMessage *msg = reinterpret_cast<RecMessage *>(data_raw);
...
RecMessageReadFromDisk(const char *fpath)
{
  ...
  if (RecFileRead(h_file, reinterpret_cast<char *>(msg) + msg_hdr.o_start, msg_hdr.o_end - msg_hdr.o_start, &bytes_read) ==
      REC_ERR_FAIL) {
    goto Lerror;
  }
...
RecMessageWriteToDisk(RecMessage *msg, const char *fpath)
{
  ...
  if ((h_file = RecFileOpenW(fpath)) != REC_HANDLE_INVALID) {
    if (RecFileWrite(h_file, reinterpret_cast<char *>(msg), msg_size, &bytes_written) == REC_ERR_FAIL) {
      RecFileClose(h_file);
      return REC_ERR_FAIL;
    }
    RecFileClose(h_file);
