...
public:
  ...
  NumericType(raw_type const t ///< Initialized value.
              );
...
template <typename T, typename X>
NumericType<T, X> &
NumericType<T, X>::operator=(self const &that)
{
  _t = that._t;
  return *this;
}
...
template <typename T, typename X> NumericType<T, X> &NumericType<T, X>::operator++()
{
  ++_t;
  return *this;
}
...
template <typename T, typename X> NumericType<T, X> NumericType<T, X>::operator++(int)
{
  self tmp(*this);
  ++_t;
  return tmp;
