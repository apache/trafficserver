...
private:
 ...
 public:
    ...
    */
   HeaderFieldName(const std::string &name) { name_  = name;}
   ...
   /**
     ...
     */
   operator const char*() { return name_.c_str();}

   /**
    * @return the length of this HeaderFieldName
    */
   size_type length() { return name_.size(); }
...
public:
  ...
  MOCK_CONST_METHOD0(name, HeaderFieldName ());
  /**
  ...
  MOCK_METHOD1(values, std::string (const char*));
  MOCK_METHOD0(values, std::string ());
  ...
  MOCK_METHOD1(values, std::string (const std::string&));
  ...
  MOCK_METHOD1(erase, bool (iterator));
  /**
  ...
  MOCK_METHOD1(append, bool (const std::string&));
  /**
   * Append a value or a seperated list of values to this HeaderField
  ...
  MOCK_METHOD2(append, bool (const char*, int));
  MOCK_METHOD1(append, bool (const char*));
   ...
   */
  MOCK_CONST_METHOD1(compare, bool(const char*));
  bool operator==(const char *field_name) const { return compare(field_name); }

  /** Compares the name of the header field only (not the values).
   * @note This is a case insensitive comparison.
   * @return true if the name is equal (case insensitive comparison).
   */
  //MOCK_CONST_METHOD1(compare, bool(std::string &));
  bool operator==(const std::string &field_name) const { return compare(field_name.c_str()); }
   ...
   */
  MOCK_METHOD1(assign, bool(const char *));
  bool operator=(const std::string &field_value) { return assign(field_value.c_str()); }
   ...
   * @return true if the value is sucessfully changed.
   */
  bool operator=(const char *field_value) { return assign(field_value); }
  ...
  /**
    ...
    * change the value.
    */
  MOCK_METHOD1(at, std::string(const int));
  std::string operator[](const int index) { return at(index); }

  /**
    * Get a string representing all the header field's values
    * @return a string representation of all the header fields
    */
  friend std::ostream& operator<<(std::ostream& os, HeaderField& obj) { return os; }
...
public:
    ...
    MOCK_METHOD0(increment, header_field_iterator& ());
    header_field_iterator& operator++() { return increment(); }

    /**
     * Advance the current iterator to the next header field
     * @return a new iterator which points to the next element
     */
    header_field_iterator operator++(int) { return increment(); }
    ...
    MOCK_METHOD0(nextDup, header_field_iterator& ());

    /**
     * Comparison operator, compare two iterators
     * @return true if the two iterators point to the same HeaderField
     */
    MOCK_CONST_METHOD1(compare, bool(const header_field_iterator&));
    bool operator==(const header_field_iterator& rhs) const { return compare(rhs); }
 
    /*
     * Inequality Operator, compare two iterators
     * @return false if the two iterators are the same.
     */
    MOCK_CONST_METHOD1(equals, bool(const header_field_iterator&));
    bool operator!=(const header_field_iterator& rhs) const { return !equals(rhs); }

    /**
     * Dereference an iterator
     * @return a HeaderField pointed to by this iterator
     */
    MOCK_METHOD0(dereference, HeaderField());
    HeaderField operator*() { return dereference(); }

    //friend class HeaderField;
    //friend class Headers;

    typedef std::map<std::string, std::vector<std::string> > HeadersMap;
    typedef std::map<std::string, std::vector<std::string> >::iterator HeadersMapIter;

    static HeadersMap headers_;
    static HeadersMapIter headers_iter_;
};

/**
 * @brief Encapsulates the headers portion of a request or response.
 */
//using this MOCK Class requires to set the headers_ with the field name.
...
public:
  ...
  MOCK_METHOD2(reset, void (void*, void*));
  ...
  MOCK_CONST_METHOD0(lengthBytes, size_type ());
  /**
  ...
  /**
    ...
    */
  //bool erase(iterator it);
  MOCK_METHOD1(erase, bool (iterator it));
  /**
  ...
  MOCK_METHOD1(erase, size_type (const std::string&));
  /**
    * Erase all headers whose name matches key (this is a case insensitive match).
    * @param the name of the header fields to erase
  ...
  MOCK_METHOD2(erase, size_type (const char*, int));
  MOCK_METHOD1(erase, size_type (const char*));
  ...
  MOCK_METHOD2(count, size_type (const char*, int));
  MOCK_METHOD1(count, size_type (const char*));
  ...
  MOCK_METHOD1(count, size_type (const std::string&));
  /**
  ...
  MOCK_METHOD2(values, std::string (const std::string&, const char*));
  MOCK_METHOD1(values, std::string (const std::string&)); // gmock does not support default arguments.
  /**
    * Join all headers whos name is key with the optionally specified join string
    * @param the name of the headers to join into a single string
  ...
  MOCK_METHOD2(values, std::string (const std::string&, const std::string&));
  /**
  ...
  MOCK_METHOD2(value, std::string (const std::string, size_type));
  MOCK_METHOD1(value, std::string (const std::string));
  ...
  MOCK_METHOD1(find, iterator (const std::string&));
  /**
    * Returns an iterator to the first HeaderField with the name key.
    * @param key the name of first header field ot find.
  ...
  MOCK_METHOD2(find, iterator (const char*, int));
  MOCK_METHOD1(find, iterator (const char*));
  /**
  ...
  MOCK_METHOD2(append, iterator (const std::string&, const std::string&));
  /**
  ...
  MOCK_METHOD2(set, iterator (const std::string&, const std::string&));
  /**
...
*/
  ...
  /**
    ...
    */
  //std::string wireStr();
  MOCK_METHOD0(wireStr, std::string ());

  friend std::ostream& operator<<(std::ostream &os, Headers &obj) { return os; }

  ~Headers() {  }


  typedef std::vector<std::string>  headers_;
