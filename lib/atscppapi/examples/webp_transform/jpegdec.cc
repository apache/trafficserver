...
void JpegDec::SaveMetadataMarkers() {
...
int JpegDec::CompareICCPSegments(const void* a, const void* b) {
  ...
  for (marker = dinfo_.marker_list; marker != NULL; marker = marker->next) {
    if (marker->marker == JPEG_APP2 &&
      ...
      if (segment_size == 0 || count == 0 || seq == 0) {
      	TS_DEBUG(TAG, "[ICCP] size (%d) / count (%d) / sequence number (%d) cannot be 0!",
...
JpegDec::ExtractMetadataFromJPEG(Metadata* const metadata) {
  ...
  for (marker = dinfo_.marker_list; marker != NULL; marker = marker->next) {
    ...
    for (i = 0; jpeg_metadata_map_[i].marker != 0; ++i) {
      if (marker->marker == jpeg_metadata_map_[i].marker &&
          marker->data_length > jpeg_metadata_map_[i].signature_length &&
          !memcmp(marker->data, jpeg_metadata_map_[i].signature,
          		jpeg_metadata_map_[i].signature_length)) {
        MetadataPayload* const payload =
            (MetadataPayload*)((uint8_t*)metadata +
            		jpeg_metadata_map_[i].storage_offset);
...
JpegDec::ReadImage(WebPPicture* const pic, Metadata* const metadata) {
  ...
  if (metadata != NULL) SaveMetadataMarkers();
