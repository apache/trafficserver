...
PngDec::ErrorFunction(png_structp png, png_const_charp error) {
  if (error != NULL) TS_DEBUG("img_transform_png", "libpng error: %s\n", error);
...
int
PngDec::ProcessRawProfile(const char* profile, size_t profile_len,
...
uint8_t*
PngDec::HexStringToBytes(const char* hexstring,
        ...
        if (!strcmp(text->key, png_metadata_map_[j].name)) {
          MetadataPayload* const payload =
              (MetadataPayload*)((uint8_t*)metadata +
              		png_metadata_map_[j].storage_offset);
          ...
          } else if (!png_metadata_map_[j].process(text->text, text_length,
                                                 payload)) {
...
PngDec::ReadImage(WebPPicture* const pic, Metadata* const metadata) {
  ...
  if (metadata != NULL &&
      !ExtractMetadataFromPNG(metadata)) {
