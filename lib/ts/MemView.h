...
inline constexpr StringView::StringView(const char *s) : _ptr(s), _size(strlen(s))
{
}
...
StringView::find(self delimiters) const
{
  ...
  // Load the bits in the array. This should be faster because this iterates over the delimiters
  // exactly once instead of for each byte in @a this.
...
StringView::ltrim(self delimiters)
{
  ...
  while (*this && valid[static_cast<uint8_t>(**this)])
    ++*this;
...
namespace detail
{
  // These are templated in order to not require including std::ostream but only std::iosfwd.
  // Templates allows the use of specific stream mechanisms to be delayed until use at which point
  // the caller will have included the required headers if needed but callers who don't won't need
  // to.
  ...
  template <typename Stream>
  void
  aligned_stream_write(Stream &os, const StringView &b)
  {
  ...
  extern template void aligned_stream_write(std::ostream &, const StringView &);
