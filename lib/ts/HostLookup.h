...
struct HostBranch {
  ...
  struct Linkage {
    ...
    key_of(HostBranch *elt)
    {
      return elt->key;
    }
    static auto
    hash_of(std::string_view key)
    {
      return hasher(key);
    }
    static bool
    ...
    static HostBranch *&
    next_ptr(HostBranch *elt)
    {
      return elt->link.next;
    }
    static HostBranch *&
    prev_ptr(HostBranch *elt)
    {
      return elt->link.prev;
    }

    HostBranch *next{nullptr};
    HostBranch *prev{nullptr};
  } link;

  using HostTable = ts::IntrusiveHashMap<Linkage>;
